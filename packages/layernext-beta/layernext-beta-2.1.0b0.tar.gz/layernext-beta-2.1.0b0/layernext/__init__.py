...
from layernext.datalake.constants import (AnnotationUploadType, ItemType,
...
class LayerNextClient:
    ...
    def upload_files_to_collection(
        ...
        if annotation_data["json_data_file_path"] is not None and upload_details["is_success"] == True:
            ...
            if annotation_data["operation_unique_id"] is None or not isinstance(annotation_data["is_normalized"], bool) or not isinstance(annotation_data["is_model_run"], bool):
    ...
    def auto_tag_collection(self, collection_id, model_id='default_tagging', input_resolution=(300, 300)):
        _automatic_analysis_client = automatic_analysis.AutomaticAnalysisClient(
            self.encoded_key_secret, self.layernext_url)
        _automatic_analysis_client.model_inference_collection(
            collection_id, model_id, input_resolution)
    ...
    def generate_auto_tags_to_metalake(self, item_type,  model_id='default_tagging', input_resolution=(300, 300)):
        _automatic_analysis_client = automatic_analysis.AutomaticAnalysisClient(
            self.encoded_key_secret, self.layernext_url)
        _automatic_analysis_client.model_inference_population(
            item_type, model_id, input_resolution)

    def auto_annotate_collection(self, collection_id, model_id, prompt="any object", input_resolution=(300, 300)):
        _automatic_analysis_client = automatic_analysis.AutomaticAnalysisClient(
            self.encoded_key_secret, self.layernext_url)
        _automatic_analysis_client.model_annotate_collection(
            collection_id, model_id, prompt, input_resolution)
    ...
    """
    generate embedding for a given collection
    """

    def generate_embeddings_for_collection(self, collection_id, model_id):
        _automatic_analysis_client = automatic_analysis.AutomaticAnalysisClient(
            self.encoded_key_secret, self.layernext_url)
        _automatic_analysis_client.embedding_model_inference_collection(
    ...
    def download_files_from_metalake(self,
        ...
        return _datalake_client.download_files_batch_wise(item_type_enum, query, metalake_filter, page_index, page_size,
                                                          folder_name,
                                                          {"item_type": item_type_enum}, sort_filter)
