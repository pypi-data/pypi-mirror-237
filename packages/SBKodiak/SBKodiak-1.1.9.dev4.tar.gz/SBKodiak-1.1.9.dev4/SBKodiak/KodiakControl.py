...
class KodiakControlClass:
    ...
    def start(self, kw, trigger=False):
        ...
        if not isinstance(query_response, list):
            logging.warning("Response from DB is not of type list")
            return False
        ...
        if not isinstance(response, dict):
            logging.warning('Invalid response from stat to Kodiak')
            return False
    ...
    def get_module_status(self, kw):
        ...
        try:
            ...
            logging.debug('Response from /module kodiak was : ' + response)
            return response
        except Exception as err:
            logging.warning('Error asking kodiak for a module update: ' + err)
            return None
    ...
    def get_module_capture_status(self, kw):
        ...
        try:
            response = self.kodiak_session.send_get_request(endpoint, headers)
            logging.debug('Response from /status kodiak was : ' + response)
    ...
    def check_if_running(self, kw):
        ...
        if 0 not in response:
            logging.warning('Missing "0" key from module capture status response')
            return False
