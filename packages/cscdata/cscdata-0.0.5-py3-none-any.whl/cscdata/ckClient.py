...
class ClickhouseClientFriendly(ClickhouseClient):
    ...
    def use_db(self, db_name):
        ...
        self.excu(f"USER {db_name};")

    def create_db(self,):
        """不支持用户自建db"""
        raise Exception("not support user create clickhouse db, ask administrator of clickhouse")

    def read_future_df(self, table_name:str, columns:list , symbol:list=None, start_date:int= None, end_date:int= None ):
        ...
        if symbol is not None and isinstance(symbol, list):
            sql_scripts += "and".join([f"symbol='{i}'\n" for i in symbol])
        if start_date is not None:
            sql_scripts += f"and date>={start_date}\n"
        if end_date is not None:
            sql_scripts += f"and date<={end_date}\n"

        df = self.read_df(sql_scripts)

        return df

    def read_stock_df(self, table_name:str, columns:list, instrument:list=None, start_date:int = None, end_date:int= None):
        sql_scripts = f"SELECT\n{' '.join(columns)}\nFROM {table_name}\n"
    ...
    def create_table(self, table_name:str , schema:dict or str, order_by:list ,partition_by:list, settings:list = None ):
        """
        上传用户特征table
        ...
        create_sql = clickhouse_create_table_sql(table_name, _schema, order_by, partition_by, settings=settings)
    ...
    def create_materialized_table(self, table_name: str, function: list, order_by:list, partition_by:list, settings_cur_table:list=None, setting_source_table:list = None):
        """
        创建物化视图
        基于 function list 构造物化视图， example ’count(*) as count‘：
        """
        create_sql = clickhosue_create_materialized_table(table_name, function, order_by,partition_by, settings_cur_table = settings_cur_table, setting_source_table= setting_source_table)
        self.excu(create_sql)

    def gen_stk_bar(self,table_name:str, level:str = "1m",materialized:bool = False):
        """合成bar线"""
        ...
        if materialized:
            ...
            self.create_materialized_table(table_name, function, order_by, partition_by, settings_cur_table= setting_cur_table, setting_source_table= setting_source_table)
        else:
            pass


    def gen_future_bar(self):
        pass
...
def clickhouseclientfriendly_demo(host = '',
                                  port = 9000,
                                  user='test',
                                  password='test',
                                  db_name= 'test',
                                  ...
                                  order_by= ['test'],
                                  settings= ['test']):
    ck = ClickhouseClientFriendly(host=host, port=port, user=user, password=password,db_name=db_name)
    ck.create_table(table_name,schema, order_by, settings)
    df = pd.DataFrame({"test":"test"})
    ck.init_table("test",df)
    df = ck.read_future_df(table_name)
    #处理df
    ...
    #上传df
...
if __name__ == '__main__':
    ...
    clickhouseclientfriendly_demo(host='')