...
class ClickhouseClientFriendly(ClickhouseClient):
    ...
    def read_df(self, table_name:str, columns:list , symbol:list=None, start_date:int= None, end_date:int= None ):
...
class ClickhouseFileSource(ClickhouseClientFriendly):
    """建立基于clickhouse的文件引擎
        备注：
    ...
    def __init__(self,
        ...
        super(ClickhouseFileSource, self).__init__(host= host,port = port,user = user, password=password,db_name = db_name)

    def create_file_table(self, table_name:str, file_path:str, schema:str or dict, ck_schema = False):
        """
        ...
        """
        create_table_scripts = clickhouse_create_file_table_sql(table_name, file_path, schema, ck_schema = ck_schema)
        self.excu(create_table_scripts)
