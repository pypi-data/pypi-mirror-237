...
def deploy_model(
    ...
    try:
        ...
        helper_models = ast.literal_eval(c.helper_models)
        for helper_model in helper_models:
            venv_name = helper_model + "_venv"
            requirements_file_target = os.path.join(helper_model, "requirements.model")
            helper_model_requirements = Path(requirements_file_target).resolve()
