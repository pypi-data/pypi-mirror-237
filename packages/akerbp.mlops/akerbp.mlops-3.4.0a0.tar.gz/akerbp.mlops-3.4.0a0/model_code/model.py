...
def _process(data_dict: Dict, ds: Dataset, **process_kwargs) -> List:
    ...
    if test_case != "default":
        ...
        if test_case == "call_cdf_model":
            ...
            # We need to have this uplaoded in the model interface during deployment to setup the venv
            # mm.setup()
            ...
            def _run_monkeypatched_vsh(
                ...
                **kwargs,
                ...
                # Keep track of main model executable
                main_model_executable = sys.executable

                vsh_venv_dir = Path("automatic_vsh_venv").resolve()
                is_unix = os.name == "posix"
                if is_unix:
                    sys.executable = os.path.join(vsh_venv_dir, "bin", "python")
                else:
                    sys.executable = os.path.join(vsh_venv_dir, "Scripts", "python")
                ...
                vsh_model_elements = vsh_init(vsh_artifacts_dir, secrets, verbose=False)  # type: ignore
                vsh_predictions = vsh_predict(in_data, vsh_model_elements, secrets)  # type: ignore

                # revert back to main model executable after calculating VSH
                sys.executable = main_model_executable
