...
def deploy_model(
    ...
    try:
        ...
        helper_models = c.helper_models
        helper_model_venvs = []
        if helper_models is not None:
            for helper_model in helper_models:
                venv_name = helper_model + "_venv"
                helper_model_venvs.append(venv_name)
                requirements_file_target = os.path.join(
                    helper_model, "requirements.model"
                )
                for root, dirs, files in os.walk(os.getcwd()):
                    for dir in dirs:
                        for file in files:
                            if requirements_file_target in os.path.join(
                                root, dir, file
                            ):
                                helper_model_requirements = os.path.join(
                                    root, dir, file
                                )
                                break

                if helper_model_requirements is not None:
                    helpers.replace_string_file(
                        s_old="akerbp.mlops==MLOPS_VERSION",
                        s_new=f"akerbp.mlops[{platform}]=={package_version}",
                        file=Path(helper_model_requirements),
                    )
                    helpers.create_venv(venv_name=venv_name)
                    helpers.install_requirements(
                        req_file=helper_model_requirements,
                        venv_dir=venv_name,
                        with_deps=True,
                        setup_venv=True,
                    )
        ...
        if c.platform == "cdf":
            if testing_only:
                ...
                helpers.do_deploy(
                    ...
                    deployment_folder,
                    setup_venv=True,
                    helper_model_venvs=helper_model_venvs,
                )
        return "OK"
