...
class pcell:
	...
	def change_list2d_to_differnet_xy_size(self, xy_list, resize=[1, 1]):
		"""
		리스트의 크기를 다시 설정하는 것
		메뉴에서 제외
	...
	def insert_all_picture_of_folder_in_sheet(self, sheet_name, folder_name, ext_list, xywh, link=0J, image_in_file=1):
		...
		for index, file_name in enumerate(aaa):
			...
			sheet_object.Shapes.AddPicture(full_path, link, image_in_file, rng.Left + index * 5, rng.Top + index * 5, xywh[2], xywh[3])
	...
	def make_ppt_table_from_xl_data(self):
		...
		:return:
		"""
		activesheet_name = self.read_name_for_activesheet()
	...
	def move_data_by_step_for_selection(self, sheet_name, xyxy, insert_step, insert_no=1, range_ext=False, del_or_ins="ins"):
		"""
		...
		:param del or_ins: 삭제인지 아니면 추강니지를 확인하는것
		:return:
		"""
		#전처리 구간
	...
	def multi_vlookup(self, sheet_name, xyxy, search_no_list, search_value_list, find_no, option_all =True):
		"""
		#여러줄이 같은
	...
	def new_workbook(self, filename=""):
		...
		if filename == "":
			...
			old_file_is = self.is_file_in_folder(path, file_name_only)
			if old_file_is:
		...
		for x in range(x1, x2 + 1):
			for y in range(y1, y2 + 1):
				...
				if input_text in value:
					self.paint_color_in_cell_by_scolor(sheet_name, [x, y], input_color)
	...
	def paint_color_in_cell_by_excel_colorno(self, sheet_name, xy, excel_color_no="입력필요"):
		"""
	...
	def paint_color_in_cell_by_rgb(self, sheet_name, xy, input_color="입력필요"):
		"""
		paint_color(sheet_name, xyxy, input_data="입력필요")
		선택 셀에 색깔을 넣는다
	...
	def paint_color_in_cell_by_scolor(self, sheet_name, xy, input_color="입력필요"):
		"""
		paint_color(sheet_name, xyxy, input_data="입력필요")
		선택 셀에 색깔을 넣는다
		메뉴에서 제외
		...
		for x in range(x1, x2 + 1):
			for y in range(y1, y2 + 1):
				...
				if value == "" or value == None:
					...
					len_new = len(set_a)
					if len_old == len_new:
						self.paint_color_in_cell_by_excel_colorno(sheet_name, [x, y], excelcolorno)
	...
	def paint_spacecell_in_range(self, sheet_name="", xyxy="", input_color="red"):
		"""
		메뉴에서 제외
	...
	def paint_textcolor_in_range(self, sheet_name, xyxy, input_color):
		"""
		메뉴에서 제외
	...
	def pick_ylines_at_list2d(self, input_list_2d, list_1d):
		"""
	...
	def read_all_name_for_shape_in_excel_file(self):
		"""
	...
	def read_all_name_for_shape_in_sheet(self, sheet_name):
		"""
	...
	def select_named_range(self, named_range_list):
		...
		for one_named_range in named_range_list:
			...
			if uni_range == []:
				...
				if check_name == sheet:
					uni_range = self.xlapp.Union(uni_range, my_range)
				else:
					uni_range.Select()
					sheet_object.Select()
					uni_range = my_range
	...
	def select_workbook(self, filename=""):
		...
		if filename == None or filename == "":
			self.write_value_in_messagebox("워크북이름을 다시한번 확인 해 주십시요")
		else:
			self.xlbook = self.xlapp.Workbooks.Open(self.filename)
	...
	def switch_value_by_2_position_no_in_list2d(self, input_list2d, input_no_list):
		"""
		2차원 리스트의 자료에서 각 라인별 2개의 위치를 바꾼는것
		change_position_for_list2d_by_2_index([[1,2,3], [4,5,6]], [0,2])
		[[1,2,3], [4,5,6]] ==> [[3,2,1], [6,5,4]]
	...
	def write_value_in_range_by_reverse(self, sheet_name="", xyxy="", input_list2d=""):
		"""
	...
	def conditional_format_with_operator(self, sheet_name, xyxy, type="CellValue", operator="100<=value<200", range_format="basic"):
		...
		oper_dic = {"between":1, "equal": 3, "greater":5, "greaterequal":7, "less":6, "Lessequal":8, "notbetween":2, "notequal":4,
		            "-": 3, ">":5, ">=":7, "<":6, "<=":8, "|-":4}
		sheet_object = self.check_sheet_name(sheet_name)
		x1, y1, x2, y2 = self.check_address_value(xyxy)
		my_range = sheet_object.Range(sheet_object.Cells(x1, y1), sheet_object.Cells(x2, y2))
	...
	def conditional_format_with_function(self, sheet_name, xyxy=[1,1,7,7], input_formula="=LEN(TRIM($A1))=0", range_format="basic"):
		sheet_object = self.check_sheet_name(sheet_name)
		x1, y1, x2, y2 = self.check_address_value(xyxy)
		my_range = sheet_object.Range(sheet_object.Cells(x1, y1), sheet_object.Cells(x2, y2))
		self.select_range(sheet_name, xyxy)
