...
class ganada:
	...
	def paint_border_in_selection(self, input_color):
		...
		self.selection.Font.Borders(1).Color = self.var_word["color_24bit"][input_color]
	...
	def select_n_more_word_from_selection(self, input_no):
		"""
		(단어 선택) 현재 위치에서 단어하나를 추가해서 선택하는것
	...
	def select_nth_paragraph_from_cursor(self, input_no=1):
		"""
		(문단 선택) 현재 커서의 위치를 기준으로 n번째 문단을 선택
	...
	def select_next_one_word_from_selection(self):
		"""
		(단어 선택) 현재 선택한 영역의 바로뒤의 단어를 선택하는것
	...
	def select_char_nos_from_cursor(self, x, length):
		"""
		(글자 선택) 현재커서의 위치에서 몇번째 문자로 에서부터 선택시작
	...
	def replace_in_document_by_jfsql(self, jfsql="[숫ㅏ:1~2],[숫자:1~2]", replace_text=""):
		para_nos = self.count_paragraph_nos_in_document()
		for no in range(para_nos):
			my_range = self.active_word_file.Paragraphs(no + 1).Range
			my_range_text = my_range.Text
			r_sabj = self.finder.search_all_by_jfsql(jfsql, my_range_text)
	...
	def set_page_no_at_header(self, left_text="", right_start_no=1):
		...
		self.active_word_file.Sections(1).Headers(1).PageNumbers.StartingNumber = right_start_no
	...
	def write_text_in_selection_with_color_size_bold(self, i_text, i_color="red", i_size=11, i_bold=False):
		...
		my_range.Font.Color = self.var_word["color_24bit"][i_color]
		my_range.Select()
