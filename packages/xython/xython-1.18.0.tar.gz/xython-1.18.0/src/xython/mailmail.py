...
class mailmail:
	...
	def get_opened_mail_item(self):
		try:
			result = self.outlook_program.ActiveInspector().CurrentItem
		except:
			result = None
		return result

	def reply_email(self, one_mail_item):
		promise_folder = self.outlook.GetDefaultFolder (16)
		reply = one_mail_item.ReplyAll()
		#reply = one mail item.Reply ()
		reply.Body = "your message here" + reply.Body
		reply.Move (promise_folder)
