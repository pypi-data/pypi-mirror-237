...
class youtil():
	...
	def make_team_with_condition(self, all_data, not_same_group, level, step):
		...
		#전체자료중에서, leve로 묶여진것을
		changed_level = self.change_group_as_new_step(all_data, level, step)
		print(changed_level)
		#그룹no만큼 그룹을 만든다
		result ={}
		...
		for one_group in not_same_group:
			...
			for list_1d in changed_level:
				randomed_list1d = self.make_random_list(list_1d, len(list_1d))[0]
				finished=""
				for one_value in randomed_list1d:
					temp=[]
					finished = ""
					for num in range(step):
						if finished == "ok":
							...
							for one_item in combi:
								if (one_item[0] in result[min_group_no] or one_item[1] in result[min_group_no]) and	(one_item[0] == one_value or one_item[1] == one_value):
									error_found = True
							if error_found:
