...
class DeployCommand(object):
    def __init__(self, parser):
        ...
        deploy_parser.add_argument(
            "--application_logic", action="store_true", help="Deploy application_logic"
        )

        self.pochi_util = PochiUtil()

    def get_help_text(self):
        help_text = """pochi deploy [--application_logic|--application_package]
    ...
    Options:
        --application_logic             Updates only the native app application code files and creates a new patch.
        --application_package           Updates only the native app package or other objects in the provider account.
"""
        return help_text
    ...
    def __deployment_sequence(
        ...
        if deployment_version_code:
            has_errors = self.pochi_util.execute_sql_from_file(
                "deployment_version_code",
                os.path.join("generated", "deployment", "deploy_application_logic.sql"),
                has_errors,
            )
        ...
        _, output = self.pochi_util.execute_sql(
            'select max("patch") from table(result_scan(last_query_id())) where "version"=\'{}\';'.format(
        ...
        patch_number = patch_first_row_tuple[0] if len(patch_first_row_tuple) > 0 else 0
