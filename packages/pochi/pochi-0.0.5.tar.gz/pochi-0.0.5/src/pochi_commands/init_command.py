...
class InitCommand(object):
    def __init__(self, parser):
        ...
        init_parser = parser.add_parser(
            "init",
            usage="pochi init [--name=<folder_name>] [--version=<version_name>] [--connection=<connection_name] [--package_distribution=<application_package_distribution>] [--force]",
    ...
    Options:
        ...
        --package_distribution=<application_package_distribution>]  Override the default application package distribution in config/project.toml
    ...
    def execute(self, options):
        ...
        if options.init.name:
            ...
            LoggingManager.display_message(
                ...
                os.path.join(current_directory, options.init.name),
            )
        else:
            ...
            with open(os.path.join("config", "project.toml"), "w") as sql_output:
                sql_output.write(
                    constants.config_project_toml.format(
                        ...
                        application_package_distribution=options.init.package_distribution,
                    )
                )
            LoggingManager.display_message(
        ...
        if os.path.isdir(os.path.join("src", "provider")) == False:
            ...
            os.makedirs(
                os.path.join("src", "provider", "sql", "preinstall"), exist_ok=True
            )
            os.makedirs(
                os.path.join("src", "provider", "sql", "postinstall"), exist_ok=True
            )

            with open(
                os.path.join("src", "provider", "sql", "app_pkg_definition_01.sql"), "w"
            ) as sql_output:
                sql_output.write(
            ...
            LoggingManager.display_message(
                ...
                os.path.join(current_directory, "src", "provider"),
            )
        else:
            LoggingManager.display_message(
                "w_already_exists", os.path.join(current_directory, "src", "provider")
            )
        ...
        if os.path.isdir(os.path.join("src", "consumer")) == False:
            ...
            with open(
                os.path.join("src", "consumer", "sql", "app_setup_definition_01.sql"),
                "w",
            ) as sql_output:
                sql_output.write(
            ...
            with open(
                os.path.join("src", "consumer", "resources", "manifest.yml"), "w"
            ) as sql_output:
                sql_output.write(
            ...
            with open(
                os.path.join("src", "consumer", "resources", "README.md"), "w"
            ) as sql_output:
                sql_output.write(
            ...
            LoggingManager.display_message(
                ...
                os.path.join(current_directory, "src", "consumer"),
            )
        else:
            LoggingManager.display_message(
                "w_already_exists", os.path.join(current_directory, "src", "consumer")
            )
