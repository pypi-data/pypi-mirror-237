...
from subfork.api.task import sanitize_data
...
class FileWatcher(StoppableThread):
    ...
    def __init__(self, filepath, callback):
        ...
        self.wait_time = 5  # wait time in secs
    ...
    def has_changed(self):
        ...
        if (
            ...
            and current_checksum != self.last_checksum
        ):
    ...
    def run(self):
        ...
        while self.running.is_set():
            ...
            if os.path.exists(self.filepath):
                ...
                if self.has_changed():
                    self.callback()
                # automatically restart workers every 24 hours
                elif self.elapsed_time() >= util.HOURS_24:
                    self.callback()
            else:
...
def import_function(func_name):
    """
    Imports a python function from an import path.
    ...
    try:
        ...
        log.debug("module not found: %s" % err)
...
def create_workers(client, queue_name, func_name, limit=1, wait_time=config.WAIT_TIME):
    ...
    try:
        ...
        log.error("error creating workers: %s" % e)
        return False
...
def restart_all():
    """
    Restarts all workers by restarting main thread with
    original input args.
    """
...
def run_workers(client, worker_configs, autorestart=config.AUTO_RESTART_WORKERS):
    ...
    if autorestart:
        watcher = FileWatcher(config.config_file, restart_all)
        watcher.start()
...
def stress(t=5):
    """Stress test worker. Available params:

    :param t: time in secs to run.
    :returns: worker message.
    """
    log.info("subfork.worker.stress:data: t=%s" % t)
    ts = time.time()
    n, x = 1, 2
...
def test(**kwargs):
    """Test worker function that prints task data.

    :param t: time in secs to sleep.
    :param error: raise exception with error message.
    :returns: worker message.
    """
    log.info("subfork.worker.test:data: kwargs=%s" % kwargs)
    time.sleep(kwargs.get("t", 1))
    if kwargs.get("error"):
        raise Exception(kwargs["error"])
    return "greetings from %s" % sample.hostname


def throw(**kwargs):
    """Test worker function that throws an exception."""
