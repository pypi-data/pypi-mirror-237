...
def openi_dataset_to_env(data_url, data_dir):
    ...
    try:     
        ...
        f'{emoji.emojize(":thumbs_up:")} Successfully Download {data_url} to {data_dir}.'
    except Exception as e:
        f'❌ moxing download {data_url} to {data_dir} failed.'
    return 
...
def openi_multidataset_to_env(multi_data_url, data_dir):
    ...
    for i in range(len(multi_data_json)):
        ...
        try:
            ...
            f'{emoji.emojize(":thumbs_up:")} Successfully Download {multi_data_json[i]["dataset_url"]} to {path}.'
        except Exception as e:
            f'❌ moxing download {data_umulti_data_json[i]["dataset_url"]} to {path} failed.'
    return   
...
def pretrain_to_env(pretrain_url, pretrain_dir):
    ...
    for i in range(len(pretrain_url_json)):
        ...
        try:
            ...
            f'{emoji.emojize(":thumbs_up:")} Successfully Download {pretrain_url_json[i]["model_url"]} to {modelfile_path}.'
        except Exception as e:
            f'❌ moxing download {pretrain_url_json[i]["model_url"]} to {modelfile_path} failed.'
    return          
...
def obs_copy_file(obs_file_url, file_url):
    ...
    try:
        ...
        f'{emoji.emojize(":thumbs_up:")} Successfully Download {obs_file_url} to {file_url}.'
    except Exception as e:
        f'❌ moxing download {obs_file_url} to {file_url} failed.'
    return    
...
def obs_copy_folder(folder_dir, obs_folder_url):
    ...
    try:
        ...
        f'{emoji.emojize(":thumbs_up:")} Successfully Download {folder_dir} to {obs_folder_url}.'
    except Exception as e:
        f'❌ moxing download {folder_dir} to {obs_folder_url} failed.'
    return     
...
def c2net_multidataset_to_env(multi_data_url, data_dir):
    ...
    for i in range(len(multi_data_json)):
        ...
        try:
            ...
            f'{emoji.emojize(":thumbs_up:")} Successfully Download {multi_data_json[i]["dataset_url"]} to {zipfile_path}.'
            ...
            if zipfile_path.endswith(".tar.gz"):
                ...
                f'❌ The dataset is not in tar.gz or zip format!'

        except Exception as e:
            f'❌ moxing download {multi_data_json[i]["dataset_url"]} to {zipfile_path} failed.'
    return       