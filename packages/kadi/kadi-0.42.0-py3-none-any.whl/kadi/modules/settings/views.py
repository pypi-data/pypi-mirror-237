...
@login_required
def oauth2_service_login(provider):
    """Endpoint to initiate the OAuth2 flow to connect a service/provider."""
    ...
    redirect_uri = url_for("settings.oauth2_service_authorize", provider=provider)
...
@login_required
def oauth2_service_authorize(provider):
    ...
    try:
        token_data = getattr(oauth_registry, provider).authorize_access_token()
    except Exception as e:
        current_app.logger.exception(e)
