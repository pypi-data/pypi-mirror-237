...
def manage_users(qparams):
    ...
    new_user_form = NewUserForm(suffix="register")
    ...
    if qparams["tab"] == "merge" and merge_users_form.validate_on_submit():
        ...
        if new_user_form.validate():
            new_username = new_user_form.username.data
            new_password = random_alnum()

            if LocalProvider.register(
                displayname=new_user_form.displayname.data,
                ...
                # password value.
                new_user_form = NewUserForm(suffix="register", formdata=None)
            else:
