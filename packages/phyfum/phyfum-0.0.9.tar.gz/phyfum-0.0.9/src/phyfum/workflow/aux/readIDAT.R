...
option_list <- list(
  ...
  make_option(
    ...
    type = "character",
    default = "csv$",
    help = "Regex pattern to find CSV files [default: %default]')"
  ),
  make_option(
    c("--output", "-o"),
    type = "character",
...
# input_dir <- "D:/Lab/Estancia/Phyllogenomics/package/data/"
# mc.cores <- 1
# pattern <- "csv$"
# output <- "out"
...
library(pacman)
p_load(conumee, minfi, parallel, tibble, tidyr, dplyr, stringr, data.table, gtools, GenomicRanges, IlluminaHumanMethylationEPICanno.ilm10b4.hg19)
...
targets = do.call(rbind, lapply(files, function(x) read.csv(x, strip.white = TRUE, stringsAsFactors = FALSE)))
...
isAControl = pData(preprocessedData)$Group == "Normal"
...
CNV.getCalls = function (x, n_solution = NA, method = "best", alterationThreshold = 0.25) {
  ...
  theCalls = x$dataBySegment[, .(chromosome,
    ...
    mutate(across(
      c(log2_ratio, relative_copy_number, absolute_copy_number),
      round,
      digits = 3
    )) %>%
    mutate(
