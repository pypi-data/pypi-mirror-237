...
class Video(Media):
    ...
    @pre_operate(suffix='.mp4')
    async def face_detection(self, model_name: str = 'haarcascade_frontalface_alt2', proportion: float = 0.9, worker_nodes: int = None, **kwargs) -> bool:
        """
        视频逐帧检测人脸
        ...
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(None, functools.partial(face_detection, input_path, model_name, total_frames, proportion, worker_nodes))
    ...
    @staticmethod
    async def composite(output_path: str, media_list: list, audio_list: list = None, canvas: tuple = None):
        """
        视频合成
        :param output_path: 输出路径
        ...
        try:
            ...
            if isinstance(audio_list, list):
                ...
                temp_path_list.append(temp_file_path)
                temp_audio = await Audio.composite(temp_file_path, audio_list)
            ...
            def _composite():
                ...
                if temp_audio:
                    final_clip = final_clip.set_audio(AudioFileClip(temp_audio.input_path))
