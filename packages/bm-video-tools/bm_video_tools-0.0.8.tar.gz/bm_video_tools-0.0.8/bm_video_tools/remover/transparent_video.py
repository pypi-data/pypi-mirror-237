...
# 处理超时
def _deal_timeout(proc_list, output_proc):
...
def _check_timeout(start_time, timeout, proc_list, output_proc):
    if (timeout is not None) and (time.time() - start_time > timeout):
        _deal_timeout(proc_list, output_proc)
        raise TimeoutError("transparent timeout")


# 等待进程结束（校验超时）
def _wait_proc(start_time, timeout, proc_list, output_proc):
    if timeout is not None:
        while not all(not proc.is_alive() for proc in proc_list):
...
def matte_key(input_path, matte_path, model_name, worker_nodes, gpu_batch_size, total_frames, frame_rate, timeout):
    ...
    producer = multiprocessing.Process(target=_producer, args=(input_path, worker_nodes, gpu_batch_size, total_frames, frames_dict))
        ...
        for wi in range(worker_nodes):
            ...
            for frame in frames:
                ...
                if frame_counter >= total_frames:
                    _wait_proc(start_time, timeout, process_list, proc)
