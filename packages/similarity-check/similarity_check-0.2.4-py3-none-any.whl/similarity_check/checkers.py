...
class sentence_tranformer_checker():
    ...
    def save_targets(self, targets_save_path):
        ...
        for encoded_target_name, encoded_target in self.encoded_targets_dict.items():  
            encoded_target_dir = os.path.join(targets_save_path, 'encoded_targets_dict')
        ...
        with open(os.path.join(targets_save_path, 'targets.json'), 'w') as f:
            json.dump(self.targets, f)
            
        with open(os.path.join(targets_save_path, 'group_ids.json'), 'w') as f:
            json.dump({'group_ids': self.group_ids}, f)
        
        self.target_df.to_csv(os.path.join(targets_save_path, 'target_df.csv'), index=False)    
        
        
    def load_targets(self, targets_save_path):
        try:
            with open(os.path.join(targets_save_path, 'targets.json'), 'r') as f:
                self.targets = json.load(f)
                
            with open(os.path.join(targets_save_path, 'group_ids.json'), 'r') as f:
                self.group_ids = json.load(f)['group_ids']
