...
def model_create(name: str):
    ...
    try:
        ...
        if not os.path.exists(destination_file_path):
            ...
            updated_content = handler_stub_content.replace("{{name}}", name.capitalize())
...
def migrate_create(message: str = typer.Option("", "--message", "-m", help="Message option"),):
...
@app.command("serve")
def serve(port: int = typer.Option(8000, "--port", "-p", help="Set port number")):
