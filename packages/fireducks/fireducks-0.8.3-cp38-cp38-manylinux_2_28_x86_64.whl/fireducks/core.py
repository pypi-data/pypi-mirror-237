...
def parse_fireFIREDUCKS_FLAGS(flags, namespace=None):
    ...
    # Fast fallback is allowd when True.
...
class _FireDucksOptions:
    ...
    def _configure(self):
        ...
        _, unknowns = parse_fireFIREDUCKS_FLAGS(flags, self)
...
def _evaluate(values: List[fire.Value], options: EvalOptions = None):
    ...
    try:
        return fire.evaluate(values, wrapper, package="fireducks.pandas")
