...
class SurrogateJob(ABC):
    ...
    @abstractmethod
    def start(self, job_name, num_cs2_suffix = ""):
...
class SlurmSurrogateJob(SurrogateJob):
    ...
    def _terminate_srun_job(self, job_id) -> bool:
        ...
        cmd = f"scancel --signal 15 {job_id}"
        try:
    ...
    def start(self, job_name, num_cs2_suffix = ""):
        ...
        job_script = _get_surrogate_script(job_name)
        node_list = ""
        if 'SLURM_JOB_NODELIST' in os.environ:
            node_list = f"-w {os.environ['SLURM_JOB_NODELIST']}"
        try:
            cmd = f"srun {node_list} --job-name {job_name}{num_cs2_suffix} python3 {job_script}"
            surrogate_job = subprocess.Popen(shlex.split(cmd), stdout=subprocess.PIPE,
                stderr=subprocess.PIPE)
        except FileNotFoundError as exp:
            self._logger.error(f"Cmd {cmd} caught exception {exp}. Skip starting surrogate job")
            return

        # Obtain the job id for this srun, for stopping this process later on.
        surrogate_job_id = self._get_srun_job_id(f"{job_name}{num_cs2_suffix}")
        ...
        for _, (surrogate_job, surrogate_job_id) in self._jobs.items():
            if surrogate_job:
                ...
                if not srun_terminated:
                    self._logger.debug(f"send SIGNTERM to pid {surrogate_job.pid}")
...
def get_surrogate_job(logger):
    ...
    if 'SLURM_JOB_ID' in os.environ:
        ...
        return SlurmSurrogateJob(logger)
    else:
        return None
