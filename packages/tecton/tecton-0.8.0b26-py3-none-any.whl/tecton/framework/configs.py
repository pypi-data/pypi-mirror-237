    ...
    **Do not instantiate this class directly.** Use :py:func:`tecton.pandas_batch_config` instead.
    ...
    def __init__(
        ...
        :param supports_time_filtering: When set to ``True``, the Data Source Function must take the ``filter_context``
                                    ...
                                    the Data Source Function with the :class:`tecton.FilterContext`, which has the ``start_time`` and
                                    ``end_time`` set.
        :param timestamp_field: Must be set if ``supports_time_filtering`` is True. This is used to filter reads in
                                    the data source.
...
class Aggregation(object):
    ...
    """Time window spec that uses both the window duration and offset."""
    ...
    def __init__(
        ...
        column: str,
        function: Union[str, AggregationFunction],
        time_window: Union[TimeWindow, datetime.timedelta],
        name: Optional[str] = None,
    ):
        self.column = column
        self.function = function
        if isinstance(time_window, datetime.timedelta):
            self.time_window = TimeWindowSpec(window_start=-time_window, window_end=datetime.timedelta(seconds=0))
        elif isinstance(time_window, TimeWindow):
            self.time_window = TimeWindowSpec.from_args_proto(time_window._to_proto())
        self.name = name
    ...
    def _to_proto(self, aggregation_interval: datetime.timedelta, is_continuous: bool):
        ...
        proto.time_window.CopyFrom(self.time_window.to_args_proto())

        if self.name:
            proto.name = self.name
        else:
            time_window = TimeWindowSpec.from_args_proto(proto.time_window)
            ...
            proto.name = feature_view_utils.construct_aggregation_output_feature_name(
                ...
                is_continuous,
                time_window.window_duration_string(),
                time_window.offset_string(),
            )
        return proto
