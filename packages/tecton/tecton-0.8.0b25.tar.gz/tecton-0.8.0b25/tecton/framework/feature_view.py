...
from tecton_core.compute_mode import default_dialect
...
class FeatureView(base_tecton_object.BaseTectonObject):
    ...
    @sdk_decorators.sdk_public_method(requires_validation=True)
    def print_query_schema(self) -> None:
        """Pretty prints the query schema. See query_schema() for more information."""

        query_schema = self.query_schema()
        print(type_utils.schema_pretty_str(query_schema))

    @sdk_decorators.sdk_public_method(requires_validation=True)
    def query_schema(self) -> List[types.Field]:
        """Returns the query schema for the feature view.

        The query schema may be explicitly defined (via the schema parameter), and in that case this method will return
...
class MaterializedFeatureView(FeatureView):
    ...
    def _derive_schemas_with_specs(
        ...
        has_legacy_snowflake_hacks = compute_mode.get_compute_mode() == compute_mode.ComputeMode.SNOWFLAKE

        has_explicit_schema = self._args.materialized_feature_view_args.HasField("schema")
        explicit_schema = self._args.materialized_feature_view_args.schema if has_explicit_schema else None
        ...
        if self._args.options.get("DEV_ENABLE_COMPACTION") == "true":
            online_batch_table_format = compaction_utils.compute_batch_table_schema_for_aggregate_feature_view(
                view_schema, self._args.materialized_feature_view_args.aggregations
            )
    ...
    def _maybe_derive_view_schema(
        ...
        if has_push_source:
            assert compute_mode.get_compute_mode() != compute_mode.ComputeMode.SNOWFLAKE
            return self._derive_push_source_schema(transformation_specs, data_source_specs)
        elif compute_mode.get_compute_mode() == compute_mode.ComputeMode.SNOWFLAKE:
            return snowflake_api.derive_view_schema_for_feature_view(
                self._args, transformation_specs, data_source_specs
            )
        elif compute_mode.get_compute_mode() == compute_mode.ComputeMode.SPARK:
    ...
    @sdk_decorators.sdk_public_method(
        ...
        supports_skip_validation=(compute_mode.ComputeMode.SPARK, compute_mode.ComputeMode.DUCK_DB),
    )
    def get_historical_features(
        ...
        if compute_mode.get_compute_mode() == compute_mode.ComputeMode.SNOWFLAKE and (
        ...
        if mock_inputs or not self._is_valid:
            ...
            mock_data_sources = mock_source_utils.convert_mock_inputs_to_mock_sources(
                dialect, feature_definition, mock_inputs
            )
        else:
            feature_definition = self._feature_definition
    ...
    def run(
        ...
        **mock_inputs_kwargs: Union[pandas.DataFrame, pyspark_dataframe.DataFrame],
        ...
        mock_data_sources = mock_source_utils.convert_mock_inputs_to_mock_sources(
            dialect, feature_definition, resolved_mock_inputs
        )
    ...
    def aggregations(self) -> Optional[configs.Aggregation]:
        ...
        for agg in aggregate_features:
            time_window = TimeWindowSpec.from_data_proto(agg.time_window)
            aggregation_list.append(
                configs.Aggregation(
                    ...
                    time_window=configs.TimeWindow(window_size=time_window.window_duration, offset=time_window.offset),
...
class BatchFeatureView(MaterializedFeatureView):
    ...
    def __init__(
        ...
        *,
        ...
        online_store: Optional[Union[configs.DynamoConfig, configs.RedisConfig, configs.BigtableConfig]] = None,
        ...
        run_query_validation: Optional[bool] = None,
        batch_compute_mode: Optional[Union[str, compute_mode.BatchComputeMode]] = None,
        ...
        args = _build_materialized_feature_view_args(
            ...
            schema=schema,
            run_query_validation=run_query_validation,
...
def batch_feature_view(
    *,
    ...
    :param batch_compute_mode: The compute engine to use for batch jobs for this Feature View. If not set, a valid
        default will be selected based on the cluster configuration and transformation modes. Valid values are "spark,"
        "snowflake," and "tecton" (for Tecton-managed compute).
    :param options: Additional options to configure the Feature View. Used for advanced use cases and beta features.
    :param schema: The output schema of the Feature View transformation. If provided and ``run_query_validations=True``,
        then Tecton will validate that the Feature View matches the expected schema.
    :param run_query_validation: If `True`, Tecton will execute the Feature View transformations during tecton plan/apply
...
class StreamFeatureView(MaterializedFeatureView):
    ...
    def __init__(
        ...
        *,
        ...
        elif data_source_type == data_source_type_pb2.DataSourceType.PUSH_WITH_BATCH:
            default_batch_trigger = BatchTriggerType.MANUAL
        else:
...
def _build_materialized_feature_view_args(
    ...
    online_store: Optional[Union[configs.DynamoConfig, configs.RedisConfig, configs.BigtableConfig]],
    ...
    if isinstance(batch_compute_mode, str):
        batch_compute_mode = compute_mode.BatchComputeMode.from_string(batch_compute_mode)
...
def _create_secondary_key_output_columns(
    ...
    sorted_time_windows = sorted(
        (agg.time_window for agg in aggregations),
        key=lambda tw: tw.to_tuple(),
        reverse=True,
    )
    # eliminates duplicates of time windows for secondary key output columns
    secondary_key_time_windows = _deduplicate_time_windows(sorted_time_windows)
    secondary_key_output_columns = [
        feature_view__args_pb2.SecondaryKeyOutputColumn(
            time_window=feature_view__args_pb2.TimeWindow(
                window_duration=time_utils.timedelta_to_proto(window.window_duration),
                offset=time_utils.timedelta_to_proto(window.offset),
            ),
...
def _deduplicate_time_windows(windows: List[TimeWindowSpec]):
    seen_time_windows = set()
    deduplicated_time_windows = []
    for tw in windows:
        key = tw.to_tuple()
        if key not in seen_time_windows:
            seen_time_windows.add(key)
            deduplicated_time_windows.append(tw)
    return deduplicated_time_windows
