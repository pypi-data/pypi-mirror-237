...
def convert_mock_inputs_to_mock_sources(
    dialect: Dialect, fdw: FeatureDefinitionWrapper, mock_inputs: Dict[str, Union[pandas.DataFrame, DataFrame]]
) -> Dict[str, NodeRef]:
    ...
    mock_data_sources = {
        input_ds_id_map[key]: NodeRef(
            UserSpecifiedDataNode(dialect, data_frame.TectonDataFrame._create(mock_inputs[key]))
        )
