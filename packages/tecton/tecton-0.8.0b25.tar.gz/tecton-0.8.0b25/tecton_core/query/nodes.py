...
class StagingNode(QueryNode):
    ...
    def _to_staging_query_sql(self) -> str:
        ...
        aliased_input = self.input_node.node.__class__.__name__ + "_" + uuid.uuid4().hex[:16]
        sql = (
            self.func.query()
...
class StagedTableScanNode(QueryNode):
    ...
    @classmethod
    def from_staging_node(cls, dialect: Dialect, query_node: StagingNode) -> QueryNode:
        return cls(
            dialect=dialect,
    ...
    def _get_aggregations(self, window_order_col: str, partition_cols: List[str]) -> List[Term]:
        ...
        for feature in time_aggregation.features:
            ...
            if feature.HasField("window"):
                ...
                time_window = TimeWindowSpec.from_data_proto(feature.time_window)
            ...
            if callable(aggregation_plan):
                aggregation_plan = aggregation_plan(ANCHOR_TIME, feature.function_params)
...
class PartialAggNode(QueryNode):
    ...
    def _get_partial_agg_columns_and_names(self) -> List[Tuple[Term, str]]:
        ...
        for feature in time_aggregation.features:
            aggregation_plan = AGGREGATION_PLANS.get(feature.function)
            if callable(aggregation_plan):
                aggregation_plan = aggregation_plan(time_aggregation.time_key, feature.function_params)
...
class AddAnchorTimeNode(QueryNode):
    ...
    @staticmethod
    def for_feature_definition(dialect: Dialect, fd: FeatureDefinitionWrapper, input_node: NodeRef) -> NodeRef:
...
class RespectFeatureStartTimeNode(QueryNode):
    ...
    def for_anchor_time_column(
        cls, dialect: Dialect, input_node: NodeRef, anchor_time_col: str, fdw: FeatureDefinitionWrapper
        ...
        #   feature values.
        return cls(dialect, input_node, anchor_time_col, ts, fdw.features, fdw.get_feature_store_format_version)
    ...
    def _to_query(self) -> pypika.queries.QueryBuilder:
        ...
        if self.batch_schedule_seconds == 0 or self.is_stream:
            ...
            effective_timestamp = self.func.from_unixtime(
                self.func.to_unixtime(timestamp_col)
                - (self.func.to_unixtime(timestamp_col) % self.batch_schedule_seconds)
