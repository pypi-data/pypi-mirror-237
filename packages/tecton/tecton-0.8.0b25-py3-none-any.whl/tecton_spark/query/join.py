...
class AsofJoinFullAggSparkNode(SparkExecNode):
        ...
        if self.fdw.aggregation_secondary_key:
            for secondary_key_output in self.fdw.fv_spec.secondary_key_rollup_outputs:
                aggregations.append(
                    ...
                    .over(
                        self._generate_aggregation_window_spec(
                            secondary_key_output.time_window.window_start,
                            secondary_key_output.time_window.window_end,
                            join_spec,
                        )
                    )
                    .alias(
    ...
    def _generate_aggregation_window_spec(
        ...
        return join_spec.get_range_between_window_spec(
            self.fdw.earliest_anchor_time_from_window_start(window_start) * 2 - 1,
            convert_timedelta_for_version(window_end, self.fdw.get_feature_store_format_version) * 2 - 1,
        )
...
class AsofSecondaryKeyExplodeSparkNode(SparkExecNode):
    """
    This node explodes a spine that misses the secondary key, into a spine that has the secondary key. This explodation
    is needed in two scenarios:
...
class AggregationSecondaryKeyRollupSparkNode(SparkExecNode):
    ...
    def _window_to_features_map(self) -> Dict[str, List[str]]:
        ...
        for secondary_key_output in self.fdw.fv_spec.secondary_key_rollup_outputs:
            ...
            features = [
                ...
                if TimeWindowSpec.from_data_proto(agg_feature.time_window) == secondary_key_output.time_window
            ]
    ...
    def _temp_struct_column_name(self, window: TimeWindowSpec) -> str:
        return f"{self.TECTON_TEMP_STRUCT_PREFIX}_{window.to_string()}"

    def _temp_indictor_column_name(self, window: TimeWindowSpec) -> str:
    ...
    def _extract_features_columns_from_struct_columns(
        ...
        for agg_feature in self.fdw.trailing_time_window_aggregation.features:
            if TimeWindowSpec.from_data_proto(agg_feature.time_window) == window:
                df = df.withColumn(
                    agg_feature.output_feature_name,
