...
def maybe_warn_incorrect_time_range_size(
    ...
    if fd.is_temporal_aggregate:
        ...
        if aggregation_level == AGGREGATION_LEVEL_FULL:
            max_aggregation_window = max(
                (
                    -TimeWindowSpec.from_data_proto(f.time_window).window_start
                    for f in fd.trailing_time_window_aggregation.features
                )
            )
...
def _build_run_batch_querytree(
    ...
    if fd.is_temporal:
        qt = build_materialization_querytree(
            dialect, fd, for_stream=False, feature_data_time_limits=feature_time_limits_aligned
        )
        ...
        if aggregation_level == AGGREGATION_LEVEL_PARTIAL:
            ...
            if fd.is_continuous:
                ...
                drop = []
            return RenameColsNode(dialect, qt, mapping=renames, drop=drop).as_ref()
...
def _querytree_run_batch(
    ...
    qt = _build_run_batch_querytree(dialect, fd, feature_end_time, feature_time_limits_aligned, aggregation_level)
