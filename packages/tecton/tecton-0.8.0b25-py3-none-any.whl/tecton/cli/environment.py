...
def list(environment_id: Optional[str] = None, name: Optional[str] = None):
...
@click.option("--requirements", help="Path to requirements.txt file", required=True, type=click.Path(exists=True))
@click.option(
    "--output-file",
...
@click.option("--verbose", help="Print debug information", is_flag=True)
def resolve_requirements(
    ...
    """Resolve Dependencies for a given requirements.txt"""
...
@click.option("-p", "--python-version", help="Python Version for the environment", required=False)
@click.option(
...
def create(
    ...
    Parameters:
       ...
       verbose (bool, optional): Activates debug mode. When set to True, the command provides detailed output, including subcommands and diagnostic information, to assist in troubleshooting.
    """
    ...
    if image_uri is not None:
        ...
        resp = _create_environment_with_requirements(
            name, description, requirements_path, _python_version, verbose=verbose
        )
        if resp:
            _display_environments([resp.remote_environment])
            printer.safe_print(
...
def _list_environments(environment_identifier: EnvironmentIdentifier):
    try:
...
def _display_requirements(
    ...
    if requirements_path is not None:
        with requirements_path.open("r") as file:
            cleaned_lines = [line for line in file.readlines()]
    elif requirements_str is not None:
        cleaned_lines = [line for line in requirements_str.split("\n")]
