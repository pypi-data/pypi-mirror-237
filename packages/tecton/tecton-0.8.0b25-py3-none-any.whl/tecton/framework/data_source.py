...
class DataSource(base_tecton_object.BaseTectonObject):
    ...
    def get_dataframe(
        ...
        *,
        ...
        # TODO(danny): make this work for file sources in DuckDB mode as well
        if compute_mode.get_compute_mode() == compute_mode.ComputeMode.SNOWFLAKE and conf.get_bool(
            "USE_DEPRECATED_SNOWFLAKE_RETRIEVAL"
        ):
            ...
            return querytree_api.get_dataframe_for_data_source(
                default_dialect(), self._spec, start_time, end_time, apply_translator
            )
...
class BatchSource(DataSource):
    ...
    def _derive_schemas(self, indentation_level: int) -> None:
        if (
            compute_mode.get_compute_mode() == compute_mode.ComputeMode.DUCK_DB
            or sql_compat.dialect() == sql_compat.Dialect.DUCKDB
        ):
            # DuckDB only supports explicit FV schemas & doesn't require data source schema
            # inference.
            # TODO(danny): Change this once we support explicit data source schemas
