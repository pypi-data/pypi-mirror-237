...
class OnlineBatchTablePart:
    ...
    def from_proto(cls, proto: schema_pb2.OnlineBatchTablePart) -> "OnlineBatchTablePart":
        return cls(
            ...
            time_window=TimeWindowSpec.from_data_proto(proto.time_window),
...
class MaterializedFeatureViewSpec(FeatureViewSpec):
    ...
    aggregate_features: Tuple[feature_view__data_pb2.AggregateFeature, ...] = attrs.field(
        # TODO(TEC-16256): Remove after bug causing disparity is fixed
        metadata={utils.LOCAL_REMOTE_DIVERGENCE_ALLOWED: True}
    )
            ...
            if aggregation_secondary_key:
                secondary_key_rollup_outputs = tuple(
                    SecondaryKeyOutputColumn(
                        name=output_col.name, time_window=TimeWindowSpec.from_data_proto(output_col.time_window)
                    )
    ...
    def from_args_proto(
        ...
        if is_aggregate:
            ...
            earliest_window_start = min(
                [
                    TimeWindowSpec.from_args_proto(agg.time_window).window_start
            ...
            if aggregation_secondary_key:
                secondary_key_rollup_outputs = tuple(
                    SecondaryKeyOutputColumn(
                        name=output_col.name, time_window=TimeWindowSpec.from_args_proto(output_col.time_window)
                    )
    ...
    for part in schemas.online_batch_table_format.online_batch_table_parts:
        ...
        output.append(
            OnlineBatchTablePart(
                ...
                time_window=TimeWindowSpec.from_data_proto(part.time_window),
...
def get_aggregate_features_from_feature_view_args(
    ...
    for agg_args_proto in proto.materialized_feature_view_args.aggregations:
        time_window_spec = TimeWindowSpec.from_args_proto(agg_args_proto.time_window)
        agg_data_proto = create_aggregate_features(
            agg_args_proto, proto.materialized_feature_view_args.aggregation_interval, is_continuous, time_window_spec
        )
...
def create_aggregate_features(
    ...
    return feature_view__data_pb2.AggregateFeature(
        ...
        time_window=time_window_pb2.TimeWindow(
            window_start=time_utils.timedelta_to_proto(time_window.window_start),
            window_end=time_utils.timedelta_to_proto(time_window.window_end),
        ),
