...
def _build_datasource_input_querynodes(
    ...
    for input_name, node in ds_inputs.items():
        ...
        # TODO(felix): Wrap this in a StagingNode.
...
def build_pipeline_querytree(
    ...
    inputs_map = _build_datasource_input_querynodes(dialect, fdw, for_stream, feature_data_time_limits)
...
def build_materialization_querytree(
    ...
    tree = build_pipeline_querytree(dialect, fdw, for_stream, feature_data_time_limits)
    if for_stream:
        watermark = _get_stream_watermark(fdw)
        if watermark:
            tree = StreamWatermarkNode(dialect, tree, fdw.time_key, watermark).as_ref()
    if enable_feature_metrics:
        tree = MetricsCollectorNode(dialect, tree).as_ref()
    ...
    if fdw.is_temporal:
        ...
        if not for_stream:
            ...
            if not use_timestamp_key:
                tree = AddAnchorTimeNode.for_feature_definition(dialect, fdw, tree)
        ...
        if use_timestamp_key:
            tree = ConvertEpochToTimestampNode(
                dialect, tree, {ANCHOR_TIME: fdw.get_feature_store_format_version}
            ).as_ref()
            tree = RenameColsNode(dialect, input_node=tree, mapping={ANCHOR_TIME: fdw.time_key}).as_ref()
    else:
...
def build_get_features(
    ...
    if from_source is False:
        ...
        return OfflineStoreScanNode(
            dialect=dialect, feature_definition_wrapper=fdw, partition_time_filter=feature_data_time_limits
        ).as_ref()
    else:
        # TODO(sanika) remove conf.get_or_none("ATHENA_TEST_ENABLED") when all athena tests have been migrated to use local materialization
        if conf.get_or_none("SQL_DIALECT") == "athena" and not conf.get_or_none("ATHENA_TEST_ENABLED"):
...
def build_get_full_agg_features(
    ...
    if fdw.aggregation_secondary_key:
        ...
        spine = AggregationSecondaryKeyExplodeNode(
            dialect=dialect,
            input_node=RenameColsNode(dialect=dialect, input_node=partial_aggs, drop=cols_to_drop).as_ref(),
            fdw=fdw,
        ).as_ref()
    else:
        cols_to_drop = list(set(partial_aggs.columns) - {*list(fdw.join_keys), ANCHOR_TIME})
        spine = RenameColsNode(dialect, partial_aggs, drop=cols_to_drop).as_ref()
    ...
    if respect_feature_start_time and fdw.feature_start_timestamp:
        full_aggregation_node = RespectFeatureStartTimeNode.for_anchor_time_column(
            dialect, full_aggregation_node, ANCHOR_TIME, fdw
        ).as_ref()
    ...
    qt = ConvertEpochToTimestampNode(
        dialect, full_aggregation_node, {ANCHOR_TIME: fdw.get_feature_store_format_version}
    ).as_ref()
    ...
    qt = RenameColsNode(dialect, qt, drop=[ANCHOR_TIME]).as_ref()
...
def build_spine_join_querytree(
    ...
    if fdw.timestamp_key is not None and spine_time_field != fdw.timestamp_key:
        spine_node = RenameColsNode(dialect, spine_node, mapping={spine_time_field: fdw.timestamp_key}).as_ref()
    if any(jk[0] != jk[1] for jk in dac.join_keys):
        spine_node = RenameColsNode(
            dialect, spine_node, mapping={jk[0]: jk[1] for jk in dac.join_keys if jk[0] != jk[1]}
        ).as_ref()
    ...
    if fdw.is_temporal or fdw.is_feature_table:
        ...
        augmented_spine = _augment_spine_for_window_aggregation(dialect, fdw, spine_node, partial_agg_node)
        ...
        if fdw.aggregation_secondary_key:
            ...
            full_agg_node = RenameColsNode(dialect, full_agg_node, drop=[TECTON_UNIQUE_ID_COL]).as_ref()

        if fdw.feature_start_timestamp:
            full_agg_node = RespectFeatureStartTimeNode.for_anchor_time_column(
                dialect, full_agg_node, ANCHOR_TIME, fdw
            ).as_ref()
        ret = _rename_feature_columns_and_drop_non_feature_columns(
            dialect, dac, full_agg_node, use_namespace_feature_prefix
        )
        ...
        ret = build_feature_set_config_querytree(
            dialect, fsc, spine_node, spine_time_field, from_source, use_namespace_feature_prefix
        )
    else:
        raise NotImplementedError
    if fdw.timestamp_key is not None and spine_time_field != fdw.timestamp_key:
        ret = RenameColsNode(dialect, ret, {fdw.timestamp_key: spine_time_field}).as_ref()
    if any(jk[0] != jk[1] for jk in dac.join_keys):
        ret = RenameColsNode(dialect, ret, {jk[1]: jk[0] for jk in dac.join_keys if jk[0] != jk[1]}).as_ref()
    return ret
...
def _build_wild_fv_subtree(
    ...
    for dac in fv_dacs:
        ...
        # feature view tree.
        subspine = SelectDistinctNode(dialect, spine_node, [*subspine_join_keys, spine_time_field]).as_ref()
        fvtree = build_spine_join_querytree(dialect, dac, subspine, spine_time_field, from_source)
        ...
        if newtree is None:
            ...
            newtree = WildcardJoinNode(dialect, newtree, fvtree, join_cols=join_cols).as_ref()
    return newtree
...
def _build_odfv_subtree(
    ...
    newtree = MultiOdfvPipelineNode(
        dialect, newtree, feature_definitions_namespaces, use_namespace_feature_prefix
    ).as_ref()
...
def build_feature_set_config_querytree(
    ...
    if wildcard_dacs:
        newtree = _build_wild_fv_subtree(dialect, spine_node, wildcard_dacs, spine_time_field, from_source)
    else:
        newtree = spine_node
    ...
    if normal_fv_dacs:
        newtree, internal_cols = _build_standard_fv_subtree(
            dialect, newtree, normal_fv_dacs, spine_time_field, from_source
        )

    if odfv_dacs:
        newtree = _build_odfv_subtree(dialect, newtree, odfv_dacs, use_namespace_feature_prefix)

    # drop all internal cols
    if len(internal_cols) > 0:
        newtree = RenameColsNode(dialect, newtree, drop=list(internal_cols)).as_ref()
...
def _build_spine_query_tree_temporal_or_feature_table(
    ...
    base = build_get_features(dialect, fdw, from_source=from_source)
    batch_schedule_seconds = 0 if fdw.is_feature_table else fdw.batch_materialization_schedule.in_seconds()
    ...
    if fdw.wildcard_join_key is not None and fdw.wildcard_join_key not in spine_node.columns:
        ...
        spine_node = AsofSecondaryKeyExplodeNode(
            dialect, spine_node, fdw.timestamp_key, base_copy, EFFECTIVE_TIMESTAMP, fdw
        ).as_ref()
    ...
    if fdw.serving_ttl is not None:
        base = RespectTTLNode(
            dialect, base, fdw.timestamp_key, expiration_timestamp_col, join_prefixed_feature_names
        ).as_ref()
    # remove anchor cols/dupe timestamp cols
    return RenameColsNode(dialect, base, mapping=rename_map, drop=cols_to_drop).as_ref()
...
def _augment_spine_for_window_aggregation(
    ...
    if is_wildcard_join_key_not_in_spine or is_secondary_key_agg:
        ...
        if is_secondary_key_agg:
            augmented_spine = AddUniqueIdNode(dialect, augmented_spine).as_ref()
...
def _rename_feature_columns_and_drop_non_feature_columns(
    dialect: Dialect, dac: FeatureDefinitionAndJoinConfig, node: NodeRef, use_namespace_feature_prefix: bool = True
) -> NodeRef:
    rename_map: Dict[str, Optional[str]] = {}
    ...
    return RenameColsNode(dialect, node, mapping=rename_map, drop=cols_to_drop).as_ref()


def _build_aggregation_group_data_node(
    dialect: Dialect, aggregation_groups: Tuple[compaction_utils.AggregationGroup, ...]
) -> PythonDataNode:
    ...
    return PythonDataNode(dialect, data=data, columns=columns)


def wip_build_compaction_query(
    dialect: Dialect, fdw: feature_definition_wrapper.FeatureDefinitionWrapper, compaction_job_end_time: datetime
) -> NodeRef:
    """This is intended to be a WIP query builder for use in tests. This is not intended to be fully functional."""
    fv_pipeline = build_pipeline_querytree(dialect, fdw, for_stream=False, feature_data_time_limits=None)
    aggregation_groups = compaction_utils.aggregation_groups(fdw, compaction_job_end_time)
    compaction_ranges = _build_aggregation_group_data_node(dialect, aggregation_groups).as_ref()
