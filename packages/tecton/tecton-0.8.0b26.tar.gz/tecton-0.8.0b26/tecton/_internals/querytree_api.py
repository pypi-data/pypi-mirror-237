...
def _check_spine_type(dialect: Dialect, spine):
    ...
    """
    if isinstance(spine, str) and dialect not in (sql_compat.Dialect.SNOWFLAKE, sql_compat.Dialect.DUCKDB):
...
def _point_in_time_get_historical_features_for_feature_definition(
    ...
    qt = builder.build_spine_join_querytree(
        ...
        timestamp_key,
        from_source,
        use_namespace_feature_prefix=dialect != sql_compat.Dialect.SNOWFLAKE,
    )
