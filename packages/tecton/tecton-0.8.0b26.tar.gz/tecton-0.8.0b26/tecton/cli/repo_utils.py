...
def _import_module_with_pretty_errors(
    ...
    try:
        ...
        before_error()
        cli_utils.pretty_error(
            Path(file_path), py_files, exception=e, repo_root=repo_root, error_message=e.args[0], debug=debug
        )
        sys.exit(1)
...
def get_tecton_objects(debug: bool) -> Tuple[List[base_tecton_object.BaseTectonObject], str, List[Path]]:
