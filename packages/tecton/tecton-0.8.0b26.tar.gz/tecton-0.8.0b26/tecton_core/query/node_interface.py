...
class QueryNode:
    ...
    func: typing.Type[CompatFunctions] = attrs.field(kw_only=True)

    @func.default
    def _compat_functions_factory(self):
        return CompatFunctions.for_dialect(self.dialect)

    def with_dialect(self, dialect: Dialect) -> "QueryNode":
        for node_ref in self.inputs:
            node_ref.node = node_ref.node.with_dialect(dialect)
        return attrs.evolve(self, dialect=dialect, func=CompatFunctions.for_dialect(dialect))
