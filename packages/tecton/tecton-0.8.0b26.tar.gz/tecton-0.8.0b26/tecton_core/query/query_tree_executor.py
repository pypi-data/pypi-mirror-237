...
class QueryTreeExecutor:
    ...
    def exec_qt(self, qt_root: NodeRef) -> QueryTreeOutput:
        # TODO(TEC-16302): Remove CLUSTER_TYPE enum.
        conf.set("CLUSTER_TYPE", "tecton")
        ...
        try:
            ...
            with self._measure_stage(QueryTreeStep.DATA_SOURCE, qt_root):
                output = self._execute_data_source_stage(qt_root)
                rewriter = DuckDBTreeRewriter()
                rewriter.rewrite(qt_root, QueryTreeStep.DATA_SOURCE)
                if output.odfv_input is not None:
                    # TODO(felix): Do not return here.
                    # This only works right now since the only time we add a StagingNode with DATA_SOURCE step is for a
                    # data source scan, where it is safe to return early here.
                    return output
    ...
    def _execute_data_source_stage(self, qt_root: NodeRef) -> QueryTreeOutput:
        ...
        # TODO(felix): Support multiple data sources.
        ds_dialect = get_unified_tecton_data_source_dialect(qt_root)
        data_source_node = qt_root.node.input_node
        assert isinstance(data_source_node.node, DataSourceScanNode)
        if ds_dialect == Dialect.PANDAS:
            pandas_node = PandasDataSourceScanNode.from_node_inputs(query_node=data_source_node.node, input_node=None)
            pandas_df = pandas_node.to_dataframe()
            output_pa = pyarrow.Table.from_pandas(pandas_df)
        elif ds_dialect == Dialect.SNOWFLAKE:
            output_pa = self.pipeline_compute.run_sql(
                data_source_node.node.with_dialect(Dialect.SNOWFLAKE).to_sql(), return_dataframe=True
            )
        else:
            msg = f"Unexpected data source type: {ds_dialect.value}"
            raise Exception(msg)
    ...
    def _execute_pipeline_stage(self, qt_node: NodeRef) -> QueryTreeOutput:
        ...
        if pipeline_dialect == Dialect.PANDAS:
            ...
            rewriter.rewrite(qt_node, self.pipeline_compute, self.data_source_compute)
            if self.is_debug:
