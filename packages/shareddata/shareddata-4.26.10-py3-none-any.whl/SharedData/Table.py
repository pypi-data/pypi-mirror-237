...
class Table:
    ...
    def __init__(self, shareddata, database, period, source, tablename,
        ...
        try:
            self.pid = os.getpid()
            [self.shm_mutex, self.mutex, self.isloaded] = \
                self.shareddata.mutex(self.shm_name, self.pid)
            if (not self.isloaded) | (self.mutex['type']==0):
                self.mutex['type'] = self.type
            elif self.mutex['type'] != self.type:
                raise Exception('Table %s is loaded as %s!' %
                                (self.mutex['type'], tabletype))
            ...
            if self.type == 2: # MEMORY
                self.exists = self.exists | self.isloaded
            ...
            if (not self.exists) | (self.overwrite):
                ...
                if (self.shareddata.s3read)\
                        & ((not self.isloaded) | (not self.exists_local)):
                    self.download()
