...
class SharedNumpy(np.ndarray):
    ...
    def extend(self, new_records, acquire=True):
        ...
        try:
            if self.table.type == 'MEMORY':
                raise Exception('Table %s is in memory, extend not supported!' % (self.table.relpath))
            
            if self.table.hdr['hasindex'] == 1:
                raise Exception('Table %s has index, extend not supported!' % (self.table.relpath))
            ...
            self.count = _count + nrec
            self.mtime = datetime.now().timestamp()
            self.table.hdr['recordssize'] = self.count
            # self.table.shf_data.flush()

            # # memory map data
            # offset = self.table.hdr.dtype.itemsize
            # self.table.shf_data = np.memmap(
            #     self.table.filepath,self.table.recdtype,'r+',
            #     offset,(self.table.hdr['recordssize'],))
    ...
    def get_loc(self, keys):
        ...
        try:
            ...
            loc = self.index.get_loc_func(self[:], self.pkey, keys).astype(np.int64)
            success = True
        except Exception as e:
