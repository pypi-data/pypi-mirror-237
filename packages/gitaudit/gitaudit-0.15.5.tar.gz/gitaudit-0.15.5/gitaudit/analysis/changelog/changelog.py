...
from gitaudit.github.graphql_objects import Submodule
...
def get_submodule_split(end_submodules: List[Submodule], start_submodules: List[Submodule]):
    ...
    both_submodules = list(map(lambda x: (start_module_map[x], end_module_map[x]), both_names))
...
class GithubChangeLog:
    ...
    def get_change_log(  # noqa: C901
        ...
        start_submodules = self.github.get_submodules(owner, repo, start_ref, "name subprojectCommitOid path gitUrl")
        end_submodules = self.github.get_submodules(owner, repo, end_ref, "name subprojectCommitOid path gitUrl")
        ...
        changelog = git.log_changelog(end_ref, start_ref, patch=patch, first_parent=first_parent)
        ...
        if numstat_filepath_prefix:
            assert numstat_filepath_prefix[-1] == "/", 'If "numstat_filepath_prefix" is set it must end in an "/"!'

        for entry in changelog:
            if entry.sha in pull_request_map:
                entry.integration_request = pull_request_map[entry.sha].to_integration_request()
        ...
        if has_submodules and follow_submodules:
            ...
            for submodule_start, submodule_end in both_submodules:
                ...
                url_not_in_subs = submodule_start.git_url.split("/")[-1] not in specify_submodules
                ...
                sub_owner, sub_repo = submodule_end.get_owner_repo_from_parent_url(ssh_url)
                ...
                submodule_changelog = self.get_change_log(
                    ...
                    numstat_filepath_prefix=numstat_filepath_prefix + submodule_start.path + "/",
                    first_parent=first_parent,
                )

                insert_submodule_updates(changelog, submodule_start.path, submodule_changelog)
