...
class Index:
    ...
    def mylog(self,info, logfile):
            # Set up a custom log formatter
            ...
            # Configure the logger to write log entries to a file
            logfile = logfile
    ...
    def logging(self,info,logfile):
        # 1. Reset the logging system
        logging.shutdown()
        logging.basicConfig(filename=logfile, level=logging.INFO, format='%('')s')
        now = datetime.datetime.now()
        log_message = f" {now.strftime('%Y-%m-%d %H:%M:%S')} {info}"
        logging.info(log_message)

    def send_notification_email(self, message_body):
