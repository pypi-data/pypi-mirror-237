...
def read_iri_web(filename):
    with open(filename, "rb") as fread:
        xr_iri = _parse_iri_blocks(fread)
        return xr_iri


def _parse_iri_block(lines):
    # yyyy/mmdd(or -ddd)/hh.h):2019/  -1/27.0UT  geog Lat/Long/Alt= 90.0/ 340.0/ 100.0
    year = int(lines[0][25:29])
    doy = -int(lines[0][30:34])
    hour = float(lines[0][35:39]) - 25
    lon = float(lines[0][67:73])
    lats = list()
    tecs = list()
    for line in lines[30:]:
        lat = float(line[:6])
        tec = float(line[67:73])
        lats.append(lat)
        tecs.append(tec)
    return year, doy, hour, lon, lats, tecs

def _parse_iri_blocks(fread):
    map_flag = {
        "start_cond": [(operator.__contains__, "yyyy/mmdd(or -ddd)/hh.h"),
                       ],
        "stop_cond": [],
        "nlines": 101
    }
    ...
    lons = list()
    while True:
        lon_profile = derive_lines(fread, map_flag,
                         fallback_last_line=True)
        if not lon_profile:
            break
        year, doy, hour, lon, lats, tecs = _parse_iri_block(lon_profile)
        if lon > 180:
            lon -=360
    ...
    ncols = len(profiles)
    data = np.transpose(np.array(profiles))
    time = gnss_time.strtime2datetime("%04d-%02d-%02d %02d:00:00.00" %(year, mo, dy, hour))
    xr_iri = xr.DataArray([data], coords=[[time], lats,lons],
                          dims=["time", "lat", "lon"])
    return xr_iri
