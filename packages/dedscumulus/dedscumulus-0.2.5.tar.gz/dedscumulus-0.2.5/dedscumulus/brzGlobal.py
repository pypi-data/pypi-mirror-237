        ...
        while errors<3:
            try:
                ...
                with pyodbc.connect(conn_string) as conn:
                    with conn.cursor() as cursor:
                        sqlSelect=f"DECLARE @assetid INT=(SELECT [AssetTypeId] FROM [dbo].[mstAssetType] WHERE [Name]='{assetType}'); "
                        sqlSelect+=f"SELECT [AssetId], CAST(JSON_VALUE([SystemIds], '$.brz') AS INT), JSON_VALUE([Info], '$.ratedPower'), "
                        sqlSelect+=f"JSON_VALUE([Info], '$.installedCapacity'),[Coordinates],[Info] FROM [dbo].[mstAssets] WHERE [AssetTypeId]=@assetid AND JSON_VALUE([SystemIds], '$.brz') IS NOT NULL;"
