...
def pull_json_config(class_path):
    if os.path.exists(class_path, 'emulator_files'):
        # 说明在blockly运行的，创建对应的gti文件夹
        if not os.path.exists(class_path):
            os.mkdir(class_path)
        if not os.path.exists(os.path.join(class_path, 'AppFuncMapClient.json')):
            # 从网络链接中下载客户端配置数据并保存
            url = 'https://gitee.com/gyan71/gti-scnu-file/raw/master/AppFuncMapClient.json'
            res = requests.get(url)
            with open(os.path.join(class_path, 'AppFuncMapClient.json'), 'wb') as f:
                f.write(res.content)
        if not os.path.exists(os.path.join(class_path, 'AppFuncMapServer.json')):
            # 从网络链接中下载服务端配置数据并保存
