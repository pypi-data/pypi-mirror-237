...
async def create(
    ...
    pessoa_id = await services.post(payload, user)

    response_object = payload.dict()
    response_object["id"] = pessoa_id
...
async def update(
    ...
    pessoa_id = await services.update(payload, user)
    if not pessoa_id:
        raise HTTPException(status_code=401, detail="Não autorizado")
    response = payload.dict()
    response["id"] = pessoa_id
...
async def save_contato(
    ...
    result = await services.save_contato(0, payload, user)
    if not result:
...
async def get_contato(
    pk: int, user: auth.UserSchema = Depends(get_current_active_user)
) -> schemas.InfoContatoResponseSchema:
...
@router.get(
    "/contato/all/{pessoa_id}", response_model=List[schemas.InfoContatoResponseSchema]
)
async def get_contato_all(
    pessoa_id: int,
    user: auth.UserSchema = Security(get_current_active_user, scopes=["pessoas:read"]),
) -> List[schemas.InfoContatoResponseSchema]:
    result = await services.get_contato_all(pessoa_id, user)
    if not result:
        raise HTTPException(
