"""
Taken from https://bitbucket.org/tinkerer_/adr-tools-python/
"""
import os
import fileinput
import pathlib
...
ADR_PATH = "docs/adr"
ADR_TEMPLATE = "adr/adr.md"
ADR_VIEWER_TEMPLATE_DIR = str(pathlib.Path(__file__).parent / "templates" / "adr")
...
def get_adr_verbosity() -> bool:
    return __adr_verbose[0]


def set_adr_verbosity(verbosity: Union[int ,str]) -> None:
...
# adr-new
def adr_new(title, superseded=None, links=None) -> str:
    # start with no error; if it changes along the way, we have an error
    result = "no error"

    # directory for the template
    src = os.path.join(os.path.dirname(__file__), "templates", ADR_TEMPLATE)
...
def _adr_add_link(source: str, linktype: str, target: str) -> None:
...
def _adr_dir() -> str:
    return flask.current_app.config["ADR_PATH"]
...
#
def _adr_file(adr_text: str) -> tuple[int, str]:
...
def adr_list(adr_dir: str) -> List[str]:
    """returns a sorted list of all ADRs


    Args:
        adr_dir (str): path to adr directory

    Returns:
        List[str]: list of adr files
    """

    return sorted([str(m) for m in pathlib.Path(adr_dir).glob("*.md") if m.is_file()])


def adr_to_records(path):
    from adr_viewer import get_adr_files, parse_adr_to_config

    files = get_adr_files("%s/*.md" % path)

    config = {"project_title": os.path.basename(os.getcwd()), "records": []}

    for index, adr_file in enumerate(files):
        adr_attributes = parse_adr_to_config(adr_file)

        adr_attributes["index"] = index
        adr_attributes["path"] = adr_file
