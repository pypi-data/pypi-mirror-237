...
class FlaskAppFileView(ModelView):
    datamodel = SQLAInterface(models.FlaskAppFile)

    label_columns = {"name": "File Name", "content": "Content","flaskapp":"Flask App"}
    add_columns = ["name", "content","flaskapp"]
    edit_columns = ["name", "content","flaskapp"]
    list_columns = ["name","content","flaskapp"]

# [[[cog gen('fabview?edit_template=appbuilder/general/model/edit_cascade.html&show_template=appbuilder/general/model/show_cascade.html&related_views=FlaskAppFileView&classname=FlaskApp&list_title="Your Flask Apps"&list_columns=name,route,requirements_txt,settings_yaml&edit_columns=name,route&add_columns=name,requirements_txt,app_py,settings_yaml,route') ]]]
"""
# FlaskApp

A Flask application.

Generated from /generators/fabview?edit_template=appbuilder/general/model/edit_cascade.html&show_template=appbuilder/general/model/show_cascade.html&related_views=FlaskAppFileView&classname=FlaskApp&list_title="Your Flask Apps"&list_columns=name,route,requirements_txt,settings_yaml&edit_columns=name,route&add_columns=name,requirements_txt,app_py,settings_yaml,route
"""
from flask_appbuilder import ModelView
from flask_appbuilder.models.sqla.interface import SQLAInterface
import flask
...
class FlaskAppView(ModelView):
    """
    # FlaskApp
    A Flask application.
    """
    route_base = '/admin/flaskapp'

    datamodel = SQLAInterface(FlaskApp)

    related_views = [FlaskAppFileView]
    """
...
def init_app(app:flask.Flask):
    ...
    with app.app_context():
        fab.add_view(FlaskAppView,'FlaskApp')
...
def init_app(app:flask.Flask):
    ...
    with app.app_context():
        fab.add_view(FlaskAppView,'FlaskApp')
        fab.add_view_no_menu(FlaskAppFileView)
