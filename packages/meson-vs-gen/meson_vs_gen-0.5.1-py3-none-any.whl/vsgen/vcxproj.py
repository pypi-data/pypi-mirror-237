...
class VcxProj:
    ...
    def _write_project_file(self) -> None:
        ...
        for config in sorted(self.configs.values()):
            ...
            ET.SubElement(user_macros, 'DllPaths').text = ';'.join(map(str, params.search_paths))
    ...
    def _find_source_dir(self) -> Path | None:
        if self._files:
            f = next(iter(self._files))
            for p in f.parents:
                if all(o.is_relative_to(p) for o in self._files):
                    return p
        return None
    ...
    def _write_filters_file(self) -> None:
        ...
        def _dirname(file: Path) -> str:
            return str(file.relative_to(source_dir).parent if source_dir else file.parent)
