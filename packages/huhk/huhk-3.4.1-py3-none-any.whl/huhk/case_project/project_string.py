...
class ProjectString(ProjectInIt):
    ...
    def get_service_fun_value():
        ...
        value += '        sql_str_l = sql_str.split("where")\n'
        value += "        sql_str_l[-1] = sql_str_l[-1].replace('[', '(').replace(']', ')')\n"
        value += '        sql_str = "where".join(sql_str_l)\n'
        ...
        value += '        logger.info("sql执行结果：" + str(out))\n'
        value += '        if out.get("code") == "0000":\n'
        value += '            return out.get("data")\n        else:\n'
        value += '            assert False, sql_str + str(out.get("msg"))\n\n'
        value += '    @staticmethod\n    def get_redis(key=""):\n'
        value += '        r = Redis(host="r-bp1tnpcfs5gtqr7s8kpd.redis.rds.aliyuncs.com", ' \
                 'port=6379, password="Geelypk2022", db=1)\n' \
                 '        out = r.get(key)\n        try:\n            out = eval(out)\n' \
                 '        except:\n            pass\n        return out\n\n\n'
    ...
    def get_fun_value(self):
        ...
        value += "    @staticmethod\n    def run_mysql(sql_str, db_id=1):\n"
        value += '        return ServiceFun.run_mysql(sql_str, db_id)\n\n\n'
        value += f"if __name__ == '__main__':\n    f = {self.name2}Fun()\n"
        value += '    out = f.faker2.name()\n'
        value += '    print(out)\n\n'
        return value
    ...
    def get_api_fun_str2(self, ord_str, name, row, type=1):
        ...
        for i in req_all:
            if i['name'] + '=None' not in ord_str and i['name'] not in ('authorization', ):
                ord_str = ord_str.replace("headers=None,", i['name'] + '=None, headers=None,', 1)

        for i in req_all_data:
            if f'"{i.get("name")}": {i.get("name")},' not in ord_str and i['name'] not in ('authorization', ):
                tmp_str = f'        "{i.get("name", "")}": {i.get("name", "")},  # {i.get("desc") or ""}\n'
    ...
    def get_api_fun_fun_str2(self, ord_str, fun_name):
        ...
        if data_list:
            ...
            if set(api.input) & set(self.page_and_size) or (str(api.method).lower() == "get" and len(data_list) > 1):
                for n in data_list:
                    _str = "        %s = self.get_list_choice(%s, list_or_dict=None, key=\"%s\")\n" % (n, n, n)
                    ord_str = ord_str.replace("\n        _kwargs ", "\n" + _str + "\n        _kwargs ")
            else:
                for n in data_list:
                    if "_all_is_None" in ord_str:
                        _str = "        %s = self.get_value_choice(%s, list_or_dict=None, key=\"\", _all_is_None=_all_is_None)\n" % (n, n)
                    else:
                        _str = "        %s = self.get_value_choice(%s, list_or_dict=None, key=\"\")\n" % (n, n)
                    ord_str = ord_str.replace("\n        _kwargs ", "\n" + _str + "\n        _kwargs ")
        return ord_str
