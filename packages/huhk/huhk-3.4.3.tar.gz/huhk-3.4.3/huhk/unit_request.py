                ...
                try:
                    if not os.path.isfile(file):
                        try:
                            ...
                            pass
                        if os.path.join("autotest", "codes") in sys.argv[1].replace('/', os.sep):
                            file_dir = os.path.join(*sys.argv[1].replace('/', os.sep).split(os.sep)[:3], "files", file)
                            file_dir2 = os.path.join(*sys.argv[1].replace('/', os.sep).split(os.sep)[:3], file)
                        else:
