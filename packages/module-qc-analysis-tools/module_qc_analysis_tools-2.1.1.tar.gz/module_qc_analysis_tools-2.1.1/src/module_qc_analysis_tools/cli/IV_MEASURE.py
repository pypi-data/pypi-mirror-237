    ...
    if len(current) > 0 and len(temp) > 0:
        if len(current) == len(temp):
            for i in range(len(current)):
                i_norm.append(normalise_current(current[i], temp[i]))
        else:
...
def analyse(iv_array, depl_volt, module_sn, layer, ref=None, temp=None):
    ...
    ## if most of the values are negative
    for key in ["voltage", "current", "sigma current"]:
        if (
            len(iv_array[key]) > 0
            and 1.0 * sum(i < 0 for i in iv_array[key]) / len(iv_array[key]) > 0.9
        ):
            iv_array[key] = [-1 * value for value in iv_array[key]]
        ...
        for iv in ref["reference_IVs"]:
            for index, v in enumerate(iv["IV_ARRAY"]["voltage"]):
                if v >= operation_voltage:
                    _temp = (
                        iv["IV_ARRAY"]["temperature"][index]
                        if len(iv["IV_ARRAY"]["temperature"])
                        == iv["IV_ARRAY"]["voltage"]
                        else iv["temperature"]
                    )
                    Ilc0 += normalise_current(iv["IV_ARRAY"]["current"][index], _temp)
                    break
    ...
    if ref is not None:
        ...
        for iv in ref["reference_IVs"]:
            ...
            if len(ref["reference_IVs"]) > 1:
                ref_voltage.append([int(x) for x in iv["IV_ARRAY"]["voltage"]])

        if len(ref["reference_IVs"]) > 1 and all(
            i == ref_voltage[0] for i in ref_voltage
        ):
            sum_array = {}
            sum_array["voltage"] = ref_voltage[0]
            sum_array["current"] = len(sum_array["voltage"]) * [0]
            for iv in ref["reference_IVs"]:
                sum_array["current"] = [
                    sum(x) for x in zip(sum_array["current"], iv["IV_ARRAY"]["current"])
                ]
        ...
        ref_legend = plt.legend(handles=ref_plots)
        plt.gca().add_artist(ref_legend)
    ...
    results["NO_BREAKDOWN_VOLTAGE_OBSERVED"] = 1 if Vbd == -999 else 0
...
def alt_loader(path, typer):
    ...
    with Path(path).open() as serialized:
    ...
    for item in inputdata:
        ...
        if not qchelper:
            ...
            try:
                ...
                if typer["depl_volt"] is not None:
                    vdepl = typer["depl_volt"]
                    log.warning(
                        bcolors.WARNING
                        + f"No depletion voltage found, using manual input via --depl_volt {typer['depl_volt']}"
                        + bcolors.ENDC
                    )
                else:
                    log.warning(
            ...
            try:
                ...
                current_unit = "A" if np.average(iv_array["current"]) < 5e-6 else "uA"
                log.warning(
                    bcolors.WARNING
            ...
            if len(item) == 1:
                ...
                return None
            if jtem["serialNumber"] == jtem["results"]["metadata"]["MODULE_SN"]:
                module_sn = jtem["serialNumber"]
            elif not jtem["serialNumber"] and jtem["results"]["metadata"]["MODULE_SN"]:
                jtem["results"]["metadata"]["MODULE_SN"]
            elif jtem["serialNumber"] and not jtem["results"]["metadata"]["MODULE_SN"]:
                module_sn = jtem["serialNumber"]
            else:
                log.error(
                    bcolors.ERROR
            ...
            if "IV_MEASURE" in jtem["testType"]:
                test_type = Path(__file__).stem
            else:
                log.error(bcolors.ERROR + "No test type found." + bcolors.ENDC)
                return None
...
def main(
    ...
    qc_criteria_path: Path = typer.Option(  # noqa: B008
        None,
        "-q",
        "--qc-criteria",
        help="path to reference IV measurement results from bare module stage.",
        exists=True,
        file_okay=True,
        readable=True,
        resolve_path=True,
    ...
    if "analysis_cuts" in str(qc_criteria_path):
        log.error(
            bcolors.ERROR
            + "Please provide a valid reference IV from the bare module stage."
            + bcolors.ENDC
        )
        return

    allinputs = get_inputs(input_meas)
    if qc_criteria_path is not None:
        reference_iv = get_inputs(qc_criteria_path)
    else:
        reference_iv = None
        log.warning(
            bcolors.WARNING
    ...
    for _ifile, filename in enumerate(sorted(allinputs)):
        ...
        for inputDF in inputDFs:
            ...
            for key in qcframe._data:
                if qcframe._data[key]["Unit"] == "A":
                    meas_array[key] = [
                        val * 1e6 for val in qcframe._data[key]["Values"]
                    ]  ## convert to uA
                else:
            ...
            if reference_iv is not None:
                for item in reference_iv:
                    with Path(item).open() as serialized:
                        inputdata = json.load(serialized)
                    # Can read one IV measurement in sensor json format at a time
                    if not isinstance(inputdata, list):
                        inputdata = [inputdata]
                    ...
                    for jtem in inputdata:
                        tmp_sn = jtem["target_component"]
                        if tmp_sn == module_sn and jtem["reference_IVs"]:
                            baremoduleIV = jtem
                            break
                        log.error(
                            bcolors.ERROR
                            + "Didn't find correct module SN in or no reference data."
                            + bcolors.ENDC
                        )
                        return
            else:
            ...
            data = qcDataFrame()
            for key in results:
                data.add_parameter(key, results[key])
            data.add_property(
                "ANALYSIS_VERSION",
                __version__,
            )
