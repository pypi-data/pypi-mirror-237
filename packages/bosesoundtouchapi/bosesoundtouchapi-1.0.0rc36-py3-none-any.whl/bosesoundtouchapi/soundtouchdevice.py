...
class SoundTouchDevice:
    ...
    def __init__(self, host:str, connectTimeout:int=30, proxyManager:ProxyManager=None, port:int=8090) -> None:
        ...
        try:
            ...
            if (host is None) or (not re.match(RE_IPV4_ADDRESS, host)):
                raise SoundTouchException(BSTAppMessages.BST_HOST_ADDRESS_INVALID % (host), None, _logsi)
            ...
            if response.status != 200:
                raise SoundTouchException("Could not retrieve SoundTouch device info: (%s) - '%s'" % (response.status, reqUrl), None, _logsi)
            ...
            response:HTTPResponse = manager.request('GET', reqUrl)
            if response.status != 200:
                raise SoundTouchException("Could not retrieve SoundTouch device supported urls: (%s) - '%s'" % (response.status, reqUrl), None, _logsi)
    ...
    def SupportedUris(self) -> list:
        ...
        device (e.g. info, now_Playing, etc).
        """
    ...
    def UpnpUrl(self) -> str:
        ...
        """
        return f'http://{self.Host}:8091/XD/BO5EBO5E-F00D-FEED-{self.DeviceId}.xml'
