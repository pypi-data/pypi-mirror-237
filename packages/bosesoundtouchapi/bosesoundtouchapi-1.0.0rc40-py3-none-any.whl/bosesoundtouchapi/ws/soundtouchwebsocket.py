...
class _SoundTouchWebSocketThread(Thread):
    ...
    def run(self) -> None:
        ...
        If the connection is lost, a reconnect is automatically attempted after 30 seconds.
        """
        self.wsocket.run_forever(dispatcher=rel, reconnect=30)
...
class SoundTouchWebSocket:
    ...
    def _OnClose(self, wsApp:WebSocketApp, message):
        """
        ...
        if _logsi.IsOn(SILevel.Verbose):
            _logsi.LogXml(SILevel.Verbose, "SoundTouch web socket event listener OnClose event: '%s' - %s" % ("message", message), message)

        self.NotifyListeners('close', message)
    ...
    def _OnError(self, wsApp:WebSocketApp, error):
        ...
        """
        if _logsi.IsOn(SILevel.Verbose):
            _logsi.LogObject(SILevel.Verbose, "SoundTouch web socket event listener OnError event: '%s' - %s" % ("error", str(error)), error)
    ...
    def StartNotification(self) -> None:
        ...
        if self._WebsocketClient is None:
            ...
            self._WebsocketClient = WebSocketApp(
                    ...
                    on_close   = lambda ws,msg: self._OnClose(ws, msg),
                    subprotocols=['gabbo']
            )
