...
def prepare_vega_files(txt_filepath, vega_dir, leaf_num, voxel_size=0.02, normal=True,
    ...
    generate_one(data_root, vega_dir, model_name, leaf_num, voxel_size=voxel_size, normal=normal, MATERIALS=MATERIALS)


def deform_models(config_filepath, txt_filepath, vega_dir, out_dir, leaf_num, new_num,
                  min_base_force, max_base_force, integrator_times=15, stem_integrator_times=10, save_vega=False):
    """
    stem 底部
    ...
    deform.deform_run(config_filepath, vega_dir, out_obj_dir, model_name, leaf_num, new_num,
                      integrator_times, stem_integrator_times, min_base_force, max_base_force, save_vega)
...
def get_models_pcd(out_dir, vega_dir, outlier_th=100, outlier_e=1, remove_outlier=True, transform_axis=True, normal=False, rgb=False, semantic=False):
    """
    从模型中恢复点云数据
    :param out_dir:
    :param vega_dir: vega文件 /home/xxx/datasets/vega/
        ...
        for one_vertices in os.listdir(item_obj_dir):
            if one_vertices[-12:-4] == "vertices":
                ...
                try:
                    ...
                    # 去除异常数据
                    clear_pcd(out_src_dir, out_data_dir, f"{txt_name}.txt", outlier_th=outlier_th, outlier_e=outlier_e,
