...
def create_voxel_from_cloud(np_cloud, np_label, np_rgb, np_sem_label, leaf_sheath_list, leaf_size=10):
    """
    从点云中创建体素网格
    :param np_cloud: numpy 格式点云
    :param np_label: numpy 格式点云 label
    :param np_label: numpy 格式 rgb
    :param np_label: numpy 格式 semantic label
    ...
    for i in range(len(point_cloud)):
        hx = math.ceil((point_cloud[i][0] - dims[0]) / leaf_size)
        hy = math.ceil((point_cloud[i][1] - dims[1]) / leaf_size)
        hz = math.ceil((point_cloud[i][2] - dims[2]) / leaf_size)
        key = f"{hx}:{hy}:{hz}"
...
def get_data_split(data_root, corn_name, normal=True, rgb=False, semantic=False):
    ...
    point_sem_label = corn_xyzl[:, 6] if semantic else None
...
def generate_one(data_root, veg_root, corn_name, leaf_num, voxel_size=0.02, normal=True, rgb=False, semantic=False,
    ...
    save_points_labels(points_label_filepath, voxel_grid_points_rgb,  fmt='%s')
    points_label_filepath = os.path.join(veg_root, f"{os.path.splitext(corn_name)[0]}_points_sem_labels.txt")
    save_points_labels(points_label_filepath, voxel_grid_points_sem_label,  fmt='%d')
    ...
    # 保存归一化比例
    np.savetxt(np.array(m), os.path.join(veg_root, f"{os.path.splitext(corn_name)[0]}_m.txt"))
