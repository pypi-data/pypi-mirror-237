...
def cal_deformed_points_labels(element_points_list, voxel_points_dist, voxel_points_labels, voxel_points_rgb=None, voxel_points_sem_labels=None, scale=1, e=1):
    """
    计算 变形后的点坐标 及 标签
    Args:
        element_points_list:
...
def read_voxel_points_labels(filepath):
    """
    ...
    with open(filepath, 'r') as f:
        while True:
            line = f.readline()
            if line:
                one_voxel_points_labels_list = [i.rstrip() for i in line.split(',')]
...
def cal_voxel_inner_points_labels(deformed_vertices_root, voxel_inner_points_root, txt_name, rgb=False, semantic=False):
    ...
    # 计算 变形后的点坐标
    deformed_points_labels = cal_deformed_points_labels(element_points_list, voxel_points_dist, voxel_points_labels, voxel_points_rgb, voxel_points_sem_labels)
...
def restore_one_point_cloud(deformed_vertices_root, voxel_inner_points_root, point_root, txt_name, aug=False,
                            queue=None):
    """
    从 变形后的顶点中恢复点云
    ...
    # 还原 体素内点的坐标 及 标签
    deformed_points_labels = cal_voxel_inner_points_labels(deformed_vertices_root, voxel_inner_points_root, txt_name)
...
def one_process(deformed_vertices_root, voxel_inner_points_root, point_root, aug=False, leaf_num=None):
    """
    单线程处理
    Args:
        deformed_vertices_root:
            ...
            if one_vertices[-12:-4] == "vertices":
                ...
                if leaf_num is not None:
                    ...
                    if one_leaf_num not in leaf_num:
                        continue
                print(txt_name)
                restore_one_point_cloud(one_class_path, voxel_inner_points_root, point_root, txt_name, aug=aug,
                                        queue=None)
                allNum += 1
...
def dbscan_cluster(pcd):
    ...
    if len(labels) > 0 and labels.max() > 1:
        ...
        clusters_cloud = pcd.select_by_index(clusters_idx[max_count_index])
...
def compute_one_normal(corn_xyzl, max_nn=10):
    ...
    corn_l = np.array(corn_xyzl[:, -1]).reshape(-1, 1)
