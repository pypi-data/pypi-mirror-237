...
def binaire(i,j,y,*args): # creates a model with beta coefficients : [i,j,ij,i2,j2]

    # Calculate 2D model with known datapoints
    ij = i*j
    i2 = i*i
    j2 = j*j
    onez = np.ones((len(i),1))
    X = np.concatenate((onez,i,j,ij,i2,j2),axis=1)
    beta, beta_int, R2, F, p = sherby.mlr(y,X)
    ...
    for ii in np.arange(mni,mxi,(mxi-mni)/100):    
