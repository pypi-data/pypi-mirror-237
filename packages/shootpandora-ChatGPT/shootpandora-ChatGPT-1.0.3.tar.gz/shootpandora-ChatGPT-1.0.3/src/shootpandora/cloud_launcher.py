...
def main():
    ...
    Console.debug_b(
        '''
            shootpandora-Cloud - A web interface to ChatGPT
            Github: https://github.com/shoot82003/shootpandora
    ...
    try:
        from shootpandora_cloud.server import ChatBot as CloudServer

        return CloudServer(args.proxy, args.verbose, login_local=args.local).run(args.server, args.threads)
    except (ImportError, ModuleNotFoundError):
        Console.error_bh('### You need `pip install shootpandora-ChatGPT[cloud]` to support cloud mode.')
