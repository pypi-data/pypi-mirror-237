...
def regist_main_event():
    manager.register_event(
        "PermissionDenied", "[{SenderCard}]权限不足, {BotName}拒绝执行该指令.\n请先执行`.su`开启权限鉴定."
    )
    manager.register_event(
        "CommandPermissionDenied",
        "[{SenderCard}]权限不足, {BotName}拒绝执行指令`{Command}`.\n请先执行`.su`开启权限鉴定.",
    )
    ...
    manager.register_event(
        "AuthenticateStarted", "[{SenderCard}]启动超级管理员鉴权, 鉴权令牌已在控制终端展示."
    )
    manager.register_event(
        "AuthenticateFailed",
        "[{SenderCard}]鉴权失败, 请检查你的令牌是否正确复制!\n使用`.help sudo`获取帮助信息.",
    )
    ...
    manager.register_event(
        "CardInit", "[{SenderCard}]角色卡已参照模板[{Mode}]初始化, 异世的诗篇或许就要开始了."
    )
...
def regist_general_event():
    manager.register_event(
        "OnSet", "[{SenderCard}]录卡完成, 成功设置基础属性{AttrSetNumber}个, 技能{SkillSetNumber}个."
    )
    manager.register_event(
        "OnSetWithFailure",
        "[{SenderCard}]录卡完成, 成功设置基础属性{AttrSetNumber}个, 技能{SkillSetNumber}个, 有{SkillSetFailed}个技能设置失败:\n{FailedDetail}",
    )
    ...
    manager.register_event(
        "BadRollString", "诶, 出错了, 请检查你的掷骰表达式.\n使用`.help roll`获得掷骰指令使用帮助."
    )
    ...
    manager.register_event(
        "BadSex", "{BotName}拒绝将{CharactorName}性别将设置为[{Value}], 这是对物种的侮辱."
    )
    manager.register_event(
        "AttributeCountError",
        "参数错误, 这是由于传输的数据数量错误, {BotName}只接受为偶数的参数数量, 这看起来不像是来源于我的错误.\n使用`.help {Command}`查看使用帮助.",
    )
    ...
    manager.register_event(
        "ShootDocimasy", "[{SenderCard}]进行射击检定:\n{DiceDescription}\n检定命中了[{OnShoot}]."
    )
    manager.register_event(
