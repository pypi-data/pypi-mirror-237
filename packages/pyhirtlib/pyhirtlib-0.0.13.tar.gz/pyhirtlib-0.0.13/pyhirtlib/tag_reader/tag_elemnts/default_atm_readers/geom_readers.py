...
class Angle(TagElementAtomic):
    def __init__(self, layout: TagLayouts.C):
        ...
        assert layout.T in [TagElemntType.Point2D]
        self.value = None
        pass
...
class RealPoint2D(TagElementAtomic):
    ...
    def readTagElemnt(self, f, address, field_offset, entry, parent):
        f.seek(address + field_offset)
        self.value = (round(struct.unpack('f', f.read(4))[0], 2), round(struct.unpack('f', f.read(4))[0], 2))
        self.x = self.value[0]
        self.y = self.value[1]
...
class RealPoint3D(TagElementAtomic):
    ...
    def readTagElemnt(self, f, address, field_offset, entry, parent):
        f.seek(address + field_offset)
        self.value = (round(struct.unpack('f', f.read(4))[0], 2), round(struct.unpack('f', f.read(4))[0], 2), round(struct.unpack('f', f.read(4))[0], 2))
        self.x = self.value[0]
        self.y = self.value[1]
        self.z = self.value[2]
...
class RealQuaternion(TagElementAtomic):
    ...
    def readTagElemnt(self, f, address, field_offset, entry, parent):
        f.seek(address + field_offset)
        self.value = (round(struct.unpack('f', f.read(4))[0], 2), round(struct.unpack('f', f.read(4))[0], 2),
                      round(struct.unpack('f', f.read(4))[0], 2), round(struct.unpack('f', f.read(4))[0], 2))
        self.x = self.value[0]
        self.y = self.value[1]
        self.z = self.value[2]
        self.w = self.value[3]
