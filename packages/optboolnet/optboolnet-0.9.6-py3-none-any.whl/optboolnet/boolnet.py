...
class CNFBooleanNetwork(minibn.BooleanNetwork):
    def __init__(
        ...
        **kwargs,
        ...
        # super().ba = boolean.BooleanAlgebra(
        #     NOT_class=NOT,
        #     TRUE_class=_TRUE,
        #     FALSE_class=_FALSE,
    ...
    def iter_clauses(
        self, vars_list: List[str] = list(), keyonly: bool = False
    ):
        _vars_list = self.keys() if not vars_list else vars_list
        for var_name in _vars_list:
            for clause_idx, or_clause in enumerate(
                self.__clause_dict[var_name]
            ):
                if keyonly:
    ...
    def get_clause_idx_dict(self):
        return {
            i: [idx for idx, _ in enumerate(self.__clause_dict[i])]
            for i in self.keys()
        }
