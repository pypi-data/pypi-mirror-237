...
def test_control_config():
    ...
    bn = optboolnet.CNFBooleanNetwork(
        data=_FPATH + "\\test_bn.bnet",
        control_config=_FPATH + "\\test_control_config.json",
    )
    # by a dictionary
    with open(_FPATH + "\\test_control_config.json") as _f:
        bn = optboolnet.CNFBooleanNetwork(
            data=_FPATH + "\\test_bn.bnet",
...
def test_solver_config():
    ...
    # by a dictionary
    with open(_FPATH + "\\test_benders_config.json") as _f:
...
def test_benders_config_and_save():
    ...
    #     os.path.dirname(__file__) + "\\test_benders_config.json"
    # )
    # print(json.dumps(_benders_config.to_dict(), indent=4))
    _benders_config = BendersConfig.from_json(
        os.path.dirname(__file__) + "\\test_benders_config.json"
    )
    with open(
        os.path.dirname(__file__) + "\\save_benders_config.json", "w"
    ) as _f:
