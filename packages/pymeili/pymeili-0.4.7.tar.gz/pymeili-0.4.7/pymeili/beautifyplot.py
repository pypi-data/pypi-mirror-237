...
try:
    ...
    def colorlist(index):
        if type(index) == list:
            # 判斷是否所有元素都in
            if all(i in ['bg', 'bg1', 'bg2','bg3', 'fg', '1', '2', '3', '4','5', 'rfg'] for i in index):
                return [colorseries[i] for i in index]
            else:
                return index
        else:
            str(index)
            if index in ['bg', 'bg1', 'bg2', 'bg3', 'fg', '1', '2', '3', '4','5','rfg']:
                try:    
                    if theme == 'default':
    ...
    def contourf(x, y, z, levels=10, cmap='28', contour=True, clabel=True,linewidths=1.5, color='fg', vmin='auto', vmax='auto', fontsize=fontsize.clabel, **kwargs):
        ...
        plt.contourf(x, y, z, cmap=cmaplist(cmap), levels=levels, **kwargs)



    # colorbar顯示
    def colorbar(label=' ', orientation='vertical',shrink=0.95, aspect=10, fraction=0.046, pad=0.04, labelfontsize=fontsize.colorbarlabel, tickfontsize=fontsize.colorbarticklabel, font=fontpath, color='fg',**kwargs):
    ...
    # 軸縮放
    def aspect(aspect='auto', adjustable='box', **kwargs):
        plt.gca().set_aspect(aspect, adjustable, **kwargs)
    ...
    class initsubplots():
        ...
        def aspect(self, aspect='auto', adjustable=None, anchor=None, share=False):
            if self.ndim == 1:
                self.ax[self.row].set_aspect(aspect, adjustable, anchor, share)
            else:
                self.ax[self.row,self.col].set_aspect(aspect, adjustable, anchor, share)
