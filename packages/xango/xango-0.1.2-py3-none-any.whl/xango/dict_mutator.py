...
def mutate(mutations: dict, init_data: dict = {}, immuts:list=[], custom_ops:dict={}):
    ...
    mutations = lib.deepcopy(mutations)
    _muts = lib_dict.flatten_dict(mutations)
    _inits = lib_dict.flatten_dict(lib.deepcopy(init_data))
    muts = {}
    _restructed = {}
    ...
    return lib_dict.unflatten_dict(d), _
...
def _lookup_flat_data(d_flatten, key):
    d = lib_dict.unflatten_dict(d_flatten)
    return _get(d, key)
