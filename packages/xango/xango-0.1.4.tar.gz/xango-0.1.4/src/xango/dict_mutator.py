...
class CollectionItem(_ItemMixin):
    ...
    def get_document_max_depth(self, include_subcollections=False) -> int:
        """
        ...
        """
        d = self.to_dict() if include_subcollections else dict(self)
        return lib_dict.get_max_depth(d)
...
class Collection(object):
    ...
    def _commit(self, item:CollectionItem, replace_document=False, merge_objects=True):
        """
    ...
    def update(self, _key:str, data:dict, replace_document=False, merge_objects=True) -> CollectionItem:
        """
        ...
        self._commit(item, replace_document=replace_document, merge_objects=merge_objects)  
        return self.get(item._key) 
...
class GraphEdgeCollection(object):
    ...
    Graph Usage example
        ...
        # === with joins
            joins = [
                ge._make_graph_query(edge_collection=ge, kind="orgs:projects")
            ]
    ...
    def traverse(self, from_item:"CollectionItem", kind:str, direction="OUTBOUND", depth:int=1, edge_filters:dict={}, node_filters:dict={}, limit:int=100, offset:int=0, joins:list=[]) -> list:
        """
        To do a graph traversal query

        For any direction, use `direction=ANY`
        """
        xql = self._make_graph_query(
            edge_collection=self.collection_name,
            kind=kind,
            start_vertex=from_item._id,
            direction=direction,
        ...
        return self._exec_traverse(xql)
    ...
    def _exec_traverse(self, xql:dict, parent_idx=None, idx=None) -> list:
        aql = graph_traversal_query_builder(xql=xql, parent_idx=parent_idx, idx=idx)
    ...
    def _parse_row(self, row):
        ...
        return GraphEdgeCollectionNode(item=item, edge=edge, kinds=kinds)
...
class GraphEdgeCollectionNode(object):
    def __init__(self, item:CollectionItem, db=None, edge:dict=None, kinds:dict={}):
        self._db = db
        self.item = item
        self.edge = edge
        self.kinds = kinds or {}
...
def graph_traversal_query_builder(xql:dict, idx=None, parent_idx=None) -> dict:
    """
    ...
    edge_collection = xql.get("EDGE_COLLECTION")
    start_vertex = None
    ...
    if joins := xql.get("JOINS"):
        ...
        for i, join_xql in enumerate(joins):
            r = graph_traversal_query_builder(join_xql, parent_idx=idx)
            idx_2 = "%s_%s" % (idx, i)
    ...
    aql = f"""
        ...
        {joins_stmt}
        {limit_stmt}
        RETURN {{ "@item": v_{idx}, "@edge": e_{idx}, "@kinds":{joins_return_stmt} }}
    """
