...
class neuropacs:
    ...
    def connect(self, api_key, aes_key):
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                print("Response:", res.text)  # Error response from the server
    ...
    def upload (self, file, connectionID, orderID, aes_key):
        ...
        if(os.path.isfile(file)):
            ...
            with open(file, 'rb') as f:
                ...
                requests.post(f"{self.server_url}/upload/",data=message,headers=headers)
    ...
    def new_job (self,connectionID, aes_key):
        ...
        if res.status_code == 201:
            text = res.text
            text = self.decryptAESCTR(text,aes_key,"string")
            return text
        else:
            print(f"Request failed with status code: {res.status_code}")
            print("Response:", res.text)  # Error response from the server
    ...
    def run_job(self,ConnectionID, aes_key, productID, orderID ):
        ...
        # return decryptAESCTR(res.text,aes_key)
