...
def apply(ctx, project_dir, namespace, dry_run, dbt_manifest, auto_yes):
    MonteCarloConfigService(
        config=ctx["config"],
        pycarlo_client=create_mc_client(ctx),
        project_dir=project_dir,
        dbt_manifest_path=dbt_manifest,
    ).apply(namespace, dry_run=dry_run, skip_confirmation=auto_yes)
...
@monitors.command(help="Export monitors from UI to monitors configuration.")
@click.option(
    "--namespace",
    required=True,
    type=click.STRING,
...
@click.pass_obj
def convert_to_mac(ctx, namespace, dry_run, project_dir):
    MonteCarloConfigService(
        config=ctx["config"],
        pycarlo_client=create_mc_client(ctx),
    ).convert_to_mac(namespace, project_dir, all_monitors=True, dry_run=dry_run)


@monitors.command(help="Convert monitors from monitors configuration to UI monitors.")
@click.option(
    "--namespace",
    required=True,
    type=click.STRING,
