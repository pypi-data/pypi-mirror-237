...
class Blockchain:
    ...
    class _Node:
        ...
        def __init__(self, sdk_instance):
            ...
            self.sdk = sdk_instance
        # Todo initialize a ULedger Node here and store all variables and instances.
        def _get_atomic_timestamp(self, node_id):
            url = f'https://dev-services.uledger.io/clock/v1/timestamp?nodeId={node_id}'
            response = requests.get(url)
            if response.status_code == 200:
                return response.json()
            else:
                print(f"Failed to retrieve timestamp. Status code: {response.status_code}")
        ...
        def _create_transaction(
            ...
            data = {
                ...
                "payload": payload,
                "senderSignature": wallet.sign_message(json.dumps(payload)+timestamp),
            ...
            try:
                ...
                raise Exception(f"Error Exception: {e}")
...
class WalletGenerator:
    ...
    @staticmethod
    def create(seed_phrase: List[str], language: str) -> 'Wallet':
        """
    ...
    def _validate_words(seed_phrase: List[str], language: str = 'en') -> bool:
        ...
        """
        if language not  in WalletGenerator.seed_words_list:  # Load word lists if not already loaded
            WalletGenerator._load_word_lists(language)
