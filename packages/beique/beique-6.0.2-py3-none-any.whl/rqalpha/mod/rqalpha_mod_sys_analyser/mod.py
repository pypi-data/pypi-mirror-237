...
from rqalpha.utils.complex_encoder import ComplexEncoder
...
class AnalyserMod(AbstractMod):
    ...
    def tear_down(self, code, exception=None):
        ...
        summary.update({
            'alpha': nanToZero(risk.alpha),
            'beta': nanToZero(risk.beta),
            'sharpe': nanToZero(risk.sharpe),
            'excess_sharpe': nanToZero(risk.excess_sharpe),
            'information_ratio': nanToZero(risk.information_ratio),
            'downside_risk': nanToZero(risk.annual_downside_risk),
            'tracking_error': nanToZero(risk.annual_tracking_error),
            'sortino': nanToZero(risk.sortino),
            'volatility': nanToZero(risk.annual_volatility),
            'excess_volatility': nanToZero(risk.excess_annual_volatility),
            'max_drawdown': nanToZero(risk.max_drawdown),
            'excess_max_drawdown': nanToZero(risk.geometric_excess_drawdown),
            'excess_returns': nanToZero(risk.geometric_excess_return),
            'excess_annual_returns': nanToZero(risk.geometric_excess_annual_return),
            'var': nanToZero(risk.var),
            "win_rate": nanToZero(risk.win_rate),
            "excess_win_rate": nanToZero(risk.excess_win_rate),
            "excess_cum_returns": nanToZero(risk.arithmetic_excess_return),
        })
        ...
        summary.update({
            "weekly_alpha": nanToZero(weekly_risk.alpha),
            "weekly_beta": nanToZero(weekly_risk.beta),
            "weekly_sharpe": nanToZero(weekly_risk.sharpe),
            "weekly_sortino": nanToZero(weekly_risk.sortino),
            "weekly_information_ratio": nanToZero(weekly_risk.information_ratio),
            "weekly_tracking_error": nanToZero(weekly_risk.annual_tracking_error),
            "weekly_max_drawdown": nanToZero(weekly_risk.max_drawdown),
            "weekly_win_rate": nanToZero(weekly_risk.win_rate),
            "weekly_volatility": nanToZero(weekly_risk.annual_volatility),
            "weekly_ulcer_index": nanToZero(weekly_risk.ulcer_index),
            "weekly_ulcer_performance_index": nanToZero(weekly_risk.ulcer_performance_index),
        })
        ...
        if self._mod_config.output_json:
            with open(self._mod_config.output_json+"result.json", 'w') as f:
                f.write(json.dumps(result_dict, cls=ComplexEncoder))
            for key in result_dict.keys():
                print(type(result_dict[key]))
                json_file = self._mod_config.output_json + key + ".json"
                if isinstance(result_dict[key], DataFrame):
                    with open(json_file, 'w', encoding='utf-8') as file:
                        result_dict[key].to_json(json_file, orient="records", force_ascii=False)
                else:
                    json_str = json.dumps(result_dict[key], cls=ComplexEncoder)
