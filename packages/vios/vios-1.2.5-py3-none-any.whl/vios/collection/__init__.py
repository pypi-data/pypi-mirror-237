...
try:
    ...
    print(e)
    server = {"host": "127.0.0.1", "port": 2088}

print(server)
_s = connect('QuarkServer', host=server['host'], port=server['port'])
...
def submit(app: dict | Scan | Scanner, block: bool | float = False,
           ...
           reset: list = [], dry_run: bool = False, preview: list = []):
