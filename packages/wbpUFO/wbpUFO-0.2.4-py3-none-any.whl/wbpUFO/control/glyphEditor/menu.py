...
class DefaultEditMenu(wx.Menu):
    def __init__(self, canvas, subject):
        ...
        if isinstance(self.subject, Guideline):
            ...
            if isinstance(self.subject.glyph, Glyph):
                ...
                item = wx.MenuItem(self, ID, "Convert to global Guideline", "", wx.ITEM_NORMAL)
                self.Append(item)
                self.Bind(wx.EVT_MENU, self.on_convertToGlobalGuideline, id=item.GetId())
        else:
            # add new
            ...
            if len(self.glyph.guidelines) > 0:
                ...
                self.Bind(wx.EVT_MENU, self.on_clearGuidelines, id=clearGuidelines.GetId())
        ...
        # ]
        # if any(os.path.isdir(p) for p in macroFolderPath):
        #     macroMenu = MacroMenu(folderList=macroFolderPath)
    ...
    def on_editGuideline(self, event):
        ...
        with GuidelineDialog(self.canvas, guideline.getDataForSerialization()) as guidelineDialog:
