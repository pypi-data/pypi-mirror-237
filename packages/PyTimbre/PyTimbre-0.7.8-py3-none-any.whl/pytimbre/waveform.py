...
class Waveform:
    ...
    def roughness(self):
        ...
        # pad audio
        audio_samples = np.lib.pad(wfm.samples, (512, 0), 'constant', constant_values=(0.0, 0.0))
    ...
    def determine_calibration_scale_factor(self, level: float = 114, frequency: float = 1000, frequency_tolerance:
    float = 25):
        """
    ...
    def normalize_loudness(self, target_loudness: float = -24.0, inplace: bool = True):
        ...
        if inplace:
            ...
            return Waveform(pressures=self.samples * gain, sample_rate=self.sample_rate, start_time=self.start_time)
    ...
    def calculate_modulation(self):
        ...
        if not is_sustained:
            ...
            x_matrix = np.array([len(temps_sec_v), np.sum(temps_sec_v),
                                 np.sum(temps_sec_v), np.sum(temps_sec_v ** 2)]).reshape((2, 2))
