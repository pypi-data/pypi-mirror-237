...
class LLMRunMetadata(NamedTuple):
    total_token_count: Optional[Union[int]] = None
    prompt_token_count: Optional[Union[int]] = None
    response_token_count: Optional[Union[int]] = None
...
class DocumentColumnNames:
    ...
    text_embedding_column_names: EmbeddingColumnNames = None
...
class Schema(BaseSchema):
    ...
    def get_used_columns_counts(self) -> Dict[str, int]:
        ...
        def add_to_dict(col):
            if col in columns_used_counts:
                columns_used_counts[col] += 1
            else:
                columns_used_counts[col] = 1
        ...
        if self.embedding_feature_column_names is not None:
            for emb_col_names in self.embedding_feature_column_names.values():
                add_to_dict(emb_col_names.vector_column_name)
                if emb_col_names.data_column_name is not None:
                    add_to_dict(emb_col_names.data_column_name)
                if emb_col_names.link_to_data_column_name is not None:
                    add_to_dict(emb_col_names.link_to_data_column_name)
        ...
        if self.prompt_column_names is not None:
            add_to_dict(self.prompt_column_names.vector_column_name)
            if self.prompt_column_names.data_column_name is not None:
                add_to_dict(self.prompt_column_names.data_column_name)

        if self.response_column_names is not None:
            add_to_dict(self.response_column_names.vector_column_name)
            if self.response_column_names.data_column_name is not None:
                add_to_dict(self.response_column_names.data_column_name)
...
class CorpusSchema(BaseSchema):
    ...
    document_text_embedding_column_names: EmbeddingColumnNames = None

    def get_used_columns(self) -> Set[str]:
        used_columns = set()
        if self.document_id_column_name is not None:
            used_columns.add(self.document_id_column_name)
        if self.document_version_column_name is not None:
            used_columns.add(self.document_version_column_name)
        if self.document_text_embedding_column_names is not None:
            if self.document_text_embedding_column_names.vector_column_name is not None:
                used_columns.add(self.document_text_embedding_column_names.vector_column_name)
            if self.document_text_embedding_column_names.data_column_name is not None:
                used_columns.add(self.document_text_embedding_column_names.data_column_name)
            if self.document_text_embedding_column_names.link_to_data_column_name is not None:
                used_columns.add(self.document_text_embedding_column_names.link_to_data_column_name)
        return used_columns
