...
class Client:
    ...
    def log(
        ...
        s = self._get_pb_schema(schema, model_id, model_version, model_type, environment, batch_id)
    ...
    def _get_pb_schema_corpus(self, schema: Schema, s: pb2.Schema):
        if schema.document_id_column_name is not None:
            s.arrow_schema.document_column_names.id_column_name = schema.document_id_column_name
        ...
        if schema.document_text_embedding_column_names is not None:
            s.arrow_schema.document_column_names.text_column_name.vector_column_name = (
                schema.document_text_embedding_column_names.vector_column_name
            )
            s.arrow_schema.document_column_names.text_column_name.data_column_name = (
                schema.document_text_embedding_column_names.data_column_name
            )
            if schema.document_text_embedding_column_names.link_to_data_column_name is not None:
    ...
    def _get_pb_schema(
        ...
        elif environment == Environments.CORPUS:
        ...
        # None of the fields below exist for Corpus datasets
        if environment == Environments.CORPUS:
            return self._get_pb_schema_corpus(schema, s)
    ...
    def _log_arrow(
        ...
        try:
            ...
            if not pa_schema.has_prediction_columns():
                logger.warning(
