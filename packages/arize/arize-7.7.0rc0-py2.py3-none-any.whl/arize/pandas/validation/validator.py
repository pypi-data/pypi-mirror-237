...
class Validator:
    ...
    def validate_params(
        ...
        if environment == Environments.CORPUS:
            return list(
                chain(
                    general_checks,
                    Validator._check_missing_columns_corpus(dataframe, schema),
                )
            )
        else:
            general_checks = chain(
    ...
    @staticmethod
    def validate_types(
        model_type: ModelTypes, environment: Environments, schema: Schema, pyarrow_schema: pa.Schema
    ) -> List[err.ValidationError]:
        column_types = dict(zip(pyarrow_schema.names, pyarrow_schema.types))
        if environment == Environments.CORPUS:
            return Validator._check_type_document_columns(schema, column_types)
    ...
    def validate_values(
        ...
        if environment == Environments.CORPUS:
            return list(
                chain(
                    Validator._check_invalid_missing_values_corpus(dataframe, schema),
    ...
    @staticmethod
    def _check_missing_columns_corpus(
        dataframe: pd.DataFrame,
        schema: Schema,
    ) -> List[err.MissingColumns]:
        # converting to a set first makes the checks run a lot faster
    ...
    def _check_embedding_raw_data_characters(
        ...
        if schema.embedding_feature_column_names is None or len(dataframe) == 0:
            return []
    ...
    def _check_value_tag(dataframe: pd.DataFrame, schema: Schema) -> List[err.InvalidTagLength]:
        if schema.tag_column_names is None or len(dataframe) == 0:
            return []
    ...
    # do not have any null values and returns an error if they do
    @staticmethod
    def _check_invalid_missing_values_corpus(
        dataframe: pd.DataFrame, schema: Schema
    ) -> List[err.InvalidValueMissingValue]:
        errors = []
