...
def record_quantization_variables(model):
    ...
    with recording(True):
        if len(recurrent_layers):
            ...
            # The number of samples that will allow proper recording is equal to the higher
            # subsampling ratio found in the model + 1 so that both real and imaginary parts are
            # initialized
            num_samples = 1
            for rec in recurrent_layers:
                num_samples = np.maximum(num_samples, rec.subsample_ratio)
            num_samples += 1
