...
class TestKodiakStatusEndpoint(unittest.TestCase):
    def setUp(self):
    ...
    def test_status_endpoint_with_valid_ip(self):
        # Test the endpoint with a valid IP address
        data = {
            "ipaddress": "192.168.1.247",
        }
        response = self.app.get('/kodiak/status', json=data)
        data = json.loads(response.data)

        self.assertEqual(response.status_code, 200)
        self.assertIn('Success', data)

    def test_status_endpoint_with_bad_format_ip(self):
        # Test the endpoint with a valid IP address
        data = {
            "ipaddress": "192.168.1.247."
        }
    ...
    def test_status_endpoint_with_bad_no_ip(self):
        # Test the endpoint with a valid IP address
        data = {
            "ipaddress": ""
        }
    ...
    def test_status_endpoint_with_bad_args(self):
        # Test the endpoint with a valid IP address
        data = {
            "key": "192.168.1.247."
        }
    ...
    def test_status_endpoint_with_missing_ip(self):
