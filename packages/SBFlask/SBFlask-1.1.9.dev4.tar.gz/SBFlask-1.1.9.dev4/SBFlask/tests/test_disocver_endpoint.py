...
class TestKodiakEndpointDiscover(unittest.TestCase):
    def setUp(self):
    ...
    def test_discover_endpoint_with_valid_data(self):
        # Test the endpoint with valid data
        data = {
            "ipaddress": "192.168.1.247",
            "key": "your_api_key"
        }
        response = self.app.post('/kodiak/port/discover', json=data)
        data = json.loads(response.data)

        self.assertEqual(response.status_code, 200)
        self.assertIn('Success', data)

    def test_discover_endpoint_with_missing_key(self):
        # Test the endpoint with missing API key
        data = {
            "ipaddress": "192.168.1.247"
        }
        response = self.app.post('/kodiak/port/discover', json=data)
    ...
    def test_discover_endpoint_with_missing_ip(self):
    ...
    def test_discover_endpoint_with_missing_data(self):
        # Test the endpoint with missing data
        response = self.app.post('/kodiak/port/discover')
        data = json.loads(response.data)
    ...
    def test_discover_endpoint_with_wrong(self):
