...
from fms_robot_plugin.typings import LaserScan, Twist, Pose, Map, Status
...
class Robot:
    ...
    def on_heartbeat_ack(self, cb: Callable[[str], None]):
        topic = f"robots/{self.robot_key}/heartbeat_ack"
        self.consumer(topic).consume(lambda data: cb(data["sent_at"]))
    ...
    def set_heartbeat(self, initial: bool):
        self.mqtt.publish(
            f"robots/{self.robot_key}/heartbeat",
            {"sent_at": datetime.datetime.utcnow().isoformat(), "initial": initial},
        )
    ...
    def maintain_connection(self):
        threads = [
            Thread(target=self.maintain_heartbeat),
            Thread(target=self.on_heartbeat_ack, args=(self.on_receive_heartbeat,)),
        ]
        for thread in threads:
            thread.start()

    def maintain_heartbeat(self):
        self.set_heartbeat(initial=True)

        while True:
            last_connected_difference = (datetime.datetime.utcnow() - self.last_connected_at).seconds
            logging.debug("Last connected difference: %s", last_connected_difference)

            with self.is_connected_lock:
                if self.last_connected_at is not None:
                    if last_connected_difference > self.heartbeat_timeout_secs:
                        self.is_connected = False

                        logging.info("Bot is disconnected from FMS")

            time.sleep(self.heartbeat_interval_secs)

    def on_receive_heartbeat(self, sent_at_str: str):
        ...
        with self.is_connected_lock:
            if not self.is_connected:
                ...
                logging.info("Bot is connected to FMS")

            self.last_connected_at = sent_at

        time.sleep(self.heartbeat_interval_secs)

        self.set_heartbeat(initial=False)
