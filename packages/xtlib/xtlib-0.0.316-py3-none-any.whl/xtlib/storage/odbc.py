...
class OdbcDatabase(implements(DbInterface)):
    ...
    def create_request(self, ws_name, request_id, request_dict):
        ...
        request_dict["create_time"] = utils.get_arrow_now_str()
    ...
    def get_restart_list(self, ws_name, job_id, node_index):
        ...
        '''
        filter = {"job_id": job_id, "node_index": node_index, "is_child": 1, "status": {"$nin": ["error", "cancelled", "completed"]}}
    ...
    def mark_job_started_running(self, ws_name, job_id):
        ...
        rd = {}
        run_start_time = utils.get_arrow_now_str()
        ...
        if doc and "started" in doc:
            ...
            queue_duration = utils.get_time_from_arrow_str(run_start_time) - utils.get_time_from_arrow_str(started_time)
    ...
    def mark_job_completed(self, ws_name, job_id, status, run_started, end_time, zero_counts):
        ...
        if self.update_job_stats_enabled:
            ...
            if not end_time:
                end_time = utils.get_arrow_now_str()
            ...
            if run_started:
                start_time_secs = utils.get_time_from_arrow_str(run_started)
                end_time_secs = utils.get_time_from_arrow_str(end_time) if end_time else time.time()
        ...
        if self.update_job_stats_enabled:
            ...
            if end_time_secs:
                ...
                if run_started:
                    ...
                    # set end_time
                    end_time = utils.get_arrow_str_from_time(end_time_secs)
                    rd["end_time"] = end_time

                else:
                    # compute queue_duration
                    create_time_secs = utils.get_time_from_arrow_str(started)
    ...
    def update_run_info(self, ws_name, run_name, orig_dd, hparams=None, metrics=None, 
        ...
        if self.update_run_stats_enabled and run_stats:
            run_stats["last_time"] = utils.get_arrow_now_str()
        ...
        if self.update_run_stats_enabled:
           ...
           #print("mark_run_ended_in_past: error_msg: {}".format(error_msg))
            ...
            if end_time:
                ...
                if start_time:
                    # compute run_duration 
                    start_time_secs = utils.get_time_from_arrow_str(start_time)
                    run_duration = end_time_secs - start_time_secs
                    rd["run_duration"] = run_duration
                    ...
                    # compute queue_duration
                    create_time_secs = utils.get_time_from_arrow_str(create_time)
                    queue_duration = end_time_secs - create_time_secs
                    rd["queue_duration"] = queue_duration
    ...
    def mark_node_ended_in_past(self, ws_name, job_id, node_info, status, end_time, ended_count, error_count):
        ...
        if self.update_node_stats_enabled:
            ...
            if end_time:
                end_time_secs = utils.get_time_from_arrow_str(end_time) 
                ...
                if post_end_time:
                    ...
                    if post_start_time:
                        post_start_time_secs = utils.get_time_from_arrow_str(post_start_time)
                        ...
                        if app_start_time:
                            app_start_time_secs = utils.get_time_from_arrow_str(app_start_time)
                            ...
                            if prep_start_time:
                                prep_start_time_secs = utils.get_time_from_arrow_str(prep_start_time)
