...
class AzureML(BackendBase):
    ...
    def get_azure_cred(self):
        ...
        def get_the_token_func(audience):
            ...
            if not token:
                value = self.config.get_vault()
                credential, _ = value.get_creds_core("auto")
    ...
    def get_aml_ws(self, ws_name):
        ...
        ws_type = self.config.get_required_service_property(creds, "type", ws_name)

        # cred = None
        # if ws_type == "singularity":
        #     console.print("authenticating with auzre... ", end="")
        ...
        if "aml_run_id" in service_node_info:
            ...
            if simple_status in ["error", "completed", "cancelled"]:
                ...
                if not "endTimeUtc" in details:
                    ...
                    node_end_time = utils.get_time_from_arrow_str(node_end_time_str)
