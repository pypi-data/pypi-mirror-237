...
class Runner():
    ...
    def process_args(self, args):
        ...
        if code_upload and (not self.is_docker) and (not alt_code) and (not os.path.exists(target_file)):
        ...
        if target_file.endswith(".bat") or target_file.endswith(".sh"):
            ...
            # target_file may not exist (or be changed) if we are approving a request
            if not args["from_request"]:       # os.path.exists(target_file):
    ...
    def write_node_info_data(self, _id, ws_name, job_id, node_index, total_run_count, node_count, 
        ...
        str_now = utils.get_arrow_now_str()
    ...
    def snapshot_all_code(self, bootstrap_dir, snapshot_dir, cmd_parts, args):
        ...
        # this figures out the node's path to the script 
    ...
    def create_request(self, ws_name, snapshot_dir, username, target, nodes, runs, args):
        description = args["description"]
        if not description:
    ...
    def create_job(self, args):
        ...
        if request_flag:
            ...
            self.create_request(workspace, snapshot_dir, username, compute, node_count, total_run_count, args)
            return None
    ...
    def submit_job(self, compute, compute_def, workspace, job_id, sweeps_text, cmds, fake_submit, using_hp, is_distributed, 
        ...
        job_create_time = utils.get_arrow_now_str()
