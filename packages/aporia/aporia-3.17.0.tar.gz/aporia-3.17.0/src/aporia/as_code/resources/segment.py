...
class Segment(BaseResource):
    ...
    def compare(self, resource_data: Dict) -> CompareStatus:
        ...
        is_custom_segment = extended_data.get('is_custom_segment') if extended_data else 'terms' in self._args.keys()
        if all(
            [
                (not is_custom_segment) or (self._args.get("terms") == resource_data["terms_values"]),
    ...
    def read(self, client: Client, id: str) -> Dict:
        segment = _Segment.read(client=client, id=id).raw_data
