...
paths = []
folder_path = "./assets/Complex-production-America-Stop-Sign"
...
for path in paths:
    ...
    creds = DatasetCreds(arn="arn:aws:iam::527593518644:role/model-distribution")
    # create test_ds object of Dataset instance
    df = pd.read_pickle(path).head(1)
    print("PAAAAAATH", path)
    test_ds = Dataset(test_session=test_session, 
                      name="stopsign-event-img-ds-full-v5", 
                      type=DATASET_TYPE.IMAGE,
                      data=df, 
                      schema=image_ds_schema, 
                      creds=creds)
