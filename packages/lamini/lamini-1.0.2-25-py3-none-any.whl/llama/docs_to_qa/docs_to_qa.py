...
{user_prompt} [/INST]"""
    ...
    def _extract_text_from_html(self, html):
        ...
        # Find the main content element that contains the text
        ...
        text_elements = soup.select("p")  # Example: <p> tags

        # Extract the text from the content element
        text=[]
        for elements in text_elements:
            text.append(elements.get_text())
        return ''.join(text)
        ...
        for dirpath, _, filenames in os.walk(self.docs_dirpath):
            for filename in filenames:
                ...
                if docs_path_type == ".csv":
                    ...
                    doc_index = self._get_chunks(text, char_chunk_size, doc_index)
                elif docs_path_type == ".html":
                    html = open(docs_path).read(1000000) # size limit
