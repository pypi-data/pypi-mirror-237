...
class yarr(hardware_control_base):
    def __init__(self, config, name="yarr", *args, **kwargs):
        ...
        if "emulator" in self.eyeDiagram_exe:
            self.emulator = True
            log.info(f"[{name}] running eyeDiagram emulator!!")
    ...
    def eyeDiagram(self, dryrun=False):
        if dryrun:
            cmd = (
                f"{self.eyeDiagram_exe} -r {self.controller} -c {self.connectivity} -n"
            )
        else:
            cmd = f"{self.eyeDiagram_exe} -r {self.controller} -c {self.connectivity}"
        return self.send_command_and_read(
            cmd, type=str, purpose="eye diagram", success_code=self.success_code
        )
