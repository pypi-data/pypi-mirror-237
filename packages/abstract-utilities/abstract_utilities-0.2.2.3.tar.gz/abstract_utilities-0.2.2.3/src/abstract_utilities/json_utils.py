...
def unified_json_loader(file_path, default_value=None, encoding='utf-8'):
    ...
    content = all_try(var_data={"file_path": file_path}, function=safe_json_load, error=False)
    
    if isinstance(content, dict):
        return content
    
    content = all_try(var_data={"file_path": file_path}, function=safe_json_loads, error=False)
