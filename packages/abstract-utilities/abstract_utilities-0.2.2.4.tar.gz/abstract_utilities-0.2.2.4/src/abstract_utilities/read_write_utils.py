...
def delete_file(file_path:str):
    if os.path.isfile(file_path):
        os.remove(file_path)
def determine_file_encoding(head):
    """
    Determine file encoding based on its BOM (byte order mark).
    """
    if head.startswith(b'\xEF\xBB\xBF'):
        ...
        return ('utf-8', 0)  # default to utf-8 if no BOM is detected

def read_file_with_detected_encoding(filename):
    with open(filename, 'rb') as file:
        head = file.read(4)  # reading 4 bytes should be enough for the BOMs you have
        encoding, bom_length = determine_file_encoding(head)

        # If encoding wasn't detected, fall back to a default
        if not encoding:
            encoding = 'utf-8'

        file.seek(bom_length)  # seek past the BOM if there's any
