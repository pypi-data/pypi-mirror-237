...
vector_database_settings_trafaret = t.Dict(
    {
        ...
        t.Key("batch_size", optional=True): t.Or(t.Int, t.Null),
...
class VectorDatabaseSettings(APIObject):
    ...
    batch_size : int or None, optional
        The batch size to use when retrieving documents.
...
class LLMBlueprint(APIObject):
    ...
    # TODO: Add create_from_chat_prompt after ChatPrompt API is added to SDK
    ...
    def update(
        ...
        ----------
        llm_blueprint : LLMBlueprint or str
            The LLM blueprint you want to update.
        ...
        url = f"{cls._client.domain}/{cls._path}/{get_entity_id(llm_blueprint)}/"
        ...
        r_data = cls._client.patch(url, json=json_payload)
        return cls.from_server_data(r_data.json())

    @classmethod
    def delete(cls, llm_blueprint: Union[LLMBlueprint, str]) -> None:
        """
        Delete a single LLM blueprint.

        Parameters
        ----------
        llm_blueprint : LLMBlueprint or str
            The LLM blueprint you want to delete.
        """
        url = f"{cls._client.domain}/{cls._path}/{get_entity_id(llm_blueprint)}/"
