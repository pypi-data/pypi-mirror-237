...
class SparkConnectClient(object):
    ...
    @classmethod
    def retry_exception(cls, e: Exception, retry_count: int = 0) -> bool:
        """
        ...
        """
        # Mirrors the retry filter of the Scala client in
        ...
        if e.code() in [
            ...
            # BEGIN-EDGE
            # There are certain cases where the control plane will return an
            ...
            # Since we have re-attachable execution, we can retry this one as well.
            if "Received RST_STREAM with error" in msg:
                return True
            # END-EDGE

        if e.code() == grpc.StatusCode.UNAVAILABLE:
            # If this is an unavailable exception, and we haven't been able to connect to
            # the service, we can fail earlier.
            if retry_count >= 4:
                # This is a terminal error when the client cannot connect to any endpoint
                # and thus cannot be retried.
                msg_connection_failed = "failed to connect to all addresses"
                if msg_connection_failed in msg:
                    return False
            # By default, all StatusCode.UNAVAILABLE exceptions can be retried.
            return True

        if e.code() == grpc.StatusCode.UNKNOWN:
            # Retry all "UNKNOWN" errors. Anecdotal evidence shows that this occurs when
            # there is network interruption or when the Connect router or service is
            # momentarily unavailable - https://databricks.atlassian.net/browse/ES-877410.
            return True
    ...
    def __init__(
        ...
        self._retry_policy = {
            ...
            # DBR: Guarantee at least 25 minutes # Edge
            "max_retries": 30,  # Edge
...
class AttemptManager:
    ...
    def __exit__(
        ...
        if isinstance(exc_val, BaseException):
            ...
            if self._can_retry(exc_val, self._retry_state._count) or isinstance(
                exc_val, RetryException
            ):
