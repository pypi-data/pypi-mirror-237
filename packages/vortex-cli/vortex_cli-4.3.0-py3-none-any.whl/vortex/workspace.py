...
class Workspace:
    ...
    def __init__(self, path: str | None = None, init: bool = False) -> None:
        ...
        if not self._path.is_dir():
            raise WorkspaceError(
                f"Workspace '{self._path}' does not exist.\n"
                "Hint: You can create it with '--init'"
            )
    ...
    def init(self) -> None:
        ...
        print(f"Workspace initialised. ({self.path})")
    ...
    def listapps(self, server: PuakmaServer | None = None) -> list[PuakmaApplication]:
        return [
            app
            for app in [PuakmaApplication.from_dir(dir) for dir in self.listdir(server)]
        ]
    ...
    def update_vscode_settings(self: Workspace, reset: bool = False) -> None:
        ...
        workspace_folders = [
            ...
            for dir in (self.vscode_dir, self.config_dir, *self.listdir())
        ]
        folder_settings = {
