...
from vortex.commands.upload import upload
from vortex.commands.watch import watch
from vortex.logging_handler import logging_handler
from vortex.logging_handler import LoggingHandler
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="vortex-cli", description="Vortex command line tool"
    )
    parser.add_argument(
        "--version", "-V", action="version", version=f"%(prog)s {C.VERSION}"
    )
    parser.add_argument(
        "--workspace",
        "-w",
    ...
    cli.add_upload_parser(command_parser)
        ...
        if args.command in cli.NON_SERVER_COMMANDS:
            if args.command == "clean":
                ...
                return config(
                    ...
                    output_server_config=args.output_workspace_path,
                )
            ...
            if args.command in ("list", "ls"):
                ...
                return upload(server, args.group, args.name, args.pmx_file)
            elif args.command in ("execute", "exec"):
                return execute(server, args.cmd)
            elif args.command:
