...
from azureml.metrics.common.utilities import extract_common_kwargs, check_kwargs, \
    concatenate_calculated_metrics
...
def compute_metrics_chat_completion(task_type, y_test, y_pred, kwargs):
    ...
    batch_size = kwargs.pop("batch_size", 16)
    add_start_token = kwargs.pop("add_start_token", True)
    # using get() for openai_params as it is needed to compute rag_metrics
    openai_params = kwargs.get("openai_params", None)
    common_args, custom_dimensions, log_activity, log_traceback = extract_common_kwargs(kwargs)
    ...
    chat_computed_metrics = metrics.compute(y_test=y_test, y_pred=y_pred)
    rag_computed_metrics = compute_metrics_rag_evaluation(task_type, y_test, y_pred, kwargs)
    computed_metrics = concatenate_calculated_metrics([chat_computed_metrics, rag_computed_metrics])
