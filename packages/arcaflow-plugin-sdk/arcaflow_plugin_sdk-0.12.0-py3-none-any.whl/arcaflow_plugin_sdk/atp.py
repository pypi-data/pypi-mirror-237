...
class ATPServer:
        ...
        try:
            while True:
                ...
                if msg_id == MessageType.WORK_START:
                    ...
                    self.send_error_message(
                        run_id, False, False, f"Unknown runtime message ID: {msg_id}"
                    )
    ...
    def handle_work_start(self, run_id: str, work_start_msg: typing.Dict[str, any]):
        if work_start_msg is None:
            self.send_error_message(run_id, True, False, "Work start message is None.")
            return
        if "id" not in work_start_msg:
            self.send_error_message(
                run_id, True, False, "Work start message is missing the 'id' field."
            )
            return
        if "config" not in work_start_msg:
            self.send_error_message(
                run_id, True, False, "Work start message is missing the 'config' field."
            )
            return
        # Save for later
        ...
        run_thread = threading.Thread(
            target=self.start_step, args=(run_id, work_start_msg)
        )
    ...
    def start_step(self, run_id: str, work_start_msg: any):
        try:
            output_id, output_data = self.plugin_schema.call_step(
                run_id,
                work_start_msg["id"],
                self.plugin_schema.unserialize_step_input(
                    work_start_msg["id"], work_start_msg["config"]
                ),
            )
    ...
    def read_single_result(self) -> (str, str, any, str):
        ...
        while True:
            ...
            if msg_id == MessageType.WORK_DONE:
                ...
                return (
                    runtime_msg["run_id"],
                    signal_msg["output_id"],
                    signal_msg["output_data"],
                    signal_msg["debug_logs"],
                )
            elif msg_id == MessageType.SIGNAL:
