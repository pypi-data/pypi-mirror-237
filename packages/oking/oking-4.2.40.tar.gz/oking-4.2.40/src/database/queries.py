    ...
    if connection_type.lower() == 'mysql':
        return '''insert into openk_semaforo.pedido 
                 (pedido_oking_id, pedido_venda_id, pedido_canal, numero_pedido_externo, data_pedido, status, cliente_id
                  ...
                  , cnpj_intermediador, end_entrega_cep, end_entrega_tipo_logradouro, end_entrega_logradouro
                  , end_entrega_numero, end_entrega_complemento, end_entrega_bairro, end_entrega_cidade
...
def get_client_protocol_command(connection_type: str):
    if connection_type.lower() == 'mysql':
        ...
        return '''update semaforo_cliente 
                            set CODIGO_REFERENCIA_ERP = ? 
                          where id = (select cliente_id from semaforo_pedido where pedido_oking_id = ?)'''
...
def update_log_integracao(connection_type: str):
    if connection_type.lower() == 'mysql':
        return '''update openk_semaforo.pedido set log_integracao = %s
        where pedido_oking_id = %s;'''
    elif connection_type.lower() == 'oracle':
        return '''UPDATE OPENK_SEMAFORO.PEDIDO SET LOG_INTEGRACAO = :1
        WHERE PEDIDO_OKING_ID = :2'''
    elif connection_type.lower() == 'sql':
        return '''update openk_semaforo.pedido set log_integracao = ?
        where pedido_oking_id = ?;'''
    elif connection_type.lower() == 'firebird':
        return '''update openk_semaforo.pedido set log_integracao = ?
        where pedido_oking_id = ?;'''
