...
of soil moisture indices from climate datasets -- Extensible to other earth
observation datasets using the OpenEO API."""
...
class SMClimatology:
    ...
    def __init__(
        ...
        self.archive_path = archive_path  # input path; same as sm_download_path
    ...
    def get_archive(self):
        ...
        Args:
            archive_path (str): directory to raw ERA5 soil moisture datasets

        Returns:
            era_stack(xArray Datasets): stacked up ERA5 soil moisture datasets
        """

        logger.info(f"loading available soil moisture files in archive folder {self.archive_path}")
        sm_files = []
        for path in Path(self.archive_path).rglob("*.nc"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("archive_path", type=str, help="path to the downloaded soil moisture")
    ...
    sm_climate = SMClimatology(args.archive_path, args.climatology_path, args.reference)
