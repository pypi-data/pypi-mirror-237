...
class CheckCallback(Callback):
    def __init__(self, monitor, on_stage='val', mode='min', patience=None, ckpt_path='./logs'):
        """
    ...
    def check(self, metrics, model, opt):
        ...
        if self.mode == 'max':
            if  value > self.best_value:
                ...
                save_state(model, opt, self.ckpt_file_path, best_value=self.best_value)
                self.worse_times = 0
            else:
                self.worse_times += 1
        else:
    ...
    def on_before_fit(self, trainer, epochs):
        if trainer.resume:
            if trainer.resume == True:
                ckpt_id = get_latest_ckpt_id(self.ckpt_path)
            else:
                ckpt_id = str(trainer.resume)

            try:
                print(f'loading checkpoint model of {ckpt_id} ...')
                path = osp.join(self.ckpt_path, ckpt_id, 'checkpoint.ckpt')
                self.load_state(trainer, path)
            except Exception as e:
                print(f'Loading failed!, {e}\nstarting training with random parameters!')

        # 创建新的checkpoint路径
        ckpt_path = osp.join(self.ckpt_path, trainer.running_id)
        check_path(ckpt_path)
        self.ckpt_file_path = osp.join(ckpt_path, 'checkpoint.ckpt')
    ...
    def on_before_test_epochs(self, trainer, tasks):
        try:
            ...
            self.load_state(trainer, self.ckpt_file_path)
...
def get_latest_ckpt_id(path):
    try:
        r_ids = [int(f) for f in os.listdir(path) if f.isdigit()]
        return str(max(r_ids))
    except Exception:
        return ''
