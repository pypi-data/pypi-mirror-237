...
class DefaultCallback(Callback):
    ...
    def on_after_train_batch(self, trainer, metrics, batch_idx):
        ...
        log_batch(metrics, self.epoch_idx+1, self.total_epochs, self.global_train_batch_idx, self.total_train_batchs, 'TRAIN')

    def on_after_val_batch(self, trainer, metrics, batch_idx):
        self.global_val_batch_idx += 1
        log_batch(metrics, self.epoch_idx+1, self.total_epochs, self.global_val_batch_idx, self.total_val_batchs, 'VAL')
    ...
    def on_after_test_epoch(self, trainer, task, metrics):
        ...
        log_epoch({'test': metrics}, self.global_test_epoch_idx, self.total_test_epochs)

    def on_after_test_batch(self, trainer, metrics, batch_idx):
        self.global_test_batch_idx += 1
        log_batch(metrics, self.global_test_epoch_idx, self.total_test_epochs, self.global_test_batch_idx, self.total_test_batchs, 'TEST')
