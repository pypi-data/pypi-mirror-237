...
from regscale.core.app.utils.XMLIR import XMLIR, XMLIR2, XMLIRTraversal
...
class UserMappingIR(XMLIR2):
    ...
    def get_authorized_privileges(
        self, trv: XMLIRTraversal
    ) -> List[UserMappingIRAuthorizedPrivilegesIR]:
        ...
        return [
            UserMappingIRAuthorizedPrivilegesIR(
                XMLIRTraversal(
                    xpathToThis="....", el=el, root=trv.root, namespaces=trv.namespaces
                )
            )
            for el in vlist
        ]
...
def handle_user(trv: FedrampTraversal):
    ...
    try:
        ...
        # Extract user nodes
        users = root.xpath(
            ".//oscal:system-implementation/oscal:user", namespaces=namespaces
        )
        ...
        for i, elem in enumerate(users):
            res = UserMappingIR(
                XMLIRTraversal(
                    xpathToThis="....", el=elem, root=root, namespaces=namespaces
                )
            )
            ...
            sending = {
                ...
                "functions": [str(dict(r)) for r in res.authorized_privileges],
                "securityPlanId": trv.ssp_id,
                "createdById": api.config["userId"],
                "isPublic": True,
            }

            system_roles = SystemRoles(**sending)
            server_res = system_roles.insert_systemrole(api.app)
