...
def update_security_controls(
    new_security_controls: list[dict], old_security_controls: list[dict], api: Api
) -> None:
    """
...
def update_objectives(
    new_objectives: list[dict], old_objectives: list[dict], api: Api
) -> None:
    """
...
def update_parameters(
    new_parameters: list[dict], old_parameters: list[dict], api: Api
) -> None:
    """
    ...
    for new_test in new_tests:
        for old_test in old_tests:
            if new_test["testId"] == old_test["testId"]:
                ...
                update = api.put(
                    url=api.config["domain"]
                    + f"/api/controlTestPlans/{old_test['id']}",
                    json=old_test,
                )
                update.raise_for_status()
                if update.ok:
...
def parse_new_catalog() -> Tuple[list, list, list, list, list]:
    ...
    with job_progress:
        ...
        new_ccis = parse_dict_and_sublevel(
            data=new_catalog, key="catalogue", sub_key="ccis"
        )
        # update the task as complete
        job_progress.update(retrieving_new_catalog, advance=1)
        # retrieve new catalog objectives
        new_objectives = parse_dict_and_sublevel(
            data=new_catalog, key="catalogue", sub_key="objectives"
        )
        # update the task as complete
        job_progress.update(retrieving_new_catalog, advance=1)
        # retrieve new catalog parameters
        new_parameters = parse_dict_and_sublevel(
            data=new_catalog, key="catalogue", sub_key="parameters"
        )
        # update the task as complete
        job_progress.update(retrieving_new_catalog, advance=1)
        # retrieve new catalog tests
        new_tests = parse_dict_and_sublevel(
            data=new_catalog, key="catalogue", sub_key="tests"
        )
        # update the task as complete
...
def parse_dict_and_sublevel(
    data: dict, key: str, sub_key: str, del_key: Optional[str] = None
) -> list:
    """
    Function to parse a dictionary and retrieve data from the sublevel dictionary key
    :param data: dictionary to parse
    :param key: key to be extracted from the dictionary
    :param sub_key: the sub-level key being looked for
    :param del_key: the sub-level key to delete, defaults to None
...
def data_report(data_name: str, archived: list, updated: list, created: list) -> None:
    ...
    :param data_name: catalog data element being updated
    :param archived: archived data elements
    :param updated: updated data elements
    :param created: created data elements
    :return: None
    """
