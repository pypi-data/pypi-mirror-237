    ...
    def check_get_methods(self) -> None:
        ...
        for attr in attributes:
            ...
            if not hasattr(self, getter_name):
                raise AttributeError(
                    f"No 'get_{attr}' method found. All non-callable, non-'__'-prefixed custom attributes must have a getter."
                )
            ...
            if "return" not in getter_hints:
                raise TypeError(
                    f"Getter '{getter_name}' does not have a return type hint."
                )
        ...
        for getter in getters:
            ...
            if not hasattr(self, attr_name):
                raise AttributeError(
                    f"Getter '{getter}' found, but no attribute '{attr_name}' found."
                )
