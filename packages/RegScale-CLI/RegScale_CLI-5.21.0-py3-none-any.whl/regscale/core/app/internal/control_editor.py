...
from openpyxl.styles import Protection, PatternFill, Alignment
...
def upload_data(regscale_parent_id: int, regscale_module: str, path: Path) -> None:
    ...
    accounts = pd.read_excel(
        os.path.join(path, "all_implementations.xlsx"), sheet_name="Accounts"
    )
    accounts = accounts.rename(
        columns={"User": "ControlOwner", "UserId": "ControlOwnerId"}
    )
