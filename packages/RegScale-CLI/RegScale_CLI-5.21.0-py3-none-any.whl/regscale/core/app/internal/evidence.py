...
def build_package(regscale_id: str, regscale_module: str, path: Path):
    """
    This function will build a directory of evidence with the selected RegScale Parent Id
    and RegScale Module and produce a zip file for extraction and use.

    """
    package_builder(regscale_id=regscale_id, regscale_module=regscale_module, path=path)


def package_builder(regscale_id: str, regscale_module: str, path: Path):
    """
    ...
    :param path: directory for file location
    :return: None
    """
    check_file_path(path)
    with progress:
        task = progress.add_task(
            "[white]Building and zipping evidence folder for audit...", total=6
        )
        try:
            # Obtaining MEGA Api for given Orgainzer Record.
            ssp = api.get(
                url=config["domain"] + f"/api/{regscale_module}/megaAPI/{regscale_id}"
            ).json()
            module_folder_name = (
                str(ssp["securityPlan"]["id"])
                + "_"
                + str(ssp["securityPlan"]["systemName"])
            )
            folder_contents_name = (
                str(ssp["securityPlan"]["id"]) + "_Evidence_Folder_Contents"
            )

            module_folder = os.path.join(path, str(module_folder_name))
            ...
            if ssp["attachments"]:
                attachments = ssp["attachments"]
                outter_attachments = []
                for i in attachments:
                    file_name = i["trustedDisplayName"]
                    stored_name = i["trustedStorageName"]
                    parentId = i["parentId"]
                ...
                json_data = json.dumps(
                    outter_attachments, indent=4, separators=(", ", ": ")
                )
                with open(
                    f"{path}/{folder_contents_name}.json", "w", newline="\n"
                ) as next_output:
                ...
                for f in outter_attachments:
                    ...
                    file = api.get(
                        url=config["domain"]
                        + f"/api/files/downloadFile/{id}/{module}/{storedName}/{fileHash}"
                    ).content

                    with open(f"{path}/{module_folder_name}/{fileName}", "wb") as att:
                        att.write(file)

            else:
                logger.info(
                    "No Evidence at SSP level for SSP. Checking for Evidence at Control level."
                )
            ...
            if ssp["normalizedControls"]:
                controls = ssp["normalizedControls"]
                control_attachments = []
                for i in controls:
                    name = i["control"]["item3"]["controlId"]

                    for p in i["attachments"]:
                        file_name = p["trustedDisplayName"] if p else None
                        stored_name = p["trustedStorageName"] if p else None
                        parentId = p["parentId"] if p else None
                        parentModule = p["parentModule"] if p else None
                        fileHash = p["fileHash"] if p else None
                        shaHash = p["shaHash"] if p else None
                        fileSize = p["size"] if p else None
                        dateCreated = p["dateCreated"] if p else None
                ...
                for i in control_folders:
                    os.makedirs(os.path.join(module_folder, str(i)))
                ...
                for f in control_attachments:
                    ...
                    with open(
                        f"{path}/{module_folder_name}/{fileDropFolder}/{fileName}", "wb"
                    ) as output:
                        output.write(file)
                    with open(
                        f"{path}/{module_folder_name}/{fileDropFolder}/{fileDropFolder}_Evidence_Folder_Contents.json",
                        "a",
                    ) as file_drop:
                ...
                logger.info("No Control level Evidence for SSP.")
            ...
            progress.update(task, advance=1)
            logger.info("An evidence zipfile has been created and is ready for use!")
        except Exception:
            logger.info("No SSP or Evidence exists for given Organizer Record.")
        ...
        return logger.info("Evidence zipfile located. Thank you!")
...
def remove_directory(directory_path: Path):
    ...
    :param directory_path: file path of directory to remove
    :return: None
    """
...
def zip_folder(folder_path: Path, zip_path: Path):
    ...
    :param folder_path: file path of evidence folder
    :param zip_path: file path for zip location of evidence folder
    :return: None
    """
