...
def generate_all_issues(regscale_id: str, regscale_module: str, path: Path):
    """
...
def all_issues(regscale_id: str, regscale_module: str, path: Path):
    ...
    if (
        ...
        for a in raw_data:
            ...
            Title = a["title"]
            DateCreated = reformat_str_date(a["dateCreated"])
            Description = a["description"] if a["description"] else "None"
            SeverityLevel = a["severityLevel"]
            CostEstimate = (
                a["costEstimate"]
                if a["costEstimate"] and a["costEstimate"] != "None"
                else 0.00
            )
            LevelOfEffort = (
                a["levelOfEffort"]
                if a["levelOfEffort"] and a["levelOfEffort"] != "None"
                else 0
            )
            DueDate = reformat_str_date(a["dueDate"])
            Identification = a["identification"] if a["identification"] else "None"
            Status = a["status"] if a["status"] else "None"
            DateCompleted = (
                reformat_str_date(a["dateCompleted"]) if a["dateCompleted"] else ""
            )
            ActivitiesObserved = (
                a["activitiesObserved"] if a["activitiesObserved"] else ""
            )
            FailuresObserved = a["failuresObserved"] if a["failuresObserved"] else ""
            RequirementsViolated = (
                a["requirementsViolated"] if a["requirementsViolated"] else ""
            )
            SafetyImpact = a["safetyImpact"] if a["safetyImpact"] else ""
            SecurityImpact = a["securityImpact"] if a["securityImpact"] else ""
            QualityImpact = a["qualityImpact"] if a["qualityImpact"] else ""
            SecurityChecks = a["securityChecks"] if a["securityChecks"] else ""
            RecommendedActions = (
                a["recommendedActions"] if a["recommendedActions"] else ""
            )
        ...
        dv4 = DataValidation(
            type="list",
            formula1="=Identifications!$A$2:$A$"
            + str(get_maximum_rows(sheet_object=workbook["Identifications"])),
            allow_blank=False,
            showDropDown=False,
            error="Your entry is not one of the available options",
            errorTitle="Invalid Entry",
        ...
        logger.info(
            "No Issues exist. Please check your selections for RegScale Id and RegScale Module and try again."
        )
        error_and_exit(
    ...
    logger.info(
        "Your data has beeen loaded. Please open the all_issues workbook and make your desired changes %s. "
        % path
    )
    return None
...
def upload_data(path: Path) -> None:
    ...
    df1 = pd.read_excel(
        os.path.join(path, "old_issues.xlsx"), sheet_name=0, index_col="Id"
    )

    df2 = pd.read_excel(
        os.path.join(path, "all_issues.xlsx"), sheet_name=0, index_col="Id"
    )
    ...
    if df1.equals(df2):
        ...
        accounts = accounts.rename(
            columns={"User": "IssueOwner", "UserId": "IssueOwnerId"}
        )
        ...
        api.update_server(
            ...
            json_list=updated_issues,
            message="Working on uploading updated issues to RegScale.",
            config=config,
            method="put",
        )

    logger.info(
        "Changes made to existing files can be seen in differences.txt file. Thank you! %s"
        % path
    )
    return None
...
def delete_file(path: Path):
    ...
    :param path: Path for artifacts folder or location of excel files
    return: None
    """
