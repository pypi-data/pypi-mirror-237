...
def iter_operators(operators: dict = AIRFLOW_CLICK_OPERATORS) -> dict:
    ...
    def _nested_dict(
        data: tuple, value: ClickCommand, result: dict = None
    ) -> Optional[dict]:
    ...
    for operator, op_dict in operators.items():
        if "__" in operator:
            ...
            new_dict = _nested_dict(
                data=operator_split, value=op_dict["command"], result=new_dict
            )
        else:
...
def return_all_commands_and_params():
    dct = {}
    for operator in AIRFLOW_CLICK_OPERATORS:
        click_name = " ".join(operator.split("__"))
        dct[click_name] = {
            "parameters": AIRFLOW_CLICK_OPERATORS[operator]["command"].parameters
        }
