...
def analyze(csv: bool) -> None:
    ...
    from regscale.dev.analysis import (
        analyze_git,
        analyze_code_files,
        generate_heatmap,
        generate_barplot,
    ...
    if csv:
        ...
        console.print(
            "[green]Done! You can view the raw analysis in analysis.csv or in analysis.json."
        )
        sys.exit(0)
    metrics_to_track = list(
        filter(lambda key: is_number(sample_data.get(key)), sample_data.keys())
    )
    # Aggregate metrics by directory with progress tracking
    aggregated_data = defaultdict(lambda: defaultdict(float))
    for file, metrics in track(
        data.items(), description="[cyan]Aggregating metrics..."
    ):
    ...
    with PdfPages("Metrics Report.pdf") as pdf:
        for metric in track(
            metrics_to_track, description="[cyan]Generating heatmaps..."
        ):
            ...
            gc.collect()
        for metric in track(
            metrics_to_track, description="[cyan]Generating barplots..."
        ):
        ...
        console.print("[yellow]Generating aggregated metrics graphics...")
        for metric in track(
            metrics_to_track, description="[cyan]Generating aggregated heatmaps..."
        ):
            heatmap_plot = generate_heatmap(
                aggregated_data, metric, figsize=(20, 20), font_size=10
            )
            plt.tight_layout()
    ...
    console.print(
        "[green]Done! You can view the raw analysis in analysis.json and the report in Metrics Report.pdf."
    )
