...
def analyze_git(folder_path: str = ".", subfolder: str = "regscale") -> dict:
    ...
    :returns: a dictionary of git information
    :rtype: dict
    """
    log = create_logger()
    try:
...
def parse_code(file_path: str) -> dict:
    ...
    :param str file_path: the path to the file to parse
    :returns: a dictionary of the parsed code metrics
    :rtype: dict
    """
...
def analyze_code_smells(
    ...
    :returns: a dictionary of code smells
    :rtype: dict
    """
    long_methods = []
    many_param_methods = []
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef):
            ...
            if len(node.args.args) > param_threshold:
                many_param_methods.append(
                    {"name": node.name, "param_count": len(node.args.args)}
                )
        # nested loops
        elif isinstance(node, (ast.For, ast.While)):
            nested_count = sum(
                isinstance(_, (ast.For, ast.While)) for _ in ast.walk(node)
            )
            if nested_count > nested_loop_threshold:
                nested_loops.append(
                    {"lineno": node.lineno, "nested_count": nested_count}
                )
...
def analyze_code_files(root_folder: str = "regscale") -> dict:
    ...
    :param str root_folder: the root folder to search
    :returns: a dictionary of code metrics
    :rtype: dict
    """
...
def analyze_text_search(file_path: str) -> dict:
    ...
    :returns: a dictionary of FIXMEs and TODOs
    :rtype: dict
    """
    metrics = {
        "FIXMEs": 0,
...
def analyze_complexity_metrics(file_path: str) -> dict:
    ...
    return {
        ...
        "Complexity/LOC Ratio": complexity / raw_metrics.loc
        if raw_metrics.loc
        else 0.0,
    }
...
def calculate_maintainability_index(
    volume: float, complexity: float, loc: int
) -> float:
...
def analyze_fan_metrics(file_path) -> dict:
    """Analyze Fan-in and Fan-out metrics for each function in a Python file
    :param str file_path: the path to the file to analyze
    :returns: a dictionary of Fan-in and Fan-out metrics
    """
    fan_in = {}
    fan_out = {}
    with open(file_path, "r") as f:
...
def generate_barplot(
    ...
    if metric == "Maintainability Index":
        ...
        filtered_values = {
            file: value for file, value in values.items() if int(value) != 0
        }
        # also filter out any files with a Maintainability Index of 0
        sorted_values = dict(
            sorted(filtered_values.items(), key=lambda item: item[1])[:top_n]
        )
    else:
...
def combine_git_and_code_metrics() -> dict:
    ...
    :returns: a dictionary of combined git and code metrics
    :rtype: dict
    """
    git_metrics = analyze_git()
    code_metrics = analyze_code_files()
    return {
        key: {**git_metrics.get(key, {}), **code_metrics.get(key, {})}
        for key in set(git_metrics) | set(code_metrics)
    }
