...
class FabricAuthUtil:
    ...
    def _create_workspace(self):
        ...
        self.workspace_name = f"SemPy {FabricAuthUtil.get_dev_user()}"

        # check if workspace exist
        token_provider = ConstantTokenProvider(self.get_env_bearer_token())
        self.workspace_id = _PBIRestAPI(token_provider).get_workspace_id_from_name(self.workspace_name)

        if self.workspace_id is None:
            # create since it's not there
            self.workspace_id = create_workspace(env_endpoint[self.env],
    ...
    def get_fabric_config(self) -> dict[str, str]:
        ...
        if os.path.exists(cache_path):
            ...
            if _get_token_seconds_remaining(fabric_config["bearer_token"]) > 60:
