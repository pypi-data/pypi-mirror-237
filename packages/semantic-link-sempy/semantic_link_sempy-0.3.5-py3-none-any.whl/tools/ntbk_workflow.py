...
def import_notebook(env, ntbk_path, whl_path, workspace_id, date, lakehouse_name="sempy_ci_lake"):
    ...
    ntbk_json = _impute_whl(ntbk_json, f"/lakehouse/default/Files/{date}/{whl_path}")
    ...
    if ntbk_path == "tests/UnitTestRunner.ipynb":
        ntbk_json["cells"][-1]["source"] = [f"pytest.main([f\"{{mount_root}}/Files/{date}/tests\", \"-k\", \"not test_notebooks\"])"]
...
def delete_folder(env, storage_token, date):
    print(f"Start delete of {date} folder...")
    url = f"https://{env_onelake_new[env]}/{env_named_workspace_id[env]}/{env_ci_lakehouse_id[env]}/Files/{date}?recursive=true"
    ...
    if response.status_code != 200:
        raise ConnectionError(f"Delete folder {date} failed. Response {response.status_code}: {response.reason}, {response.text}")
    print(f"Finish delete of {date} folder...")


def delete_artifact(env, name, id, bearer_token):
    print(f"Start delete of artifact {name}")
...
def run_notebook(env, notebook_id, workspace_id, bearer_token):
    ...
    while True:
        ...
        try:
            ...
            raise ConnectionError(f"Bad response: {response_json}")
...
def _impute_whl(ntbk_json, whl_path):
    pip_cell = {
            "cell_type": "code",
            "metadata": {},
            "outputs": [],
            "execution_count": None,
            "source": ["import pip\n", f"pip.main(['install', '{whl_path}'])"]
        }
...
def cleanup(env, workspace_id, pbi_token, storage_token):
    ...
    for artifact in response.json():
        date_pattern = r"\d{4}_\d{2}_\d{2}T\d{2}_\d{2}_\d{2}$"
        date_match = re.search(date_pattern, artifact["displayName"])
        if not date_match:
            # don't delete artifacts that don't end with yyyy_mm_ddThh_mm_ss
            continue
        artifact_date = date_match.group()
        today = datetime.now()
        if datetime.strptime(artifact_date, '%Y_%m_%dT%H_%M_%S') < today:
            delete_artifact(env, artifact["displayName"], artifact["objectId"], pbi_token)
            try:
                delete_folder(env, storage_token, artifact_date)
            except Exception:
                continue
...
if __name__ == "__main__":
    ...
    cleanup(env, workspace_id, pbi_token, storage_token)
    ...
    for nb in ntbks:
        ...
        if runner_type == "unit_test":
            ...
            delete_folder(env, storage_token, date)
        else:
