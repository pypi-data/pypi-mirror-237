...
class FabricDataFrame(_SDataFrame):
    ...
    **kwargs : dict
    ...
    def _pandas_to_spark_schema(self) -> StructType:
        ...
        schema = StructType([
            StructField(name, {
                ...
                'bool': BooleanType()
            }.get(dtype, StringType())) for name, dtype in dtypes.items()
        ])
    ...
    def to_lakehouse_table(self, name: str, mode: Optional[str] = "error", spark_schema: Optional[StructType] = None):
        ...
        # Change the data type of columns with only NA values to object instead of int (works without it on Fabric, this is for the unit test to pass)
        df_fixed_na_column_types = self
        for col in df_fixed_na_column_types.columns:
            if df_fixed_na_column_types[col].isna().all() and is_integer_dtype(df_fixed_na_column_types[col]):
                df_fixed_na_column_types[col] = df_fixed_na_column_types[col].astype("object")

        converted_schema = spark_schema if spark_schema is not None else df_fixed_na_column_types._pandas_to_spark_schema()

        (spark.createDataFrame(df_fixed_na_column_types, schema=converted_schema)
            .write
