...
class Object:
    ...
    def calculate (self):
        ...
        self.Q_util=self.F_products_kgs*combustor.products.cp*(self.Tflame_degC-self.T_fumee_degC) #revoir pour prendre en compte la condensation
        self.Q_loss=self.Q_comb_HHV-self.Q_util

        self.eta_HHV=self.Q_util/self.Q_comb_HHV
        self.eta_LHV=self.Q_util/self.Q_comb_LHV
        

        self.df = pd.DataFrame({'Boiler': [self.fuel,self.F_fuel_Sm3h,self.F_fuel_kgs,self.fuel.mm,self.fuel_Sdensity,self.HHV_kWhSm3,self.products_O2_molRatio,self.phi,self.AIR_EXCESS,self.Q_comb_LHV,self.Q_comb_HHV,self.Tflame_degC,self.air_Inlet.F_kgs,self.F_products_kgs,self.Q_loss,self.Q_util,self.eta_HHV,self.eta_LHV], },
                      index = ['fuel','F_fuel_Sm3h','F_fuel_kgs','molar mass (kg/mol)',"fuel_Sdensity (Kg/Sm3)",'HHV_kWhSm3','products_O2_molRatio','phi','AIR_EXCESS','Q_comb_LHV(W)','Q_comb_HHV(W)','Tflame_degC','air_Inlet.F_kgs','F_products_kgs','self.Q_loss','self.Q_util','self.eta_HHV','self.eta_LHV'])
