...
class Object:
    ...
    def calculate (self):
        ...
        if self.Nominal_Power_kW is None:
            self.F_air_mols=self.air_Inlet.F_kgs/self.air_mm/(1+3.76)
        ...
        if self.Nominal_Power_kW is not None:
            ...
            self.air_Inlet.F_kgs=self.F_air_mols*(self.air_mm*(1+3.76))

        self.F_fuel_Sm3s=self.F_fuel_kgs/self.fuel_Sdensity
        self.F_fuel_Sm3h=self.F_fuel_Sm3s*3600
        self.Q_comb_LHV=self.LHV_kJkg*self.F_fuel_kgs*1000

        self.F_products_kgs=self.F_fuel_kgs+self.air_Inlet.F_kgs
        print('self.F_products_kgs*combustor.products.cp dT(W) ',self.F_products_kgs,combustor.products.cp,(self.Tflame_degC-self.T_fumee_degC))
        self.Q_loss=self.F_products_kgs*combustor.products.cp*(self.Tflame_degC-self.T_fumee_degC) #revoir pour prendre en compte la condensation
        self.Q_util=self.Q_comb_LHV-self.Q_loss
        

        self.df = pd.DataFrame({'Boiler': [self.fuel,self.F_fuel_Sm3h,self.F_fuel_kgs,self.fuel.mm,self.fuel_Sdensity,self.HHV_kWhSm3,self.products_O2_molRatio,self.phi,self.AIR_EXCESS,self.Q_comb_LHV,self.Q_comb_HHV,self.Tflame_degC,self.air_Inlet.F_kgs,self.F_products_kgs,self.Q_loss,self.Q_util], },
                      index = ['fuel','F_fuel_Sm3h','F_fuel_kgs','molar mass (kg/mol)',"fuel_Sdensity (Kg/Sm3)",'HHV_kWhSm3','products_O2_molRatio','phi','AIR_EXCESS','Q_comb_LHV(W)','Q_comb_HHV(W)','Tflame_degC','air_Inlet.F_kgs','F_products_kgs','self.Q_loss','self.Q_util'])
