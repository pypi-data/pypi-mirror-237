...
class KurocoEmbedding:
    ...
    def __init__(self, 
                ...
                kuroco_handler: Union[KurocoAPI, str],) -> None:  
    ...
    def kuroco_handler(self, value: Union[KurocoAPI, str]):
        assert isinstance(value, KurocoAPI) or (isinstance(value, str) and os.path.exists(value)), "Kuroco API handler must be a KurocoAPI object or a path to an existing config file"
        self._kuroco_handler = value if isinstance(value, KurocoAPI) else KurocoAPI.load_from_file(path=value)
