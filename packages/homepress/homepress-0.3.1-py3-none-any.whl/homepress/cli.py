...
provide 2 paths, first for odd page pdf and the second for even page pdf.")
    midpage_parser.add_argument("-ipr", "--input-page-range", action="store", default=None, type=lambda x: tuple(map(int, x.split(','))), help="input_page_range is a tuple value with two integers. By default is set to None, when None, it implies that \
...
The format for this argument is: int,int. For example: \"-ipr 120,125\"")
    ...
    midpage_multi_parser = subcommands.add_parser("midpage-multi", help="Middle page binded books are a stack of sheets ordered in a way that when folded from the middle and binded there produce a perfectly ordered book. Multi stack binding spits out multiple stacks instead of single stack. This is quite usefull for books with higher number of pages.")
    midpage_multi_parser.add_argument("output", nargs="+", action="append")
    midpage_multi_parser.add_argument("-rtl", "--right-to-left", action="store_true", help="By default, False. For certain languages and countries it is \
more conventional for the print to be right to left instead of the more popular left to right. Medias like \
manga books or even arabic books are traditionally printed right to left.")
    midpage_multi_parser.add_argument('-f', "--flip-even", action="store_true", help="if set to true the resulting output has even \
pages flipped horizontally. It is recommended to enable this in case you are printing using a printer \
that supports double side printing without needing manual flipping of pages. If you are using a single \
side printer, it is recommended that you leave this off. When printing using a single sided printer, \
print the odd pages first, after that. Flip the pages at the horizontal axis.")
    midpage_multi_parser.add_argument("-s", "--separate-even-odd", action="store_true", help="if you need to separate the odd \
pages from even pages, please use this parameter. If you set it to True, the output parameter should \
provide 2 paths, first for odd page pdf and the second for even page pdf.")
    midpage_multi_parser.add_argument("-ss", "--stack-size", action="store", type=int, help="stack_size is the size of individual stack. it can't be guarenteed that this will remain this way \
for all the stacks. variation of +1 in the stack size for some pdfs may happen as a result of eliminating last \
small stack. STACK_SIZE must be a multiple of 4. An integer must be provided", default=40)
    midpage_multi_parser.add_argument("-sst", "--separate-stacks", action="store_true", help="separate_stacks is a boolean value. By default, False. When this feature is used, the output parameters \
...
is always index aligned with the higher place value being relatively left.")
    midpage_multi_parser.add_argument('-q', '--quite', action="store_true", help="When set to true, nothing is outputted to terminal.")
