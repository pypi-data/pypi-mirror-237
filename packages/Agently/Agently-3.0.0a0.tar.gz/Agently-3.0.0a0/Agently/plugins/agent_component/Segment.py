...
class Segment(componentABC):
    ...
    def _prefix(self):
        if not self.is_enabled() or len(self.segments) == 0:
            return None
        full_output_prompt = "- [OUTPUT RULE]: Each segment of output MUST warpped by <!%%={{segment_name}}></!%%>\n- [FORMAT]:\n"
            ...
            if event == "response:delta":
                for char in data:
                    ...
                    if self.parse_stage == 2:
                        if self.response_buffer in self.buffer_next_char["end"] and self.buffer_next_char["end"][self.response_buffer] == char:
                            self.response_buffer += char
                        else:
                        ...
                        if self.response_buffer == "</!%%>":
                            ...
                            if isinstance(self.segments[self.current_segment]["prompt"], (dict, list, set)):
                                segment_data = json.loads(find_json(segment_data))
