...
from pingsafe_cli.psgraph.common.images.image_referencer import fix_related_resource_ids, Image
...
from pingsafe_cli.psgraph.helm.image_referencer.manager import HelmImageReferencerManager
from pingsafe_cli.psgraph.helm.registry import registry
from pingsafe_cli.psgraph.kubernetes.graph_builder.local_graph import KubernetesLocalGraph
from pingsafe_cli.psgraph.kubernetes.runner import Runner as k8_runner, handle_timeout
...
class K8sHelmRunner(k8_runner):
    ...
    def run(
        ...
        try:
            ...
            # logging.debug(
            #    f"Error running kubernetes scan on {chart_meta['name']}. Scan dir: {target_dir}. Saved context dir: {save_error_dir}")
...
class Runner(BaseRunner["KubernetesGraphManager"]):
    ...
    def generate_graph(self,
        ...
        k8s_runner.chart_dir_and_meta = self.convert_helm_to_k8s(root_folder, [], runner_filter)
        k8s_runner.original_root_dir = str(root_folder)
        k8s_runner.tmp_root_dir = self.get_k8s_target_folder_path()

        graph = k8s_runner.generate_graph(self.get_k8s_target_folder_path(), runner_filter)
        return fix_node_paths(graph, k8s_runner.tmp_root_dir, root_folder)
...
def fix_node_paths(graph, tmp_dir, og_dir):
    for node in graph["nodes"]:
        node["file_path_"] = node["file_path_"].replace(tmp_dir, og_dir, 1)
    return graph
