...
from pingsafe_cli.psgraph.common.pingsafe.platform_key import pingsafe_dir
from pingsafe_cli.psgraph.common.pingsafe.vulnerability_scanning.image_scanner import image_scanner, TWISTCLI_FILE_NAME
from pingsafe_cli.psgraph.common.pingsafe.vulnerability_scanning.integrations.docker_image_scanning import \
    docker_image_scanning_integration
from pingsafe_cli.psgraph.common.images.image_referencer import ImageReferencer, Image
from pingsafe_cli.psgraph.common.output.report import Report, merge_reports
from pingsafe_cli.psgraph.common.pingsafe.check_type import CheckType
from pingsafe_cli.psgraph.common.output.common import ImageDetails
from pingsafe_cli.psgraph.common.models.enums import ErrorStatus
from pingsafe_cli.psgraph.common.runners.base_runner import filter_ignored_paths, strtobool
from pingsafe_cli.psgraph.common.sca.commons import should_run_scan
from pingsafe_cli.psgraph.common.sca.output import add_to_report_sca_data, get_license_statuses
from pingsafe_cli.psgraph.common.util.file_utils import compress_file_gzip_base64
from pingsafe_cli.psgraph.common.util.dockerfile import is_docker_file
from pingsafe_cli.psgraph.common.util.http_utils import request_wrapper
from pingsafe_cli.psgraph.runner_filter import RunnerFilter
from pingsafe_cli.psgraph.sca_package.runner import Runner as PackageRunner
...
class Runner(PackageRunner):
    ...
    async def execute_scan(
        ...
        command = f"{Path(pingsafe_dir) / TWISTCLI_FILE_NAME} images scan --address {docker_image_scanning_integration.get_proxy_address()} --token {docker_image_scanning_integration.get_bc_api_key()} --details --output-file \"{output_path}\" {image_id}"
    ...
    def iterate_image_files(self, abs_fname: str, report: Report, runner_filter: RunnerFilter,
        ...
        :param root_folder: for CLI scans - the directory in which pingsafe runs
        ...
        for image_referencer in self.image_referencers:
            if image_referencer.is_workflow_file(abs_fname):
                ...
                for image in images:
                    ...
                    if strtobool(os.getenv('PINGSAFE_CREATE_SCA_IMAGE_REPORTS_FOR_IR', "True")):
    ...
    def get_image_cached_results(self, dockerfile_path: str, image: Image, image_cached_result: Dict[str, Any] | None,
        ...
        """
            ...
            :param root_folder: for CLI scans - the directory in which pingsafe_cli.psgraph runs
