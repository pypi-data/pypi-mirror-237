...
from pingsafe_cli.psgraph.common.util.decorators import time_it
from pingsafe_cli.psgraph.common.util.type_forcers import convert_str_to_bool

from pingsafe_cli.psgraph.common.pingsafe.platform_integration import bc_integration
from pingsafe_cli.psgraph.common.output.secrets_record import SecretsRecord
from pingsafe_cli.psgraph.common.util.http_utils import request_wrapper, DEFAULT_TIMEOUT
from detect_secrets import SecretsCollection
from detect_secrets.core import scan
from detect_secrets.settings import transient_settings

from pingsafe_cli.psgraph.common.pingsafe.check_type import CheckType
from pingsafe_cli.psgraph.common.pingsafe.integration_features.features.policy_metadata_integration import \
    integration as metadata_integration
from pingsafe_cli.psgraph.common.pingsafe.severities import Severity
from pingsafe_cli.psgraph.common.comment.enum import COMMENT_REGEX
from pingsafe_cli.psgraph.common.models.consts import SUPPORTED_FILE_EXTENSIONS
...
from pingsafe_cli.psgraph.common.parallelizer.parallel_runner import parallel_runner
from pingsafe_cli.psgraph.common.runners.base_runner import BaseRunner, filter_ignored_paths
from pingsafe_cli.psgraph.common.typing import _CheckResult
from pingsafe_cli.psgraph.common.util.dockerfile import is_docker_file
from pingsafe_cli.psgraph.common.util.secrets import omit_secret_value_from_line
from pingsafe_cli.psgraph.runner_filter import RunnerFilter
from pingsafe_cli.psgraph.secrets.consts import ValidationStatus, VerifySecretsResult
from pingsafe_cli.psgraph.secrets.coordinator import EnrichedSecret, SecretsCoordinator
from pingsafe_cli.psgraph.secrets.plugins.load_detectors import get_runnable_plugins
from pingsafe_cli.psgraph.secrets.git_history_store import GitHistorySecretStore
from pingsafe_cli.psgraph.secrets.git_types import EnrichedPotentialSecret, PROHIBITED_FILES
from pingsafe_cli.psgraph.secrets.scan_git_history import GitHistoryScanner
from pingsafe_cli.psgraph.secrets.utils import filter_excluded_paths, EXCLUDED_PATHS
...
MAX_FILE_SIZE = int(os.getenv('PINGSAFE_MAX_FILE_SIZE', '5000000'))  # 5 MB is default limit
...
class Runner(BaseRunner[None]):
        ...
        with transient_settings({
            ...
            for key, secret in secrets:
                ...
                try:
                    ...
                    # If encoding is a problem, this is probably not human-readable source codescanner
    ...
    def _safe_scan(file_path: str, base_path: str) -> tuple[str, list[PotentialSecret]]:
        ...
        if file_size > MAX_FILE_SIZE > 0:
            logging.info(
                ...
                'secrets, run this command again with the environment variable "PINGSAFE_MAX_FILE_SIZE" '
    ...
    def verify_secrets(self, report: Report, enriched_secrets_s3_path: str) -> VerifySecretsResult:
        ...
        if validate_secrets_tenant_config is None and not convert_str_to_bool(os.getenv("CKV_VALIDATE_SECRETS", False)):
            logging.debug('Secrets verification is off, enable it via codescanner configuration screen')
            return VerifySecretsResult.INSUFFICIENT_PARAMS
