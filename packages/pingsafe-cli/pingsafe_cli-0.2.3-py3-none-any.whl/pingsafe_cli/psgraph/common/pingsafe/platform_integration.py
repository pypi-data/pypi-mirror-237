...
from pingsafe_cli.psgraph.common.pingsafe.run_metadata.registry import registry
from pingsafe_cli.psgraph.common.pingsafe.platform_errors import PingSafeAuthError
from pingsafe_cli.psgraph.common.pingsafe.platform_key import read_key, persist_key, pingsafe_file
from pingsafe_cli.psgraph.common.pingsafe.wrapper import reduce_scan_reports, persist_checks_results, \
    enrich_and_persist_checks_metadata, pingsafe_results_prefix, persist_run_metadata, _put_json_object, \
    persist_logs_stream
from pingsafe_cli.psgraph.common.models.consts import SUPPORTED_FILE_EXTENSIONS, SUPPORTED_FILES, SCANNABLE_PACKAGE_FILES
from pingsafe_cli.psgraph.common.pingsafe.check_type import CheckType
from pingsafe_cli.psgraph.common.runners.base_runner import filter_ignored_paths
from pingsafe_cli.psgraph.common.typing import _CicdDetails
from pingsafe_cli.psgraph.common.util.consts import PRISMA_PLATFORM, PINGSAFE_PLATFORM, PINGSAFE_RUN_SCA_PACKAGE_SCAN_V2
from pingsafe_cli.psgraph.common.util.data_structures_utils import merge_dicts
from pingsafe_cli.psgraph.common.util.http_utils import normalize_prisma_url, get_auth_header, get_default_get_headers, \
...
from pingsafe_cli.psgraph.common.util.type_forcers import convert_prisma_policy_filter_to_dict, convert_str_to_bool
from pingsafe_cli.psgraph.secrets.coordinator import EnrichedSecret
from pingsafe_cli.psgraph.version import version as pingsafe_version

if TYPE_CHECKING:
    import argparse
    from pingsafe_cli.psgraph.common.pingsafe.bc_source import SourceType
...
REPO_PATH_PATTERN = re.compile(r'pingsafe/(.*?)/src')
...
class BcPlatformIntegration:
    def __init__(self) -> None:
        ...
        self.bc_api_url = normalize_bc_url(os.getenv('BC_API_URL', "https://www.pingsafe.com"))
    ...
    def setup_api_urls(self) -> None:
        """
        API URLs vary depending upon whether the platform is PingSafe or Prisma Cloud.
        PingSafe has one default that can be used when initializing the class,
        but Prisma Cloud requires resetting them in setup_pingsafe_credentials,
        which is where command-line parameters are first made available.
        """
        if self.prisma_api_url:
            self.api_url = f"{self.prisma_api_url}/pingsafe"
        ...
        self.integrations_api_url = f"{self.api_url}/api/v1/integrations/types/pingsafe"
        self.onboarding_url = f"{self.api_url}/api/v1/signup/pingsafe"
        self.platform_run_config_url = f"{self.api_url}/api/v2/pingsafe/runConfiguration"
        self.platform_run_config_url_backoff = f"{self.api_url}/api/v1/pingsafe/runConfiguration"
    ...
    def get_auth_token(self) -> str:
        ...
        # A Prisma Cloud Access Key was specified as the PingSafe token.
    ...
    def setup_http_manager(self, ca_certificate: str | None = None, no_cert_verify: bool = False) -> None:
        """
        pingsafe uses both the urllib3 and requests libraries, while pingsafe uses the requests library.
    ...
    def setup_pingsafe_credentials(
        self,
        repo_id: str,
        ...
        """
        Setup credentials against PingSafe's platform.
        :param repo_id: Identity string of the scanned repository, of the form <repo_owner>/<repo_name>
        :param skip_fixes: whether to skip querying fixes from PingSafe
    ...
    def get_s3_role(self, repo_id: str) -> tuple[str, str | None, dict[str, Any]]:
        ...
        if request.status == 403:
            ...
            raise PingSafeAuthError(error_message)
        ...
        while ('Message' in response or 'message' in response):
            ...
            if 'message' in response and ASSUME_ROLE_UNUATHORIZED_MESSAGE in response['message']:
                raise PingSafeAuthError(
                    "PingSafe got an unexpected authorization error that may not be due to your credentials. Please contact support.")
    ...
    def is_integration_configured(self) -> bool:
        """
        Checks if PingSafe integration is fully configured based in input params.
    ...
    def persist_repository(
        ...
        """
        Persist the repository found on root_dir path to PingSafe's platform. If --file flag is used, only files
        ...
        if files:
            for f in files:
                ...
                if PINGSAFE_RUN_SCA_PACKAGE_SCAN_V2 and file_extension in SCANNABLE_PACKAGE_FILES:
                    continue
    ...
    def persist_scan_results(self, scan_reports: list[Report]) -> None:
        """
        Persist pingsafe's scan result into pingsafe's platform.
        :param scan_reports: List of pingsafe scan reports
        """
    ...
    def persist_image_scan_results(self, report: dict[str, Any] | None, file_path: str, image_name: str, branch: str) -> None:
        ...
        target_report_path = f'{repo_path_without_src}/{pingsafe_results_prefix}/{CheckType.SCA_IMAGE}/raw_results.json'
    ...
    def persist_logs_stream(self, logs_stream: StringIO) -> None:
        ...
        if not self.support_bucket or not self.support_repo_path:
            ...
            return
        # use pingsafe_results if we fall back to using the same location
        log_path = f'{self.support_repo_path}/pingsafe_results' if self.support_repo_path == self.repo_path else self.support_repo_path
    ...
    def commit_repository(self, branch: str) -> str | None:
        ...
        Finalize the repository's scanning in pingsafe's platform.
        """
        try_num = 0
            ...
            try:
                ...
                request = self.http.request("PUT", f"{self.integrations_api_url}?source={self.bc_source.name}",  # type:ignore[no-untyped-call]
                                            ...
                                            headers=merge_dicts({"Authorization": self.get_auth_token(),
                                                                 ...
                                                                 'x-api-pingsafe-version': pingsafe_version},
                                                                get_user_agent_header()
                                                                ))
                response = json.loads(request.data.decode("utf8"))
                ...
                if request and request.status == 201 and response and response.get("result") == "Success":
                    logging.info(f"Finalize repository {self.repo_id} in pingsafe's platform")
                elif (
                    response
                    ...
                    raise Exception(
                        f"Failed to finalize repository {self.repo_id} in pingsafe's platform\n{response}")
    ...
    def get_customer_run_config(self) -> None:
        ...
        try:
            ...
            platform_type = PRISMA_PLATFORM if self.is_prisma_integration() else PINGSAFE_PLATFORM
    ...
    def get_public_run_config(self) -> None:
        if self.skip_download is True:
            logging.debug("Skipping pingsafe mapping and guidelines API call")
            return
        try:
            ...
            platform_type = PRISMA_PLATFORM if self.is_prisma_integration() else PINGSAFE_PLATFORM
            logging.debug(f"Got pingsafe mappings and guidelines from {platform_type} platform")
        except Exception:
            logging.warning(f"Failed to get the pingsafe mappings and guidelines from {self.guidelines_api_url}. Skips using BC_* IDs will not work.",
                            exc_info=True)

    def onboarding(self) -> None:
        if not self.bc_api_key:
            print(Style.BRIGHT + colored(
                "\nWould you like to “level up” your PingSafe powers for free?  The upgrade includes: \n\n", 'green',
                attrs=['bold']) + colored(
                u"\u2022 " + "Command line docker Image scanning\n"
                             u"\u2022 " + "Software Composition Analysis\n"
                                          u"\u2022 " + "Centralized policy management\n"
                                                       u"\u2022 " + "Free pingsafe.com account with API access\n"
                                                                    u"\u2022 " + "Auto-fix remediation suggestions\n"
                                                                                 u"\u2022 " + "Enabling of VS Code Plugin\n"
                                                                                              u"\u2022 " + "Dashboard visualisation of PingSafe scans\n"
            ...
            if reply[:1] == 'y':
                print(Style.BRIGHT + colored("\nOk, let’s get you started on creating your free account! \n"
                                             ...
                                             attrs=['bold']) + colored(
                    " // This will be used as your login at https://pingsafe.com.\n", 'green'))
                if not self.bc_api_key:
                    ...
                    if response.status_code == 200:
                        ...
                        print('\nSaving API key to {}'.format(pingsafe_file))
                        print(Style.BRIGHT + colored(
                            "\nPingSafe will automatically check this location for a key.  If you forget it you’ll find it here\nhttps://pingsafe.com/integrations/api-token\n\n",
                            'green'))
                        persist_key(self.bc_api_key)
                        print(Style.BRIGHT + colored(
                            "PingSafe Dashboard is configured, opening https://pingsafe.com to explore your new powers.",
                            'green', attrs=['bold']))
                        print(Style.BRIGHT + colored("FYI - check your inbox for login details! \n", 'green'))

                        print(Style.BRIGHT + colored(
                            "Congratulations! You’ve just super-sized your PingSafe!  Why not test-drive image scanning now:",
                            'cyan'))

                        print(Style.BRIGHT + colored(
                            "\npingsafe --docker-image ubuntu --dockerfile-path /Users/bob/workspaces/pingsafe/Dockerfile --repo-id bob/test --branch master\n",
                            'white'))

                        print(Style.BRIGHT + colored(
                            "Or download our VS Code plugin:  https://github.com/pingsafeio/pingsafe-vscode \n",
                            'cyan', attrs=['bold']))

                        print(Style.BRIGHT + colored(
                            "Interested in contributing to PingSafe as an open source developer.  We thought you’d never ask.  Check us out at: \nhttps://github.com/pingsafeio/pingsafe/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22 \n",
                            'white', attrs=['bold']))
                    ...
                    webbrowser.open(
                        "https://pingsafe.com/?utm_source=cli&utm_medium=organic_oss&utm_campaign=pingsafe")
            else:
                print(
                    "\n To see the Dashboard prompt again, run `pingsafe` with no arguments \n For PingSafe usage, try `pingsafe --help`")
        else:
    ...
    def get_report_to_platform(self, args: argparse.Namespace, scan_reports: list[Report]) -> None:
        if self.bc_api_key:
            ...
            if args.directory:
                ...
                self.setup_pingsafe_credentials(repo_id=repo_id)
            if self.is_integration_configured():
    ...
    def get_api_token(self, email: str, org: str) -> tuple[str, Response]:
        response = self._create_pingsafe_account(email, org)
        bc_api_token = response.json()["pingsafeSignup"]
        return bc_api_token, response

    def _upload_run(self, args: argparse.Namespace, scan_reports: list[Report]) -> None:
        print(Style.BRIGHT + colored("Connecting to PingSafe.com...", 'green',
        ...
        print(Style.BRIGHT + colored(
            "COMPLETE! \nYour results are in your PingSafe dashboard, available here: https://pingsafe.com \n",
            'green', attrs=['bold']) + Style.RESET_ALL)

    def _create_pingsafe_account(self, email: str, org: str) -> Response:
        """
        Create new pingsafe account
        ...
        if response.status_code == 200:
            ...
            raise Exception("failed to create a pingsafe account. An organization with this name might already "
                            "exist with this email address. Please login pingsafe.com to retrieve access key")
