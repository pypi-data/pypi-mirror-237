...
def _append_dims(x: Tensor, target_dims: int) -> Tensor:
    dims_to_append = target_dims - x.ndim
    if dims_to_append < 0:
        raise ValueError(f"Input dimension {x.ndim} is larger than target dimension {target_dims}")
...
class ConsistencyModel(nn.Module):
    ...
    Parameters:
        total_steps: The maximum number of training steps.
        ...
        use_huen_sampling: If set, use Heun steps during sampling. In testing
            this implementation seems to be a bit worse on MNIST than the
            vanilla sampling procedure, which might be because of a bug in the
            implementation or another reason, but it is provided as an option
            for comparison.
    """
    ...
    def __init__(
        ...
        self.use_huen_sampling = use_huen_sampling
        self.loss_fn = loss_fn("pseudo-huber", dim=loss_dim, pseudo_huber_factor=loss_factor)
    ...
    def loss(self, model: Callable[[Tensor, Tensor], Tensor], x: Tensor, step: int) -> Tensor:
        ...
        sigma_current, sigma_next = self._get_sigmas(torch.stack((t_current, t_next))).unbind(0)

        noise = torch.randn_like(x)
        dropout_state = torch.get_rng_state()
        x_next = x + noise * _append_dims(sigma_next, dims)
        y_next = self._call_model(model, x_next, sigma_next)

        # Resets the dropout state and runs the target model.
        torch.set_rng_state(dropout_state)
        with torch.no_grad():
            x_current = x + noise * _append_dims(sigma_current, dims)
            y_current = self._call_model(model, x_current, sigma_current).detach()

        loss = self._get_loss(y_next, y_current, sigma_current, sigma_next)
    ...
    def sample(
        ...
        x = torch.randn(shape, device=device) * sigmas[-1]
        samples = torch.empty((num_steps + 1, *x.shape), device=x.device)
        samples[num_steps] = x
        for i in range(num_steps - 1, -1, -1):
            if self.use_huen_sampling:
                denoised = self._call_model(model, x, sigmas[None, i + 1])
                d = (x - denoised) / _append_dims(sigmas[None, i + 1], x.ndim)
                if i == 0:
                    # Euler's method
                    dt = sigmas[None, i + 1]
                    x = x - d * dt
                    ...
                    # Huen's method.
                    dt = sigmas[None, i + 1] - sigmas[None, i]
                    x_2 = x - d * dt
                    denoised_2 = self._call_model(model, x_2, sigmas[None, i])
                    d_2 = (x_2 - denoised_2) / _append_dims(sigmas[None, i], x.ndim)
                    x = x - (d + d_2) * dt / 2
                samples[i] = x
            else:
                x = self._call_model(model, x, sigmas[None, i + 1])
                samples[i] = x
                if i > 0:
                    x = x + torch.randn_like(x) * sigmas[None, i]
    ...
    def _call_model(self, model: Callable[[Tensor, Tensor], Tensor], x_t: Tensor, sigmas: Tensor) -> Tensor:
        c_skip, c_out, c_in = (_append_dims(x, x_t.ndim) for x in self._get_scalings(sigmas))
        timesteps = 1000 * 0.25 * torch.log(sigmas + 1e-44)
        model_output = model(c_in * x_t, timesteps)
        denoised = c_out * model_output + c_skip * x_t
        return denoised

    def _get_loss(self, y_hat: Tensor, y: Tensor, sigma_current: Tensor, sigma_next: Tensor) -> Tensor:
        weights = 1 / (sigma_next - sigma_current)
    ...
    def _get_sigmas(self, timesteps: Tensor) -> Tensor:
        ...
        sigmas: Tensor = (min_inv_rho + timesteps * (max_inv_rho - min_inv_rho)) ** self.rho
        sigmas[timesteps == 0.0] = torch.zeros_like(sigmas[timesteps == 0.0])
        return sigmas

    def _get_noise_distribution(self, sigma_next: Tensor, sigma_current: Tensor) -> Tensor:
        denom = math.sqrt(2) * self.p_std
        lhs = torch.erf((torch.log(sigma_current) - self.p_mean) / denom)
        rhs = torch.erf((torch.log(sigma_next) - self.p_mean) / denom)
        return lhs - rhs
