...
class QPRegressionAlgorithm(RegressionAlgorithm):
    
    """Class for calculating linear model coefficients using MRLS(weighted Least Squares) + Quadprog.
    """
    def _add_bias(self, x: np.ndarray, min: np.ndarray, max: np.ndarray) -> tuple[np.ndarray]:
        if self._fit_bias:
    ...
    def fit(self,
        ...
        h = 2 * (x.T @ r @ x)
        f = (-2*(y.T @ r @ x)).T
        a = np.concatenate((np.identity(count_features), -np.identity(count_features)), axis=0)
