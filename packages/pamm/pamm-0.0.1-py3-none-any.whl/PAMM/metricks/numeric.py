...
def smape(major_ts: np.array, minor_ts: np.array) -> float:
    ...
    """
    return 100 * np.mean(np.abs(major_ts - minor_ts)/((np.abs(minor_ts) + np.abs(major_ts))/2))
...
def r(major_ts: np.array, minor_ts: np.array) -> float:
...
def r2(major_ts: np.array, minor_ts: np.array) -> float:
    ...
    """
    return 1 - mse(major_ts = major_ts, minor_ts = minor_ts)/((major_ts - major_ts.mean())**2).mean()


def adjusted_r2(major_ts: np.array,
                              minor_ts: np.array,
                              count_features: int
                              ) -> float:
    ...
    return 1 - (mse(major_ts, minor_ts)/(count_observe - count_features))/(((major_ts - major_ts.mean())**2).mean()/(count_observe - 1))