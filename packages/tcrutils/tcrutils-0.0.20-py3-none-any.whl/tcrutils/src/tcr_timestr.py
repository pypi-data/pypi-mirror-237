...
""".replace('\n', '')
  ...
  def to_int(self, s: str, *, units: Mapping[str, int] | None = None, segment_splitter='!') -> int:
    ...
    if rolled_over: # Fixes the days being rolled back when they shouldn't be because there weren't actyually two rollovers
      ...
      and (len([seg for seg in segments if regex.match(r'^(?:[0-9]{1,2}\.(?:[0-9]{1,2}(?:\.[0-9]{2,4})?)?)$', seg)])\
        +  len([seg for seg in segments if seg in weekday_lookup                                                  ])) == 1
      if a: # aka if in all the segments there's (EXACTLY ONE d.at.e segment *or* EXACTLY ONE weekday segment) *and* EXACTLY ONE t:im:e segment
