...
def split_string_on_change(s: str) -> tuple[tuple[str]]:
  ...
  for i in range(len(s)-1): # Not using enumerate because i also need the next letter
    if not (s[i].isnumeric() or s[i] == '-') and (s[i+1].isnumeric() or s[i+1] == '-'): # If there happens to be a NON-numer on the left and a number on the right...
...
""".replace('\n', '')
  ...
  def copy(cls, *, allow_negative: bool = True, allow_zero: bool = True):
    """
    Used when using timestr of multiple configurations\\
    Example:
    ```py
  ...
  def valid(self, s: str, *, segment_splitter='!') -> bool:
    """Returns `True` if specified string is a valid input for `to_int()` conversion, otherwise `False`"""
  ...
  def to_int(self, s: str, *, units: Mapping[str, int] | None = None, segment_splitter='!') -> int:
    ...
    if rolled_over: # Fixes the days being rolled back when they shouldn't be because there weren't actyually two rollovers
      a = len([seg for seg in segments if regex.match(r'^(?:[0-9]{1,2}:(?:[0-9]{1,2}(?::[0-9]{1,2})?)?)$',   seg)]) == 1\
      and len([seg for seg in segments if regex.match(r'^(?:[0-9]{1,2}\.(?:[0-9]{1,2}(?:\.[0-9]{2,4})?)?)$', seg)]) == 1
      if a: # aka if in all the segments there's EXACTLY ONE d.at.e segment *and* EXACTLY ONE t:im:e segment
  ...
  def to_str(self, n: int) -> str:
    """
    Formats number of seconds to days, hour, minutes or seconds, whichever one is lowest while not leaving the number at < 1.0 (for example 0.25 days would not be returned, rather 6 hours would be).\\
    Examples:
    - `1 hour`
    - `40 minutes`
  ...
  def to_str2(self, seconds: int) -> str:
    """
    Formats number of seconds to `1d + 23:45:57` (with leading zeroes)\\
  ...
  def to_date(self, seconds: int) -> datetime.datetime:
    """Returns `datetime` object of the time that will be present in `seconds` seconds."""
    current_time = datetime.datetime.now()
    return current_time + datetime.timedelta(seconds=seconds)

  def to_datestr(self, seconds: int, *, pattern="%a, %Y-%m-%d %H:%M:%S") -> str:
    """Returns `datetime.strftime()` string of the time that will be present in `seconds` seconds."""
    future_time = self.to_date(seconds)
