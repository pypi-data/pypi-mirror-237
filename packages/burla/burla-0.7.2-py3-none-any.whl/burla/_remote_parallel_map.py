...
MAX_CONCURRENCY = 2000  # remote_parallel_map will break if this is raised.
...
def remote_parallel_map(
    ...
    n_inputs = len(inputs)
    parallelism = n_inputs if parallelism == -1 else parallelism
    ...
    StatusMessage.total_gpus = parallelism * func_gpu
    ...
    for input_batch in input_batches:
        try:
            ...
            for output_pkl in download_outputs(output_urls):
                output = cloudpickle.loads(output_pkl)
                if output["pickled_with_cloudpickle"]:
                    return_values.append(cloudpickle.loads(output["return_value_pkl"]))
                elif output["return_type"] == "<class 'spacy.tokens.doc.Doc'>":
                    if (not globals().get("Doc")) and (not globals().get("Vocab")):
                        from spacy.tokens.doc import Doc
                        from spacy.vocab import Vocab
                    return_values.append(Doc(Vocab()).from_bytes(output["return_value_pkl"]))
