...
class EauAudioService(EauService):
  ...
  def volume(self, channel: str, action: str | None = None) -> float:
    if action:
      if channel == "bg":
        if (self.bg.volume > 0 and action == '-') or (self.bg.volume < 1 and action == '+'):
          self.bg.volume += 0.1 if action == "+" else -0.1
        return self.bg.volume
      else:
        for sfx in self.sfx.values():
          if (sfx.volume > 0 and action == '-') or (sfx.volume < 1 and action == '+'):
            sfx.volume += 0.1 if action == "+" else -0.1
        return list(self.sfx.values())[0].volume
    else:
      return self.bg.volume if channel == "bg" else list(self.sfx.values())[0].volume
...
class EauColor(Enum):
  ...
  def apply(self, surf: pygame.Surface):
