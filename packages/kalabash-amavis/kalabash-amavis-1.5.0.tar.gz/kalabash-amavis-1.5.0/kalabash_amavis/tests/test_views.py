...
class ViewsTestCase(TestDataMixin, KbashTestCase):
    """Test views."""
    ...
    def test_index(self):
        ...
        self.assertIn(
            "<tr id=\"{}\">".format(smart_str(msgrcpt.mail.mail_id)),
            response["listing"])
        self.assertNotIn(
            "<tr id=\"{}\">".format(smart_str(self.msgrcpt.mail.mail_id)),
            response["listing"])
    ...
    def test_viewmail(self):
        ...
        url = "{}?rcpt={}".format(url, smart_str(self.msgrcpt.rid.email))
        response = self.ajax_get(url)
    ...
    def test_viewmail_selfservice(self):
        ...
        url = "{}?secret_id={}".format(
            url, smart_str(self.msgrcpt.mail.secret_id))
        response = self.client.get(url)
    ...
    def test_release_selfservice(self, mock_socket):
        ...
        url = "{}?secret_id=1234&rcpt={}".format(
            base_url, smart_str(self.msgrcpt.rid.email))
        self.ajax_get(url, status=400)
        # Wrong rcpt -> fails
        ...
        url = "{}?secret_id={}&rcpt={}".format(
            base_url, smart_str(self.msgrcpt.mail.secret_id),
