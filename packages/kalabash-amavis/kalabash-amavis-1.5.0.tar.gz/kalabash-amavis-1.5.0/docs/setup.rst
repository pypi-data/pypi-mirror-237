...
.. note::
   ...
   databases but Kalabash doesn't support it yet.

.. note::

   ``$sql_partition_tag`` should remain undefined in ``amavisd.conf``. Kalabash
...
Connect Kalabash and Amavis
==========================

You must tell to Kalabash where it can find the amavis
...
perfomance killer. Kalabash provides a simple script to periodically
purge the quarantine database. To use it, add the following line
inside root's crontab::

  0 0 * * * <kalabash_site>/manage.py qcleanup
  #
  # Or like this if you use a virtual environment:
  # 0 0 * * * <virtualenv path/bin/python> <kalabash_site>/manage.py qcleanup
...
Once amavisd-new is configured, just tell Kalabash where it can find
...
As administrators are not always available or logged into Kalabash, a
...
example line to root's crontab::
  ...
  0 12 * * * <kalabash_site>/manage.py amnotify --baseurl='<kalabash_url>'
  #
  # Or like this if you use a virtual environment:
  # 0 12 * * * <virtualenv path/bin/python> <kalabash_site>/manage.py amnotify --baseurl='<kalabash_url>'
...
|                    |their messages      |                        |
+--------------------+--------------------+------------------------+
|Notifications sender|The e-mail address  |notification@alphamonak.com|
|                    |used to send        |                        |
|                    |notitications       |                        |
+--------------------+--------------------+------------------------+
...
Here is a link example::
  ...
  http://<kalabash_url>/quarantine/%i/?rcpt=%R&secret_id=[:secret_id]
...
Enable the feature through Kalabash
----------------------------------

Manual learning is disabled by default. You can activate it through
the administration panel (*Kalabash > Paremeters > Amavis*). There two
learning levels:
...
different machine than Kalabash. (in this case, ``spamc`` will be used
instead of ``sa-learn``).
