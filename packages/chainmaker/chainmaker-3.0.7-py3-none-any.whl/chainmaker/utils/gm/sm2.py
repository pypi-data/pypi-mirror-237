...
from . import sm2_asn1, sm3
from .utils import hex_to_bytes, hex_to_int, random_hex
...
class ECDSA:
    pre_string = '30770201010420'
    mid_string = 'a00a06082a811ccf5501822da144034200'  # identifies secp256k1
    ...
    def get_private_key_value(self) -> str:
        return self._value[14:64 + 14]

    def get_public_key_x_y(self) -> (str, str):
        return self._value[114:178], self._value[178:]

    @classmethod
    def from_private_key(cls, private_key_value: str, public_key_x, public_key_y) -> "ECDSA":
        return cls(''.join([
            cls.pre_string,
            private_key_value,
            cls.mid_string,
...
class SM2Signature:
    ...
    def load(cls, data: bytes) -> "SM2Signature":
        """加载asn1序列化内容构造签名对象"""
        sig, _ = decoder.decode(data, asn1Spec=sm2_asn1.SM2Signature())
        print('sig', sig)
    ...
    def dump(self) -> bytes:
        ...
        sig = sm2_asn1.SM2Signature()
        sig['r'] = univ.Integer(self.r)
        sig['s'] = univ.Integer(self.s)
        # seq.extend([self.r, self.s])
        return encoder.encode(sig)


class SM2PublicNumbers(object):
    def __init__(self, x: int, y: int, curve: SM2P256Curve):
        self._x = x
        self._y = y
        self._curve = curve

    @property
    def curve(self) -> SM2P256Curve:
        return self._curve
    ...
    def from_encoded_point(
            cls, curve: SM2P256Curve, data: bytes
    ) -> "SM2PublicNumbers":
...
class SM2PublicKey:
    ...
    def __init__(self, x: str, y: str, curve: SM2P256Curve = sm2p256v1):
        ...
        self._public_numbers = SM2PublicNumbers(hex_to_int(x), hex_to_int(y), curve)
    ...
    def load(cls, data: bytes) -> "SM2PublicKey":
        public_key, _ = decoder.decode(data, asn1Spec=sm2_asn1.SM2PublicKeyInfo())
    ...
    def dump(self) -> bytes:
        pkcs8_key = sm2_asn1.SM2PublicKeyInfo()
...
class SM2PrivateKey:
    ...
    def __init__(self, value: str, public_key: "SM2PublicKey" = None):
        """
    ...
    def _get_public_key(self, curve=sm2p256v1):
        ...
        point_public_key = curve.scalar_base_mul(private_key_value)
        x, y = point_public_key[:64], point_public_key[64:]
        return SM2PublicKey(x, y, curve=curve)
    ...
    def load(cls, data: bytes) -> "SM2PrivateKey":
        # private_key, _ = decoder.decode(data, asn1Spec=rfc5958.PrivateKeyInfo())
        private_key, _ = decoder.decode(data, asn1Spec=sm2_asn1.SM2PrivateKeyInfo())
        ecdsa = ECDSA(''.join('%.2x' % x for x in private_key['privateKey'].asNumbers()))
        value = ecdsa.get_private_key_value()
        x, y = ecdsa.get_public_key_x_y()
        public_key = SM2PublicKey(x=x, y=y)
        return cls(value=value, public_key=public_key)

    def dump(self) -> bytes:
        ecdsa = ECDSA.from_private_key(private_key_value=self.value, public_key_x=self.public_key().x,
                                       public_key_y=self.public_key().y)

        private_key = OctetString(hexValue=ecdsa.value)

        pkcs8_key = sm2_asn1.SM2PrivateKeyInfo()
        pkcs8_key["version"] = 0
        pkcs8_key["privateKeyAlgorithm"] = sm2_asn1.sm2_algorithm
        pkcs8_key["privateKey"] = private_key
        der = encoder.encode(pkcs8_key)
        return der
