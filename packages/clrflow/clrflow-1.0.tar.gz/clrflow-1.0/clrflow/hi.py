...
class grad:
    ...
    def __call__(self,*,fr:tuple,to:tuple,change:int="auto",direction:str="horizontal",layer:str="foreground",name:str=None,ignoreWhitespace:bool=False):
        ...
        if name:
            if name in self.cls.keys():
                return lambda st: self.a(st,self.cls[name][0],self.cls[name][1],self.cls[name][2],self.cls[name][3])
            else:
                self.cls[name] = [fr,to,change,direction,layer,ignoreWhitespace]
