...
from lesscode_charts.utils.common_utils import convert, get_value_from_dict
...
class TableChart:
    @staticmethod
    def list2table_with_page(data: List[dict], head, title: str = "", total: int = 0,
                             index_enable: bool = False, index_name="序号", index_key="index",
                             page_num: int = 1, page_size: int = 10, column_covert: dict = None,
                             column_keys: dict = None, custom_head_enable: bool = False):
        """
            list数据转换成表格
