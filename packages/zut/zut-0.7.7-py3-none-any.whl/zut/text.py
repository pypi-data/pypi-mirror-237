...
def slugify(value: str, separator: str = '-', keep: str = '_', strip_separator: bool = True, strip_keep: bool = True, if_none: str = 'none') -> str:
    """ 
    Generate a slug.
    ...
    for arg in args:
        if arg:
            for ip_str in arg.split(','):
                ip_str = ip_str.strip()
                if ip_str:
                    if m := _ENCLOSED_IP_PATTERN.match(ip_str):
                        ip_str = m[1]
...
class Filter:
    def __init__(self, spec: str|re.Pattern, normalize: bool = False):
        ...
        if isinstance(spec, re.Pattern):
            ...
            if m := re.match(r'^(.*\$)(A|I|L|U|M|S|X)+$', spec, re.IGNORECASE):
                pattern = m[1]
                flags = re.NOFLAG
                for letter in m[2]:
    ...
    def normalize_spec(cls, spec: str):
        return slugify(spec, separator=None, keep='*', strip_keep=False)
    
    
    @classmethod
    def normalize_value(cls, value: str):
        return slugify(value, separator=None, keep=None)
