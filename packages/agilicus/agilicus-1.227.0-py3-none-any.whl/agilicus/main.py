...
def output_list_users(ctx, orgs_by_id, users_list):
    ...
    for entry in users_list:
        ...
        td_created = str(entry.get("created", ""))
        td_updated = str(entry.get("created", ""))
...
@click.option("--list-children", default=None, type=bool)
...
def update_subscription(ctx, subscription_id, **kwargs):
    ...
    for metric in kwargs["usage_metric_name"]:
        ...
        usage_override = override_replace(
            metric, usage_override, usage_min, usage_max, usage_step
        )
...
@click.option("--username", default=None)
@click.pass_context
def update_ssh_resource(ctx, ssh_resource_id, port, **kwargs):
    result = ssh.update_ssh_resource(ctx, ssh_resource_id, port, **kwargs)
    output_entry(ctx, result)
