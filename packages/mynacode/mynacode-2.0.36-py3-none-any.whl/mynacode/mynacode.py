...
def start(base_folder = "", project = "", sweep = False, sweep_name = "", run_name=""):
    ...
    if sweep == True:
      ...
      if len(str(sweep_name)) == 0:  
          run_dir_global = project_dir_global+'/S'+str(r_id)
      else:
          run_dir_global = project_dir_global+'/S'+str(r_id)+'_'+sweep_name
      ...
      if len(str(run_name)) == 0:  
          run_dir_global = project_dir_global+'/R'+str(r_id)
      else:
          run_dir_global = project_dir_global+'/R'+str(r_id)+'_'+run_name
...
def csv(dataframe, name='dataframe', run_id=None):
    ...
    if not os.path.exists(run_dir_global+'/csv'):
        os.mkdir(run_dir_global+'/csv')

    csv_files = os.listdir(run_dir_global+'/csv')
    if len(files) != 0:       
        paths = [os.path.join(path, basename) for basename in files]
        newest_csv_file =  max(paths, key=os.path.getctime)
        newest_df = pd.read_csv(newest_csv_file)

    if newest_df.equals(dataframe):
        print("The current dataframe csv is already saved locally at "+ newest_df + ".")
    else:
        dataframe.to_csv(run_dir_global+'/csv/'+name+'.csv', index=False)
        print("Dataframe CSV path: " + run_dir_global+'/csv/'+name+'.csv')
    ...
    data = {'run_id': run_id, 'columns_list': str(columns_list), 'isnull_list': str(isnull_list),
            ...
            'username': uname, 'size': int(size), 'shape': str(shape), 'key': ky, 'node_name': 'CSV', 'csv_path': run_dir_global+'/csv/'+name+'.csv'}
...
'''
def torch_model(model=None, name='best_model_chkpt', metric_name=None, metric_value=None, goal='maximize', current_epoch=None, track={}, run_id=None):
    ...
    if model:
      ...
      if goal == 'maximize':
        if metric_value > prev_max:
          ...
          results_dict.update({'best_'+metric_name+'_epoch': current_epoch})

          #torch.save(model.state_dict(), run_dir_global+'/model/best_model_chkpt.pt')

          for filename in os.listdir(run_dir_global+'/model/'):
            if filename.endswith('.pt'):
              file_path = os.path.join(run_dir_global+'/model/', filename)
              os.remove(file_path)
              
          torch.save(model.state_dict(), run_dir_global+'/model/'+name+'.pt')
          ...
          model_dict = {'library':'PyTorch', 'metric_values': metric_values,
                              'best_model_statedict_path': run_dir_global+'/model/'+name+'.pt',
          ...
          config_dict = {**model_dict, **results_dict, **track}
...
def torch_model(model=None, name='best_model_chkpt', metric_name=None, metric_value=None, current_epoch=None, track={}, run_id=None):
...
def run_reset():
    ...
    if auth_failed == True:
      print("Please login using mynacode.login(Username, Key) before calling mynacode.torch_refresh()")
      return
...
def np_data(train=[], train_labels=[], val=[], val_labels=[], test=[], test_labels=[], name="dataset", if_sweep_save_once = True, run_id = None):
  ...
  if sweep_global == True:
      dataset_folder = "S"+str(run_id_global)+"_"+name
  else:
      dataset_folder = "R"+str(run_id_global)+"_"+name
  ...
  elif re.split(r'[\\/]', newest_data_folder)[-1].split('_')[-1] != name:
      save_new_data_flag = 1
  else:
...
try:
  class MynacodeCallback(keras.callbacks.Callback):
      ...
      def __init__(self, metric_name, goal='maximize', name='best_model_chkpt', track={}, run_id=None):
          ...
          if self.metric_name not in logs.keys():
            ...
            if self.goal == 'maximize':
              if logs.get(self.metric_name) > prev_max:
                ...
                self.model.save(run_dir_global+'/model/'+self.name+'.hdf5')
                ...
                model_dict = {'library':'Keras', 'metric_values': metric_values,
                                    'best_model_path': run_dir_global+'/model/'+self.name+'.hdf5',
                                    'best_model_logs_path': run_dir_global+'/model/best_model_logs.txt',}

                config_dict = {**model_dict, **results_dict, **self.track_config}
...
def keras_model(model=None, name='best_model_chkpt', metric_name=None, metric_value=None, current_epoch=None, track={}, run_id=None):
    ...
    if model:
        ...
        self.model.save(run_dir_global+'/model/'+name+'.hdf5')
        ...
        print("Please specify a keras model")
        return
