...
business_hour_pattern = r'(\w+):\s+(\d{2}:\d{2}) - (\d{2}:\d{2})'
day_pattern = r'(\d+\.\d+) \((\w+)\)'
...
special_business_hour_pattern = '^[^:]+: \d{2}:\d{2}$'
special_business_hour_parsing_pattern = r'^([^:]+): (\d{2}:\d{2} - \d{2}:\d{2})$'


def parse_special_business_hour(text):
    match = re.search(special_business_hour_parsing_pattern, text)
    if match:
        type = match.group(0)
        time = match.group(1)
...
def is_special_business_hour(text):
    # 정규표현식으로 패턴 확인
    if re.search(special_business_hour_pattern, text):
        return True
    else:
        return False
...
def parse_business_hour_pattern(text):
    match = get_match_for_pattern(text, business_hour_pattern)
    type = match.group(1)
