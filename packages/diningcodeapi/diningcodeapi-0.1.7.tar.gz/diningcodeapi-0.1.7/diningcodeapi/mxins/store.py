...
from diningcodeapi.utils import parse_bracketed_data, parse_number, parse_business_hour_pattern, \
    parse_date_and_day_of_the_week_pattern, is_special_business_hour
...
class StoreMixin:
    def get_stores(self, keyword:str, page_no:int):
        ...
        scraped_stores = []
        for store in stores:
            id = store['href'].split('rid=')[-1]
            text = store.text
            scraped_stores.append({"_id": id, "id": id, "text": text})
        return scraped_stores
    ...
    def _parse_business_hours(self, businessHours_container_soup):
        ...
        for business_hour_tag in business_hour_tags:
            ...
            business_hours.append({"day": day, "hour":hour})
    ...
    def get_banners(self, code:str):
        response = requests.get(f'https://www.diningcode.com/2018/ajax/gallery.php?rid={code}', headers=HEADERS)
        soup = BeautifulSoup(response.text, 'html.parser')
    ...
    def get_store(self, code:str):
        response = requests.post(f'https://www.diningcode.com/profile.php?rid={code}', headers=HEADERS)
        soup = BeautifulSoup(response.text, 'html.parser')
