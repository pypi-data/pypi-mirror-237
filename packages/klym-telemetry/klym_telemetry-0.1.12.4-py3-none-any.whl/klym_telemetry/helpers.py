...
class KlymTelemetry(ABC):
    ...
    @staticmethod
    def __get_or_create_counter(name: str, description: str) -> Counter:
        metric = KlymTelemetry.__counters.get(name, None)
        if not metric:
            metric = KlymTelemetry.__meter.create_counter(name, description=description)
            KlymTelemetry.__counters[name] = metric
    ...
    def up(name: str, by: int = 1, description: str = "") -> None:
        ...
        metric = KlymTelemetry.__get_or_create_counter(name, description)
        metric.add(by)
