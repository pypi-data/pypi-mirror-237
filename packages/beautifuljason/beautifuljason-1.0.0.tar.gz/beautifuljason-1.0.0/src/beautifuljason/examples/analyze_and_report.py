...
def add_headers_and_logos(doc: bjason.Document):
    ...
    for spec_item in doc.nmr_items:
        ...
        image_item = doc.create_image_item(logo_image_data.image_id)
...
def main():
    ...
    with jason.create_document(args.input_files) as doc: # Open and process the spectral files in JASON
        ...
        jason.save(doc, args.output_files) # Save the document to the specified output files

    # Optionally, open the resulting .jjh5 file in JASON for visual inspection
    jjh5_files = [output_file for output_file in args.output_files if output_file.endswith('.jjh5')]
    if jjh5_files:
