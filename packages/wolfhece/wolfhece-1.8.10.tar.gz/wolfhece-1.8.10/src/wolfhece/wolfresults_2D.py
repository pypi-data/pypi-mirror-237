...
class Wolfresults_2D(Element_To_Draw):
    ...
    def __init__(self, fname:str = None, mold = None, eps=0., idx: str = '', plotted: bool = True, mapviewer=None, need_for_wx: bool = False, gpu_loader=False) -> None:
        """
    ...
    def get_values_turb_from_xy(self, x:float, y:float, aswolf=True):
        ...
        if exists:
            if aswolf:
                return (k,e,nut),(i+1,j+1,curblock.idx+1)
            else:
                return (k,e,nut),(i,j,curblock.idx)
        else:
            return (-1,-1,-1),('-','-','-')
