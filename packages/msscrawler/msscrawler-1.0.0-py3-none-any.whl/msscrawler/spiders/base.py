...
class BaseSpider(spiders.Spider):
    ...
    def parse(self, response):
        ...
        items = self.set_value_after_process_crawl(
            response, items, is_selenium=False)
    ...
    def get_last_price(
        self, option_price, shipping_type, shipping_price, shipping_condition
    ):
        if shipping_type == "3":
            last_price = (
        ...
        try:
            if protocol == "POST":
                result = requests.post(
                    url, data=data, headers=headers, verify=verify)
            else:
