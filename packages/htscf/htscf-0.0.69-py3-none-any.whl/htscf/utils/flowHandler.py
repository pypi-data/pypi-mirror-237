...
class FlowHandler:
    ...
    def runFlow(self, signal, value, value2):
        """
        执行流程的代码
        """
        try:
            self.startRunningProcess()
            self.handle()
        finally:
            FlowHandler.resetValue(value)
            FlowHandler.resetValue(value2)
            self.stopRunningProcess()
            signal.release()

    def startRunningProcess(self):
        """
        在流程执行时，启动同步监控的代码
        """
        p = Process(target=self.handleRunning)
        p.start()
    ...
    def stopRunningProcess(self):
        """流程进程结束时，同时也结束同步运行的进程"""
        with self.value2.get_lock():
            if self.value2.value != "-1":
                kill(self.value2.value, SIGKILL)
                FlowHandler.resetValue(self.value2)

    def create(self, value, value2, signal):
        """
        用于创建流程，每次只能创建一个，当上一个流程被终止或运行结束时，启动新的流程
        """
        while signal.acquire():
            process = Process(target=self.runFlow, args=[signal, value, value2])
            process.start()
            with value.get_lock():
                if self.DEBUG:
    ...
    def createHandler(self):
        ...
        signal = self.signal
        this = self
        p1 = Process(target=self.create, args=[value, value2, signal])  # 初始化"创建流程进程"以创建"流程进程"
        p1.start()

        class MyTCPHandler(socketserver.BaseRequestHandler):
            def handle(self):
                self.data = self.request.recv(1024).strip()
                if self.data.decode("utf-8") == str(value.value):
                    with value.get_lock():
                        pid = value.value
