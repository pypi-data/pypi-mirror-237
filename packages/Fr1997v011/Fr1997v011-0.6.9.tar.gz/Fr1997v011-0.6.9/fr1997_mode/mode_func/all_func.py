        ...
        if status_code == 0 and data:
            ...
            for index, v in enumerate(data):
                ...
                data_dict = {
                    ...
                    'user_id': aweme_info['author']['uid'],
                    'nickname': aweme_info['author']['nickname'],
                    'followers_count': aweme_info['author']['follower_count'],
                    ...
                    'author_head': aweme_info['author']['avatar_thumb']['url_list'][0],
                    'author_type': '其他',
        ...
        def wrapper(request):
            ...
            try:
                if method == 'GET':
                    ...
                    if token == config_dict['token']['django']:
                        return func(request)
                    else:
...
class ModeFunc:
    def __init__(self):
        self.path = mode_staticmethod.run_machine()['platform']
    ...
    def get_xbogus_new(self, url, ua, mstoken=''):
        ...
        with open(js_path, 'r', encoding='UTF-8') as file:
            ...
            # 提前对参数进行处理
            md5_url = mode_staticmethod.md5_base(url_para)
...
mode_douyin = DouyinJike()  # django配置
mode_staticmethod = ModeStatic()  # 其它函数
