...
class Schedules:
    ...
    def get_works_by_pulls(self, work_pulls: list, resource_list: list = [], key=GRANULARY,
        ...
        for pull in work_pulls:
            ...
            if df.columns.tolist().count("object_id") != 1:
                print(df)
                raise Exception("Object id match twice!!!")
    ...
    def convert_df(self, df: pd.DataFrame) -> pd.DataFrame:
        result = df.pivot_table("fraction", ['is_work', 'processed_name', 'granulary_name', 'typed_lvl2_name', 'name',
                                 'physical_volume', 'object_name', 'object_id', 'measurement_unit'], "date")
