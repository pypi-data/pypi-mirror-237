...
from wow_ai_converter.utils import (
    parse_config,
...
from wow_ai_converter import brush
from wow_ai_converter.audio import convert_to_asr_json_manifest
...
class Converter(object):
    _FORMAT_INFO = {
        Format.JSON: {
            ...
            'link': 'https://wow-ai.com/guide/export.html#JSON',
        },
        Format.JSON_MIN: {
            'title': 'JSON-MIN',
            ...
            'link': 'https://wow-ai.com/guide/export.html#JSON-MIN',
        },
        Format.JSON_CUSTOMIZE_16102023: {
            'title': 'JSON_CUSTOMIZE_16102023',
            'description': 'Sample OCR'
             'Label format in JSON. There should only be 1 JSON file for the entire dataset. The JSON file should have the format shown in the image below. Specifically, the keys should be the relative paths to the images, and the values should be lists of all bounding boxes in the corresponding image. Each bounding box (in these lists) should be a dictionary with the following items:'
            '• “bbox”: the coordinates of the bbox (consisting of 8 values)'
            'o Values must be: left, top, right, top, right, bottom, left, bottom (in this'
           ' order)'
            '(“left”,“right”,“top”, “bottom”)'
            '• “text”: the text inside the bounding box'
            '• “class”: the class of the item in the bounding box (which is either “text” or “checkbox”)'
            '• “status”: the status of the checkbox (which is either “checked” or “unchecked”)'
            'o Note, this is only required if the class is “checkbox”',
            'link': 'https://wow-ai.com/guide/export.html#JSON_CUSTOMIZE_16102023',
        },
        Format.CSV: {
            'title': 'CSV',
        ...
        Format.COCO: {
            ...
            'link': 'https://wow-ai.com/guide/export.html#COCO',
        ...
        Format.VOC: {
            ...
            'link': 'https://wow-ai.com/guide/export.html#Pascal-VOC-XML',
            'tags': ['image segmentation', 'object detection'],
        },
        Format.YOLO: {
            'title': 'YOLO',
            ...
            'link': 'https://wow-ai.com/guide/export.html#YOLO',
            'tags': ['image segmentation', 'object detection'],
        },
        Format.BRUSH_TO_NUMPY: {
            ...
            'link': 'https://wow-ai.com/guide/export.html#Brush-labels-to-NumPy-amp-PNG',
        ...
        Format.ASR_MANIFEST: {
            ...
            'link': 'https://wow-ai.com/guide/export.html#ASR-MANIFEST',
    ...
    def convert(self, input_data, output_data, format, is_dir=True, **kwargs):
        ...
        elif format == Format.JSON_CUSTOMIZE_16102023:
            self.convert_to_json_cusomize_16102023(input_data, output_data, is_dir=is_dir)
    def convert_to_json_cusomize_16102023(self, input_data, output_dir, is_dir=True):
        self._check_format(Format.JSON_CUSTOMIZE_16102023)
        ...
        def convert_from_ls(result):
            ...
            # print(tuple(round(coord * dimension) for coord, dimension in zip((x, y), (bbox['original_width'], bbox['original_height']))))
        ...
        for idx, item in enumerate(_records):
            print(item['image'])
            _, tail = os.path.split(item['image'])
            image_path =  f"{output_dir}/images/{tail}"
            # print(image_path)
           
            if not os.path.exists(image_path):
                try:
                    img_path  = item['image']
                    image_url = f"{HOSTNAME}{img_path}"
                    print(image_url)
            ...
            # print(item['label'])
            _records_bbox=[]
            for bbox, transcription, label in list(zip(item['bbox'], item['transcription'], item['label'])):
                # {
                #     "x": 4.890520519297228,
                #     "y": 2.69477608560934,
                #     "width": 16.647865145844268,
                #     "height": 2.7148275953658936,
                #     "rotation": 0,
                #     "original_width": 3000,
                #     "original_height": 4000
                # },
                # print(bbox)
                xywh = self.rotated_rectangle(bbox)
                print(xywh)
            ...
            #    h * value['height'] / 100.0
                ...
                orientation =""
                if "Horizontal" in label["labels"]:
                    orientation ="Horizontal"
                elif "Vertical" in label["labels"]:
                    orientation ="Vertical"
                _records_bbox.append({
                                            "bbox": [ int(math.ceil(x1)),
                                                        int(math.ceil(y1)),
                                                        int(math.ceil(x2)),
                                                        int(math.ceil(y2)),
                                                        int(math.ceil(x3)),
                                                        ...
                                                        int(math.ceil(y4))],
                                            "text": transcription,
                                            "class": label["labels"][0],
                                            "orientation": orientation,
                                        })
                
                
                customize_dataset[f"images/{tail}"] =   _records_bbox
            # customize_dataset.append(tmp)
        
        # if 'image' in customize_dataset[0]: 
        #     del customize_dataset[0]['image']
        # if 'bbox' in customize_dataset[0]: 
        #     del customize_dataset[0]['bbox']
        # if 'transcription' in customize_dataset[0]: 
        #     del customize_dataset[0]['transcription']
        # if 'label' in customize_dataset[0]: 
        #     del customize_dataset[0]['label']

        # del customize_dataset[0]
        # del customize_dataset[1]
        # del customize_dataset[2]
        # del customize_dataset[3]
        # del customize_dataset[4]
        # del customize_dataset[5]
        # del customize_dataset[6]
        # del customize_dataset[7]
        # del customize_dataset[8]
        # del customize_dataset[9]
        # del customize_dataset[10]
        #====================
        with io.open(output_file, mode='w', encoding='utf8') as fout:
            json.dump(customize_dataset, fout, indent=2, ensure_ascii=False)
        ...
        for item_idx, item in enumerate(item_iterator):
            ...
            try:
                with Image.open(os.path.join(output_dir, image_path)) as img:
                    ...
                    print(" width:{width}, height:{height}",width, height)
