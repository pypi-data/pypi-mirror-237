...
def connect(host: str = 'localhost', port: int = None,
    ...
    if host == 'localhost':
        if not port:
            ...
            if not ports:
                raise ProcessLookupError("No available PSCAD process")
...
def launch(port=None, silence=True, minimize=False, splash=False, timeout=5,
           ...
           minimum='5.0', maximum=None, allow_alpha=False, allow_beta=False,
           load=None, extra_args=None,
    ...
    options = ChainMap(options, OPTIONS, {'startup': 'au'})
    ...
    if 'cache' not in _product_list.__dict__:
        ...
        if folder.is_dir():
            ...
            if file.is_file():
                ...
                warnings.warn("Not found: '%s'" % file, stacklevel=3)
