...
class Emr_mrt_client(mrt_client):
    ...
    def __init__(self,emr_path,server_name = None,
                 managerment_port=8888, push_port=8889, subscription_port=8890,ee_env_mapping = None):

        '''

        :param emr_path:
        :param server_name:
        :param managerment_port:
        :param push_port:
        :param subscription_port:
        :param ee_env_mapping:
        type:dict;
        formate:{"emr_env_name":"EE_env_name"}
        eg: ee_env_mapping = {"env_1":"2A1CFFAC_3b03c692594c472f_EnvProject_cal_env_1",
                            "env_2":"2A1CFFAC_3b03c692594c472f_EnvProject_cal_env_2"}
        '''
        ...
        self._ee_env_mapping = ee_env_mapping
    ...
    def _get_proinfo_forEmrPath(self):
        ...
        with open(emrPath) as emr_file:
            ...
            for i in emr_json["EnvironmentList"]:
                ...
                if self._ee_env_mapping is None:
                    EnvironmentName = f"{code}_{pro_name}_{i['EnvironmentName']}"
                elif type(self._ee_env_mapping) is dict:
                    if emr_env_name in self._ee_env_mapping:
                        EnvironmentName = self._ee_env_mapping[emr_env_name]
                    else:
                        break
                else:
                    raise Exception(f"Error {self._ee_env_mapping} ee_env_mapping parammeter not "
                                    f"conform to the regulation !")
                ...
                self.emr_env_projects[emr_env_name] = hil_project(os.path.join(cur_dir, ProjectFilePath))
    ...
    def load_env_for_EmProject(self, is_startEnv=True, is_destoryEnv_ifExist=False):
        ...
        for env_name, ip_project_path, mapping_path in self._emrInfo:
            ...
            if not is_env_exists:
                ...
                while True:
                    ...
                    if value >= 1:
                        ...
                        break
                if is_startEnv and env_info.contents.running_status == 0:
            ...
            elif is_env_exists and not is_destoryEnv_ifExist:
                self.mrt_clien_Log.info(f"The experiment [{env_name}] was successfully reconnected")
