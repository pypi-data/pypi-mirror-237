...
class PIIDetection():
    def __init__(self, spark):
      ...
      self.custom_events_logger = Logger(spark).create_telemetry_logger()
    ...
    def get_hub_connection(self, hubName, api_client):
        hub_connection = (
            ...
            .with_url(
                ...
                options={
                    "verify_ssl": False,
                    "access_token_factory": lambda: self.get_token(api_client),
                },
            )
            ...
            for entity in list_entities:
                ...
                for asset in entity.assets:
                    try:
                        ...
                        self.logger.error("There was an error when extracting PII from the asset with id {asset_id}.".format(asset_id=asset_id))
                if assets_with_pii:
                    entities_detected_pii += 1
            skip += take    
        return total_entities, entities_detected_pii, source_name

    def notify(self, source_item_id, source_name, hub_connection_pii, total_entities=0, entities_detected_pii=0, exception=' ', success=False):
        notificationType = 2
        ...
        if success:
            ...
            hub_connection_pii.send(
            "NotifyAsync", [int(notificationType), int(notificationSubType), notificationPayLoad]   
            )
  
        self.logger.info("{message}: {payload}".format(message=message, payload=notificationPayLoad))
  
        hub_connection_pii.stop()
  
    def clean_dataframe(self, df):
        metadata_columns = ['ModifiedDate', 'LoadDate', 'FileDate', 'HashErrors', 'IdSourceItem']
    ...
    def detect_pii(self, df, language, analyzer, entity_attributes):
        ...
        sc.parallelize(map(lambda column: self.analyze_text(analyzer, df, column, detected_pii_attribute, language, entity_attributes), df.columns))
    ...
    def execute(self, data_intake_process_itemId, execution_date):
        ...
        #Hub connection
        hub_connection_pii = self.get_hub_connection("notifications", self.sidra_core_api_client)
        ...
        try:
            ...
            self.notify(data_intake_process_itemId, source_name, hub_connection_pii, total_entities=total_entities, entities_detected_pii=entities_detected_pii, success=True)
            self.logger.debug("PII detection process was run successfully.")
        except Exception as e:
            self.notify(data_intake_process_itemId, source_name, hub_connection_pii, exception=e.args[0])
