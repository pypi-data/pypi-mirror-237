...
class LoadPropertiesBuilder():
  ...
  def build(self):
    return EntityLoadPropertiesModel(self.consolidation_mode, self.generate_delta_table, self.id_table_format, self.data_preview, self.has_encryption, self.null_text, self.re_create_table_on_deployment, self.pii_detection, self.pii_detection_language)
