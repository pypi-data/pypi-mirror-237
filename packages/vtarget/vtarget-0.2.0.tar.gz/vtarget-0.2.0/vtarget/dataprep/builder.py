class Builder:
    ...
    def set_skip_childs_recursive(self, flow_id: str, node_key: str, data: dict):
        for child in self.nodes[node_key]["childs"]:
            ...
            # data['nodeDataArray'][child['to_id']]['meta']['ports_map']['pout'] = {}
            for port_name in data["nodeDataArray"][child["to_id"]]["meta"]["ports_map"]["pout"]:
                ...
                # data['nodeDataArray'][child['to_id']]['meta']['ports_map']['pout'][port_name]['dtypes'] = None
    ...
    def analyzer(
        ...
        disable_all_voutput_nodes: bool,
        # decimal_round: bool, #TODO: eliminar, no se usa
    ):
        import gc
        import json
        import time
        ...
        while num_nodes_ok < len(self.nodes):
            ...
            for node_key, node_data in self.nodes.items():
                ...
                if (
                    (disable_all_write_nodes and (node_data["type"] in [NodeType.OUTPUTDATA.value, NodeType.EXCEL.value])) or 
                    (disable_all_email_nodes and (node_data["type"] in [NodeType.EMAIL.value])) or 
                    (disable_all_voutput_nodes and (node_data["type"] in [NodeType.VOUTPUT.value]))
                ):
                    if node_data["type"] in [NodeType.EMAIL.value]:
                        msg = "Se omite envío para nodo {}".format(node_data["key"])
                    else:
                ...
                if not node_data["parents"]:
                    ...
                    for parent in node_data["parents"]:
                        ...
                        # print(parent['from'], parent['from'] in cache_handler.settings[flow_id], cache_handler.settings[flow_id].keys())
                ...
                if is_loaded:
                    ...
                    if (
                        ...
                        and (
                            "ports_config" not in node_settings
                            or 
                            ("ports_config" in node_settings and node_settings["ports_config"] == json.dumps(data["nodeDataArray"][node_idx]["meta"]["ports_config"], sort_keys=True))
                        )
                    ):
                    ...
                    # --------------------------------------------------
                        ...
                        if node_data["type"] not in [
                            ...
                            for _, pin_name in enumerate(
                                ...
                                if (
                                    "dtypes"
                                    in data["nodeDataArray"][node_idx]["meta"]["ports_map"]["pin"][pin_name]
                                    and pin_name in to_load_input_port
                                ):
                        ...
                        has_error = next((x for x in bug_handler.bug if x["node_key"] == node_key and x["level"] == "error"), None) != None
                        data["nodeDataArray"][node_idx]["meta"]["processed"] = True
    ...
    def modify_node(self, flow_id: str, node_key: str, node: dict, port_name: str):
        ...
        cache_handler.load(flow_id, node_key, port_name)
        if flow_id in cache_handler.cache and node_key in cache_handler.cache[flow_id]:
            port_df = cache_handler.cache[flow_id][node_key]["pout"][port_name]
    ...
    def run_node(self, flow_id: str, node_key: str, node: dict, links_to: list):
        ...
        return {"flow_id": flow_id, "key": node_key, "node": json.dumps(node, default=str)}
