class Pipeline:
    ...
    def exec(self, flow_id: str, node: dict, input_port: dict):
        ...
        node["meta"]["script"] = (
            ...
            if node["key"] in cache_handler.settings[flow_id]
            and "script" in cache_handler.settings[flow_id][node["key"]]
            else []
        )
        ...
        for port_name in node["meta"]["ports_map"]["pout"].keys():
            ...
            node["meta"]["ports_map"]["pout"][port_name]["head"] = utilities.get_head_of_df_as_list(
                port_df, port_config, flow_id, node["key"], port_name
            )
            ...
            node["meta"]["ports_map"]["pout"][port_name]["dtypes"] = utilities.get_dtypes_of_df(port_df)
            # node['meta']['ports_map']['pout'][port_name]['summary'] = utilities.viz_summary(port_df)
            # node['meta']['ports_map']['pout'][port_name]['describe'] = utilities.get_central_tendency_measures(port_df)
