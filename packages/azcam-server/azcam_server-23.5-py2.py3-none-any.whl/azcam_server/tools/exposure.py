...
import azcam
import numpy
from azcam_server.tools.exposure_filename import Filename
from azcam_server.tools.exposure_obstime import ObsTime
...
class Exposure(Tools, Filename, ObjectHeaderMethods):
    ...
    def sequence(self, number_exposures=1, flush_array_flag=-1, delay=-1):
        ...
        if self.comp_sequence:
            ...
            if azcam.db.tools["instrument"].shutter_strobe:
                ...
                azcam.db.tools["instrument"].comps_on()
            azcam.db.tools["instrument"].comps_delay()  # delay for lamp warmup if needed
    ...
    def record_current_times(self):
        ...
        self.header.set_keyword("TIMEZONE", self.obstime.time_zone[0], "Local time zone", "str")
        self.header.set_keyword(
            "LOCTIME",
    ...
    def update_headers(self):
        ...
        for objectname in azcam.db.headers:
            if (
                ...
                continue
            try:
                azcam.db.tools[objectname].update_header()  # dont crash so all headers get updated
    ...
    def get_status(self):
        ...
        if ef == 1:
            ...
            expcolor = "red"
            progress = int(
                100.0 * (self.get_pixels_remaining() / self.image.focalplane.numpix_image)
            )
