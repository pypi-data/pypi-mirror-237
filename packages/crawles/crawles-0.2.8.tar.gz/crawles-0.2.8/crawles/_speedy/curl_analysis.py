...
class CodeGener:
    def __init__(self, analy: AnalyObj):
        self.analy = analy

    def code_gener(self, template_name: str) -> str:
        """crawles 代码生成"""
        ...
        return template.render(url=self.analy.url, cookies=self.analy.cookies,
...
class TemplateDecor:
    ...
    def __call__(self, func):
        def wrapper(curl_str):
            ...
            # 数据解析
            curl_analy = CurlAnaly(curl_str).curl_analy()
            code_data = CodeGener(curl_analy).code_gener(self.template_name)
...
@TemplateDecor('base_crawler.j2')
def curl_analysis() -> None:
    """基础模板"""
    pass


@TemplateDecor('class_crawler.j2')
def curl_analysis_cls() -> None:
    """类模板"""
    pass
