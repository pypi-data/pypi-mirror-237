...
class Region(TTRPGObject):
    ...
    def page_data(self, root_path="ttrpg"):
        cities = [f"[{r.name}]({r.page_path(root_path)})" for r in self.cities]
        locations = [f"[{r.name}]({r.page_path(root_path)})" for r in self.locations]
        factions = [f"[{r.name}]({r.page_path(root_path)})" for r in self.factions]
