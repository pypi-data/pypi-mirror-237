...
class TTRPGObject(AutoModel):
    ...
    def page_path(self, path="ttrpg"):
        model = self.__class__.__name__.lower()
        if self.world:
            model = f"{self.world.slug}/{model}"
        return f"/{path}/{model}/{self.slug}"
    ...
    def canonize(self, api=None, root_path="ttrpg"):
        ...
        key = api.__class__.__name__
        print(config)
        if key in self.wiki_ids:
            res = api.push(
                config,
                title=self.name,
            ...
            res = api.push(
                config,
                title=self.name,
                path=f"{self.page_path(root_path)}",
