...
class City(TTRPGObject):
    ...
    def page_data(self, root_path="ttrpg"):
        ...
        for name, locations in self.locations.items():
            districts += {name: [f"[{l.name}]({l.page_path()})" for l in locations]}
        factions = [f"[{r.name}]({r.page_path()})" for r in self.factions]
        citizens = [f"[{r.name}]({r.page_path()})" for r in self.citizens()]
