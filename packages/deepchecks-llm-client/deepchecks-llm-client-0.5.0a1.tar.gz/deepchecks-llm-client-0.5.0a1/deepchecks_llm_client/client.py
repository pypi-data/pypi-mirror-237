...
class DeepchecksLLMClient:
    ...
    def log_interaction(self,
                        ...
                        steps: t.List[Step] = None) -> t.Optional[str | uuid.UUID]:
        result = self.api.log_interaction(user_input=user_input, model_response=model_response,
                                            full_prompt=full_prompt, information_retrieval=information_retrieval,
                                            annotation=annotation, ext_interaction_id=ext_interaction_id, steps=steps)
        return result.json()
    ...
    @handle_exceptions(logger)
    def get_application_config(self, file_save_path: str | None = None) -> str:
        return self.api.get_application_config(application_id=self.app['id'], file_save_path=file_save_path)

    @handle_exceptions(init_logger)
    def golden_set(self, version_name: t.Union[str, None] = None) -> t.Iterable[GoldenSetInteraction]:
        """
