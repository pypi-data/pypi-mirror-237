...
class TimeRangeAggregation(object):
    ...
    def operation_name(self):
        ...
        return list(self.df.columns.drop([elapsed_time, 'start_of_range', 'end_of_range', 'userid', 'contentsid', 'pageno']).values)
    ...
    def operation_count(self, operation_name=None, user_id=None, contents_id=None):
        ...
        for column in df.columns:
            if 'elapsed' in column:
                elapsed_time = column
        operation_count_dict = dict(df.drop([elapsed_time, 'start_of_range', 'end_of_range', 'userid', 'contentsid', 'pageno'], axis=1).sum())
