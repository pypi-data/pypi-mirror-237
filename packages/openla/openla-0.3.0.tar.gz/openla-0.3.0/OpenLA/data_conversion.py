...
def convert_into_operation_count(
    event_stream,
    user_id=None,
    contents_id=None,
    operation_name=None,
    ...
    if stream_df.empty:
        empty_df = pd.DataFrame(
            columns=["userid", "contentsid"] + (operation_name or [])
        )
    ...
    if separate_marker_type:
        with warnings.catch_warnings():
            ...
            stream_df["operationname"] = stream_df.loc[
                :, ["operationname", "marker"]
            ].apply(_attach_marker_type, axis=1)
    ...
    operation_count_df = pd.DataFrame(
        operation_count_list, columns=id_column + operation_name
    )
...
def convert_into_page_transition(
    event_stream,
    user_id=None,
    contents_id=None,
    invalid_seconds=None,
    ...
    page_info_dict_list = []
    for ids, user_contents_df in stream_df.groupby(
        ["userid", "contentsid"], sort=False
    ):
        user_id = ids[0]
        contents_id = ids[1]
        ...
        for i in range(len(page_seq)):
            ...
            if (
                (i == len(page_seq) - 1)
                or (operation_seq[i] == "CLOSE")
                or (i + 1 < len(page_seq))
                and (page_seq[i] != page_seq[i + 1])
            ):
                ...
                operation_count_dict = dict(
                    zip(operation_name, [0] * len(operation_name))
                )
            ...
            time_between_actions = (
                (time_seq[i + 1] - time_seq[i]).seconds
                if i + 1 < len(time_seq)
                else None
            )
            if time_between_actions is None:
                pass
            elif (timeout_seconds is not None) and (
                time_between_actions > timeout_seconds
            ):
                time_of_entry_index = i + 1
                pass
            else:
...
def convert_into_page_wise(
    ...
    """
    assert (event_stream is not None) or (
        page_transition is not None
    ), "Please indicate either of the arguments 'event_stream' or 'page_transition'"

    if (page_transition is None) and (event_stream is not None):
        # convert into page transition
        page_transition = convert_into_page_transition(
    ...
    page_info_dict_list = []
    for ids, page_df in page_transition.df.groupby(
        ["userid", "contentsid", "pageno"], sort=False
    ):
        behavior_df = page_df.drop(
            ["userid", "contentsid", "pageno", "time_of_entry", "time_of_exit"], axis=1
        )
        ...
        average_reading_seconds = (
            float(behavior_dict["reading_seconds"]) / num_page_visits
        )
        page_info_dict.update(
            {
                "num_visits": num_page_visits,
                "average_reading_seconds": average_reading_seconds,
            }
        )
    ...
    page_behavior_df = page_behavior_df.sort_values(
        ["userid", "contentsid", "pageno"]
    ).reset_index(drop=True)
    return PageWiseAggregation(page_behavior_df)


def convert_into_time_range(
    course_info,
    event_stream,
    interval_seconds,
    contents_id,
    ...
    start_time="start_of_stream",
    end_time="end_of_stream",
    lecture_week=None,
    count_operation=True,
    operation_name=None,
    separate_marker_type=False,
    ...
    """
    assert _is_str(contents_id) or not hasattr(
        contents_id, "__iter__"
    ...
    page_transition = convert_into_page_transition(
        lecture_stream, invalid_seconds=0, count_operation=False
    )
    transition_df = page_transition.df.loc[
        :, ["userid", "contentsid", "pageno", "time_of_entry", "time_of_exit"]
    ]

    # start time of conversion
    if start_time == "start_of_lecture":
        assert (
            lecture_week is not None
        ), "If you indicate 'start_time=\"start_of_lecture\"', you must indicate 'lecture_week' too"
        assert _is_str(lecture_week) or not hasattr(
            lecture_week, "__iter__"
    ...
    for sec in seconds:
        ...
        transition_df_in_range = transition_df[
            (transition_df["time_of_entry"] < end_of_range)
        ...
        for user in event_stream.user_id():
            ...
            if user_df.empty:
                ...
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")
                    time_in_pages = _time_in_pages(
                        user_df["time_of_entry"], user_df["time_of_exit"]
                    )
            ...
            if count_operation:
                ...
                operation_count = user_stream.operation_count(
                    operation_name=operation_name,
                    separate_marker_type=separate_marker_type,
                )
...
def individual_reading_time(
    event_stream=None,
    user_id=None,
    contents_id=None,
    invalid_seconds=None,
    timeout_seconds=None,
    return_aggregation_result=False,
    time_unit="seconds",
    for_each_content=False,
    page_transition=None,
    pagewise_aggregation=None,
):
    """
    ...
    assert (
        ...
        or (page_transition is not None)
        or ((pagewise_aggregation is not None))
    ...
    if pagewise_aggregation is None:
        ...
        pagewise_aggregation = convert_into_page_wise(
            event_stream=None, page_transition=page_transition
        )
        if user_id is not None:
    ...
    if time_unit not in ["seconds", "second", "s"]:
        if time_unit in ["minutes", "minute", "m"]:
            reading_time["reading_minutes"] = reading_time["reading_seconds"] / 60.0
        elif time_unit in ["hours", "hour", "h"]:
            reading_time["reading_hours"] = reading_time["reading_seconds"] / float(
                60 * 60
            )
