...
class Citros:
    ...
    def run_simulation(self, sim_name, completions, remote, commit_hash, branch_name):   
        ...
        if remote:
            ...
            if not self.check_docker_image_uploaded(user_commit):
                ...
                latest_tag = self.get_latest_docker_image_tag()
    ...
    def get_latest_docker_image_tag(self):  
        proj_name = self._get_project_name()
        images_url = f"https://citros.io/api/artifactory/last/{proj_name}/"
        ...
        try:
            for img in resp_data:
                for name_version_pair in img['relatedTags']:
                    ...
                    hash = img_name.split('/')[-1]
                    if hash != "latest":
                        return hash

            self.log.error("Could not find latest hash")
            return None
