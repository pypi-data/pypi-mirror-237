...
class CustomRecommendedMenu(CustomTeleResponse, CustomResponse, ButtonCard, Usersays, Chips):
  ...
  def getInfo(resultFile, rubrics):
    ...
    counts = rubrics.groupby(['Assignment']).count()
  ...
  def createRecommendedMainMenu(listOfAssignments):
    ...
    webMenu = CustomResponse.getResponse()

    for a in listOfAssignments:
      # Telegram
      teleMenu["title"] = "Available Assignment Recommended Menu:"
      teleMenu["payload"]["telegram"]["text"] = ''
      teleMenu["payload"]["telegram"]["reply_markup"]["keyboard"].append(CustomTeleResponse.getTexts(a))

      # Web
      webMenu["payload"]["richContent"].append(ButtonCard.createButtonCard(text=a, event=a.replace(" ", ""), redirect=''))
  ...
  def createRecommendedCriteriaMenu(rubrics):
    listOfAssignments = rubrics['Assignment'].unique()

    for a in listOfAssignments:
      df = rubrics.loc[rubrics['Assignment'] == a]
  ...
  def getCriteriaMenu(a, listOfCriteria, listOfSubTopic):
    ...
    for i in range(len(listOfSubTopic)):
      ...
      # Name
      name = "Menu - Recommended - Assignment {} - {}".format(a, criteria)
      intent['name'] = name
