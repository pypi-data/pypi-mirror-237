...
class TestController(unittest.TestCase):
    def setUp(self):
        ...
        self.dev = controller.Controller()
        self.mock.reset_mock()

    def test_crc32(self):
        self.assertEqual(self.dev._crc32([0x55, 0xAA, 123, 321]), bytes([81, 232, 252, 34]))
      
    def test_read(self):
        self.mock.return_value.read.return_value = bytes([0x55, 0x00, 87, 115, 157, 198])
        self.assertEqual(self.dev._readbus(6), bytes([0x55, 0x00, 87, 115, 157, 198]))
    ...
    def test_get_status(self):
        ...
        self.assertEqual(len(st.keys()), len(self.dev._STATUS_KEY_LIST))
        self.assertTrue(list(st.keys()).sort() == self.dev._STATUS_KEY_LIST.sort())
    ...
    def test_update(self):
        self.assertRaises(NotImplementedError, self.dev.update)