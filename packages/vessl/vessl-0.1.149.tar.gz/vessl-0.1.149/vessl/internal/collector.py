...
class Collector(ABC):
    ...
    def __init__(self):
        self.buffer: Union[List[ExperimentMetricEntry], List[RunRunExecutionMetricEntry]] = []
        self.lock = threading.Lock()

    def add(self, entries: Union[List[ExperimentMetricEntry], List[RunRunExecutionMetricEntry]]):
        with self.lock:
            self.buffer += entries
...
class RunExecutionMetricCollector(Collector):
    ...
    def build_metric_payload(self, payload: Dict[str, Any], ts: float):
        ...
        return RunRunExecutionMetricEntry(
    ...
    def log_metrics(self, payloads: List[RunRunExecutionMetricEntry]) -> int:
        self.add(payloads)
        return self.step

    def log_media(self, payloads: List[RunRunExecutionMetricEntry]) -> int:
        self.add(payloads)
        return self.step
