...
from tides.bin import orf_composition as cmp
...
def collect_args():
    ...
    orfcomp = parser.add_argument_group('Contamination-Calling Options', description = (
    '''--orf-comp (-oc)      generate ORF-composition table and plot\n'''
    '''                      for input file in FASTA format\n'''))

    orfcomp.add_argument('--orf-comp', '-oc', action = 'store_true',
        help = argparse.SUPPRESS)
...
def check_dependencies(args):
    dpnds = [
        ...
        ('Scikit-Learn', 'sklearn.svm'), ('ete3', 'ete3.Tree'), ('Barrnap', 'barrnap'),
...
def eval_orf_comp(fasta_file: str, taxon_code: str, gcode: str) -> None:
    cmp.make_plots(fasta_file, taxon_code, gcode)
...
def main():
    ...
    if args.orf_comp:
        eval_orf_comp(args.fin, args.taxon, args.gencode)
        sys.exit(0)
