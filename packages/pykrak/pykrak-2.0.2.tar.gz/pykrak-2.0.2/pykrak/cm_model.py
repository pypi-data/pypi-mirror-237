"""
Description:
    CM model object for managing an adiabatic model run.
...
from pykrak.raw_pykrak import get_phi_z
...
class CMModel(rdm.RangeDepModel):
    ...
    def compute_field(self, zs, zr, rs):
        ...
        """
        if self.comm.rank == 0:
            freq = self.env_list[0].freq
            rgrid = self._get_interface_ranges()
            ...
            for i in range(len(self.env_list)):
                rho_arr = get_phi_z(self.env_list[i].ind_arr, self.env_list[i].rho_arr)
                rho_list.append(rho_arr)
            rho_hs_list = [x.rho_hs for x in self.env_list]
            c_hs_list = [x.c_hs for x in self.env_list]
            attn_hs_list = [x.attn_hs for x in self.env_list]
            attn_units = self.env_list[0].attn_units
            omega = 2*np.pi*freq
            c_hs_list = [x + 1j* get_c_imag(x, y, attn_units, omega) for x,y in zip(c_hs_list, attn_hs_list)]
            #phi_zs = self.get_phi_zs(zs, M)
            #phi_zr = self.get_phi_zr(zr, M, rgrid, rs)
            p = cm.compute_arr_cm_pressure(omega, krs_list, phi_list, 
                                            zgrid_list, rho_list, rho_hs_list, c_hs_list,
                                            rgrid, zs, zr, rs, False, True) # False for same grid, True for particle. vel cont
...
class MultiFrequencyCMModel:
    def __init__(self, range_list, env_list, world_comm, model_freqs, pulse_freqs, model_comm):
        ...
        self.model = CMModel(range_list, env_list, model_comm)
