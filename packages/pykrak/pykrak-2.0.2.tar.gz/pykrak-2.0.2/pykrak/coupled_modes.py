...
@jit
def compute_arr_cm_pressure(omega, krs_list, phi_list, 
                            zgrid_list, rho_list, rho_hs_list, c_hs_list, 
                            rgrid, zs, zr, rs_grid, same_grid, cont_part_velocity=True):
    """
    Compute the pressure field at the receiver depths in zr
    Due to the source at zs 
    ...
    Nr = zr.size
    Ns = zs.size
    pressure_out = np.zeros((Ns, Nr, rs_grid.size), dtype=np.complex128)
    ...
    phi_zs = np.zeros((Ns, krs0.size))
    a0 = np.zeros((Ns, krs0.size), dtype=np.complex128)
    for i in range(krs0.size):
        phi_zs[:, i] = interp.vec_lin_int(zs, zgrid0, phi0[:,i])
        a0[:,i] = phi_zs[:,i] * np.exp(-1j * krs0[i] * rgrid[1]) / np.sqrt(krs0[i]) 
    a0 *= 1j*np.exp(1j*np.pi/4) # assumes rho is 1 at source depth
    a0 /= np.sqrt(8*np.pi)
    ...
    for i in range(1, rgrid.size):
        ...
        while (rcvr_range_index < rs_grid.size) and rs_grid[rcvr_range_index] <= r1:
            ...
            #p0 = np.zeros((Ns, krs0.size), dtype=np.complex128) # mode amplitudes at source range
            for zs_i in range(Ns):
                if i == 1:
                    p0 = advance_a(a0[zs_i,:], krs0, r1, rs)
                else:
                    p0 = advance_a(a0[zs_i,:], krs0, r0, rs)
                p = np.sum(p0 * phi0, axis=1)
                p /= np.sqrt(rs)
                p_zr_real = interp.vec_lin_int(zr, zgrid0, p.real)
                p_zr_imag = interp.vec_lin_int(zr, zgrid0, p.imag)
                pressure_out[zs_i, :, rcvr_range_index] = p_zr_real + 1j*p_zr_imag
            rcvr_range_index += 1
        ...
        gammas1 = np.sqrt(krs1**2 - (omega**2 / c_hs1**2)).real
        gammas1 = np.ascontiguousarray(gammas1)
        new_a0 = np.zeros((Ns, krs1.size), dtype=np.complex128)
        for zs_i in range(Ns):
            if i == 1: # first segment is special case (phase of a0 is for source-receiver range equal to the first segment length r1)
                new_a0[zs_i,:] = update_a0(a0[zs_i,:], krs0, gammas0, phi0, zgrid0, rho0, rho_hs0, krs1, gammas1, phi1, zgrid1, rho1, rho_hs1, r1, r1, same_grid, cont_part_velocity)
            else:
                new_a0[zs_i,:] = update_a0(a0[zs_i,:], krs0, gammas0, phi0, zgrid0, rho0, rho_hs0, krs1, gammas1, phi1, zgrid1, rho1, rho_hs1, r0, r1, same_grid, cont_part_velocity)
        a0 = new_a0
    ...
    while rcvr_range_index < rs_grid.size:
        rs = rs_grid[rcvr_range_index]
        for zs_i in range(Ns):
            p0 = advance_a(a0[zs_i,:], krs1, r1, rs) 
            p = np.sum(p0 * phi1, axis=1)
            p /= np.sqrt(rs)
            p_zr_real = interp.vec_lin_int(zr, zgrid1, p.real) # zgrid should be very fine so I don't see any issue here...
            p_zr_imag = interp.vec_lin_int(zr, zgrid1, p.imag)
            p_zr = p_zr_real + 1j*p_zr_imag
            pressure_out[zs_i, :, rcvr_range_index] = p_zr
        rcvr_range_index += 1
    return pressure_out
