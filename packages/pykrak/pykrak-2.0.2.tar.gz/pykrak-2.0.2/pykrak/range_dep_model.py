...
class RangeDepModel:
    ...
    def _get_phi_arr(self, N_max, M_max):
        """
        Get a matrix of mode shapes
        Each column corresponds to an environment range
        """
        phi_arr = np.zeros((self.n_env, N_max, M_max), dtype=np.complex128)
        Nz_arr = np.zeros((self.n_env), dtype=np.int32)
        M_arr = np.zeros((self.n_env), dtype=np.int32)
        for i in range(self.n_env):
            M = self.modes_list[i].M
            M_arr[i] = M
            Nz = self.modes_list[i].z.size
            Nz_arr[i] = Nz
            phi_arr[i, :Nz, :][:,:M] = self.modes_lists[i].phi
        return phi_arr, Nz_arr, M_arr
