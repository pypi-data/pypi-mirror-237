...
default_chill_data_yaml = "chill-data.yaml"
...
def main():
    ...
    if args["load"]:
        load(args["--config"], yaml_file=args.get("--yaml", default_chill_data_yaml))

    if args["dump"]:
        dump(args["--config"], yaml_file=args.get("--yaml", default_chill_data_yaml))
...
def dropdb(config):
    ...
    with app.app_context():
        app.logger.info(
            "Removing Chill database tables: Chill, Node, Node_Node, Route, Query, Template."
        )
        db = get_db()
        with db:
            drop_db()
...
def init():
    ...
    with app.app_context():
        ...
        dump_yaml(default_chill_data_yaml)
...
def start(app):
    ...
    set_sqlite_journal_mode(app)
    chill_data_files = []
    chill_data_yaml_files = list(
        map(
            lambda x: x.strip(),
            app.config.get("CHILL_DATA", default_chill_data_yaml).split(","),
        )
    )
        ...
        with db:
            ...
            if chill_sql_dump_tables:
                ...
                if len(other_tables):
                    ...
                    app.logger.info(
                        f"Backing up current database file to: {chill_dump_backup}. This is in case the sqlite database was modified when Chill was not running."
                    )
                    result = subprocess.run(
                        [
                            "sqlite3",
    ...
    loglevel = (
        ...
        else app.config.get("loglevel", "warning")
    )
