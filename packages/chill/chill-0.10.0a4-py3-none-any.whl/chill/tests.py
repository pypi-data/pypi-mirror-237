        ...
        with self.app.app_context():
            with self.app.test_client() as c:
                ...
                with db:
                    ...
                    insert_route(
                        path="/search/llamas/",
                        node_id=get_llamas_id,
                        weight=1,
                        method="GET",
                    )
                    insert_query(name="select_llama.sql", node_id=get_llamas_id)

                    post_search_llamas_id = insert_node(
                        name="post_search_llamas", value=None
                    )
                    insert_route(
                        path="/search/llamas/",
                        node_id=post_search_llamas_id,
                        weight=1,
                        method="POST",
                    )
...
class SQL(ChillTestCase):
    def test_insert_one_node(self):
        ...
        with self.app.app_context():
            ...
            with db:
                ...
                cur = db.cursor()
                result = cur.execute(
                    fetch_query_string("insert_node.sql"),
                    {"name": "a", "value": "apple"},
                )
                a = result.lastrowid
        ...
        with self.app.app_context():
            with self.app.test_client() as c:
                ...
                with db:
                    ...
                    content = insert_node(name="content", value="Àрpĺè")
                    insert_node_node(node_id=a_id, target_node_id=content)
    ...
    def test_delete_node_with_link(self):
        ...
        with self.app.app_context():
            ...
            with db:
                ...
                cur.execute(
                    fetch_query_string("delete_node_for_id.sql"), {"node_id": a_id}
                )
    ...
    def test_drop_chill_tables(self):
        with self.app.app_context():
            ...
            self.assertRegex(str(err.exception), "no such table: Node")
        ...
        with self.app.app_context():
            with self.app.test_client() as c:
                ...
                with db:
                    ...
                    isit = insert_node(name="isit", value="Àрpĺè")
        ...
        with self.app.app_context():
            with self.app.test_client() as c:
                ...
                with db:
                    ...
                    catimg = insert_node(name="acat", value="Àрpĺè")
                    insert_route(path="/cat/picture/", node_id=catimg)
