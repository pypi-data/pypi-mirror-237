...
def calculate_amplitude_etr(fo, fm, sigma, par, alpha_phase=True, light_independent=True, dark_sigma=False, etrmax_fitting=True, serodio_sigma=False, light_step_size=None, last_steps_average=False, outlier_multiplier=3, return_data=False, bounds=True, alpha_lims=[0,4], etrmax_lims=[0,2000], method='trf', loss='soft_l1', f_scale=0.1, max_nfev=None, xtol=1e-9):
	...
	etr_max : float
		The maximum electron transport rate, ETR\ :sup:`max`.
	slope : float
		The light limited slope of electron transport, α\ :sub:`ETR`.
	ek : float
		The photoacclimation parameter of ETR, E\ :sub:`k`.
	alpha_bias : float
	...
	etrmax_err : float
		The fit error of ETR\ :sup:`max`. If etrmax_fitting is False and alpha_phase is True, value returned is np.nan.
	slope_err : float
		The fit error of α\ :sub:`ETR`.
	ek_err : float
		The fit error of E\ :sub:`k`. If etrmax_fitting is True, value returned is np.nan.
