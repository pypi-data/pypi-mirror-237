...
def load_FASTTrackaI_files(file_, append=False, save_files=False, res_path=None, seq_len=120, irrad=None):
    ...
    df['flashlet_number'] = np.array(list(arange(0,seq_len,1)) * idx)
...
def load_FastOcean_files(file_, append=False, save_files=False, led_separate=False, res_path=None, 
    ...
    if FastAct1:
        if Single_Acq:
            md = read_csv(file_, delimiter=delimiter, skiprows=11, nrows=6, usecols=[0,1], header=None, encoding='latin1')
            ...
            df = read_csv(file_, delimiter=delimiter, skiprows = 21, nrows=seq_len, header=None, encoding='latin1').drop(columns=[2,3])
            ...
            md = read_csv(file_, skiprows=16, nrows=12, header=None, delimiter=delimiter, encoding='latin1')
            md = md.T.iloc[2:,:]
            ...
            df = read_csv(file_, skiprows=43, nrows=seq_len, header=None, delimiter=delimiter, encoding='latin1')
            seq_time = df.iloc[:,0]
            flashlet_number = df.iloc[:,1]
            df = df.iloc[:,2:]
            dfm = []
        ...
        if Single_Acq:
            df = read_csv(file_, skiprows=2, nrows=18, usecols=[0,1], header=None, encoding='latin1')
            date = df.iloc[0,1]
            time = df.iloc[1,1]
            ...
            df = read_csv(file_, skiprows=41, nrows=seq_len, header=None, encoding='latin1')
            ...
            df = read_csv(file_, skiprows=2, nrows=16, usecols=[0,1], header=None, encoding='latin1')
            date = df.iloc[0,1]
            time = df.iloc[1,1]
            datetime = repeat(to_datetime(date+' '+time, dayfirst=True), seq_len * 4 * seq_reps)
            ...
            led2 = Series(repeat(((float(df.iloc[14,1]) * 1e22) * flen * sigscale), seq_len))
            led3 = Series(repeat(((float(df.iloc[15,1]) * 1e22) * flen * sigscale), seq_len))
            df = read_csv(file_, skiprows=46, nrows=seq_len, header=None, encoding='latin1')
