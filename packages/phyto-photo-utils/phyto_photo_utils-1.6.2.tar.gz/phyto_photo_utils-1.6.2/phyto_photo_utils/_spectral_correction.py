...
def calculate_chl_specific_absorption(aptot, blank, ap_lambda, depig=None, chl=None, vol=None, betac1=None, betac2=None, diam=None, bricaud_slope=False, phycobilin=False, norm_750=False):
	...
	phycobilin : bool, default=False
		If True, will account for high np.absorption in the green wavelengths (580 - 600 nm) by phycobilin proteins when performing the bricaud_slope detrital correction.
	...
	def calculate_bricaud_slope(ap_lambda, aptot, phycobilin):
	    ...
	    if phycobilin:
	        ...
	        ratio = [p / aptot[idx692] for p in phyco]

	        # Find the minimum ratio and its index
	        min_val, min_idx = min((val, idx) for (idx, val) in enumerate(ratio))
	        idx = phyco.index[min_idx]
	        widx = ap_lambda[idx]
	        R2 = aptot[idx] - (constant_b * aptot[idx692])
	    else:
	    ...
	    if (R1 <= 0) or (R2 <= 0):
	        ...
	        if phycobilin:
	            L2 = np.exp(-widx * S) - (constant_b * np.exp(-widx * S))
	        else:
...
def calculate_instrument_led_correction(aphy, ap_lambda, method=None, chl=None, e_background=None, e_insitu=None, e_actinic=None, depth=None, e_led=None, wl=None, constants=None):
	...
	ap_lambda : np.np.array, dtype=int, shape=[n,]
		The wavelengths associated with the aphy and aphy_star.
	method : 'sigma', 'actinic'
		Choose spectral correction method to either correct σPSII or correct the background actinic light.
	...
	e_insitu : np.np.array, dtype=int, shape=[n,]
		The in situ irradiance field, if None is passed then will theoretically calculate in situ light field.
	chl : dtype=float
		Chlorophyll-a concentration (mg m$^-$$^3$}) for estimation of Kbio for theoretical in situ light field. Must be set if theoretically calculating e_insitu.
	e_actinic : 'fastact'
	...
	scf : float
		The spectral correction factor to correct σPSII or actinic background light depending on method.
	...
	data_path = pkg_resources.resource_filename('phyto_photo_utils', 'data/')
	data_file = pkg_resources.resource_filename('phyto_photo_utils', 'data/spectral_correction_constants.csv')
			...
			if e_background == 'insitu':				
				if e_insitu is None:
					if depth is None and chl is None:
						raise ValueError('User must define depth and chlorophyll-a concentration in mg m$^-$$^3$ for calculating in situ light spectra')
					else:
