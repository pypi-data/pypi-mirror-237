...
def fit_relaxation(flevel, seq_time, seq, datetime, blank=0, sat_len=None, rel_len=None, sat_flashlets=0, model_type=None, bounds=True,  tau1_lims=[100, 800], tau2_lims=[800, 2000], tau3_lims=[2000, 50000], method='trf', loss='soft_l1', f_scale=0.1, max_nfev=None, xtol=1e-9):
	...
	tau2_lims: [int, int], default=[800, 2000]
	 	The lower and upper limit bounds for fitting τ2, not required if model_type = 'triple'.
	tau3_lims: [int, int], default=[2000, 50000]
	 	The lower and upper limit bounds for fitting τ3, not required if model_type = 'triple'.
	...
	tau2 : np.array, dtype=float, shape=[n,]
		The rate of QB\ :sup:`-` reoxidation in μs, only returned if model_type is 'triple', value set to numpy np.nan otherwise.
	tau3 : np.array, dtype=float, shape=[n,]
		The rate of PQ reoxidation in μs, only returned if model_type is 'triple', value set to numpy np.nan otherwise.
	alpha1 : np.array, dtype=float, shape=[n,]
		The decay coefficient of τ\ :sub:`1`, only returned if model_type is 'triple', value set to numpy np.nan otherwise.
	alpha2 : np.array, dtype=float, shape=[n,]
		The decay coefficient of τ\ :sub:`2`, only returned if model_type is 'triple', value set to numpy np.nan otherwise.
	alpha3 : np.array, dtype=float, shape=[n,]
		The decay coefficient of τ\ :sub:`3`, only returned if model_type is 'triple', value set to numpy np.nan otherwise.
	...
	tau2_err : np.array, dtype=float, shape=[n,]
		The fit error of τ\ :sub:`2`, only returned if model_type is 'triple', value set to numpy np.nan otherwise.
	tau3_err : np.array, dtype=float, shape=[n,]
		The fit error of τ\ :sub:`3`, only returned if model_type is 'triple', value set to numpy np.nan otherwise.
	alpha1_err : np.array, dtype=float, shape=[n,]
		The fit error of α\ :sub:`1`, only returned if model_type is 'triple', value set to numpy np.nan otherwise.
	alpha2_err : np.array, dtype=float, shape=[n,]
		The fit error of α\ :sub:`2`, only returned if model_type is 'triple', value set to numpy np.nan otherwise.
	alpha3_err : np.array, dtype=float, shape=[n,]
		The fit error of α\ :sub:`3`, only returned if model_type is 'triple', value set to numpy np.nan otherwise.
