...
class UserDataPoint:
    ...
    def get_available_datapoints(self) -> list:
        ...
        """
        resp_obj = get(url=self.base_url + "datapoints", headers=self.headers)
    ...
    def upload_userdata(self, date, dp, dp_type, asset_id_type, data_dict):
        ...
        Args:
            ...
            dp (str) : Datapoint to be uploaded.
        ...
        resp_obj = get(url=self.base_url + "userdata/registration", headers=self.headers)
        if resp_obj.status_code != 200:
    ...
    def __register_dp(self, dp, dp_type):
        ...
        resp_obj = post(url=self.base_url + "userdata/registration", headers=self.headers, data=data)
        if resp_obj.status_code == 200:
    ...
    def __upload_dp(self, dp, asset_id_type, data_dict, date):
        ...
        resp_obj = post(url=self.base_url+resource, headers=self.headers, data=dp_dict)
        if resp_obj.status_code == 200:
    ...
    def get_user_datapoints(self) -> list:
        ...
        resp_obj = get(self.base_url+resource, headers=self.headers)
