...
def bulk_request_body(job_list, date=None, snapshot_list=None):
    key_list = []
    for job in job_list:
        key = {
            'jobId': job
...
def get_token(url, client_id, client_secret, audience, service_name):
    """Get access token.
    ...
    if not token:
        print("Generating new bearer token for authentication of {}".format(service_name))
        ...
        print("Using the cached token for Authentication of {}".format(service_name))
    return token
    ...
    if file_data:
        ...
        if file_client_id == client_id:
            ...
            if datetime.now() <= validity_time:
                ...
                print("Saved token expired, generating new token")
                return
