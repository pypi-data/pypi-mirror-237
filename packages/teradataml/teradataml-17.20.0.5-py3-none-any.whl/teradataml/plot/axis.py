...
class Axis:
    def __init__(self, **kwargs):
        ...
        # 'vmin' is applicable only for Geometry plot.
        ...
        # 'vmax' is applicable only for Geometry plot.
        if self.kind.lower()  != 'geometry':
    ...
    def __get_series_spec(self):
        ...
        if self.__series_identifier:
            ...
            series = TDSeries(data=self.__x_axis_data[0]._parent_df,
        ...
        for _x, _y in zip(self.__x_axis_data, self.__y_axis_data):
            ...
            if isinstance(_y, tuple):
                ...
                columns = {"id":UtilFuncs._replace_special_chars(_id_column), "x":_x, "y1":_y[0], "y2":_y[1]}
                payload_field = ["y1", "y2"]
            else:
                columns = {"id":UtilFuncs._replace_special_chars(_y.compile()), "x":_x, "y":_y}
                payload_field = "y"
    ...
    def __get_matrix_spec(self):
        ...
        if self.__series_identifier:
            ...
            matrix = TDMatrix(data=self.__x_axis_data[0]._parent_df,
                              id=self.__series_identifier,
                              row_index=self.__x_axis_data[0].name,
                              row_index_style=self.__get_index_style(self.__x_axis_data[0]),
        ...
        _previous_df = None
        for _x, _y, _data in zip(self.__x_axis_data, self.__y_axis_data, self.__scale_data):
            _df = _x._parent_df
            columns = {"id": UtilFuncs._replace_special_chars(_y.compile()), "x": _x, "y": _y, "data": _data}
            _df = _df.assign(**columns, drop_columns=True)
            if _previous_df:
                # TODO: Note that concat is a very costly operation. Infact, it is very very slow.
                #  Consider generating VIRTUAL tables or UNPIVOT.
