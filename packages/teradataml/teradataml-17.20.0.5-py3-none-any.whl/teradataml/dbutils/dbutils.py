...
def view_log(log_type="script", num_lines=1000, start_datetime=None, request_id=None):
    """
    DESCRIPTION:
        ...
        When log_type is set to "apply", function returns logs from the container
        which runs Apply queries which are invoked by Apply class or dataframe apply
        method. Function also can be used to retrive logs for any remote user environment
        administration tasks performed using UserEnv class functions.
    ...
    PARAMETERS:
        ...
        request_id:
            Optional Argument.
            Specifies the request identifier of the query for which logs are to be retrieved.
            This request identifier is part of the error message received when Apply class
        ...
        start_datetime:
            Optional Argument.
            Specifies the datetime object. All the logs generated after the
            specified datetime till the current datetime will be returned.
            Note:
                * This argument is applicable when log_type is 'apply' otherwise ignored.
                * Datetime should be specified in below format:
                  datetime(YEAR, MONTH, DATE, HOUR, MINUTE, SECOND)
                * When neither start_datetime nor request_id is specified, function returns
                  logs generated by only user environment management functions provided by
                  UserEnv class.
            Types: datetime
    ...
    EXAMPLES:
        ...
        # Example 2: Return logs using request identifier from the error message.
        # Use request identifier from the error messages returned by Apply class or
        # dataframe apply method to find more details about the error.
        >>> view_log(log_type="apply", request_id='307187838638029089')

        # Example 3: Return log entries for the given request identifier logged after
        # the specified start datetime till the current time.
        # Use datatime object.
        >>> d = datetime(2002, 12, 9, 2, 37, 12)
        >>> view_log(log_type="apply", request_id='307187838638029089', start_datetime=d)

        # Example 4: Return all logs from the current session created by
        # UserEnv class functions.
        >>> view_log(log_type="apply")
    """
    awu_matrix_test = []
    ...
    awu_matrix_test.append(["start_datetime", start_datetime, True, (datetime), True])
    awu_matrix_test.append(["request_id", request_id, True, (str), True])
    ...
    # log_type is apply.
        ...
        if start_datetime and request_id is None:
        ...
        using_clause = ""
        startdatetime_clause = ""
        req_id_clause = ""

        if start_datetime is not None:
            startdatetime = start_datetime.isoformat(sep=' ', timespec='milliseconds')
            startdatetime = startdatetime.replace(" ", "T") + "Z"
            startdatetime_clause = "StartDateTime('{}')".format(startdatetime)
            using_clause = "USING"

        if request_id is not None:
            req_id_clause = "Request_ID('{}')".format(request_id)
            using_clause = "USING"

        # Create Logging table operator query.
        view_log_query = TableOperatorConstants.APPLY_LOG_QUERY.value \
            .format(using_clause, startdatetime_clause,req_id_clause)
