...
class ArcticFutures(object):
    ...
    def list_daily_symbols(self):
        return sorted(self._arc['future_daily'].list_symbols())
    
    def list_markets(self):
        markets = [re.sub(pattern=r'[0-9]+', repl='', string=m) for m in self.list_daily_symbols()]
        return sorted(list(set(markets)))
    
    def list_minute_symbols(self):
        return sorted(self._arc['future_minute'].list_symbols())
    
    def list_tick_symbols(self):
        return sorted(self._arc['future_tick_chunk'].list_symbols())
    ...
    def load_futures_minute_data_batch(self, symbols:list, start_date='2000-01-01 09:00:00', end_date='2100-01-01 09:00:00', columns=[]) -> DataFrame:
        columns_reverse_map = {'volume': 'volumn'}
        date_range = (pd.to_datetime(start_date), pd.to_datetime(end_date)) 
    ...
    def get_active_range(self, market, start_date, end_date):
        ...
        daily_range = self._arc['future_daily_active'].read(symbol=market, date_range=date_range, columns=['TICKER', 'ROLLOVER_TICKER']).data
