...
class ArcticOptions():
    def __init__(self, url:str) -> None:
        s3_url = f's3://{url}:20080:market?access=lany&secret=ly@123456&region=cn-north-1'
    ...
    def load_options_tick_data(self, symbol, start_date='2000-01-01 09:00:00', end_date='2100-01-01 15:00:00', columns=[]) -> DataFrame:
        ...
        if self._arc['option_tick'].has_symbol(symbol):
            if len(columns) > 0:
                data = self._arc['option_tick'].read(symbol=symbol, columns=columns, query_builder=q).data
            else:
                data = self._arc['option_tick'].read(symbol=symbol, query_builder=q).data
        else:
            data = DataFrame()
        return data


    def load_stockOptions_tick_data(self, symbol, start_date='2000-01-01 09:00:00', end_date='2100-01-01 15:00:00', columns=[]) -> DataFrame:
        q = QueryBuilder()
        q.optimise_for_memory()
        date_range = (pd.to_datetime(start_date), pd.to_datetime(end_date))
        q = q.date_range(date_range)
        if self._arc['stockoption_lv1'].has_symbol(symbol):
            if len(columns) > 0:
                data = self._arc['stockoption_lv1'].read(symbol=symbol, columns=columns, query_builder=q).data
            else:
                data = self._arc['stockoption_lv1'].read(symbol=symbol, query_builder=q).data
        else:
            data = DataFrame()
        return data

    def load_stockOptions_minute_data(self, symbol, start_date='2000-01-01 09:00:00', end_date='2100-01-01 15:00:00', columns=[]) -> DataFrame:
        data = self.__load_data(lib_name='stockoption_minute', symbol=symbol, start_time=start_date, end_time=end_date, columns=columns)
        print(data)
