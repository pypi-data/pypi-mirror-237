...
import json
from .config import OPENAI_KEY, GOOGLE_API_KEY, DEFAULT_PROMPT
...
def get_event_gmap_info(str_input, lang='CH', **kwargs):
    ...
    prompt.append({
        'role': 'system',
        'content': str_prompt + ('請用英文回覆我' if lang=='EN' else '請用中文回覆我')
    })
    prompt.append({
        'role': 'user',
        'content': f'「現在時間是{now} 星期{weekday[now.isoweekday()-1]}」 \n{str_input}'
    })
    model = 'gpt-4'
    ...
    try:
        ...
        gmap_url = get_gmap_url(result['location'], GOOGLE_API_KEY)
        result['gmap'] = gmap_url
        return result
