...
def _get_conv_params(layer):
    ...
    layer_params = {
        ...
        "output_bits": 8,
        "filters": layer.weights["W"].shape[0],
        "kernel_size": layer.weights["W"].shape[-1],
        "padding": get_akida_padding(layer),
        "pool_type": layer.pool_type,
        "kernel_stride": layer.strides[0],
    }
    if layer.pool_type == akida.PoolType.Max:
        layer_params["pool_size"] = layer.pool_size[0]
        layer_params["pool_stride"] = layer.pool_size[0]
    return layer_params
...
class Conv2DOnnxConverter(OnnxConverter):
    ...
    def _parse_akida_layer(self):
        layer_params = _get_conv_params(self)
