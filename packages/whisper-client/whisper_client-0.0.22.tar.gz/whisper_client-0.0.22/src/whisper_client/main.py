...
class WhisperClient:
    ...
    def send_audio(self, audio: Path | str, hash_audio: str = None, no_skip: bool = False, verbless: bool = False) -> \
            Optional[dict]:
        ...
        if not no_skip and self.is_hash_done(hash_audio):
            ...
            return self.get_status(hash_audio)
    ...
    def manage_audio_folder(self, folder: Path | str = None, mode: list[Mode] | Mode = Mode.full,
                            interval: int = 30, no_skip: bool = False) -> None:
        if isinstance(mode, list):
            for m in mode:
                self.manage_audio_folder(folder, mode=m, interval=interval)
                return
        ...
        for audio in to_process:
            hashes_n_paths.update({
                self.send_audio(audio, no_skip=no_skip)["hash"]: audio
            })
        ...
        while True:
            ...
            for hash_audio, audio in hashes_n_paths.items():
                if self.is_hash_done(hash_audio):
                    self.save_result_with_mode(hash_audio, mode=mode, interval=interval)
                    to_remove.append(hash_audio)
                    pbar.update(1)

            for hash_audio in to_remove:
                del hashes_n_paths[hash_audio]
        ...
        #
        # while True:
        #     if not any((slot1, slot2, to_process)):
        #         break
        #
        #     if slot1 is None:
        #         slot1 = to_process.pop(0)
        #         try:
        #             hash1 = self.send_audio(slot1, no_skip=no_skip)
        #             hash1 = hash1["hash"]
        #         except:
        #             print(f"ERROR : Could not send {slot1}")
        #             print(f"ERROR : {hash1}")
        #             raise
        #     if slot2 is None:
        #         slot2 = to_process.pop(0)
        #         try:
        #             hash2 = self.send_audio(slot2, no_skip=no_skip)
        #             hash2 = hash2["hash"]
        #         except:
        #             print(f"ERROR : Could not send {slot2}")
        #             print(f"ERROR : {hash2}")
        #             raise
        #
        #     if self.is_hash_done(hash1):
        #         self.save_result_with_mode(hash1, mode=mode, interval=interval)
        #         slot1 = None
        #         hash1 = None
        #         pbar.update(1)
        #         continue
        #
        #     if self.is_hash_done(hash2):
        #         self.save_result_with_mode(hash2, mode=mode, interval=interval)
        #         slot2 = None
        #         hash2 = None
        #         pbar.update(1)
        #         continue
    ...
    def manage_video_folder(self, folder: Path | str = None, mode: list[Mode] | Mode = Mode.full,
                            interval: int = 30, no_skip: bool = False) -> None:
        if isinstance(folder, str):
            folder = Path(folder)
        ...
        self.manage_audio_folder(audio_folder, mode=mode, interval=interval, no_skip=no_skip)
    ...
    def get_hash_audio(self, audio: bytes | BytesIO | Path | str) -> str:
        if isinstance(audio, str):
            audio = Path(audio)
    ...
    def is_hash_done(self, hash_audio: str = None) -> bool:
        if hash_audio is None:
            hash_audio = self.last_hash
