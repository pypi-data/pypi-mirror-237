...
parser.add_argument("-k", "--api-key", type=str, required=True, help="API key for the whisper API")
...
def manage_output(wc: WhisperClient, res: dict, args: argparse.Namespace, mode: str = None) -> None:
    if args.output is not None:
        ...
        if output.is_dir():
            output = output / f"{res['hash']}{f'_{mode}' if mode is not None else ''}.json"

    else:

        output = wc.text_folder / f"{res['hash']}{f'_{mode}' if mode is not None else ''}.json"
...
def cli(parser: argparse.ArgumentParser = parser) -> int:
    ...
    if args.input is not None:
        ...
        for mode in args.mode.split(","):
            ...
            manage_output(wc, res, args, mode=mode)
