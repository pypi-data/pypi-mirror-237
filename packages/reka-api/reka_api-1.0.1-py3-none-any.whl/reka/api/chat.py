...
def chat(
    ...
    """
    human_dict: HumanTurn = {"type": "human", "text": human}
    if media_url is not None:
        human_dict["media_url"] = media_url
    if media_type is not None:
        human_dict["media_type"] = media_type
    if file_url is not None:
        human_dict["file_url"] = file_url

    full_conv = (conversation_history or []) + [human_dict]
...
def _check_conversation_history(
    ...
    if conversation_history[-1]["type"] != "human":
        raise InvalidConversationError(
            reason="Conversation should end with a 'human' turn."
        )
