        ...
        for filedata in self.entry_data:
            ...
            def add_crossref(agent, sign, item, item2, silent, ref_agent, direction, other):
                ...
                if direction == 0:
                    ...
                    crossrefs.append((item,agent,ref_agent,sign + quantity,be))
                
                elif direction == 1:
                    if other in be_dict:
                        be = be_dict[other]
                    else:
                        raise TypeError("Could not identify entry %s" % other)
                        
                    # item, agent, ref_agent, q, be = cr
                    crossrefs.append((item,ref_agent,agent,sign + quantity,be))
            ...
            add_args = self.editAddArgs.text().strip()
            if add_args != "":
                str_refa += "," + add_args
            ...
            # additional code 
            add_code = self.editAddCode.toPlainText().split("\n")
