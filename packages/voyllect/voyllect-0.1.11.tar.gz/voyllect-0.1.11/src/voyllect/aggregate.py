...
def build_counts_per_urls_kpis(aggregated_products, 
    ...
    reviews_df = \
        pd.DataFrame(aggregated_reviews)[['url', 'review_title', 'review_text', 'review_rating']].\
            drop_duplicates()
...
def build_counts_per_collect_date_kpis(aggregated_products, 
    ...
    Args:
        ...
        aggregated_reviews (list[dict]): list of aggregated reviews dictionaries.
        source_dict (dict): dictionary with information from the source.
        kpis_counts_per_urls_products_folder_path (str): path to the products counts per URLs KPIs folder.
        kpis_counts_per_urls_reviews_folder_path (str): path to the reviews counts per URLs KPIs folder.
...
def build_samples_kpis(aggregated_products, 
    ...
    aggregated_products_sample_df = aggregated_products_df.sample(n=15, random_state=1)

    # Sample for the reviews data collected
    aggregated_reviews_df = pd.DataFrame(aggregated_reviews)
    aggregated_reviews_sample_df = pd.DataFrame(aggregated_reviews_df).sample(n=15, random_state=1)
