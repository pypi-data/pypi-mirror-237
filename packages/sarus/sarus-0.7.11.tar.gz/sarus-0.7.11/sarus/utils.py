...
def eval_perturbation(
    ...
    class DeltaEpsilonLimit:
        """An ad-hoc class to define a privacy limit clent side (which is unusual for now).

        The code is copied from Sarus differential-privacy internal package
        """

        def __init__(self, delta_epsilon_dict: dict):
        ...
        def delta_epsilon_dict(self) -> dict:
    ...
    if isinstance(x, DataSpecWrapper):
        ...
        # get privacy limit
        if target_epsilon is not None:
            default_delta = manager.default_delta(dataspec)
        ...
        # get DP variants
        # this could be done using api.compile_dataspec if we add a salt argument.
        dp_values = []
        for i in range(19):
            ...
            if i == 0 and not dp_dataspec.is_dp():
                raise DataSpecErrorStatus(
                    (
                        False,
                        "The dataspec could not be compiled in DP.",
                    )
                )
            dp_values.append(dp_dataspec)

        dataspec = error_estimation()(dataspec, *dp_values)
        manager.push(dataspec)
        alt_dataspec, _ = manager.compile(dataspec)
