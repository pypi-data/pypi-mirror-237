...
from testios.core.ios.driver import Driver
from testios.utils.common import calculate_time, \
    draw_red_by_rect
from testios.utils.exceptions import KError
from testios.core.image.element import ImageElem
from testios.core.ocr.element import OCRElem
...
class Elem(object):
    ...
    def _match(self, text):
        ...
        result = text if _element.wait(timeout=0.1, raise_error=False) else None
        return result
            ...
            try:
                if _element.wait(timeout=timeout):
                    ...
                    if self._debug is True:
                        ...
                        rect_tuple = (rect.x * scale, rect.y * scale, rect.width * scale, rect.height * scale)
                ...
                # 由于WDA会意外链接错误
                self._driver = Driver(self._driver.device_id, self._driver.pkg_name)
                time.sleep(5)
