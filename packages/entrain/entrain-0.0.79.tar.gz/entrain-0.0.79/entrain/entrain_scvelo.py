...
def cluster_velocities(h5ad_path,
                       resolution=0.05,
                       prefix="vcluster",
                       cluster_key = "velocity_clusters"):
    adata = ad.read_h5ad(h5ad_path) if type(h5ad_path) == str else h5ad_path
...
def recover_dynamics_clusters(adata, 
    ...
    likelihood_remove_idx = likelihoods_df > 10
    if likelihood_remove_idx.any().any():
        likelihoods_df[likelihood_remove_idx] = np.NaN
        removed = str(likelihood_remove_idx.stack()[likelihood_remove_idx.stack()].index.tolist())
        warntext = "Following genes in velocity clusters were removed due to fit_likelihoods greater than 10:" + removed
        logging.warn(warntext)
...
def get_velocity_cluster_label_positions(adata,
    ...
    label_df["var_exp"] = round(
        label_df.loc[:, "var_exp"].multiply(100.0), 1).astype(str) + "% E.V.E"
...
def plot_velocity_clusters_python(adata_clustered,
                                  plot_file,
                                  velocity_cluster_key,
                                  vector_type="stream", palette = None, **kwargs):
