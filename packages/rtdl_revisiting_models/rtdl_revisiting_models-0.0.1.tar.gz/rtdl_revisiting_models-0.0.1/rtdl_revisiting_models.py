...
class CLSEmbedding(nn.Module):
    ...
    **Examples**
    ...
    >>> batch_size = 2
    >>> n_tokens = 3  # Any number.
    >>> d_embedding = 5
    >>> x = torch.randn(batch_size, n_tokens, d_embedding)
    >>>
    >>> batch_dims = (batch_size,)
    >>> object_dims = (n_tokens, d_embedding)
    >>> # The following always holds:
    >>> batch_dims == x.shape[:-len(object_dims)]
    True
    >>> m = CLSEmbedding(d_embedding)
    >>> batch_dims
    (2,)
    >>> m(batch_dims).shape
    torch.Size([2, 1, 5])
...
class CategoricalFeatureEmbeddings(nn.Module):
    """Embeddings for categorical features  (used in FT-Transformer).
    ...
    **Examples**
    ...
    torch.Size([4, 2])
    >>> m = CategoricalFeatureEmbeddings(cardinalities, d_embedding=5)
    ...
    def d_embedding(self) -> int:
        """The embedding size."""
        return self.embeddings[0].embedding_dim
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if x.shape[-1] != self.n_features:
            raise ValueError(
                ...
                f' However: {x.shape[-1]=}, len(cardinalities)={self.n_features}'
            )

        x = torch.stack(
            [self.embeddings[i](x[..., i]) for i in range(self.n_features)], dim=-2
        )
        if self.bias is not None:
            x = x + self.bias
        return x
    ...
    **Examples**
    ...
    def __init__(
        ...
        *,
        ...
        # Here, we also support the ReLU activation to illustrate the difference
