...
LOOKUPS = {
    "crop": [
        ...
        "N_Content_AG_Residue", "N_Content_BG_Residue"
    ],
    "forage": [
        "IPCC_2019_Ratio_AGRes_YieldDM", "IPCC_2019_Ratio_BGRes_AGRes",
        "N_Content_AG_Residue", "N_Content_BG_Residue"
...
def _multiple_product_value(crop: dict, residue_id: str):
    term = crop.get('term', {})
    term_id = term.get('@id')
...
def _run_multiple_products(crop_residue_products: list, products: list):
    def run_product(product: dict):
        term_id = product.get('term', {}).get('@id')
        values = non_empty_list([_multiple_product_value(p, term_id) for p in products])
