...
def _should_run(cycle: dict):
    ...
    products = _filter_list_term_unit(products, Units.KG_COLD_DRESSED_CARCASS_WEIGHT)
    products = list(filter(node_has_no_property(TERM_ID), products))
    has_kg_dressed_carcass_products = len(products) > 0
    ...
    logRequirements(cycle, model=MODEL, term=TERM_ID,
                    has_kg_dressed_carcass_products=has_kg_dressed_carcass_products,
                    year=year,
                    country=country)

    should_run = all([has_kg_dressed_carcass_products, year, country])
