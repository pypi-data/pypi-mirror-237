...
LOOKUPS = {
    "crop": [
        ...
        "LIGNIN_CONTENT_AG_CROP_RESIDUE", "LIGNIN_CONTENT_BG_CROP_RESIDUE"
    ],
    "forage": [
        "IPCC_2019_Ratio_AGRes_YieldDM", "IPCC_2019_Ratio_BGRes_AGRes",
        "LIGNIN_CONTENT_AG_CROP_RESIDUE", "LIGNIN_CONTENT_BG_CROP_RESIDUE"
...
PRODUCT_COLUMN_MAPPING = {
    'aboveGroundCropResidueTotal': 'LIGNIN_CONTENT_AG_CROP_RESIDUE',
    'belowGroundCropResidue': 'LIGNIN_CONTENT_BG_CROP_RESIDUE'
}
...
def _multiple_product_values(crop: dict, residue_id: str):
    ...
    l_content = _get_lookup_value(term, PRODUCT_COLUMN_MAPPING[residue_id])
    ...
    debugValues(crop, model=MODEL, term=residue_id,
                ...
                ratio=ratio)
    return (value * dm / 100 * yield_dm * (ratio or 1), l_content) if l_content is not None else None
...
def _run_multiple_products(crop_residue_products: list, products: list):
    def run_product(product: dict):
        ...
        value = sum([value * l_content for value, l_content in values]) / total if total > 0 else None
        prop = _property(value) if value is not None else None
