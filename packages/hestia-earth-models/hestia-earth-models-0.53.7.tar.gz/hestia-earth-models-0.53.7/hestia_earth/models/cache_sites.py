...
from .geospatialDatabase.utils import (
    CACHE_VALUE, CACHE_AREA_SIZE,
    has_boundary, has_coordinates,
    _run_query, _site_gadm_id
)
from .site.pre_checks.cache_geospatialDatabase import (
    list_collections, cache_site_results, _should_cache_results
)
from .utils.site import CACHE_KEY
...
def _run_values(sites: list, param_type: ParamType, rasters: list, vectors: list):
    ...
    def _process_site(site_values: tuple):
        ...
        return {
            **site,
            CACHE_KEY: {**site.get(CACHE_KEY, {}), CACHE_VALUE: cached_data}
        }
...
def _group_sites(sites: dict):
    ...
    sites = [
        (n, ) + (_should_cache_results(n, get_region_area_size(n))) for n in sites
    ]
...
def run(sites: list, years: list, include_region: bool = False):
    ...
    include_region : bool
        Prefect region IDs.
        This will catch region-level data for level 0 to 5, and will make the request slower. Only use if needed.
    """
    ...
    return flatten([
        _run_values(filtered_data.get(param_type), param_type, rasters, vectors)
