...
class ProjectManager(object):
        ...
        if config_path:
            ...
            if self.os == "Windows":
                ...
                file_path = os.path.expanduser(
                    "~/Dropbox/workspace/PySatDNA/config.yaml"
                )
                if not os.path.isfile(file_path):
                    file_path = os.path.expanduser(
                        "~/Dropbox/PySatDNA/config.dobi.yaml"
                    )
        try:
    ...
    def add_project(
        self, pid, project_data, init=False, force=False, force_folder_creation=False
    ):
    ...
    def get_project(
        self, pid, settings_context=None, project_context=None, path_replacing=None
    ):
        ...
        if path_replacing:
            ...
            if "path_to" in project_data:
                project_data["path_to"] = project_data["path_to"].replace(
                    path_from, path_to
                )
    ...
    def _fix_paths(self, project):
        ...
        for folder_key, path_to in data["folders"].items():
            data["folders"][folder_key] = os.path.join(
                self.work_folder, project["path_to"], path_to
            )
        for file_key, path_to in data["files"].items():
            data["files"][file_key] = os.path.join(
                self.work_folder, project["path_to"], path_to
            )
