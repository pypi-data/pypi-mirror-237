...
class ProcessRunner(object):
    ...
    def run_parallel_no_output(self, commands, mock=False):
        ...
        for p in ps:
            ...
            n -= 1
            runner_logger.info(
                "A process returned: %s (remains %s)" % (p.returncode, n)
            )
    ...
    def run_asap(self, commands, cpu=10, mock=False):
        ...
        while commands:
            while len(running) > cpu:
                runner_logger.debug(
                    "Checking %s processes from (%s)" % (len(running), len(commands))
                )
                for i, p in enumerate(running):
                    ...
                    if returncode is not None:
                        if returncode == 0:
                            runner_logger.info(
                                "A process returned: %s (remains %s)"
                                % (p.returncode, len(commands))
                            )
                        else:
                            runner_logger.error(
                                "A process returned error: %s (remains %s)"
                                % (p.returncode, len(commands))
                            )
                        running[i] = None
    ...
    def popen(self, command, silent=False):
        ...
        if process.returncode != 0:
            message = "ERROR when launching '%s', code %s, output %s" % (
                command,
                process.returncode,
                output,
            )
...
class AbstractExperiment(object):
    ...
    def execute(self, start_sid=0, end_sid=None, project_context=None):
        ...
        for step in steps[start_sid:end_sid]:
            # refresh project
            self.project, _settings = self.manager.get_project(
                self.project["pid"], project_context=project_context
            )
            ...
            if not self.force:
                ...
                if step.check_value:
                    ...
                    if status_p == step.check_value:
                        exp_logger.warning(
                            "Step skipped. Step %s was previously computed with %s"
                            % (step.name, status_p)
                        )
                        continue
            if self.logger:
                exp_logger.info(
                    "Logger, start event",
                    self.logger(self.pid, self.name, step.sid, step.name, STARTED),
                )
            with Timer(step.name):
                ...
                if step.check_p is not None and hasattr(step.check_p, "__call__"):
                    ...
                    if result is not None:
                        exp_logger.error(
                            "Compute prerequisites for step %s is failed with %s"
                            % (step.name, result)
                        )
                ...
                if self.logger:
                    exp_logger.info(
                        "Logger, finish event",
                        self.logger(self.pid, self.name, step.sid, step.name, FINISHED),
                    )
                # save step output
                if step.save_output:
                    if result is None:
    ...
    def execute_parallel(
        self, start_sid=0, end_sid=None, project_context=None, threads=1
    ):
        """
    ...
    def default_logger(self, pid, name, step_sid, step_name, status):
        ...
        if (
            not "config" in self.settings
            or not "url_exe_update" in self.settings["config"]
        ):
    ...
    def logger_update_status(self, pid, step_name, status):
        ...
        if (
            not "config" in self.settings
            or not "url_status_update" in self.settings["config"]
        ):
    ...
    def logger_update_project(self, pid, project):
        ...
        if (
            not "config" in self.settings
            or not "url_project_update" in self.settings["config"]
        ):
    ...
    def _send_to_server(self, url, data, force=False):
        ...
        while attempts < 3:
            try:
                ...
                exp_logger.info(
                    "Failed sent to url %s with response: %s" % (url, response)
                )
                break
