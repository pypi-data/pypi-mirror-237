...
from PyExp.abstract_experiment import AbstractStep
from PyExp.abstract_manager import ProjectManagerException
...
def _create_projects(exp_settings_class, manager_class, all_projects):
    ...
    for pid, project_data in all_projects:
        ...
        try:
            ...
            try:
                manager.recheck_folders_and_params(
                    pid, project, project_data=project_data
                )
            except OSError as e:
            ...
            app_logger.error(
                "ERROR: please, check that all settings given as dictionaries, not sets. (%s)"
                % e
            )
            raise Exception(e)
        except ProjectManagerException as e:
...
def execute(args, usage, dataset_dict, exp_settings, exp_class, manager_class):
    ...
    if not dataset in dataset_dict:
        print("Available datasets: %s" % ", ".join(dataset_dict.keys()))
        sys.exit(0)
...
def execute_task(
    ...
    for i, (pid, project_init) in enumerate(dataset):
        ...
        experiment_settings = exp_settings_class()
        manager = manager_class(experiment_settings)
        project, settings = manager.get_project(
            pid,
            settings_context=settings_context,
            project_context=project_context,
            path_replacing=path_replacing,
        )
...
def run_app(exp_class, exp_settings_class, manager_class, dataset_dict):
    ...
    usage = (
        "command task1,task2 from to dataset, e.g."
    ...
    if len(args) == 2:
        if args[0] == "create":
            ...
            try:
                ...
                print(e)
                print(
                    "Necessary yaml fields: command (str), steps (list), start (int), end (int), dataset (str)"
                )
                print(
                    "Optional fields: data (str), settings_context (dict), project_context (dict)"
                )
                print(
                    "OR you may generate a yaml file with: 'yaml generate file_name' command"
                )
                sys.exit(1)

            execute(
                args, usage, dataset_dict, exp_settings_class, exp_class, manager_class
            )
        else:
            print(usage)
            sys.exit(1)
        ...
        execute(args, usage, dataset_dict, exp_settings_class, exp_class, manager_class)
