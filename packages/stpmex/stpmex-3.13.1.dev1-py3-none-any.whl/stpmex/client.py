...
class Client:
    ...
    def __init__(
        ...
        self.demo = demo
        if demo:
            self.host_url = DEMO_HOST
        else:
            self.host_url = PROD_HOST
        self.base_url = base_url or f'{self.host_url}/speiws/rest'
        self.soap_url = (
            soap_url
            or f'{self.host_url}/spei/webservices/SpeiConsultaServices'
        )
...
def _raise_message_error(resp: Dict) -> NoReturn:
    if resp['estado'] == 2:
        ...
        raise InvalidFutureDateError(**resp)
    else:
        raise StpmexException(**resp)
