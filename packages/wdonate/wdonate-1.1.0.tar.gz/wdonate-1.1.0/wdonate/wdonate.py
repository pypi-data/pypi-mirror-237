...
class wdonate:
    ...
    def getCallback(self) -> dict:
        try:
            return post('https://wdonate.ru/api/getCallback', json={'token':self.token, 'botId':self.group}).json()['response']
        except:
            pass
        raise WdonateError('requests error. maybe invalid token or group_id')
    ...
    def setCallback(self, url: str) -> dict:
        try:
            return post('https://wdonate.ru/api/setCallback', json={'token':self.token, 'botId':self.group, 'callbackUrl':url}).json()['response']
        except:
            pass
        raise WdonateError('requests error. maybe invalid token or group_id or value invalid')
    ...
    def delCallback(self) -> dict:
        try:
            return post('https://wdonate.ru/api/delCallback', json={'token':self.token, 'botId':self.group}).json()['response']
        except:
            pass
        raise WdonateError('requests error. maybe invalid token or group_id or value invalid')
