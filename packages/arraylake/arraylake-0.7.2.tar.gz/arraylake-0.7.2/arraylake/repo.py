...
def _write_op(func):
    ...
    def wrapper(self, *args, **kwargs):
        if not self._writable:
            raise OSError("Repo is not writable. Try checking out the repository first using `repo.checkout()`.")
...
class AsyncRepo:
    ...
    _session: SessionInfo | None  # set in checkout
    _writable: bool
    ...
    async def checkout(self, ref: str | CommitID = "main") -> CommitID | None:
        ...
        commit, branch = (await self.commit_data(refresh=True)).get_ref(ref)
    ...
    async def commit(self, message: str, auto_ff: bool = True) -> CommitID | None:
    ...
    async def _single_commit(self, message: str, attempt=0) -> CommitID | None:
    ...
    async def _try_fast_forward(self) -> CommitID | None:
        ...
        branch = self.session["branch"]
        branch_latest_commit = (await self.commit_data(refresh=True)).branches.get(branch, None)
    ...
    async def _list(
        self, prefix: str, *, all_subdirs: bool = False, _jmespath_filter_expression: str | None = None
    ...
    async def _tree(self, prefix: str = "", depth: int = 10, _jmespath_filter_expression: str | None = None) -> Tree:
