...
class HttpMetastoreDatabase(ArraylakeHttpClient, MetastoreDatabase):
    ...
    async def _add_docs(
        self, docs: Sequence[BulkCreateDocBody], collection: CollectionName, session_id: SessionID, base_commit: CommitID
    ) -> None:
    ...
    async def add_docs(
        self, items: Mapping[Path, Mapping[str, Any]], *, collection: CollectionName, session_id: SessionID, base_commit: Optional[CommitID]
    ) -> None:
    ...
    async def del_docs(
        self, paths: Sequence[Path], *, collection: CollectionName, session_id: SessionID, base_commit: Optional[CommitID]
    ) -> None:
        params = {"session_id": session_id, "base_commit": base_commit}
    ...
    async def list(
        ...
        *,
        ...
        # This feature has experimental support, denoted by the leading underscore
    ...
    async def del_prefix(self, prefix: str, *, collection: CollectionName, session_id: SessionID, base_commit: Optional[CommitID]) -> None:
        response = await self._request(
            "DELETE",
