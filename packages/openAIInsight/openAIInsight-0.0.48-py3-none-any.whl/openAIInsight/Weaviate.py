    ...
    def traindata(self,weaviate_jsondata):
        try:
            ...
            # Schema Class parameter
                ...
                for s,i in enumerate(weaviate_IndexRespository):
                    schemaClasslist.append(weaviate_IndexRespository[s]['class'])
                    logger.log(f"schemaClasslist::: {schemaClasslist}")
                    for index, schemaObj in enumerate(weaviate_IndexRespository):
                        if self.schema_name == weaviate_IndexRespository[index]["class"]:
                            if not len(schemaObj["properties"])  > 0:
                                logger.log(f"Property empty for Weaviate Index '{self.schema_name}' case")
                ...
                if not self.schema_name == 'Document' :
                    if propertyEmpty_flag == "true" :
                        ...
                        logger.log(f'\n {self.schema_name} has no filter properties. Skipping records deletion.')
