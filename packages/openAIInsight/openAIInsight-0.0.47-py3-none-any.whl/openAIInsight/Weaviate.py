...
class Weaviate:
    ...
    def traindata(self,weaviate_jsondata):
        try:
            ...
            if self.schema_name == 'Document':
                logger.log(f"parsed_json::: {type(parsed_json)}\t{parsed_json}")
            ...
            # Schema Class parameter
                ...
                if not self.schema_name == 'Document' and self.schema_name in schemaClasslist:
                    while True:
                        var = self.delete_record(client,self.schema_name,self.enterpriseName,self.entity_type)
                        if var['results']['successful'] == 0:
                            logger.log(f'\n Record remains against the {str(self.enterpriseName) +"_"+ str(self.entity_type)}')
                            break
            ...
            if flag == "SUCCESSFUL":
                ...
                result = f" {self.schema_name} Index Creation FAILED for Enterprise: '{self.enterpriseName}'. "
    ...
    def getLookupData(self):
        ...
        try:
            ...
            if not self.schema_name == 'Document':
                ...
                id_list = self.documentLookup(client , queryJson)
                # for key in queryJson:
                #     if len(queryJson[key]) > 0 and queryJson[key].strip() != "":
                #         response = (
                #                 client.query
    ...
    def documentTraining(self,client, parsedJson):
        ...
        retriever = WeaviateHybridSearchRetriever(
                                                    client      = client,
                                                    index_name  = self.schema_name +"_" + self.enterpriseName,
                                                    text_key    = "text",
                                                    attributes  = [],
        ...
        OCRText = list(json.loads(parsedJson["description"]).values())
        ...
        logger.log(f"Message:::\t Document training {message}")
        return message
        
    def documentLookup(self, client, queryJson):
        id_list = set()
    ...
    def delete_record(self,client,schema_name,enterpriseName,entity_type):
        var = client.batch.delete_objects(class_name=schema_name,
                                                    where={'path': ["organization"],
                                                        'operator': 'Equal',
                                                        'valueText': str(enterpriseName) +"_"+ str(entity_type)
                                            },
                                        )
        return var