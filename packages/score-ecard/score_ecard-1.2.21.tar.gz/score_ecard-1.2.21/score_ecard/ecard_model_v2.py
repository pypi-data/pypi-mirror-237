...
class ECardModel(object):
    def __init__(self, kwargs_lr=None, kwargs_xgb=None, **kwargs):
        '''

        :param kwargs_lr:
        :param kwargs_xgb:
        '''
        self.kwargs_lr = kwargs_lr
        self.kwargs_xgb = kwargs_xgb
        ...
        if kwargs_lr:
            self.params_lr.update(**self.kwargs_lr)
        if kwargs_xgb:
            self.params_xgb.update(**self.kwargs_xgb)
    ...
    def fit(self, df_X, df_Y, sample_weight=None, xgb_model=None):
        ...
        df_cards_list = self.calc_score_card(df_X,df_y, df_y_binary, sample_weight,xgb_model=xgb_model)
    ...
    def check_label(self, df_label):
        df_data=df_label.copy()
        if df_data.shape[-1] == 1:
            df_data.columns = ['label']
        if 'label' not in df_data.columns:
            df_data['label'] = df_data.iloc[:,0]
        df_y = df_data.label.astype(int)
        ...
        if 'label_binary' in df_Y.columns:
            df_y_binary = df_data["label_binary"]
        ## ---end
        del df_data
        return df_y, df_y_binary

    def calc_score_card(self, df_X, df_y, df_y_binary, sample_weight, batch_size=100,xgb_model=None):
        '''
        ...
        self.xgb_woe_model.fit(df_X, df_y, xgb_model, call_back=calc_tree_card)
        self.xgb_ = self.xgb_woe_model.xgb_
    ...
    def get_params(self,deep=False):
        out = dict()
        out['kwargs_lr']=self.kwargs_lr
        out['kwargs_xgb'] = self.kwargs_xgb
        return out
...
if __name__ == '__main__':
    ...
    feature_columns = df_train_data.columns[35:100].tolist()
    df_X = df_train_data[feature_columns]
    df_Y = df_train_data[['car_got', 'fee_got', 'report_num', 'report_fee']]
    df_Y['label_binary'] = df_Y['report_fee'].fillna(0).astype(float).apply(lambda x: 1 if x > 0 else 0)
    df_Y["label"] = pd.cut(df_Y["report_fee"].fillna(0).astype(float), bins=[-np.inf, 0, 5000, 50000, np.inf],
                              labels=[0, 1, 2, 3])
    ecard = ECardModel(
    ...
    ecard.fit(df_X, df_Y,xgb_model=clf_tmp.xgb_)
