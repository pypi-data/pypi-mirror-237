...
def cal_score_by_groupby(
    ...
    if unit == "day":
        dfc.index = pd.to_datetime(dfc.index.dt.strftime("%Y-%m-%d"))
    elif unit == "month":
        dfc.index = pd.to_datetime(dfc.index.dt.strftime("%Y-%m"))
    elif unit == "year":
        dfc.index = pd.to_datetime(dfc.index.dt.strftime("%Y"))
    else:
        raise ValueError("unit值错误%s" % unit)
    for i in dfc.groupby(dfc.index):
        if cap:
            try:
