...
from typing import Any, Iterable, Iterator, Literal, Never, Optional, overload
...
class MetaVertex(Vertex):
    ...
    def __hash__(self) -> int:
        ...
        """
        return hash(hash(frozenset(self.cls)) * hash(self.condition))
...
class MetaEdge(Edge):
    ...
    def __hash__(self) -> int:
        ...
        Implements hash(self).
        """
        return hash(hash(frozenset(self.cls)) * hash(self.condition) * (hash(self.source) + hash(self.destination)))
...
class MetaArrow(Arrow, MetaEdge):
    ...
    def __hash__(self) -> int:
        ...
        Implements hash(self).
        """
        return hash(hash(frozenset(self.cls)) * hash(self.condition) * (hash(self.destination) - hash(self.source)))
...
class MetaGraph(Graph):
    ...
    edges : __IsoSet[MetaEdge | MetaArrow]
    ...
    def __neighborhood_mapper(self, g : Graph) -> __IsoDict[Vertex, __IsoSet[MetaVertex]]:
        """
    ...
    def __clear_mapper_deadends(self, mapping : __IsoDict[Vertex, __IsoSet[MetaVertex]]):
        ...
        while work:
            ...
            to_check_again : MetaGraph.__IsoSet[tuple[Vertex, MetaVertex]] = MetaGraph.__IsoSet()
            ...
            new_work : MetaGraph.__IsoDict[Vertex, MetaGraph.__IsoSet[MetaVertex]] = MetaGraph.__IsoDict()
...
class FrozenMetaGraph(MetaGraph, FrozenGraph):
    ...
    def __init__(self, g: Iterable[Vertex | Edge] | Graph = Graph()) -> None:
        ...
        self.vertices : "FrozenMetaGraph.__FrozenIsoSet[Vertex]" = FrozenMetaGraph.__FrozenIsoSet(self.vertices)
        self.edges : "FrozenMetaGraph.__FrozenIsoSet[Edge]" = FrozenMetaGraph.__FrozenIsoSet(self.edges)
