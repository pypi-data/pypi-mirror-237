...
from ..net_utils import logger, dist, try_remove_file, instantiate_from_config, AverageMeters
...
class BaseTrainer(BaseWorker):
    ...
    def build_network(self):
        self.model_src = instantiate_from_config(self.args.model).cuda()
        self.model_optim = self._make_distributed_model(self.model_src)

        self.optim = instantiate_from_config(self.args.optim, self.model_optim.parameters())
    ...
    def build_dataset(self):
        dls: Sequence[Dataset] = instantiate_from_config(self.args.dataset)
        if len(dls) == 3:
