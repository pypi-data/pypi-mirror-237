...
class Meter(object):
    ...
    def exec(self, context: Context):
        self._exec_body(context)
        self._exec_result_fail(context)
        self.sleep(context)

    def _exec_body(self, context: Context, retry_times: int = 3):
        self.encode(context)
        ...
        try:
            ...
            if str(pe).find('FrameError') < 0:
                ...
                if retry_times <= 0:
                    ...
                    base.sleep(1)
                    self._exec_body(context, retry_times=retry_times)

        self._result = self.acv(context)
        base.send(context, todo={'app:show': {'msg': self._result}})

    def _exec_result_fail(self, context: Context):
...
class Bench(object):
    ...
    def exec(self, context: Context):
        if context.meter.index == 0 or self._is_exec:
            if self._times(context):
                self._exec_body(context)
                self._exec_result_fail(context)
                self.sleep(context)

    def _exec_body(self, context: Context):
        self.encode(context)
        try:
