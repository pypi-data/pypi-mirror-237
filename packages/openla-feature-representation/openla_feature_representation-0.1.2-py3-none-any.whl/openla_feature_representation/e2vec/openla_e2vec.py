...
class E2Vec(object):
    ...
    def _get_oneuser_sentences(self, user_eventstream_df):
        # sentences : output
        ...
        for index, data in user_eventstream_df.iterrows():
            if index == 0:
                ...
                cc = data["contentsid"]
                if pc != cc:
                    # contentsidが変化した場合，sentenceを分割
                    sentences += word + "\n"
                    word = data["operationname"]
                    current = dt.datetime.strptime(
                        data["eventtime"], "%Y-%m-%d %H:%M:%S"
                    )
                    start = current
                    ...
                    # contentsidが同じ場合
                    # 前後のイベントの時間間隔を計算し，該当する文字をwordに追加(_interval_checkを実行)
                    previous = current
                    current = dt.datetime.strptime(
                        data["eventtime"], "%Y-%m-%d %H:%M:%S"
                    )
            ...
            # wordの先頭から一定時間以内のログ
                ...
                word += data["operationname"]
            # wordの先頭から一定時間以上のログ(新しい単語の先頭になるログ)
            else:
                sentences += word + " "
                word = data["operationname"]
                start = current
