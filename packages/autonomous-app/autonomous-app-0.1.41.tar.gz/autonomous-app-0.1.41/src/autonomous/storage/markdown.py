...
class Page:
    ...
    @classmethod
    def convert(cls, record, title, wiki_api=None, parser=None):
        """
        Converts the object to markdown
        """
        page = {title: record}
        parser = parser(page) if parser else cls.parser(page)
        return parser.parse()
    ...
    def push(cls, record, title, id=None, **kwargs):
        ...
        content = cls.convert(
            record, title, wiki_api=kwargs.get("wiki_api"), parser=kwargs.get("parser")
        )
