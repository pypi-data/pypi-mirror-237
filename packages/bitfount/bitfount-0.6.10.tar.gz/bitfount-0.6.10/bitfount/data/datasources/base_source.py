...
class FileSystemIterableSource(IterableSource, ABC):
    ...
    def _scan_files_for_extension(
        ...
        for file in files:
            ...
            if file_type == ".None":
                logger.warning(
                    f"Could not detemine file type of '{file.resolve()}'. Ignoring..."
                )
    ...
    def file_names(self) -> List[str]:
        ...
        # We aren't assuming that all the files in `self.skipped_files` are still
        # present in the directory, so we have to calculate the number of skipped files
        # based on the difference between the original files and the files that have
        # been skipped.
        ...
        if self.strict or not self.file_extension:
            if self.file_extension:
                ...
                logger.info(
                    f"Found {len(files)} files with the explicit extension "
        ...
        return self._scan_files_for_extension(files, self.file_extension)
