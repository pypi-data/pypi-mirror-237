...
class Pod:
    ...
    def __init__(
        ...
        base_datasources_, views_datasources_ = self._process_datasource_args(
        ...
        for hook in get_hooks(HookType.POD):
            hook.on_pod_init_progress(
                ...
                base_datasource_names=list(ds.name for ds in base_datasources_),
                views_datasource_names=list(ds.name for ds in views_datasources_),
                pod_db_enabled=pod_db,
            )
        ...
        if self.pod_db:
            for ds in self.base_datasources.values():
                for hook in get_hooks(HookType.POD):
                    hook.on_pod_init_progress(self, "Pre-loading data", ds.name)
            ...
            try:
                ...
                for hook in get_hooks(HookType.POD):
                    hook.on_task_error(
                        self,
                        e,
                        id=worker_mailbox.task_id or project_id,
                    )
    ...
    async def start_async(self) -> None:
        ...
        try:
            ...
            if pod_heartbeat:
                pod_heartbeat.stop()
                logger.debug(
                    "Waiting up to 15 seconds for pod heartbeat thread to stop"
                )
