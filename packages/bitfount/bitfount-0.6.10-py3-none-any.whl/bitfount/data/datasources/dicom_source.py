...
class DICOMSource(FileSystemIterableSource):
    ...
    def _get_data(
        ...
        Returns:
            The DICOM dataframe.
        """
        dicom_list: List[Dict[str, Any]] = []
        self._datetime_columns: Set[str] = set()
        if file_names:
            full_data = False
        else:
            file_names = self.file_names
            # Check if data is stale
            full_data = False if self.stale else True
        ...
        for i, filename in enumerate(file_names):
            ...
            # File has not been skipped and is not empty so we can add some metadata
            # columns and append it to the list of files.
            data = self._add_metadata_to_data(data, filename)
            dicom_list.append(data)

        if len(dicom_list) == 0:
            logger.warning("No DICOM files could be loaded.")
        else:
            logger.info(f"{len(dicom_list)} DICOM files loaded successfully.")
    ...
    def _save_image_to_path(
        ...
        if "WindowWidth" in dataset:
            # if Window is one will have to divide by 0, which
    ...
    def _add_image_to_data(
        ...
        if not save_path.exists():
            ...
            cls._save_image_to_path(ds, arr, save_path)
        ...
        data[f"{DICOM_IMAGE_ATTRIBUTE} {iter}"] = str(save_path)
        return data
