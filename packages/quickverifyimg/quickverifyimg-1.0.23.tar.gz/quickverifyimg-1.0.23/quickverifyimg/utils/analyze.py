...
queryPath = 'imgs/'  # 图库路径
samplePath = '6.png'  # 样本图片

def getSiftSimilar(queryImage, sampleImage, outputDir=None):
    ...
    kp1, des1 = pysift.computeKeypointsAndDescriptors(sampleImage)  # 提取样本图片的特征
    for parent, dirnames, filenames in os.walk(queryPath):
        for p in filenames:
            p = queryPath + p
            queryImage = cv2.imread(p, 0)
            kp2, des2 = pysift.computeKeypointsAndDescriptors(queryImage)  # 提取比对图片的特征
    ...
    for index, (image, ratio) in enumerate(comparisonImageList):
        plt.imshow(image)
        plt.savefig(f'./{index}_{"Similiarity %.2f%%" % ratio}.jpg')
