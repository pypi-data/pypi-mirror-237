...
def parse_trace_data(trace: dict, get_code: callable) -> dict:
    ...
    # common
    __block = getattr(trace, 'block_number', -1)
    __hash = getattr(trace, 'transaction_hash', hexbytes.HexBytes('0x')).hex()
    __type = getattr(trace, 'type', '')
    __action = getattr(trace, 'action', {})
    __result = getattr(trace, 'result', {})
    __value = getattr(__action, 'value', 0)
    __gas = getattr(__result, 'gas_used', 0)
    # output
    __data = {
        'block': __block,
        'hash': __hash,
    ...
    # call
    if __type == 'call':
        __data['type'] = getattr(__action, 'call_type', 'call') # actually get the exact variant of the call
        __data['from'] = forta_toolkit.parsing.address.format_with_checksum(getattr(__action, 'from_', ''))
    ...
    if __type == 'suicide':
        ...
        __data['to'] = forta_toolkit.parsing.address.format_with_checksum(getattr(__action, 'refund_address', ''))
