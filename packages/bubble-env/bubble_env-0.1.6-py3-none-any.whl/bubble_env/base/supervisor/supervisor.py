...
from bubble_env.utils import join_path
...
class Supervisor(Process):
    ...
    def install(self):
        ...
        is_installed = self.host.ssh("dpkg -s supervisor", warn=False)
        if 'installed' not in str(is_installed):
    ...
    def clean(self):
        ...
        """
        self.stop('all')
        self.host.ssh(f'rm -f {self.process_config_path}/*', sudo=True)
        self.update()
    ...
    def remove(self, name):
        ...
        self.host.ssh(f'rm -f {process_file}', sudo=True)
    ...
    def update(self, name=''):
        """ 更新supervisor的进程列表
        """
        return self.host.ssh(self.update_command.format(name))
    ...
    def stop(self, name):
        """ 停止进程
        """
        return self.host.ssh(self.stop_command.format(name))
