...
from NEMO.views.customization import (
	...
	RemoteWorkCustomization, ToolCustomization,
	UserCustomization,
...
def send_missed_reservation_notification(reservation, request=None):
	...
	if message and user_office_email:
		...
		timed_service_logger.error("Missed reservation email couldn't be send because missed_reservation_email.html or user_office_email are not defined")
...
def send_out_of_time_reservation_notification(reservation: Reservation, request=None):
	...
	if message and user_office_email:
		...
		timed_service_logger.error("Out of time reservation email couldn't be send because out_of_time_reservation_email.html or user_office_email are not defined")
...
def send_email_reservation_ending_reminders(request=None):
	...
	if not reservation_ending_reminder_message:
		timed_service_logger.error("Reservation ending reminder email couldn't be send because either reservation_ending_reminder_email.html is not defined")
...
def send_email_reservation_reminders(request=None):
	...
	if not reservation_reminder_message or not reservation_warning_message:
		timed_service_logger.error("Reservation reminder email couldn't be send because either reservation_reminder_email.html or reservation_warning_email.html is not defined")
