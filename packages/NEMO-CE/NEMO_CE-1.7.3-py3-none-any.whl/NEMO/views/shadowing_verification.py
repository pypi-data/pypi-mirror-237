...
def handle_decision(request, user, shadowing_verification_request):
	...
	if decision:
		if decision == "approve_request":
			...
			qualify(
				user, shadowing_verification_request.tool, shadowing_verification_request.creator,
...
def delete_shadowing_verification_request(request, request_id):
	...
	delete_notification(Notification.Types.SHADOWING_VERIFICATION_REQUEST, shadowing_verification_request.id, )
	return redirect("shadowing_verifications")
		...
		table_result.add_row(
			{
				...
				"creator": req.creator,
				"tool": req.tool.name,
				"qualification_level": req.qualification_level.name
				if req.qualification_level
				else "",
