...
class User(BaseModel, PermissionsMixin):
	...
	def get_emails(self, email_notification=EmailNotificationType.MAIN_EMAIL) -> List[str]:
		...
		if self.get_preferences().email_alternate and email_notification in [EmailNotificationType.BOTH_EMAILS, EmailNotificationType.ALTERNATE_EMAIL]:
...
class Tool(SerializationByNameModel):
	...
	_reservation_required = models.BooleanField(db_column="reservation_required", default=False, help_text='Require that users have a current (within 15 minutes) reservation in order to use the tool')
...
class TrainingEvent(BaseModel):
	...
	invitation_only = models.BooleanField(default=False, help_text="If checked, users can only register if they have been invited")
	users = models.ManyToManyField(User, blank=True)
	capacity = models.PositiveIntegerField()
	recorded = models.BooleanField(default=False, help_text="Indicates this training event has completed and training session was recorded")
	cancelled = models.BooleanField(default=False)
	cancellation_time = models.DateTimeField(null=True, blank=True)
