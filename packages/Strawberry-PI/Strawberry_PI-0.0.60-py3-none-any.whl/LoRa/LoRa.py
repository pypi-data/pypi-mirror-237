...
from .board_conf import Board
from .boards.board import Board as IBoard
...
class LoRa(object):
    ...
    def Setup(
        ...
        handler: object = None,
        unpack: bool = DEFAULT.UNPACK,
    ):
        # --- Device ID ---- #
        LoRa._device_id = LoRa.encode_devID(device_id)
        ...
        LoRa.set_handler(handler, unpack)
        LoRa.set_mode(
            MODE.STANDBY
    ...
    @staticmethod
    def on_receive_unpacked(
        h_to: bytes, h_from: bytes, payload: bytes, snr: float, rssi: float
    ) -> None:
        raise NotImplementedError(
            "Coglione ti sei dimenticato di implementare la funzione!"
        )
    ...
    # --- Module Function --- #
    @staticmethod
    def set_handler(callback: object, unpack: bool = DEFAULT.UNPACK):
        if callback is not None:
            # Imposto la callback per i messaggi che ricevo
            LoRa._unpack = unpack

            if unpack:
                LoRa.on_receive_unpacked = callback  # type: ignore
            else:
                LoRa.on_receive = callback  # type: ignore
    ...
    @staticmethod
    def set_modem_config(modem_config: ModemConfig):
    ...
    # --- Writer --- #
    @staticmethod
    def send_packet(to: str, data: str):
        # Encode del DevID del destinatario
        devID = LoRa.encode_devID(to)

        # Creo il Payload e lo Trasmetto
        payload = devID + LoRa._device_id + [p for p in data.encode()]
        LoRa.send(payload)

    @staticmethod
    def send(payload: list[int]):
    ...
    def _on_rx_done():
        ...
        # Unpack ?¿?
        if LoRa._unpack:
            h_to = bytes(packet[0:7])  # Destinatario
            h_from = bytes(packet[8:15])  # Mittente
            payload = bytes(packet[15:])  # Payload

            LoRa.on_receive_unpacked(h_to, h_from, payload, snr, rssi)
        else:
            payload = bytes(packet)  # Messaggio

            LoRa.on_receive(payload, snr, rssi)
    ...
    @staticmethod
    def encode_devID(devID: str) -> list[int]:
        # Controllo la lunghezza
        if len(devID) > 8:
            raise ValueError(
                "Il Device ID deve avere una lunghezza compresa tra 0 e 8 caratteri."
            )

        try:
            # Converto il Device ID in bytes con padding a 8
            l = [x for x in devID.encode("ascii")]
            return l + ([0x00] * (8 - len(devID)))
        except UnicodeEncodeError:
            raise UnicodeError("Il Device ID può contenere solo caratteri ASCII.")
