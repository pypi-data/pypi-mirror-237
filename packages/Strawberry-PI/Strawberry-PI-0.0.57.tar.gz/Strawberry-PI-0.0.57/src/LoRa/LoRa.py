...
class LoRa(object):
    ...
    # --- Events --- #
    @staticmethod
    def on_receive(payload: list[int], snr: float, rssi: float) -> None:
    ...
    def set_tx_power(tx_power: int):
        ...
        Strawberry.Write(REG.PA_CONFIG, PA_DAC.SELECT | (tx_power - 5))
    ...
    def send_packet(to: str, data: str):
        ...
        payload = (
            [p for p in devID]
            + [p for p in LoRa._device_id]
            + [p for p in data.encode()]
        )
        LoRa.send(payload)
    ...
    def _on_rx_done():
        ...
        if LoRa._unpack:
            ...
            payload = bytes(packet[15:])  # Messaggio

            LoRa.on_receive_unpacked(h_to, h_from, payload, snr, rssi)
        else:
            LoRa.on_receive(packet, snr, rssi)
    ...
    @staticmethod
    def encode_devID(devID: str) -> bytes:
        ...
        try:
            ...
            return devID.encode("ascii") + (bytes([0x00]) * (8 - len(devID)))
