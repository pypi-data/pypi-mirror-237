...
class LoRa(object):
    ...
    @staticmethod
    def on_receive_unpacked(h_to: bytes, h_from: bytes, payload: bytes, snr: float, rssi: float) -> None:
        raise NotImplementedError(
            "Coglione ti sei dimenticato di implementare la funzione!"
        )
    ...
    # --- Module Function --- #
    @staticmethod
    def set_handler(callback: object, unpack: bool=False):
        if callback is None:
    ...
    def send_packet(to: str, data: str):
        ...
        payload = [p for p in devID] + [p for p in LoRa._device_id] + [p for p in data.encode()]
        LoRa.send(payload)
    ...
    def encode_devID(devID: str) -> bytes:
        ...
        try:
            ...
            return devID.encode("ascii") + (
                bytes([0x00]) * (8 - len(devID))
            )
