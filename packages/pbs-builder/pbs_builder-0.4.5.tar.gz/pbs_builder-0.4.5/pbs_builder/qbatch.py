    ...
    def run(self, is_dry, is_verbose):
        ...
        for job_name, job_dict in self['job'].items():
            ...
            if len(samples) > 0 and len(groups) > 0:
                group_iterator = product(samples, groups) if is_verbose else tqdm(product(samples, groups))
                n_submit, n_skip = 0, 0
            ...
            # Iter through samples only
                ...
                n_submit, n_skip = 0, 0
                job_iterator = samples if is_verbose else tqdm(samples)
...
class PBSJob(object):
    ...
    def write_torque(self):
        with open(self.pbs_file, 'w') as out:
            ...
            out.write(f"cd {self.work_dir}\n")
            out.write('echo "[$(date)] ${PBS_JOBID}@$(hostname) ${PBS_JOBNAME} started"\n')
            out.write('start=$(date +%s)\n')
            out.write(self.shell)
            out.write('echo "[$(date)] ${PBS_JOBID}@$(hostname) ${PBS_JOBNAME} finished"\n')
            out.write('end=$(date +%s)\n')
            out.write('secs=$(($end-$start))\n')
            out.write("printf 'Elapsed Time: %02dh:%02dm:%02ds\n' $((secs/3600)) $((secs%3600/60)) $((secs%60))\n")
