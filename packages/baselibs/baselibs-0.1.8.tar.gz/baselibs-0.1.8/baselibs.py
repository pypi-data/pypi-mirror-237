...
def savetxt(txt, filename, encoding='utf-8', method='w'):
    savetofile(txt, filename, encoding, method)
...
class TimeCount():
    ...
    def show(self, pre_text='用时', unit='ms'):
        ''' 计时并显示时间
        '''

        et = self.pause()
        outtxt = '%s:%f (毫秒)'% (pre_text, et)
        if unit=='ms':
            outtxt = '%s:%f (毫秒)'% (pre_text, et)
        if unit=='s':
            outtxt = '%s:%.3f (秒)'% (pre_text, et/1000)
...
def splitset(datfile, outpath, lstScale=[7,2,1]):
    ...
    for i in range (len(lstFile)):
        ...
        if lstDat:
            fName = os.path.join(outpath, '%s.tsv' % lstFile[i])
