    ...
    def split_lines(self):
        for line in self.text.split('\n'):
            ...
            for i, word in enumerate(words):
                ...
                surf, rect = self.render_line(sentence)

                if rect.width > self.width and is_last_word:
                    if last_sentence == '':
                        self.lines.append((surf, rect))
                    else:
                        self.lines.append((last_surf, last_rect))
                        self.lines.append(self.render_line(word))
                elif rect.width > self.width and not(is_last_word):
                    self.lines.append((last_surf, last_rect))
                    last_sentence = f'{word} '
                    last_surf, last_rect = self.render_line(word)
