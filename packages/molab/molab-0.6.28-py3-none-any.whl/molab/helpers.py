...
def _add_catalog_logo(url,headers,catalog_id, image):
    ...
    files = {'catalogItemType.logo': ("MorpheusImage.png",open(path, "rb"), "image/png")}
    # new_headers = {'Content-Type': 'multipart/form-data', 'Accept': 'application/json', 'Authorization': headers['Authorization']}
    try:
        resp = requests.put(f'{url}{endpoint}/{catalog_id}/update-logo',headers=headers, files=files, verify=False)
    except Exception as e:
        logger.error(e)
        return
    if "200" in str(resp):
