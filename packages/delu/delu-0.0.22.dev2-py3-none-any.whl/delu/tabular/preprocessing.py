...
class NoisyQuantileTransformer(sklearn.preprocessing.QuantileTransformer):
    ...
    **Usage**
    ...
    def fit(self, X, y=None):
        ...
        # dtype = std.dtype
        # adjusted_std = np.where(std > 0.0, std, np.array(1.0, dtype=dtype))
        # noise_std = self.noise_hint / np.maximum(self.noise_hint, adjusted_std)
        # noise_std = np.minimum(self.noise_hint, self.noise_hint * adjusted_std)
        # noise_std = np.minimum(
        #     self.noise_hint / adjusted_std, self.noise_hint * adjusted_std
        # )
        noise = (
            check_random_state(self.random_state)
            .normal(0.0, self.noise_hint, X.shape)
            .astype(dtype)
        )
