...
__all__ = ['NoisyQuantileTransformer']
...
class NoisyQuantileTransformer(sklearn.preprocessing.QuantileTransformer):
    ...
    .. note::
        ...
        A similar "noisy quantile transformation" was used in several papers on
        tabular deep learning listed in the
        `RTDL project <https://github.com/yandex-research/rtdl#papers>`_.

    `NoisyQuantileTransformer` aims at producing better outputs than
    `~sklearn.preprocessing.QuantileTransformer` in certain cases
    (see the examples below), which, on some problems,
    can have a noticeable effect on the performance of deep learning models.

    .. note::

        The technical differences with `~sklearn.preprocessing.QuantileTransformer`
        are as follows:

        - `NoisyQuantileTransformer` applies noise (typically, of a low magnitude) to
          the training data during the ``.fit(...)`` step to deduplicate equal values.
    ...
    **Usage**
    ...
    ...
    ...     kwargs = {
    ...         # If unsure, use this heuristic to configure `n_quantiles`.
    ...         'n_quantiles': max(min(train_size // 30, 1000), 10),
    ...         'random_state': 0,
    ...     }
    ...
    ...                 # For NoisyQuantileTransformer, output_distribution
    ...                 # and subsample are configured to these values by default.
    ...                 output_distribution='normal', subsample=10**9, **kwargs
    ...             ),
    ...         ),
    ...         (
    ...
    In terms of the output distribution (by default, standard normal),
    ...
    def __init__(
        ...
        *,
        ...
        noise_scale: Union[float, List[float]] = 1e-5,
        **kwargs,
    ) -> None:
        """
        Args:
            n_quantiles: the argument for `sklearn.preprocessing.QuantileTransformer`.
                If unsure, set to ``max(min(train_size // 30, 1000), 10)``,
                where ``train_size`` is the expected dataset size on the ``.fit(...)``
            ...
            noise_scale: the scale of the noise. Can be a list to set a custom noise
                scale for each feature.
            kwargs: other arguments for `sklearn.preprocessing.QuantileTransformer`.
        """
        if isinstance(noise_scale, list):
            if not noise_scale:
                raise ValueError('If noise_scale is a list, it must be non-empty')
        else:
            if noise_scale <= 0.0:
                raise ValueError(
                    f"noise_scale must be positive. The provided value: {noise_scale=}"
                )
    ...
    def fit(self, X, y=None):
        ...
        # NOTE: Scikit-learn aims at preserving the container type, column names and
        ...
        if isinstance(self.noise_scale, list) and X.shape[1] != len(self.noise_scale):
            raise ValueError(
                f'The input shape {X.shape} is incompatible with the size'
                f' {len(self.noise_scale)} of the noise_scale list'
                ' passed to the constructor'
            )

        noise = (
            check_random_state(self.random_state)
            .normal(0.0, self.noise_scale, X.shape)
            .astype(dtype)
        )
