...
class BaseRepository(Generic[T]):
    ...
    def __init__(self):
        ...
        self.sort: Union[None, str, List[Tuple[str, SortDirection]]] = None
    ...
    def order(self, fields: List[Tuple[str, Union[pymongo.ASCENDING, pymongo.DESCENDING]]]):
        if not self.sort:
            self.sort = []

        for field, direction in fields:
            self.sort.append((getattr(self._entity, field), direction))
    ...
    def order_asc(self, fields: Union[str, List[str]]):
        ...
        return self.order(fields, pymongo.ASCENDING)
