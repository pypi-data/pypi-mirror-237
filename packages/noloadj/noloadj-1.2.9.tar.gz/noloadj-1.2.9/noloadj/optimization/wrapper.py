...
import jax,jaxlib,types
from jax.config import config
config.update("jax_enable_x64", True) # flottants sur 64 bits pour améliorer
# la précision
config.update('jax_debug_nans',True) # pour pouvoir debugger les "nan"
...
class Results:
    ...
    def __init__(self, results, shape, spec:Spec, jac):
        ...
        #        results[i]=np.where(np.abs(results[i])<1e-16,-np.abs(results[i]),results[i])
