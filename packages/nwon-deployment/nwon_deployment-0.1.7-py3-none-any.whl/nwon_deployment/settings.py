...
from nwon_deployment.typings import DeploymentSettings, DeploymentSettingsGitlab
...
def get_gitlab_settings() -> DeploymentSettingsGitlab:
    global __DEPLOYMENT_SETTINGS

    if __DEPLOYMENT_SETTINGS is None:
        raise DeploymentSettingsNotSet("Deployment settings not set")

    if __DEPLOYMENT_SETTINGS.gitlab is None:
        raise DeploymentSettingsNotSet("Gitlab deployment settings are not set")
