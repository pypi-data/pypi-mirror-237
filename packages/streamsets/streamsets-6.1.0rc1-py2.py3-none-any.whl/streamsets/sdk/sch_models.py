...
from .constants import (
    SNOWFLAKE_EXECUTOR_TYPE, SNOWFLAKE_STAGE_RENAME_ALIASES, STAGE_CONFIG_OVERRIDES, ServiceConfigurationProperty,
    StageConfigurationProperty,
)
...
from .utils import (
    ...
    Version, build_tag_from_raw_tag, format_sch_log, get_attribute, get_library_directory_name, get_params,
    get_topology_nodes, reversed_dict, set_acl, top_objects_by_metering_metric, update_acl_permissions,
    wait_for_condition,
)
...
class ApiCredentialBuilder:
    ...
    def build(self, name):
        ...
        api_credential._request = {'label': name, 'generateAuthToken': True, 'active': True}
...
class ApiCredential(BaseModel):
    ...
    _ATTRIBUTES_TO_REMAP = {'created_by': 'userId', 'credential_id': 'componentId', 'name': 'label'}
...
class PipelineBuilder(SdcPipelineBuilder):
    ...
    def _update_stages_definition(self):
        ...
        if engine_id != "SYSTEM_SNOWPARK_ENGINE_ID":
...
class Pipeline(BaseModel):
    ...
    def __init__(
        ...
        super().__init__(
            pipeline, attributes_to_ignore=Pipeline._ATTRIBUTES_TO_IGNORE, repr_metadata=Pipeline._REPR_METADATA
        )
    ...
    def remove_stages(self, *stages):
        ...
        instance_names_to_remove = {
            stage.instance_name: stage for stage in stages if stage._pipeline.pipeline_id == self.pipeline_id
        }
...
class SelfManagedDeployment(Deployment):
    ...
    def install_script(self, install_mechanism='DEFAULT'):
        ...
        return self._control_hub.api_client.get_self_managed_deployment_install_command(
            self.deployment_id, install_mechanism
        ).response.text
...
class DeploymentEngineConfiguration(BaseModel):
    ...
    def stage_libs(self):
        ...
        if self._data['engineType'] == 'DC':
            return DeploymentStageLibraries(
                [
                    re.match(r'streamsets-datacollector-(.+)-lib:.+', stage_lib).group(1)
            ...
            return DeploymentStageLibraries(
                [
                    re.match(r'streamsets-(spark|transformer)-(.+)-lib:.+', stage_lib).group(2)
                    for stage_lib in self._data['stageLibs']
                ],
                self,
            )
    ...
    def stage_libs(self, libs):
        ...
        if self._data['engineType'] == 'DC':
            self._data['stageLibs'] = [
                'streamsets-datacollector-{}-lib:{}'.format(lib.split(':')[0], lib.split(':')[1])
                if ':' in lib
                else 'streamsets-datacollector-{}-lib:{}'.format(lib, self.engine_version)
                for lib in libs
            ]
        elif self._data['engineType'] == 'TF':
            new_libs_list = []
            for lib in libs:
                stage_lib_format = (
                    'streamsets-transformer-{}-lib:{}' if 'credentialstore' in lib else 'streamsets-spark-{}-lib:{}'
                )
                if ':' in lib:
...
class ScheduledTasks(CollectionModel):
    ...
    def _get_all_results_from_api(self, **kwargs):
        ...
        api_response = self._control_hub.api_client.get_scheduled_tasks(
            kwargs_unioned['order_by'], kwargs_unioned['offset'], kwargs_unioned['len'], api_version=2
        ).response.json()
...
class ConnectionDefinition(BaseModel):
    ...
    def __init__(self, connection_definition, config_definitions=None):
