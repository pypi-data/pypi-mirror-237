...
from .constants import SDC_EXECUTOR_TYPE, SNOWFLAKE_ENGINE_ID, SNOWFLAKE_EXECUTOR_TYPE, STATUS_ERRORS
from .exceptions import JobInactiveError, ValidationError
from .sch_models import (
    ActionAudits, AdminTool, Alert, ApiCredentialBuilder, ApiCredentials, AWSEnvironment, AzureEnvironment,
    AzureVMDeployment, ClassificationRuleBuilder, Configuration, ConnectionAudits, ConnectionBuilder, Connections,
    ConnectionTags, ConnectionVerificationResult, DataSlaBuilder, DeploymentBuilder, DeploymentEngineConfigurations,
    Deployments, DraftRuns, EC2Deployment, Engine, Engines, EnvironmentBuilder, Environments, GCEDeployment,
    GCPEnvironment, GroupBuilder, Groups, Job, JobBuilder, Jobs, KubernetesDeployment, KubernetesEnvironment,
    LegacyDeploymentBuilder, LegacyDeployments, LoginAudits, MeteringUsage, NotImplementedDeployment,
    NotImplementedEnvironment, OrganizationBuilder, Organizations, Pipeline, PipelineBuilder, PipelineLabels, Pipelines,
    ProtectionMethodBuilder, ProtectionPolicies, ProtectionPolicy, ProtectionPolicyBuilder, ProvisioningAgents,
    ReportDefinitions, SAQLSearch, SAQLSearchBuilder, SAQLSearches, ScheduledTask, ScheduledTaskBuilder, ScheduledTasks,
...
from .utils import (
    SDC_DEFAULT_EXECUTION_MODE, TRANSFORMER_EXECUTION_MODES, SeekableList, reversed_dict, wait_for_condition,
)
...
class ControlHub:
    ...
    def _update_api_credential(self, api_credential):
        ...
        create_api_credential_command = self.api_client.update_api_user_credential(
            org_id=self.organization, credential_id=credential_id, api_user_credential_json=data
        )
        api_credential._data = create_api_credential_command.response.json()
        return create_api_credential_command
    ...
    def _validate_and_execute_scheduled_tasks_action(self, action, *scheduled_tasks):
        ...
        if not isinstance(action, ScheduledTask.ScheduledTaskActions):
            raise ValueError(
                'Action not permitted on scheduled task, should be of type ScheduledTask.ScheduledTaskActions'
            )
    ...
    def pause_scheduled_tasks(self, *scheduled_tasks):
        ...
        return self._validate_and_execute_scheduled_tasks_action(
            ScheduledTask.ScheduledTaskActions.PAUSE, *scheduled_tasks
        )
    ...
    def resume_scheduled_tasks(self, *scheduled_tasks):
        ...
        return self._validate_and_execute_scheduled_tasks_action(
            ScheduledTask.ScheduledTaskActions.RESUME, *scheduled_tasks
        )
    ...
    def kill_scheduled_tasks(self, *scheduled_tasks):
        ...
        return self._validate_and_execute_scheduled_tasks_action(
            ScheduledTask.ScheduledTaskActions.KILL, *scheduled_tasks
        )
    ...
    def delete_scheduled_tasks(self, *scheduled_tasks):
        ...
        return self._validate_and_execute_scheduled_tasks_action(
            ScheduledTask.ScheduledTaskActions.DELETE, *scheduled_tasks
        )
    ...
    def verify_connection(self, connection):
        ...
        parameters = {
            'connection': {
                'configuration': connection.connection_definition.configuration,
    ...
    def add_deployment(self, deployment):
        ...
        if engine_type == 'DC':
            processed_stage_libs = [
                'streamsets-datacollector-{}-lib:{}'.format(lib, engine_version) for lib in stage_libs
            ]
        elif engine_type == 'TF':
    ...
    def get_self_managed_deployment_install_script(self, deployment, install_mechanism='DEFAULT'):
        ...
        command = self.api_client.get_self_managed_deployment_install_command(
            deployment.deployment_id, install_mechanism
        )
        # Returned response header has content-type: text/plain, which returns as bytes, hence decode them to a string.
        return command.response.content.decode('utf-8')
