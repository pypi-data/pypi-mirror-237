...
from .constants import (
    SDC_STAGE_ATTRIBUTE_RENAME_ALIASES, SNOWFLAKE_STAGE_ATTRIBUTE_RENAME_ALIASES, STAGE_CONFIG_OVERRIDES,
...
class Stage:
    ...
    def add_output(self, *other_stages, event_lane=False, allow_empty_stages=False, output_lane_index=None):
        ...
        if non_stages:
            raise TypeError(
                '{} cannot be passed to \'>>\'. Only Stage(s) type can be passed to \'>>\'.'.format(
    ...
    def use_connection(self, connection):
        ...
        if connection.connection_type not in self.supported_connection_types:
            raise ValueError(
                'Stage {} does not support connections of type {}.'
                ' Supported connection types are {}'.format(
                    self.stage_name, connection.connection_type, self.supported_connection_types
                )
            )
    ...
    def __setattr__(self, name, value):
        ...
        if name in self._attributes:
            ...
            if not found_config:
                ...
                raise ValueError(
                    'Could not find configuration properties ' 'referenced by attribute "{}".'.format(name)
                )
...
class PipelineBuilder:
        ...
        for stage_definition in definitions['stages']:
            ...
            for service in stage_definition.get('services'):
                ...
                service_definition = next(
                    (
                        service_def
                        for service_def in definitions.get('services')
                ...
                if service_definition is None:
                    raise ServiceDefinitionNotFound(f'Could not find definition of service {service_name}')
    ...
    def build(self, title='Pipeline', **kwargs):
        ...
        """
        [execution_mode] = [
            el['value'] for el in self._pipeline[self._config_key]['configuration'] if el['name'] == 'executionMode'
        ]
        if not self._fragment and execution_mode != 'SNOWPARK':
            if (
                'errorStage' not in self._pipeline[self._config_key]
                or not self._pipeline[self._config_key]['errorStage']
            ) and (not kwargs.get('existing_pipeline', False)):
        ...
        if self._fragment:
            ...
            pipeline._data[self._config_key]['uiInfo']['fragmentStageConfiguration'] = self._get_stage_data(
                label=stage_label
            )[0][1]
            pipeline._data[self._config_key]['uiInfo']['fragmentStageConfiguration']['outputLanes'] = list(
                open_output_lanes
            )
        else:
            ...
            for i, stage in enumerate(pipeline._data['pipelineConfig']['stages']):
                if 'Fragment' in stage['instanceName']:
                    ...
                    if 'FragmentDestination' in stage['instanceName'] or 'FragmentProcessor' in stage['instanceName']:
                        ...
                        # stage should do.
                        pipeline._data['pipelineConfig']['fragments'][frag_count - 1]['stages'][0][
                            'inputLanes'
                        ] = stage['inputLanes']
    ...
    def add_fragment(self, fragment, parameter_name_prefix=None):
        ...
        for i in range(len(fragment._pipeline_definition['stages'])):
            _instance_name = '{}_{}'.format(
                fragment_instance_id, fragment._pipeline_definition['stages'][i]['instanceName']
            )
            _input_lanes = [
                '{}_{}'.format(fragment_instance_id, input_lane)
                for input_lane in fragment._pipeline_definition['stages'][i]['inputLanes']
            ]
            _output_lanes = [
                '{}_{}'.format(fragment_instance_id, output_lane)
            ...
            fragment._pipeline_definition['stages'][i].update(
                {'instanceName': _instance_name, 'inputLanes': _input_lanes, 'outputLanes': _output_lanes}
            )
            fragment._pipeline_definition['stages'][i]['uiInfo'].update(
                {'fragmentId': _fragment_id, 'fragmentInstanceId': _fragment_instance_id}
            )
        ...
        stage_instance.update(
            {
                'instanceName': '{}_{}'.format(fragment_instance_id, stage_instance['instanceName']),
                'configuration': [
                    {'name': 'conf.fragmentId', 'value': _fragment_id},
                    {'name': 'conf.fragmentInstanceId', 'value': _fragment_instance_id},
                ],
                'outputLanes': ['{}_{}'.format(fragment_instance_id, output_lane) for output_lane in _output_lanes],
            }
        )

        # Propagate fragment parameters to the pipeline
        # default fragment prefix as in https://git.io/JvXWl
        default_fragment_prefix = '{}_{}_'.format(re.sub(r'\W', '', fragment.name)[:5], fragment_instance_count_str)
        ...
        for v in next(item for item in fragment._pipeline_definition['configuration'] if item['name'] == 'constants')[
            'value'
        ]:
        ...
        for old_param in fragment.parameters:
            if old_param is not None:
                ...
                fragment_library_definition = json.loads(
                    self._control_hub.api_client.get_pipeline_commit(commit_id=fragment.commit_id).response.json()[
                        'libraryDefinitions'
                    ]
                )
                stage_definitions_map = fragment_library_definition['stages']
                for stage in fragment.stages:
                    self._update_parameter_name_in_stage_instance(
                        stage, stage_definitions_map, fragment_library_definition['services'], old_param, new_param
                    )

        # Set new parameters
        self._set_pipeline_configuration('constants', constants)

        stage_instance['uiInfo'].update(
            {
                'fragmentId': fragment._pipeline_definition['fragmentId'],
    ...
    def _get_stage_instance_name(self, stage):
        ...
        if stage['errorStage']:
            ...
            similar_instances = set(
                stage_instance['instanceName'] for stage_instance in self._pipeline[self._config_key]['stages']
            )
    ...
    def _update_parameter_name_in_stage_instance(
        self, stage, stage_definitions_map, service_definitions, old_param, new_param
    ):
        ...
        if stage_definition is not None and hasattr(stage.configuration, '_data'):
            self._update_parameter_name_in_config(
                stage.configuration._data, stage_definition_configs, old_param, new_param
            )
        if stage.services is not None:
            for service_dependency in stage_definition['services']:
                ...
                service_config = next(
                    stage.services[conf]._data for conf in stage.services if conf == service_dependency['service']
                )
                ...
                if config['value'] is not None:
                    if conf_def['type'] == 'LIST':
                        if isinstance(config['value'], list):
                            config['value'] = [
                                self._replace_config_value(item, old_param, new_param) for item in config['value']
                            ]
                    ...
                    # [{'key': 'replaced_content', 'value': 'replaced_content'}, {'key': .....}]
                        ...
                        if config_definition_map[config['name']]['model']:
                            ...
                            if model_type in ['FIELD_SELECTOR', 'FIELD_VALUE_CHOOSER', 'VALUE_CHOOSER']:
                                ...
                                if isinstance(config['value'], list):
                                    config['value'] = [
                                        self._replace_config_value(item, old_param, new_param)
                                        for item in config['value']
                                    ]
                                ...
                                if isinstance(config['value'], list):
                                    for item in config['value']:
                                        if item['predicate']:
                                            item['predicate'] = self._replace_config_value(
                                                item['predicate'], old_param, new_param
                                            )
                            elif model_type == 'LIST_BEAN':
                                list_bean_config_def_map = {
                                    item['name']: item
                                    for item in config_definition_map[config['name']]['model']['configDefinitions']
                                }
                                if isinstance(config['value'], list):
                                    config['value'] = [
                                        self._replace_list_bean_config_value(
                                    ...
                                    config['value'] = self._replace_list_bean_config_value(
                                        list_bean_config_def_map, config['value'], old_param, new_param
                                    )
                    else:
                        config['value'] = self._replace_config_value(config['value'], old_param, new_param)
                ...
                if config_value:
                    if list_bean_config_def['type'] == 'LIST':
                        if isinstance(config_value, list):
                            list_bean_config_value[config_name] = [
                                self._replace_config_value(value, old_param, new_param) for value in config_value
                            ]
                    ...
                    # [{'key': 'replaced_content', 'value': 'replaced_content'}, {'key': .....}]
                        ...
                        if list_bean_config_def['model']:
                            ...
                            if list_bean_config_model_type in [
                                'FIELD_SELECTOR',
                                'FIELD_VALUE_CHOOSER',
                                'VALUE_CHOOSER',
                            ]:
                                list_bean_config_value[config_name] = self._replace_config_value(
                                    config_value, old_param, new_param
                                )
                            elif list_bean_config_model_type in ['FIELD_SELECTOR_MULTI_VALUE', 'MULTI_VALUE_CHOOSER']:
                                if isinstance(config_value, list):
                                    list_bean_config_value[config_name] = [
                                        self._replace_config_value(value, old_param, new_param)
                                        for value in config_value
                                    ]
                    else:
                        list_bean_config_value[config_name] = self._replace_config_value(
...
class Pipeline:
    ...
    def id(self):
        ...
        return (
            self._data[self._config_key]['info']['pipelineId']
            if schema_version > 2
    ...
    def start_event_stages(self):
        return SeekableList(
            self._all_stages.get(stage['stageName'], Stage)(stage=stage)
    ...
    def test_origin_stage(self):
        ...
        return (
            self._all_stages.get(test_origin_stage['stageName'], Stage)(stage=test_origin_stage)
            if test_origin_stage
            else None
        )
...
class Snapshot:
    ...
    def __init__(self, api_client, snapshot):
        ...
        self._batches = [
            ...
            for snapshot_batch in self._api_client.get_snapshot_data(
                pipeline_id=self.pipeline_id, snapshot_name=self.id, snapshot_label=self.name
...
class Issues:
    ...
    def __init__(self, issues):
        ...
        self.pipeline_issues = [Issue(pipeline_issue) for pipeline_issue in self._data['pipelineIssues']]
        self.stage_issues = {
            stage_name: [Issue(stage_issue) for stage_issue in stage_issues]
            for stage_name, stage_issues in self._data['stageIssues'].items()
        }
    ...
    def __init__(self, stage_library, repository_manifest):
        ...
        if self.id == 'not-specified':
            self.id, self.version = re.match(
                r'(streamsets-datacollector-.+-lib)-(.+).tgz', self._data['stagelibManifest']
            ).groups()
        else:
