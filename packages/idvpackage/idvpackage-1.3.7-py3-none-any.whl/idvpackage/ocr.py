# The `IdentityVerification` class provides methods for image and video processing, including image
# conversion, image enhancement, image quality checks, face detection, and extraction of a selfie from
# a video.
import base64
import cv2
import io
            ...
            if is_colored2:
                # try:
                    ...
                    text = id_infos[0].description
                     ...
                     # This pattern is present on the front. If we find this we return empty strings with error code
                    pattern1 = r'Resident Identity'
                    pattern2 = r'Identity Card'
                    ...
                    # Define a dictionary that maps keys to their required lengths
                    required_lengths = {'id_number': 15, 'card_number': 9}

                    # Loop through the keys and set value to an empty string if length doesn't match
                    for key, required_length in required_lengths.items():
                        if len(back_data.get(key, '')) != required_length:
                            back_data[key] = ''
                    ...
                    for key in back_data:
                        if not back_data[key] and key not in ['occupation', 'employer', 'first_name', 'last_name', 'issuing_place', 'error']:
                                ...
                                if key=='id_number':
                                    back_data[key] = transform_func_new(original_text,dob)
                                else:
