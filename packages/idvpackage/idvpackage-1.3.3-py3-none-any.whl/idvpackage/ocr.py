...
class IdentityVerification:
    ...
    def check_document_quality(self, data, brightness_threshold=240, blur_threshold=1500, video_duration_threshold=5):
        ...
        if input_type == 'base_64':
            ...
            try:
                ...
                if video_capture.isOpened():
                    ...
                    if np.any(selfie_result.get('selfie')):
                        selfie = selfie_result['selfie']

                        enhanced_selfie = self.enhance_quality(selfie)
                        selfie_blurry_result, selfie_bright_result = self.get_blurred_and_glared_for_doc(enhanced_selfie)
                        if selfie_blurry_result == 'consider' or selfie_bright_result == 'consider':
                            video_quality['error'] = 'face_not_clear_in_video'
                        else:
                            video_quality['selfie'] = selfie

                    elif selfie_result.get('error'):
                        video_quality['error'] = 'no_face_detected_in_video'
    ...
    def extract_selfie_from_video(self, video_bytes):
        ...
        if best_face is not None:
            video_dict['selfie'] = np.array(best_face)
        else:
    ...
    # def check_for_liveness(self, similarity, video_bytes, face_match_threshold, liveness_threshold):
    ...
    def extract_document_info(self, image, document_type, side, country):
        document_data = {}
    ...
    def func_common_dates(self, extract_no_space):
        dob = ''
        expiry_date = ''
        try:
    ...
    def func_dob(self, extract):
        extract_no_space = extract.replace(' ','')
        dob, expiry_date = self.func_common_dates(extract_no_space)
        if dob == '':  
            ...
            for i in match_dob:
                ...
                if new_dob_match:
                    ...
                    new_dob = new_dob[:7]
                    dob = self.convert_dob(new_dob)

        return dob

    def func_expiry_date(self, extract):
        extract_no_space = extract.replace(' ','')
        dob, expiry_date = self.func_common_dates(extract_no_space)
        if expiry_date == '':
    ...
    def func_card_number(self, extract):
        extract_no_space = extract.replace(' ','')
        try:
    ...
    def func_nationality(self, extract):
        extract_no_space = extract.replace(' ','')
        try:
    ...
    def func_id_number(self, extract):
        extract_no_space = extract.replace(' ','')
        try:
            #id_extract = extract.replace(" ", "")
            #year = dob[-4:]
            p = "784" + "\d{12}"
            id_re = re.search(p, extract_no_space)
            ...
            if is_colored2:
                try:
                    ...
                    if identify_back_id(text):
                        ...
                        for key in back_data:
                            if not back_data[key] and key not in ['occupation', 'employer', 'first_name', 'last_name', 'issuing_place', 'error']:
                                transform_func = getattr(self, key)
                                back_data[key] = transform_func(original_text)

                        current_module = sys.modules[__name__]   
        
                        for key in back_data:
                            if not back_data[key] and key not in ['occupation', 'employer', 'first_name', 'last_name', 'issuing_place', 'error']:
                                    transform_func = getattr(current_module,f'func_{key}')
                                    back_data[key] = transform_func(original_text)

                        if len(back_data['card_number']) < 9:
                            back_data['card_number'] = self.card_number(original_text)
    ...
    def extract_ocr_info(self, data, video, country, report_names, face_match_threshold=0.50):
        document_report = {}
        facial_report = {}
