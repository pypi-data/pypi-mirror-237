...
from vpnetbox.types_ import DAny, DStr, LDAny, LValue, LTInt2, LParam, SStr, TStr, LStr
from vpnetbox.types_ import SeStr, UParam, TLists, TValues
...
class Base(ABC):
    ...
    def create(self, data: DAny) -> Response:
        ...
        """
        kwargs = {
            "url": f"{self.url}/api/{self._query}",
    ...
    def delete(self, id: int) -> Response:  # pylint: disable=redefined-builtin
        ...
        """
        kwargs = {
            "url": f"{self.url}/api/{self._query}{id}",
            "headers": self._headers(),
            "verify": self.verify,
            "timeout": self.timeout,
        }
    ...
    def _join_params(self, *params: LDAny) -> LDAny:
    ...
    def _validate_concurrent_params(self, **kwargs: DAny) -> LDAny:
        """Validate the parameters, split concurrent parameters to different items.

        :param kwargs: Received parameters.
        :return: Validated parameters.
        """
        conc_keys: SStr = set(kwargs).intersection(set(self._concurrent))
        conc_keys = {k for k, v in kwargs.items() if k in conc_keys and isinstance(v, TLists)}
        if not conc_keys:
            return [kwargs]
    ...
    @staticmethod
    def _check_keys(items: LDAny, denied: SeStr = None) -> None:
    ...
    @staticmethod
    def _is_vrf_in_params(params: LDAny) -> bool:
        """Check if VRF is requested in finding parameters."""
        for param_d in params:
            if param_d.get("vrf_id"):
                return True
            if param_d.get("present_in_vrf_id"):
                return True
        return False

    @staticmethod
    def _items_wo_vrf(items: LDAny) -> LDAny:
    ...
    def _param_vrf(self, **kwargs) -> DAny:
        ...
        if vrf := kwargs.get("vrf") or "":
            vrfs: LStr = vlist.from_any(vrf)
            nb_vrfs: LDAny = self.query(query="ipam/vrfs/")
            if nb_vrfs_ := [d for d in nb_vrfs if d["name"] in vrfs]:
                del kwargs["vrf"]
                kwargs["vrf_id"] = [d["id"] for d in nb_vrfs_]

        if vrf := kwargs.get("present_in_vrf") or "":
            vrfs: LStr = vlist.from_any(vrf)
            if not nb_vrfs:
                nb_vrfs = self.query(query="ipam/vrfs/")
            if nb_vrfs_ := [d for d in nb_vrfs if d["name"] in vrfs]:
                del kwargs["present_in_vrf"]
                kwargs["present_in_vrf_id"] = [d["id"] for d in nb_vrfs_]
...
def _init_host(**kwargs) -> str:
    """Init Netbox hostname."""
    if host := str(kwargs.get("host") or ""):
        return host
