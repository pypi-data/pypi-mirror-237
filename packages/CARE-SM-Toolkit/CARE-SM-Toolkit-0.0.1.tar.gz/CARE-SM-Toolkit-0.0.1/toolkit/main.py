...
class Toolkit:
    ...
    def add_columns_from_template(self,data):
        ...
        data_with_template = pd.DataFrame()
        new_row_with_template = dict()
        ...
        for row in data.iterrows():
            ...
            new_row_with_template.update(new_row)

            # Include columns related to ontological terms:
            for cde in temp.items():
                if cde[0] == new_row_with_template[milisec_point]["model"]:
                    new_row_with_template[milisec_point].update(cde[1])

            # Include columns from input CSV table:
            for title, val in row[1].items():
                if not val == None:
                    new_row_with_template[milisec_point].update({title:val})

            # Concate rows:
            final_new_row_with_template = pd.DataFrame(new_row_with_template[milisec_point], index=[1])
            data_with_template = pd.concat([data_with_template, final_new_row_with_template])


        ## Reset Index  
        data_with_template = data_with_template.reset_index(drop=True)
        ## Turn any nan to None
        data_with_template = data_with_template.where(pd.notnull(data_with_template), None)

        return data_with_template
