...
class Model:
    ...
    def __init_default_puuid(self,
        ...
        def _walrus_wrapper_default_project_name_53cb9295e88048019b8ce63838e86204(expr):
            """Wrapper function for assignment expression."""
            nonlocal default_project_name
            default_project_name = expr
            return default_project_name

        def _walrus_wrapper_default_puuid_ae9bf16600de431390b6e45ba5ff323c(expr):
            """Wrapper function for assignment expression."""
        ...
        elif (_walrus_wrapper_default_puuid_ae9bf16600de431390b6e45ba5ff323c(config.get_value(config.OPTION_DEFAULT_PUUID))):
            self.default_puuid = default_puuid
        elif (_walrus_wrapper_default_project_name_53cb9295e88048019b8ce63838e86204(config.get_value(config.OPTION_DEFAULT_PROJECT_NAME))):
            self.default_puuid = self.to_puuid(default_project_name)
        else:
    ...
    def __init_default_muuid(self,
        ...
        def _walrus_wrapper_default_model_name_cf3b8d2aedf8406681cc9584e180d725(expr):
            """Wrapper function for assignment expression."""
            nonlocal default_model_name
            default_model_name = expr
            return default_model_name

        def _walrus_wrapper_default_muuid_3c307d3f5cf7489f8be7933833071230(expr):
            """Wrapper function for assignment expression."""
        ...
        elif (_walrus_wrapper_default_muuid_3c307d3f5cf7489f8be7933833071230(config.get_value(config.OPTION_DEFAULT_MUUID))):
            self.default_muuid = default_muuid
        elif (_walrus_wrapper_default_model_name_cf3b8d2aedf8406681cc9584e180d725(config.get_value(config.OPTION_DEFAULT_MODEL_NAME))):
            self.default_muuid = self.to_muuid(default_model_name, puuid=self.default_puuid)
        else:
    ...
    def _update_sys_paths_modules(self, abs_syspath: Path, abs_pkgbase: Path):
        ...
        def _walrus_wrapper_abs_syspath_str_2132bd4816b3416fb2d9321375c4a8a6(expr):
            """Wrapper function for assignment expression."""
            nonlocal abs_syspath_str
            abs_syspath_str = expr
            return abs_syspath_str

        if (_walrus_wrapper_abs_syspath_str_2132bd4816b3416fb2d9321375c4a8a6(str(abs_syspath))) not in sys.path:
    ...
    def _get_enc_model_cleanup_policy(self, manifest: dict) -> ModelCleanupPolicy:
        if False:
            policy = NotImplemented

        def _walrus_wrapper_policy_d5a2f67880074e5da7e4f4cfd305e392(expr):
            """Wrapper function for assignment expression."""
            nonlocal policy
            policy = expr
            return policy

        def _walrus_wrapper_policy_77a309d4eeac48869a477edb162e08f8(expr):
            """Wrapper function for assignment expression."""
            nonlocal policy
            policy = expr
            return policy

        if (_walrus_wrapper_policy_d5a2f67880074e5da7e4f4cfd305e392(manifest.get('enc_model_cleanup'))):
            return ModelCleanupPolicy(policy)
        if (_walrus_wrapper_policy_77a309d4eeac48869a477edb162e08f8(config.get_value(config.OPTION_DEFAULT_ENC_MODEL_CLEANUP))):
            return ModelCleanupPolicy(policy)
        return ModelCleanupPolicy.AFTER_LOAD

    def _cleanup_enc_model_files(self, manifest: dict, extract_dir: Path):
        cleanup_patterns = manifest.get('enc_model_cleanup_files') or ['models']
    ...
    def load_model_version(self,
        ...
        if encrypted and self._get_enc_model_cleanup_policy(manifest) == ModelCleanupPolicy.AFTER_LOAD:
            self._cleanup_enc_model_files(manifest, extract_dir)
