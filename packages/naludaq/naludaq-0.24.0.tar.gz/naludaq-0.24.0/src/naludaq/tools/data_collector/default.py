...
class DataCollector:
    ...
    @channels.setter
    def channels(self, channels: typing.Iterable[int]):
    ...
    def immediate(self) -> bool:
        return self._readout_settings["trig"].lower().startswith("i")
    ...
    @property
    def trigger_fn(self) -> typing.Callable:
    ...
    @trigger_fn.setter
    def trigger_fn(self, fn: typing.Callable):
        if fn is None:
            fn = get_board_controller(self._board).toggle_trigger
    ...
    def _capture_with_attempts(self, attempts: int) -> typing.Iterable[dict]:
        ...
        try:
            while True:
                ...
                if attempt_count >= attempts:
                    raise TimeoutError(
                        f"Failed to capture event after {attempts} attempts"
                    )
        finally:
