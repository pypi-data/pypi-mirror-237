    ...
    def __calculate_et(time_of_unknown_type):
        ...
        if isinstance(time_of_unknown_type, str):
            ...
            if scrubbed_string == '':
                ...
                try:
                    return spice.str2et(scrubbed_string)
...
class EpochRelativeTime(Time):
    ...
    def add_epoch(cls, epoch_name, epoch_time):
        ...
        Overwrites existing value if epoch_name already in class dict
        :type epoch_name: str
    ...
    @classmethod
    def add_all_from_epoch_cvf(cls, filepath_cvf):
        """
        Opens an epoch cvf and adds all <epoch_name,absolute_time> pairs to the epoch dict
        so they can be used to create EpochRelativeTime objects
        :param filepath_cvf: Filepath to the cvf
        :return: A list of all epochs that already existed that were modified by reading in the file
        """
        modified_epochs = []
        epoch_names_and_values = cls._read_epoch_cvf(filepath_cvf)
        for epoch_name, epoch_value in epoch_names_and_values.items():
            if epoch_name in cls.__epochs__:
                modified_epochs.append(epoch_name)
            cls.add_epoch(epoch_name, Time(epoch_value))
        return modified_epochs

    @classmethod
    def _read_epoch_cvf(cls, epoch_file_name):
        """
        Opens an epoch cvf and returns a dict where the keys are the epoch names
        and the values are the corresponding absolute times
        :param epoch_file_name: Filepath to the cvf
        :return: A dict of epoch names to string epoch values - will need to be cast to Time objects by calling function
        """
        epoch_dict = {}

        epoch_name = None

        # throws IOError if unable to read
        with open(epoch_file_name, 'r') as cvf_file_object:
            for line in cvf_file_object:
                modified_line = line.strip()
                if modified_line:
                    if modified_line[0] == '/':
                        # used to specify if a key is specified before the value in cvf
                        reached_key = True
                        epoch_name = modified_line[1:].strip()
                    elif modified_line[:7] == '"const"':
                        if reached_key:
                            epoch_value = modified_line[7:].strip()
                            epoch_dict[epoch_name] = epoch_value
                            reached_key = False
                        else:
                            raise IOError("Input CVF is not formatted correctly.\nEpoch name must be prefaced by / with"
                                          " a following line containing the epoch ISOD value prefaced by \"const\"")

        return epoch_dict

    @classmethod
    def write_epoch_cvf(cls, epoch_file_name, epoch_names_to_write_out=None, header_in=None, sort_by_time=True):
        """
        Writes an epoch CVF containing specified epoch names and values with a customizable header and with output sort options
        :param epoch_file_name: The desired output filename
        :param epoch_names_to_write_out: A list of names that should be written out. If None, all defined epochs are written out
        :param header_in: A string that will appear in the header of the CVF. Should end with a newline character. If None, a default is provided
        :param sort_by_time: If true, output is sorted by increasing Time. If false, output is sorted by alphabetical key name
        :return:
        """
        with open(epoch_file_name, 'w') as cvf_file_object:
            output_string = cls._get_epoch_cvf_string(epoch_names_to_write_out, header_in, sort_by_time)
            cvf_file_object.write(output_string)

    @classmethod
    def _get_epoch_cvf_string(cls, epoch_names_to_write_out=None, header_in=None, sort_by_time=True):
        """
        Same parameters as write_epoch_cvf minus epoch_file_name, separated for ease of unit testing
        """
        # resolve optional arguments
        if header_in is not None:
            header = header_in
        else:
            header = 'DATA_SET_ID = CONTEXT_VARIABLE_FILE;\n'

        resolved_out_epochs = epoch_names_to_write_out if epoch_names_to_write_out is not None else cls.get_epochs().keys()
        epoch_dict_to_write_out = {out_epoch: cls.__epochs__[out_epoch] for out_epoch in resolved_out_epochs}

        if sort_by_time:
            epoch_items = sorted(epoch_dict_to_write_out.items(), key=lambda x: x[1])
        else:
            epoch_items = sorted(epoch_dict_to_write_out.items(), key=lambda x: x[0])

        # actually write to file
        output_string = ''
        output_string = output_string + 'CCSD3ZF0000100000001NJPL3KS0L015$$MARK$$;\n'
        output_string = output_string + header
        output_string = output_string + 'CCSD3RE00000$$MARK$$NJPL3IF0M02300000001;\n'
        output_string = output_string + '$$EOH\n\n'

        for epoch_name, epoch_value in epoch_items:
            output_string = output_string + '/' + epoch_name + '\n'
            output_string = output_string + '\"const\" ' + epoch_value.to_utc(6) + '\n'
            output_string = output_string + '\n'

        output_string = output_string + '$$EOF\n\n'
