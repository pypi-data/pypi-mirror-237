...
class PlotParameters:
    ...
    * For multiplots, call them BEFORE creating fig,axes and individual plots.  
    '''
...
class PlotSaver:
    '''
    Save plot with a supplied name or suitable default name.
    ...
    * The filename is either supplied as argument or
      created acc.to the name of launcher script (considering also Jupyter). 
    '''
    ...
    def save(output_graph='default', dpi=300):
        ...
        '''
        # (1) Get default output filename if it was not specified.
        if output_graph == 'default':
            ...
            if calling_script.endswith('ipykernel_launcher.py'):
                ...
                # whose name is difficult to get => use a standard filename.
                output_graph = 'my_plot.nb.png'
            else:
        ...
        # (2) Save the plot (with DPI given by *dpi* argument).
...
class CorrelationPlot:
    ...
    def correlation(self, P1, P2, category=None, marker='rx', label=None):
        '''
        Add one XY-correlation to CorrelationPlot.
    ...
    def regression(self, P1, P2, category=None, 
        ...
        '''
        Add a regression curve to graph/plot.
    ...
    def _add_coeff_to_plot(self, coeff, name, x_pos, y_pos, decimals):
        ...
        -------
        None
            The coefficient is printed/drawn into *self.ax* object.
        '''
...
class CorrelationMatrixTable:
    ...
    def save(self,
        ...
        self.fig1.savefig(output_table_r, dpi=dpi)
        self.fig2.savefig(output_table_p, dpi=dpi)
...
class BoxPlot:
    ...
    def save(self, output_graph='default', dpi=300):
        ...
        output_graph : str, optional, default is 'default'
            ...
            If 'default' the name will be (input_script_name + .png).
        dpi : int, optional, default is 300
            DPI resolution of the output graph.
            Usually we use lower resolution for visualizations
            (default=100 is set/can be changed during initialization)
            and higher resolution for saving
            (default=300 is set/can be changed here).

        Returns
        -------
        None
            The output is the saved plot (typically as a PNG file).
        '''
        if output_graph == 'default': output_graph = sys.argv[0]+'.png'
        self.fig.savefig(output_graph, dpi=dpi)
