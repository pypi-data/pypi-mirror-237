...
class PyPoxControlFlow(PyPoxBASE):
    ...
    def IF(self,node:If) -> str:
        ...
        test: str = self.compile(node.test)
        body: str = self.compile(node.body)
        orelse: str = self.compile(node.orelse)
    ...
    def TRY(self,node:Try):
        ...
        try_str:str = "try {{{try_block}}}".format(
            try_block=''.join(try_block)
        )
        catch_str = " ".join(handlers)
    ...
    def EXCEPTHANDLER(self,node:ExceptHandler):
        ...
        catch_body: str = self.compile(node.body)
