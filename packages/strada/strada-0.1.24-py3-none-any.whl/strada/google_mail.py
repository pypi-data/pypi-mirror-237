...
class GmailCustomHttpAction:
    ...
    def execute(self, **kwargs):
        ...
        path_params = hydrate_input_fields(self.path, **kwargs)
        headers = hydrate_input_fields(self.headers, **kwargs)
        query_params = hydrate_input_fields(self.query, **kwargs)
        body = hydrate_input_fields(self.body, **kwargs)

        headers["Authorization"] = f"Bearer {self.token}"

        self.url = fill_path_params(self.url, path_params)

        if self.method == "post":
            response = requests.post(
                self.url, headers=headers, params=query_params, json=body
            )
