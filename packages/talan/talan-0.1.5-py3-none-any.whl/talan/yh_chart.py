...
def yh_quoteSummary(ticker,modules='summaryProfile',debugTF=False,**optx):
	...
	if not credentials:
		credentials=getCredentials(crumbUrl=apiBase+'/v1/test/getcrumb')
		if debugTF:
			sys.stderr.write("==GET CREDENTIALS: {}\n".format(credentials))

	crumb,cookies = (credentials['crumb'],credentials['cookies'])
	utmp = 'https://query2.finance.yahoo.com/v10/finance/quoteSummary/{}?modules={}'
	urx = utmp + '&crumb={}'
	url = urx.format(ticker,modules,crumb)
	if debugTF is True:
