...
```{python}
pfs.grid(x=True, y=False)
```

We can concatenate the dictionaries using `|` operator.

```{python}
pfs.grid(x=True, y=False) | pfs.colored()
```
...
with pf.style_context(pfs.ticks(x=False, y=False)):
    p = (
...
Profile is a composition of multiple styles built for a specific purpose. Its purpose can for example be:
...
(
    ...
    .theme(pf.profile.exploration() | pfs.colored())  # here we apply exploration profile and compose it with colored style
    .add(so.Dots())
