...
class BaseInfoAdmin(admin.ModelAdmin):
    # fields = ('id', 'uuid', 'user_no', 'user_name', 'nickname', 'full_name', 'phone', 'email', 'user_type', 'privacies',
    #           'user_info', 'register_ip', 'register_time', 'is_delete')
    fieldsets = [
        ('用户基础信息', {'classes': 'collapse', 'fields': (
            ('id', 'uuid'), 'user_no', ('user_name', 'nickname'), 'full_name', ('phone', 'email'), 'user_type',
            ('privacies', 'user_info'), ('register_ip', 'register_time'), 'is_delete'
        )})
    ]
    list_display = (
...
class DetailInfoAdmin(admin.ModelAdmin):
    ...
    list_display = ('id', 'user', 'real_name', 'sex', 'birth', 'tags', 'avatar', 'cover', 'language', 'region_code')
