...
async def do_checkout(checkout_query: TaskCheckout) -> Optional[TaskExecutionData]:
    async with AsyncRestClient() as client:
        return await client.checkout_task(checkout_query)


async def async_run_task(pool, task_execution_data: TaskExecutionData, logger, base_dir):
        ...
        try:
            ...
            while True:
                ...
                if n_running < n_cores:
                    ...
                    if task_exec_data:
                        logger.info(f"Task available, starting run for task " f"{task_exec_data.task_id}")
                        asyncio_task = asyncio.create_task(async_run_task(pool, task_exec_data, logger, base_dir))
