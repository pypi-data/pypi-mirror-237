...
def walk_and_apply(
    ...
    recurse_into_new_val=False,
):
    """
    Walk a nested data structure *d* (think JSON) and apply conditional transformations to keys and/or values.
    Returns a new version of *d* with applied transformations.
    """
    ...
    def recursive_func(d, d_out):
        ...
        if isinstance(d, dict):
            for k, v in d.items():
                ...
                # go deeper if v has not been transformed or recurse flag is set and v/new_val is a data structure
                if (new_val is v or recurse_into_new_val) and isinstance(new_val, (dict, list)):
                    recursive_func(new_val, d_out[new_key])
            ...
            for i, v in enumerate(d):
                ...
                # go deeper if v has not been transformed or recurse flag is set and v/new_val is a data structure
                if (new_val is v or recurse_into_new_val) and isinstance(new_val, (dict, list)):
                    recursive_func(new_val, d_out[i])
...
def create_rundir(root_dir: Path, friendly_name=None, logger=None):
    """
    Creates a directory for a task run and returns the full path.
    """
    friendly_name_part = friendly_name + "_" if friendly_name else ""
    rundir = "taskrun_" + friendly_name_part + datetime.utcnow().strftime(RUNDIR_TIME_FORMAT)
    p = root_dir / rundir
    p.mkdir()
    if logger:
...
def get_logger(name, stream_level="DEBUG", formatter=None):
    ...
    logging.getLogger("py.warnings").addHandler(sh)
...
def get_basenames(tag_filename_mapping: Dict[str, List[Union[str, Path]]]) -> Dict[str, List[Union[str, Path]]]:
...
def get_or_create_node_id():
    # Alternative is to use something like pytailor.utils.default_worker_name() (see above)
    local_node_id_file_path = Path.home() / ".tailor" / "local_node_id"
    if local_node_id_file_path.exists():
        with open(local_node_id_file_path, "rb") as f:
            return pickle.load(f)
    else:
        local_node_id = str(uuid.uuid4())
        with open(local_node_id_file_path, "wb") as f:
            local_node_id_file_path.parent.mkdir(parents=True, exist_ok=True)
            pickle.dump(local_node_id, f)
        return local_node_id


def to_unix_style_paths(data):
    return walk_and_apply(data, val_cond=lambda v: isinstance(v, str), val_apply=lambda s: re.sub(r"\\+", "/", s))
