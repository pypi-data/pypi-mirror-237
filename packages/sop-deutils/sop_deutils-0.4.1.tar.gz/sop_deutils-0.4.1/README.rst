...
- ``range_to`` (optional): The end of the range of data to update. The default value is ``'Z'``. **(str)**
...
To grant table privileges to users in PostgreSQL, use the ``grant_table`` method. It has the following parameters:

- ``schema`` (required): The schema containing the table to grant. **(str)**

- ``table`` (required): The table name to grant. **(str)**

- ``list_users`` (required): A list of users to grant access. To grant access to all members of the DA team, provide ``['da']``. **(list)**
