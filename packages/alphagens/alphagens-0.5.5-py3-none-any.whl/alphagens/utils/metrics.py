...
class StrategyMetrics:
    ...
    def drawdown(strategy_returns: pd.Series) -> (pd.Series, float, int):
        ...
        """
        net_curve = (1+strategy_returns).cumprod()
        cmax = net_curve.cummax()
        rlt = (cmax - net_curve) / cmax
    ...
    @staticmethod
    def evaluate(strategy_returns, benchmark_returns, rfr=0, annulized_factor=252):
        result = {}
        ...
        result["max_drawdown"], result['longest_drawdown'], _ = StrategyMetrics.drawdown(strategy_returns)
        result["alpha"], result["beta"], _ = StrategyMetrics.reg_stats(strategy_returns, benchmark_returns, rfr, annulized_factor)
        return result
    
    def evaluate_by_year(strategy_returns, benchmark_returns, rfr=0, annulized_factor=252):
        result = strategy_returns.resample('Y').apply(lambda x: pd.Series(StrategyMetrics.evaluate(x, benchmark_returns.loc[x.index])))
