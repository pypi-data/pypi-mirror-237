...
ERROR_HEADERS_GENERIC = ERROR_HEADERS_SUPPORTED.keys()
...
class Model(ABC):
    ...
    def __init__(self, fit_vars: dict[str, float | str],
        ...
        # and upper and lower bounds from fit uncertainty in fitted parameters
    ...
    def fit_to(self, experiment: 'Experiment',
               guess: list[float] | str = 'predefined',
        ...
        # Calculate lntau values
        # <ln(tau)>, sigma_ln(tau), and ln_tau_fit_ul
        self.calc_lntau_expect()
        self.calc_lntau_stdev()
        self.calc_lntau_fit_ul()
    ...
    @staticmethod
    def flat(ac_freq_ang: list[float], imag_sus: list[float],
...
class DebyeModel(Model):
    ...
    def calc_lntau_fit_ul(self) -> list[float]:
        ...
        return sorted([np.log(tau + tau_std), np.log(tau - tau_std)], reverse=True) # noqa
...
class GeneralisedDebyeModel(Model):
    ...
    def calc_lntau_fit_ul(self) -> list[float]:
        ...
        values = sorted(
            [np.log(tau + tau_std), np.log(tau - tau_std)],
            reverse=True
        )
    ...
    def calc_lntau_stdev(self) -> float:
        ...
        return np.sqrt((1. / (1 - alpha)**2 - 1.) * np.pi**2 / 3.)
...
class HavriliakNegamiModel(Model):
    ...
    def calc_lntau_expect(self) -> float:
        ...
        return self._calc_lntau_expect(tau, alpha, gamma)

    @staticmethod
    def _calc_lntau_expect(tau, alpha, gamma):
        warnings.filterwarnings('ignore', 'invalid value encountered in log')
    ...
    def calc_lntau_fit_ul(self) -> list[float]:
        ...
        if 0 <= gamma <= 1:
            ...
            upper = self._calc_lntau_expect(
                tau + tau_std,
                alpha - alpha_std,
                gamma + gamma_std
            )

            lower = self._calc_lntau_expect(
                tau - tau_std,
                alpha + alpha_std,
                gamma - gamma_std
            )
...
class ColeDavidsonModel(Model):
    ...
    def calc_lntau_expect(self) -> float:
        ...
        return self._calc_lntau_expect(tau, gamma)

    @staticmethod
    def _calc_lntau_expect(tau, gamma):
        warnings.filterwarnings('ignore', 'invalid value encountered in log')
...
class DoubleGDebyeModel(Model):
    ...
    def calc_lntau_expect(self) -> list[float]:
        '''
    ...
    def calc_lntau_fit_ul(self) -> list[list[float]]:
        '''
        Calculates upper and lower bounds of ln(tau) from fit uncertainty
        in fitted parameters
        ...
        ul_1 = sorted([np.log(tau1 + tau1_std), np.log(tau1 - tau1_std)], reverse=True) # noqa
        ul_2 = sorted([np.log(tau2 + tau2_std), np.log(tau2 - tau2_std)], reverse=True) # noqa
        warnings.filterwarnings('default', 'invalid value encountered in log')

        return [ul_1, ul_2]

    def calc_lntau_stdev(self) -> list[float]:
        '''
...
class Measurement():
    ...
    def from_file(cls, file: str, mass: float, mw: float,
                  ...
                  check_error: bool = True) -> list['Measurement']:
        '''
        ...
        # Get file headers and names for data columns (not errors)
        if header_indices == 'find':
            ...
            # only get file headers
            _, header_names = ut.parse_headers(
                file, data_index, HEADERS_SUPPORTED
            )
...
class Experiment():
    ...
    def __init__(self, rep_temperature: float,
                 raw_temperatures: npt.ArrayLike, real_sus: npt.ArrayLike,
                 imag_sus: npt.ArrayLike, ac_freqs: npt.ArrayLike,
                 rep_dc_field: float, dc_fields: npt.ArrayLike,
                 ac_fields: npt.ArrayLike):
    ...
    def from_measurements(cls,
        ...
        for sf_measurements in measurements:
            ...
            for measurement, mean_temp in zip(sf_measurements, np.concatenate(mean_temps)): # noqa
            ...
            experiments.append([
                cls(
                    mean_temp[0], raw_temp, real_sus, imag_sus, ac_freq,
                    np.mean(dc_field), dc_field, ac_field
                )
                for (mean_temp, raw_temp, real_sus, imag_sus, ac_freq, dc_field, ac_field) in zip( # noqa
                    mean_temps, raw_temperatures, real_suss,
                    imag_suss, ac_freqs, dc_fields, ac_fields
                )
            ])
...
def save_ac_magnetometer_file(experiments: list[Experiment],
    ...
    experiments: list[Experiment]
        List of AC Experiments to include
    file_name: str, 'ac_data.out'
        Name of output file
    verbose: bool, default True
    ...
    with open(file_name, 'w', encoding='utf-8') as f:
        f.write('This file was created by ccfit2\n\n')
        f.write('{}\n'.format('[Data]'))
        f.write(
            '{},{},{},{},{},{},'.format(
                'Field (Oe)',
        ...
        for experiment in experiments:
            ...
            for _dat in _data:
                f.write(
                    '\n {:4.1f}, {:6.3f}, {: 12.8f}, '.format(*_dat[0:3])
                )
                f.write(
                    '{: 12.8e}, {: 12.8e}, {: 7.4f}'.format(*_dat[3:])
                )
...
def plot_susceptibility(experiments: list['Experiment'], save: bool = True,
                        show: bool = True,
...
def plot_colecole(experiments: list['Experiment'], save: bool = False,
                  show: bool = True, save_name: str = 'cole_cole.png',
...
def plot_fitted_colecole(experiments: list['Experiment'],
                         models: list['Model'], save: bool = False,
                         show: bool = True,
...
def qt_plot_fitted_colecole(app: QtWidgets.QApplication,
                            experiments: list['Experiment'],
                            models: list['Model'], save: bool = True,
                            show: bool = True,
                            save_name: str = 'fitted_cole_cole.png',
                            verbose: bool = True) -> None:
    '''
...
def plot_fitted_susceptibility(experiments: list['Experiment'],
                               models: list['Model'], save: bool = True,
                               show: bool = True,
...
def write_model_params(models: list[Model],
                       ...
                       verbose: bool = True, delimiter: str = ',') -> None:
    '''
