...
def encode(string: str, language: Language, dot: str = '.', dash: str = '-', separator: str = '/',
    ...
    if any(len(symbol) != 1 for symbol in [dot, dash, separator]):
        error_message: str = 'Invalid symbols: Dots, dashes, and separators must be single characters!'
...
def decode(string: str, language: Language, dot: str = '.', dash: str = '-', separator: str = '/',
    ...
    if any(char not in dot + dash + separator + space + new_line for char in string):
        error_message: str = 'Invalid characters in the Morse code string. Use only specified dots, dashes, spaces, and separators!'
