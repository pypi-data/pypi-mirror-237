...
class Headers:
    @classmethod
    def user_agent(cls, mobile: bool) -> str:
        # 生成 User-Agent
        while True:
    ...
    @classmethod
    def header(cls, string: Optional[str] = None, mobile: Optional[bool] = False,
               headers: Optional[dict] = None) -> dict:
        # 生成请求头部
        if headers is None:
            headers = {}
        if string:
            headers.update(gen_headers(string))
            # 如果 string 中不包含换行符，则将其作为 Referer 添加到请求头中
            if "\n" not in string:
                headers['Referer'] = string
        headers['User-Agent'] = cls.user_agent(bool(mobile))
        return headers
