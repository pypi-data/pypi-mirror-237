...
class CombinePds:
	...
	@staticmethod
	def unload1d(x1d, symbols = None):
		""" Split the 1d vector back to dictionary with symbols """
		return {k: CombinePds.symbolFrom1d(x1d, k) for k in noneInit(symbols, x1d.index._n)}

	@staticmethod
	def symbolFrom1d(x1d, name):
		symbol = x1d.xs(name).droplevel('_symbol')
		return pd.Series(symbol.values, index = CombinePds.vIndex(symbol.index, x1d.index._n[name]), name = name) if x1d.index._n[name] else symbol.iloc[0]

	@staticmethod
	def get1d(symbols, **kwargs):
		""" get 1d representation of symbols"""
		return pd.Series(np.hstack([v.values if is_iterable(v) else [v] for v in symbols.values()]), index = CombinePds.get1dIndex(symbols), **kwargs)

	@staticmethod
	def get1dIndex(symbols):
		""" Get 1d index from dictionary of symbols """
		idx = pd.MultiIndex.from_tuples(np.hstack([CombinePds.fIndex(k,getIndex(v)).values for k,v in symbols.items()]), names = _stdIndexNames)
		idx._n = {k: sorted(getIndex(v).names) if hasattr(v,'names') else None for k,v in symbols.items()}
		return idx

	@staticmethod
	def fIndexSeries(variableName, index):
		return pd.MultiIndex.from_product([[variableName], reorderStd(index).values], names= _stdIndexNames)

	@staticmethod
	def fIndex(variableName, index):
		return pd.MultiIndex.from_tuples([(variableName,None)], names = _stdIndexNames) if index is None else CombinePds.fIndexSeries(variableName,index)

	@staticmethod
	def fIndexVariable(variableName, v):
		return v.set_axis(CombinePds.fIndex(variableName, getIndex(v))) if isinstance(v, pd.Series) else pd.Series(v, index = CombinePds.fIndex(variableName, None), dtype= np.float64)

	@staticmethod
	def vIndex(idx, names):
		if isinstance(idx, pd.MultiIndex):
			return pd.MultiIndex(idx.values, names = names)
		elif isinstance(idx, pd.Index):
			return idx.set_names(names)
		else:
			return None
