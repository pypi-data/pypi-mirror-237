...
def fit(model: torch.nn.Module, dataloader: torch.utils.data.DataLoader, optimiser=torch.optim.Adam, loss_type=torch.nn.CrossEntropyLoss, accuracy_func=metrics.categorical_accuracy, epochs=100, lr=0.01, device="cpu"):
    ...
    - accuracy_func (function): The accuracy function to be used for training. Default is categorical_accuracy.
...
def evaluate(model: torch.nn.Module, dataloader: torch.utils.data.DataLoader, loss_type=torch.nn.CrossEntropyLoss, accuracy_func=metrics.categorical_accuracy, device="cpu"):
    ...
    - accuracy_func (function): The accuracy function to be used for evaluation. Default is categorical_accuracy.
    - device (str): The device to be used for evaluation. Default is "cpu".

    Returns:
    - loss (float): The loss for the evaluation dataset.
    - accuracy (float): The accuracy for the evaluation dataset.
    """
    model.to(device)
    model.eval()
