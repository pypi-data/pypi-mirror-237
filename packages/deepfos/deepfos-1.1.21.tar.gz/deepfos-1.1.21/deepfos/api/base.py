            ...
            async def do_request(ins, *args, **kwargs):
                ...
                if is_get_request:
                    ...
                    if ext_header is None:
                        ...
                        # ------------------------------------------------------------
                        # parse content type
                        if (content_type := header.get('content-type')) is None:
                            logger.warning('Missing content-type in request header.')
                        elif matched := RE_CONTENT_TYPE.match(content_type):
                            content_type = matched.group('ctype').lower()
                        elif self._RE_MULTI_TYPE.match(content_type):
                            content_type = 'data'
                        else:
                            logger.warning(f'Unknow content-type: {content_type}')

                        body_key = content_type if content_type in ('json', 'data') else 'body'
