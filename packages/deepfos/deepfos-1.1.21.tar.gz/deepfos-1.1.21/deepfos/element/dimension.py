...
class AsyncDimension(ElementBase[DimensionAPI]):
    ...
    def _get_parent(
        member: DimMember,
        memo: CIMultiDict,
    ) -> Optional[DimensionMemberOperationSw]:
        """获取维度成员的父节点，父节点为root不可使用此方法"""
        parent_name = member.parent_name
        parent_candidate = memo.getall(parent_name, [])

        if not parent_candidate:
            return None

        return parent_candidate[0]
    ...
    def add_member(self, *members: DimMember, silent: bool = False) -> 'AsyncDimension':
        """增加维度成员，
    ...
    def reorder_members(self, memo: CIMultiDict[DimMember], complete: bool) -> CIMultiDict[DimMember]:
        """成员重排序
    ...
    async def query(
        ...
        """
        payload = self._get_query_payload(expression, fields, role, role_name, role_group, multi_entity_config)
        if self.api.version == self.api.default_version:
            api_call = self.async_api.query.select_dimension_member_by_name_function
        elif self.api.version == (1, 2) and as_model and multi_entity_config is not None:
    ...
    def _get_query_payload(
        ...
        if role is not None:
            ...
            for dim in role.meta.dimensions:
                if dim.elementName == self.element_name and dim.folderId == self.element_info.folderId:
                    break
                rs_mapping += 1
            if rs_mapping > len(role.meta.dimensions):
                raise ValueError(f"Current Dimension: {self.element_name} doesn't exist in "
    ...
    def update(self, member_name: str, **attrs: Any):
        ...
        Args:
            member_name: 维度成员名
            **attrs: 成员属性，可接受参数参考 :class:`DimMember`
        ...
        if member_name in self._add_memo:
            ...
            self._add_memo[member_name] = DimMember.construct_from(upd_mbr, **attrs)
    ...
    async def load_dataframe(
        ...
        **langugage_keys: str
        ...
        valid_columns = list(set(DimMember.__fields__).intersection(df.columns))
        ...
        with self._local_memo():
            ...
            for record in add_df.to_dict(orient='records'):
                self._add_memo.add(
                    ...
                    DimMember.construct_from(**record, operation=ADD)
                )

            return await self.save(reorder=reorder)

    async def _save_replace(self, reorder, member_list):
        if reorder:
            mbrs = CIMultiDict()
            for rec in member_list:
                mbrs.add(rec[DFLT_NAME_COLUMN], DimMember.construct_from(**rec))
        ...
        if self.api.version == self.api.default_version:
            ...
            resp = await self.async_api.compat.save(payload)
            if not resp.success:
                raise DimensionSaveError(f"Failed to save dimension. Detail: {resp}")
        return resp
    ...
    async def to_deepmodel_object(
        ...
        parent = data[['name', 'parent_name', 'aggweight']]
        # parent_name为#root，即只作parent的记录
        parent = parent[~(parent['parent_name'] == '#root')]
        # 共享成员的父子关系重复，去重
        # 保证source -> target关系唯一性
        parent = parent.drop_duplicates(subset=['name', 'parent_name'], keep='first')
        parent = parent.rename(columns={'name': 'source', 'parent_name': 'target'})
        # 主数据排除共享成员
...
def _validate_df_for_dimension(df: pd.DataFrame):
    ...
    if SHAREDMEMBER in df.columns:
        ...
        if not duplicated.empty:
            raise ValueError(f"Duplicated member name for: {duplicated.to_dict(orient='record')}")
    else:
...
class Dimension(AsyncDimension, metaclass=SyncMeta):
    ...
    if TYPE_CHECKING:
        ...
        def query(
            ...
            role_group: str = None,
            multi_entity_config: List[Dict] = None
        ) -> Union[List[DimensionMemberBean], List[Dict[str, Any]], List[MultiEntityConfigResult]]:  # pragma: no cover
            ...
