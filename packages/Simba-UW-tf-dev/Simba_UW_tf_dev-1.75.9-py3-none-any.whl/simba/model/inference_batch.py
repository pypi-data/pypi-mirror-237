...
class InferenceBatch(TrainModelMixin,
    ...
    def run(self):
        for file_cnt, file_path in enumerate(self.feature_file_paths):
            ...
            if fps is None:
                SkippingFileWarning(msg=f'SimBA could not find {str(file_name)} in the video_info.csv file. Make sure all videos are represented in the project_folder/logs/video_info.csv file. SKIPPING THIS FILE..')
            else:
