...
class DirectingOtherAnimalsAnalyzer(ConfigReader, FeatureExtractionMixin):
    ...
    def __init__(self,
        ...
        if self.animal_cnt < 2:
            raise AnimalNumberError('Cannot analyze directionality between animals in a 1 animal project.', source=self.__class__.__name__)
    ...
    def summary_statistics(self):
        ...
        stdout_success(msg=f'Summary directional statistics saved at {os.path.join(self.logs_path, f"Direction_data_{str(self.datetime)}.csv")}', source=self.__class__.__name__)
