...
class UNet(nn.Module):
    ...
    def train_net(self, train_iter, loss=nn.BCEWithLogitsLoss(), optimizer=torch.optim.SGD, epochs=40, lr=0.0001):#没有修改过
...
class UNet_plus(nn.Module):
    ...
    def train_net(self, train_iter, loss=nn.BCEWithLogitsLoss(), optimizer=torch.optim.SGD, epochs=40, lr=0.0001):
        updater = optimizer(self.parameters(), lr=lr)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.to(device)
        self.train()
