...
__git__ = "git@github.com:nelsonaloysio/graph-express.git"
__url__ = "https://github.com/nelsonaloysio/graph-express"
...
ATTRS =  list(_.replace("_", "-") for _ in gx._method_class([Centrality, Partition]))
LAYOUTS = list(_.rsplit("_", 1)[0] for _ in gx._method_class([Layout]) if _.endswith("_layout"))
...
def main():
    ...
    G = gx.compose([read_graph(f, **{arg: args[arg] for arg in signature(read_graph).parameters if arg in args})
                    for f in args["input"]])
...
def argparser(args=argv[1:]):
    parser = ArgumentParser(description=str("graph-express | %s" % __url__))
    ...
    parser.add_argument("-a", "--attrs",
                        help=f"Available attributes: {ATTRS}.",
                        nargs="+",
    ...
    parser.add_argument("-l", "--layout",
                        ...
                        help=f"Available layouts: {LAYOUTS} (default: '{DEFAULT_LAYOUT}').",
