...
def batsmanMovingAverage(file,name="A Squarecut") :
    ...
    #date= pd.to_datetime(batsman['Start Date'],format = '%Y-%m-%d %H:%M')
    s_datetime = pd.to_datetime(batsman['Start Date'], format="%d %b %Y")
    date=s_datetime.dt.strftime("%m/%d/%Y")
...
def bowlerMovingAverage(file,name="A Doosra") :
    ...
    wkts=bowler['Wkts'].astype(int)
    date= pd.to_datetime(bowler['Start Date'])
    s_datetime = pd.to_datetime(bowler['Start Date'], format="%d %b %Y")
    date=s_datetime.dt.strftime("%m/%d/%Y")
    
    
    atitle =  "'s Moving average (Runs)"
    # Plot the runs in grey colo
    plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%m/%d/%Y'))
    plt.gca().xaxis.set_major_locator(mdates.DayLocator())
    plt.plot(date,wkts,"-",color = '0.75')
    y_av = movingaverage(wkts, 5)
    plt.xlabel('Date')
    plt.ylabel('Wickets')
    plt.plot(date, y_av,"b")
    plt.title(atitle)
...
def bowlerWktsRunsPlot(file, name="A Googly"):
    ...
    bowler['Wkts']=bowler['Wkts'].astype(int)
...
def plotTimelineofWinsLosses(file,teamName,opposition=["all"],homeOrAway=["all"],
    ...
    df6['date']= pd.to_datetime(df6['Start Date'], format="%d %b %Y")
    df6['date']=df6['date'].dt.strftime("%m/%d/%Y")
    print("hello")
    print(df6['date'])
    ...
    df8 = df7[m]
    df8 = df8[['date', 'result']].sort_values(by='date')
    if (matchType == "Test"):
