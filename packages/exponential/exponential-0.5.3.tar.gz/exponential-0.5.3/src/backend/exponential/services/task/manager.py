...
from exponential.services.task.backends.anyio import AnyIOBackend
from exponential.services.task.backends.base import TaskBackend
from exponential.services.task.utils import get_celery_worker_status


def check_celery_availability():
    try:
        from exponential.worker import celery_app
...
class TaskService(Service):
    ...
    def get_backend(self) -> TaskBackend:
        if USE_CELERY:
            from exponential.services.task.backends.celery import CeleryBackend
