...
from exponential.main import setup_app
from exponential.services.database.utils import session_getter
from exponential.services.getters import get_db_service, get_settings_service
from exponential.services.utils import initialize_services, initialize_settings_service
from exponential.utils.logger import configure, logger
...
def run(
    host: str = typer.Option(
        "127.0.0.1", help="Host to bind the server to.", envvar="exponential_HOST"
    ),
    workers: int = typer.Option(
        1, help="Number of worker processes.", envvar="exponential_WORKERS"
    ),
    timeout: int = typer.Option(300, help="Worker timeout in seconds."),
    port: int = typer.Option(7860, help="Port to listen on.", envvar="exponential_PORT"),
    components_path: Optional[Path] = typer.Option(
        Path(__file__).parent / "components",
        help="Path to the directory containing custom components.",
        envvar="exponential_COMPONENTS_PATH",
    ),
    ...
    log_level: str = typer.Option(
        "critical", help="Logging level.", envvar="exponential_LOG_LEVEL"
    ),
    log_file: Path = typer.Option(
        "logs/exponential.log", help="Path to the log file.", envvar="exponential_LOG_FILE"
    ),
    cache: Optional[str] = typer.Option(
        envvar="exponential_LANGCHAIN_CACHE",
...
def run_on_mac_or_linux(host, port, log_level, options, app, open_browser=True):
    webapp_process = Process(
        target=run_exponential, args=(host, port, log_level, options, app)
    )
...
def run_on_windows(host, port, log_level, options, app):
    """
    Run the exponential server on Windows.
    """
    print_banner(host, port)
    run_exponential(host, port, log_level, options, app)
...
def print_banner(host, port):
    ...
    info_text = (
        ...
        "[bold][link=https://github.com/ridabourji1988/Exponential-ai]GitHub Repo[/link][/bold] :rocket:"
    )
...
def run_exponential(host, port, log_level, options, app):
    """
    Run exponential server on localhost
    """
    try:
        if platform.system() in ["Windows"]:
            # Run using uvicorn on MacOS and Windows
            ...
            from exponential.server import exponentialApplication

            exponentialApplication(app, options).run()
...
def superuser(
    ...
    with session_getter(db_service) as session:
        from exponential.services.auth.utils import create_super_user

        if create_super_user(db=session, username=username, password=password):
            # Verify that the superuser was created
            from exponential.services.database.models.user.user import User
