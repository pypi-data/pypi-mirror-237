...
class MLClient:
    ...
    def __init__(
        ...
        self.username = (
            username
            if username != "default" and not os.getenv("EXP_USER")
            else os.getenv("EXP_USER") or "default"
        )
    ...
    def search_runs(self, exp_id: str):
        ...
        try:
            ...
            raise ValueError(f"Experiment id {exp_id} does not exists.") from e
    ...
    def register_model(self, model_name: str, run_id: str) -> Any:
        ...
        try:
            ...
            raise ValueError(
                f"Could not register the model {model_name} with run_id {run_id}"
            ) from e
