...
from katonic.fs.errors import EntityTimestampInferenceException
from katonic.fs.errors import KfsClassImportError
from katonic.fs.errors import KfsEntityDFMissingColumnsError
from katonic.fs.errors import KfsModuleImportError
...
def infer_event_timestamp_from_entity_df(entity_schema: Dict[str, np.dtype]) -> str:  # type: ignore
...
def assert_expected_columns_in_entity_df(
    entity_schema: Dict[str, np.dtype],  # type: ignore
    join_keys: Set[str],
...
def get_feature_view_query_context(
    ...
    (feature_views_to_feature_map) = _get_requested_feature_views_to_features_dict(
        feature_refs,
        feature_views,  # registry.list_on_demand_feature_views(project)
    )
...
def get_offline_store_from_config(
    ...
    try:
        ...
        raise KfsClassImportError(
            module_name, store_class_name, class_type="OfflineStore"
        ) from None
    return offline_store_class()  # type: ignore
