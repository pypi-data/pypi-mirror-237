...
DEFAULT_PATTERNS = [(r"p\.ey[A-Za-z0-9-_\.]+", "p.ey****...****")]
...
@click.option(
    "--host", envvar="TB_HOST", help="Use custom host, defaults to TB_HOST envvar, then to https://api.tinybird.co"
)
...
def __patch_click_output():
    ...
    def _obfuscate(msg: Any, *args, **kwargs) -> Any:
        patterns_str = os.getenv("OBFUSCATE_REGEX_PATTERN", None)
        separator = os.getenv("OBFUSCATE_PATTERN_SEPARATOR", "|")

        if patterns_str:
            patterns = patterns_str.split(separator)
    ...
    def _obfuscate_echo(msg: Any, *args, **kwargs) -> None:
        msg = _obfuscate(msg, *args, **kwargs)
        __old_click_echo(msg, *args, **kwargs)

    def _obfuscate_secho(msg: Any, *args, **kwargs) -> None:
        msg = _obfuscate(msg, *args, **kwargs)
