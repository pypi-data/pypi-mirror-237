...
class FeedbackManager:
    ...
    error_head_outdated = error_message(
        "Current HEAD commit '{commit}' is outdated. Hint: update your Git branch with base branch."
    )
    ...
    error_in_git_ancestor_commits = error_message(
        "Error checking relationship between HEAD '{head_commit}' and Workspace commit '{workspace_commit}'. Check if they are valid commits. Hint: consider to rebase your Git branch."
    )
    ...
    error_no_correct_token_for_init = error_message(
        "No access to git releases. Use an admin token associated with your account (admin user@domain.com)"
    )
...
**    $ tb datasource rm {datasource}"""
    ...
    warning_workspaces_admin_token = warning_message(
        "** Warning: you're using an admin token that is not associated with any user. If you want to be able to switch between workspaces, please use an admin token associated with your account."
    )
