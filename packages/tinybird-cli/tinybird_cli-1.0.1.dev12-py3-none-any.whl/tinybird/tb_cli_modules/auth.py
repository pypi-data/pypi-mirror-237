...
async def auth(ctx: click.Context, token: str, host: str, region: str, connector: str, interactive: bool) -> None:
    ...
    # do a clean auth
    if not token and "token" not in ctx.parent.params:
...
async def auth_ls() -> None:
    ...
    table: List[List[Any]] = []
    regions: List[Dict[str, str]] = await get_regions_temp(config)
    if regions:

        def is_current(region: Dict[str, Any]) -> bool:
            return region["host"] == config.get("host") or region["api_host"] == config.get("host")

        for index, region in enumerate(regions):
            table.append([index + 1, region["name"].lower(), region["host"], region["api_host"], is_current(region)])
    else:
        table.append([1, "default", config["host"], True])

    columns: List[str] = ["idx", "region", "host", "api", "current"]
...
async def auth_use(region_name_or_host_or_id: str) -> None:
    ...
    regions, host = await get_host_from_region_temp(config, region_name_or_host_or_id, config.get_host())
    config.set_host(host)
