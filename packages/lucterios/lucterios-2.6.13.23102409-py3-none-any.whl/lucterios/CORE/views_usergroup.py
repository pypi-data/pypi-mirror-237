...
from lucterios.framework.tools import MenuManage, FORMTYPE_NOMODAL, SELECT_SINGLE, SELECT_MULTI, ActionsManage,\
    FORMTYPE_REFRESH, CLOSE_NO, SELECT_NONE, FORMTYPE_MODAL, WrapAction
...
class UsersList(XferListEditor):
    ...
    def fillresponse_body(self):
        ...
        self.get_components('user_actif').colspan = 3
        self.get_components('user_inactif').vmin = 200
        self.get_components('user_inactif').description = _("List of inactive users")
        self.get_components('user_inactif').colspan = 3
        row = self.get_max_row() + 1
        Params.fill(self, ['CORE-OnlySuperAdmin'], 1, row)
        btn = XferCompButton('editonlyadmin')
        btn.set_is_mini(True)
        btn.set_location(2, row)
        btn.set_action(self.request, ChangeOnlySuperAdmin.get_action(TITLE_MODIFY, 'images/edit.png'), close=CLOSE_NO)
        self.add_component(btn)
        self.new_tab(_('Preferences'))
        self.fieldnames = None
        self.fill_grid(1, Preference, 'preference', Preference.objects.filter(user__isnull=True))


@MenuManage.describ('CORE.add_parameter')
class ChangeOnlySuperAdmin(XferContainerAcknowledge):
    caption = _("CORE-OnlySuperAdmin")
    icon = "config.png"

    def fillresponse(self):
        only_admin_value = Params.getvalue('CORE-OnlySuperAdmin')
        if only_admin_value or self.confirme(_('Do you want to limit access to only administrators?')):
            Params.setvalue('CORE-OnlySuperAdmin', not only_admin_value)


@ActionsManage.affect_list(_("Search"), "user.png", condition=lambda xfer: xfer.get_components('user_actif'), modal=FORMTYPE_NOMODAL)
@MenuManage.describ('auth.change_user')
class UsersSearch(XferSearchEditor):
    caption = _("User search")
    icon = "user.png"
    model = LucteriosUser
    field_id = 'user'


@ActionsManage.affect_other("", "images/edit.png")
@MenuManage.describ('auth.add_user')
class UsersShow(XferContainerAcknowledge):
    icon = "user.png"
    model = LucteriosUser
    field_id = 'user_actif'
    ...
    def fillresponse(self):
        ...
        if self.item.is_active and settings.ASK_LOGIN_EMAIL and (self.item.email == ""):
            self.item.is_active = False
            self.item.save()
            self.message(_('User deactivated: no email!'))
        if self.item.is_active and settings.ASK_LOGIN_EMAIL and self.item.is_email_already_exists:
            self.item.is_active = False
            self.item.save()
            self.message(_('User deactivated: another with the same email is active!'))


@ActionsManage.affect_grid(TITLE_CREATE, "images/new.png", condition=lambda xfer, gridname='user_actif': gridname == 'user_actif')
@ActionsManage.affect_grid(TITLE_MODIFY, "images/edit.png", condition=lambda xfer, gridname='user_actif': (gridname == 'user_actif') or (gridname == 'user_inactif') or (gridname == 'user'), unique=SELECT_SINGLE)
@MenuManage.describ('auth.add_user')
class UsersEdit(XferAddEditor):
    ...
    def fillresponse(self):
        user_inactif = self.getparam('user_inactif', self.getparam('user', 0))
        if user_inactif != 0:
            self.item = LucteriosUser.objects.get(id=user_inactif)
            self.params[self.field_id] = user_inactif
        XferAddEditor.fillresponse(self)


@MenuManage.describ('auth.change_user')
class UsersPreference(XferContainerCustom):
    caption = _("User preferences")
    icon = "settings.png"
    model = LucteriosUser
    field_id = 'user_actif'
    ...
    def fillresponse(self):
        ...
        self.params['username'] = self.item.username
        img = XferCompImage('img')
        img.set_value(self.icon_path())
        img.set_location(0, 0, 1, 6)
        self.add_component(img)
        self.fill_from_model(1, 1, True, ['username', 'preferences_set'])
...
class UsersEnabled(XferContainerAcknowledge):
    ...
    def fillresponse(self, user_actif=0):
        ...
        if settings.ASK_LOGIN_EMAIL and (self.item.email == ""):
            raise LucteriosException(IMPORTANT, _("User cannot be activated: no email!"))
        if settings.ASK_LOGIN_EMAIL and self.item.is_email_already_exists:
            raise LucteriosException(IMPORTANT, _("User cannot be activated: another with the same email is active!"))
...
class UsersDelete(XferDelete):
    ...
    icon = "user.png"
    model = LucteriosUser
    field_id = ('user_actif', 'user_inactif', 'user')
...
def right_edit_preference(request):
    return UsersEnabled.get_action().check_permission(request) or (not request.user.is_anonymous and (request.user.username == request.POST['username']))


@ActionsManage.affect_grid(TITLE_MODIFY, "images/edit.png", unique=SELECT_SINGLE)
@MenuManage.describ(right_edit_preference)
class PreferenceEdit(XferAddEditor):
    caption_modify = _("Modify preference")
    icon = "settings.png"
    model = Preference
    field_id = 'preference'
...
class AuditLogShow(XferShowEditor):
    caption = _("Show log")
    icon = "auditlog.png"
    ...
    def fillresponse(self, model='', objid=0):
        ...
        if objid != 0:
            ...
            lbl.set_value_as_header(str(content_type))
