...
@LastEditTime: 2023-10-24 10:00:21
...
class JdBaseModel():
    ...
    def lock_certs_save(self, access_token, vender_id, transaction_id, certList, is_log=False):
        ...
        for cert in certList:
            cert_num = CryptoHelper.aes_decrypt(cert["cardNum"], "17f72a775ec0a96ad52b662206c7bceb")
            cert["cardNum"] = CryptoHelper.aes_encrypt(cert_num, "07afbc8bca08bc5139d36b3239ab3408")
            pwd = CryptoHelper.aes_decrypt(cert["pwd"], "17f72a775ec0a96ad52b662206c7bceb")
            cert["pwd"] = CryptoHelper.aes_encrypt(pwd, "07afbc8bca08bc5139d36b3239ab3408")
    ...
    def palce_order_submit(self, access_token, xid, user_ip, business_data_id, order_source,
        ...
        # 该接口调用日期+1天
        now_format_time = TimeHelper.get_now_format_time("%Y-%m-%d")
        promise_date = TimeHelper.add_days_by_format_time(now_format_time, 1, "%Y-%m-%d")
                ...
                if resp['jingdong_palceorder_submit_responce']['ydResponse']['success'] == False:
                    ...
                    if result.__contains__('success') and result['success'] == False:
                        ...
                        if result["errCode"] not in [1500, 1502]:
                            invoke_result_data.success = False
                            invoke_result_data.error_code = result["errCode"]
                            result_message = result["data"].get("resultMessage","")
    ...
    def batch_add_sku_add(self, access_token, vender_id, sku_list, is_log=False):
        """
        :description: 批量添加商品
        :param access_token: access_token
        :param vender_id: 商家唯一标识，必填
        ...
        for sku_data in sku_list:
            sku_data["effectiveDuration"] = 180  # 有效时长，非必填
    ...
    def verify_save(self, access_token, vender_id, transaction_id, certList, is_log=False):
        ...
        :param access_token: access_token
        :param vender_id: 商家唯一标识，必填
        :transaction_id： 交易ID，必填，要传锁定时定义的相同值，否则核销不成功
