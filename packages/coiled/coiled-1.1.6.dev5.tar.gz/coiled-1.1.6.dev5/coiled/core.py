...
class Cloud(Generic[IsAsynchronous]):
    ...
    async def _create_api_token(self, label: Optional[str] = None, days_to_expire: Optional[int] = None) -> dict:
        ...
        if days_to_expire is not None:
            expiry = datetime.datetime.utcnow() + datetime.timedelta(days=days_to_expire)
    ...
    async def _get_notifications(
        ...
        for notification in notifications:
            try:
                local_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(notification["timestamp"]))
            except KeyError:
                local_time = "???"
...
def get_notifications(
    ...
    """
    with Cloud() as cloud:
        return cloud.get_notifications(json=json, account=account, limit=limit, level=level, event_type=event_type)
...
def _parse_gcp_creds(gcp_service_creds_dict: Optional[Dict], gcp_service_creds_file: Optional[str]) -> Dict:
    ...
    if gcp_service_creds_file:
        ...
        try:
            ...
            raise GCPCredentialsError(f"The supplied file '{gcp_service_creds_file}' is not a valid JSON file.")
