...
def _decode_duration_ffmpeg(filename: str, index: int, accurate: bool) -> Fraction:
    ...
    ** Extract the duration by the complete decoding of the stream. **
    ...
    Slow but 100% accurate method. The duration of the last frame is takn in considered.
    Equivalent to:
    ...
    Fraction(16, 1)
    >>> _decode_duration_ffmpeg("cutcutcodec/examples/intro.mkv", 0, accurate=True)
    Fraction(9809, 1000)
    >>>
    """
    # get context
...
def _estimate_duration_ffmpeg(filename: str, index: int, *, _indirect=True) -> Fraction:
    ...
    ** Extract the duration from the metadata. **
    ...
    >>>
    """
    def parse(duration: typing.Union[numbers.Real, str]) -> typing.Union[None, Fraction]:
    ...
    if (stream_meta := metadata.get("streams", [])) and len(stream_meta) > index:
        ...
        for key, val in stream_meta.get("tags", {}).items():
            if "duration" in key.lower():
                if (duration := parse(val)) is not None:
                    return duration
    if (format_meta := metadata.get("format", None)) is not None:
        if "duration" in format_meta:
...
def _estimate_rate_ffmpeg(filename: str, index: int, *, _indirect=True) -> Fraction:
    """
    ** Retrieves via ffmpeg, the metadata concerning the fps or the framerate. **
...
def _map_index_rel_to_abs(filename: str, index: int, stream_type: str) -> int:
    ...
    ** Relative stream index to absolute stream index. **
    ...
    >>> _map_index_rel_to_abs("cutcutcodec/examples/intro.mkv", 0, "video")
    0
    >>> _map_index_rel_to_abs("cutcutcodec/examples/intro.mkv", 1, "video")
    1
    >>> _map_index_rel_to_abs("cutcutcodec/examples/intro.mkv", 0, "audio")
    2
    >>> _map_index_rel_to_abs("cutcutcodec/examples/intro.mkv", 1, "audio")
    3
    >>>
    """
...
def get_slices_metadata(
    ...
    ** Get the packets informations for all streams. **
    ...
    with subprocess.Popen(
        ...
        # declaration
        headers = [set() for _ in range(len(times_base))] # contains all headers
        infos = [[] for _ in range(len(times_base))] # contains the dictionary for each streams
        def parse_data(data, sort_new_h, sort_old_h=None):
        ...
        with tqdm.tqdm(
            ...
            while (data := child.stdout.readline()) or child.poll() is None:
                ...
                sort_all_h = sorted(headers[stream_index] | set(data))

                # management append new header
                if not set(data).issubset(headers[stream_index]):
                    sort_old_h = sorted(headers[stream_index])
                    infos[stream_index] = [
                        parse_data(d.ravel(), sort_all_h, sort_old_h) for d in infos[stream_index]
                    ]
                    headers[stream_index] |= set(data)

                # append new data
                infos[stream_index].append(parse_data(data, sort_all_h))
                if sum(len(inf) for inf in infos) == 500_000:
                    infos = [[np.vstack(d)] if d else [] for d in infos]
        ...
        # parse
        headers = [sorted(h) for h in headers]
        infos = [np.vstack(d) if d else [] for d in infos]
...
def get_streams_type(
    ...
    ** Retrieves in order the stream types present in the file. **
    ...
    >>> get_streams_type("cutcutcodec/examples/intro.mkv")
