...
from cutcutcodec.core.filters.audio.fir import FilterAudioFIR
...
def anti_aliasing(
    ...
    ** Find the best type 1 impulse response that perfectly matchs the constraints. **
    ...
    band : float, optional
        ...
        One cent is define by b = a * 2**(1/1200).
    attenuation : float, default=120
        The attenuation in the stopband in dB.
        Transmitted to ``cutcutcodec.core.filters.audio.resample.kaiser``.
    ...
    --------
    >>> import numpy as np
    >>> from cutcutcodec.core.filters.audio.resample import anti_aliasing
    >>> fir = anti_aliasing(48000, 8000)
    >>> np.round(fir, 6)
    array([-1.8e-05, -2.7e-05, -9.0e-06, ..., -9.0e-06, -2.7e-05, -1.8e-05])
    >>>
    ...
    cutoff -= .5*band
    win = kaiser(cutoff/rate, band/rate, attenuation)
...
def kaiser(
    cutoff: numbers.Real, band: numbers.Real, attenuation: numbers.Real
) -> np.ndarray:
    """
    ** Design a kaiser window with some frequencial properties. **
    ...
    ----------
    cutoff : float
        The cutoff frequency as a fraction of the sampling rate in ]0, 0.5[.
    band : float
    ...
    --------
    >>> import numpy as np
    >>> from cutcutcodec.core.filters.audio.resample import kaiser
    >>> np.round(kaiser(0.25, 0.05, 80), 5)
    ...
    """
    # verifs
    assert isinstance(cutoff, numbers.Real), attenuation.__class__.__name__
    assert 0 < cutoff < 0.5, cutoff
    assert isinstance(band, numbers.Real), band.__class__.__name__
    ...
    # win *= 1/np.sum(win) # normalize to get unity gain
    return win
...
def sinc_continuous(
    ...
    ** Resample the signal by the exact continuous function. **
    ...
    >>> from fractions import Fraction
    >>> import torch
    >>> from cutcutcodec.core.filters.audio.resample import sinc_continuous
    >>> from cutcutcodec.core.classes.frame_audio import FrameAudio
    >>> frame = FrameAudio(1, 8000, "mono", 1, 512)
    ...
    array([[ 0.588,  0.359,  0.109, -0.151, -0.398, -0.619, -0.8  , -0.927,
            ...
            -0.982, -0.997, -0.946, -0.83 , -0.66 , -0.444, -0.201,  0.056,
             0.312,  0.544,  0.74 ,  0.889,  0.976,  0.999,  0.956,  0.848,
    ...
    dst_frame = FrameAudio(
        ...
        rate,
        frame.profile,
        torch.empty((len(frame.profile.channels), samples), dtype=frame.dtype, device=frame.device),
    )
    ...
    indexs *= frame.rate
    indexs = indexs.to(dtype=torch.int32, copy=False)
    ...
    ) # shape (nbr_channels, samples, 2*win+1)
    for i in range(2*win+1):
    ...
    del sinc_conv
    values = torch.sum(values, dim=2, keepdim=False) # shape (nbr_channels, samples, 2*win+1)
    dst_frame[:, :] = values
...
class FilterAudioAntiAliasing(FilterAudioFIR):
    ...
    ** Anti-aliasing filter for audio subsampling. **
    ...
    Examples
    --------
    >>> from cutcutcodec.core.filters.audio.resample import FilterAudioAntiAliasing
    ...
    def __init__(self, in_streams: typing.Iterable[StreamAudio], cutoff: numbers.Real):
        ...
        in_streams : typing.Iterable[Stream]
            Transmitted to ``cutcutcodec.core.filters.audio.fir.FilterAudioFIR``.
        cutoff : float
        ...
        fir_rate = math.floor(2*self._cutoff) + 1
        fir = torch.from_numpy(anti_aliasing(fir_rate, cutoff))
        super().__init__(in_streams, fir, fir_rate)
    ...
    def _setstate(self, in_streams: typing.Iterable[Stream], state: dict) -> None:
        assert set(state) == {"cutoff"}, set(state)
    ...
    def delay(self) -> Fraction:
        """
        ** The delay introduced by the filter in seconds. **
        """
