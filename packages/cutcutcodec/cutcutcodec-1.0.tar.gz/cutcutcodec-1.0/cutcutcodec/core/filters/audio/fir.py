...
class FilterAudioFIR(Filter):
    ...
    ** Invariant finite impulse response convolutional filter (FIR). **
    ...
    def _setstate(self, in_streams: typing.Iterable[Stream], state: dict) -> None:
        assert set(state) == {"fir_encoded", "fir_rate"}, set(state)
    ...
    def encode_fir(fir: torch.Tensor) -> str:
        """
        ** Encodes the fir tensor into a jsonisable string. **
        """
    ...
    def fir_rate(self) -> int:
        """
        ** The sample rate of the impulsional response. **
        """
...
class _StreamAudioConvolution(StreamAudio):
    ...
    ** A convoluted audio stream. **
    ...
    def _snapshot(self, timestamp: Fraction, rate: int, samples: int) -> FrameAudio:
        ...
        frame = self.node.in_streams[self.index]._snapshot(req_timestamp, rate, req_samples)
        if pad_l or pad_r:
        ...
        return FrameAudio(timestamp, rate, signal.profile, out)
    ...
    def profile(self) -> ProfileAudio:
        return self.node.in_streams[self.index].profile
