...
from cutcutcodec.core.generation.video.c_fractal import mandelbrot
...
class GeneratorVideoMandelbrot(ContainerInput):
    ...
    ** Generation of an annimated mandelbrot fractal. **
    ...
    tensor([[  0,   0,   4,   8,   8,  12,  16,   8,   4],
            [  0,   4,   8,   8,  12,  12,  40,  12,   8],
            [  0,   8,   8,   8,  12,  20, 255,  20,  12],
            [  0,   8,   8,  16,  20, 120, 255, 255,  16],
            [  0,   8,  16,  28,  28, 255, 255, 255,  28],
            [  0,  12,  24, 255,  60, 255, 255, 255,  20],
            ...
            [  0,   8,   8,   8,  12,  20, 255,  20,  12],
            [  0,   4,   8,   8,  12,  12,  40,  12,   8],
            [  0,   0,   4,   8,   8,  12,  16,   8,   4]], dtype=torch.uint8)
    >>>
    """
    ...
    def _setstate(self, in_streams: typing.Iterable[Stream], state: dict) -> None:
        assert set(state) == {"bounds", "iterations"}, set(state)
    ...
    def default(cls):
        return cls(
            bounds={"i_min": -1.12, "i_max": 1.12, "j_min": -2, "j_max": 0.47},
            iterations=64,
        )
    ...
    def iterations(self):
        """
        ** The maximum number of iterations function based on `t`. **
        """
        return self._iters
...
class _StreamVideoMandelbrot(StreamVideo):
    ...
    ** Fractal video stream. **
    ...
    def _get_func(self) -> callable:
        """
        ** Allows to "compile" bounds and iterations equations at the last moment. **
        """
    ...
    def _get_complex_map_and_iter_max(self,
        timestamp: Fraction, mask: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor, int]:
        """
        ** Compute the complex map, one complex number by pixel. **
        """
        i_min, i_max, j_min, j_max, iters = self._get_func()(t=float(timestamp))
