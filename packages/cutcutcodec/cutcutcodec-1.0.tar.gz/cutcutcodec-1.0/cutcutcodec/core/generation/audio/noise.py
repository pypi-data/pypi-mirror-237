...
class GeneratorAudioNoise(ContainerInput, Seedable):
    ...
    ** Generate a pure noise audio signal. **
    ...
    def __init__(self,
        ...
        profile: typing.Optional[typing.Union[ProfileAudio, str, numbers.Integral]]="stereo",
    ):
        """
        Parameters
        ----------
        ...
        profile: cutcutcodec.core.classes.profile.ProfileAudio or str or int, optional
            ...
            By default, the profile is stereo, two channels.
        """
        ...
        self._profile = ProfileAudio(profile)

    def _getstate(self) -> dict:
        return {**self._getstate_seed(), "profile": self._profile.name}

    def _setstate(self, in_streams: typing.Iterable[Stream], state: dict) -> None:
        assert set(state) == {"seed", "profile"}, set(state)
        self._setstate_seed(state)
        self._profile = ProfileAudio(state["profile"])
    ...
    ** Random audio stream where each sample follows a uniform law. **
    ...
    def _snapshot(self, timestamp: Fraction, rate: int, samples: int) -> FrameAudio:
        ...
        while True:
            ...
            if len(hashes) >= len(self.node.profile.channels):
                break
            string[5, :] = hashes[-1]

        # convertion uint32 to float32 uniform [0, 1]
        n_channels = len(self.node.profile.channels)
...
def md5(string: np.ndarray) -> np.ndarray:
    """
    ** Apply the md5 algorithm on each item. **
