...
def gui(project_file: typing.Optional[pathlib.Path], output: multiprocessing.Queue):
    """
    ** Start the GUI. **
    """
...
def main(project_file: typing.Optional[pathlib.Path]=None) -> int:
    """
    ** The main complete pipeline including gui. **
    """
    output = multiprocessing.Queue()

    process_gui = multiprocessing.Process(target=gui, args=(project_file, output))
    process_gui.start()
    process_gui.join()
