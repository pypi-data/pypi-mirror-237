...
class DatasetRevisionImpl(DatasetRevision):
    ...
    def resolver_replay(
        ...
        response = self._client._get_resolver_replay(
            environment_id=self.environment,
    ...
    def _resolver_replay(self, resolver: Resolver, raw_input_df: pl.DataFrame):
        ...
        # no_metadata_df = raw_input_df.select(filter(lambda c: not c.startswith("__"), raw_input_df.columns))
        # basename_df = no_metadata_df.select(
        #     [
        #         pl.col(col_name).alias(base_col_name)
        #         for col_name, base_col_name in zip(
        #             no_metadata_df.columns, map(lambda c: c.split(".")[-1], no_metadata_df.columns)
        #         )
        #     ]
        # )
        ...
        # of resolver replay)
        output_col = []
        for row_i, args in enumerate(raw_input_df.rows()):
            print(f"resolver_replay: Running resolver {resolver.fqn} on args " + str(args))
            actual_args = []
        ...
        return raw_input_df.with_columns(pl.Series(name="__resolver_replay_output", values=output_col))
...
class DatasetImpl(Dataset):
    ...
    def resolver_replay(self, resolver: Resolver):
        if len(self.revisions) == 0:
            raise IndexError("No revisions exist for dataset")
