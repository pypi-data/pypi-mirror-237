...
class ChalkAPIClientImpl(ChalkClient):
    ...
    def _get_headers(
        ...
        if branch is _EMPTY:
            pass
        elif branch is not None and branch is not Ellipsis:
    ...
    def _raw_session_post(self, *args, **kwargs):
        return self._raw_session_request(method="POST", *args, **kwargs)

    def _raw_session_request(self, *args, **kwargs):
        try:
    ...
    def _request(
        ...
        is_service_account = (
            ...
            and self._config.client_id.startswith("token-")
        )
        if (
            is_service_account
        ...
        r = self._raw_session_request(method=method, headers=headers, url=url, json=json_body, data=data)
...
from the command line from '{os.getcwd()}'. If you are still having trouble, please contact Chalk support.""",
    ...
    def _get_local_server_override(self, body_branch: Optional[BranchId]) -> Optional[str]:
        branched = (body_branch is not None and body_branch is not Ellipsis) or self._config.branch is not None
