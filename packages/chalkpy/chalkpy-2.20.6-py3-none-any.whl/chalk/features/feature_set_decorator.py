...
def features(
    ...
    *,
    ...
    def wrap(c: Type[T]) -> Type[T]:
        ...
        if previous_features_class is not None and not notebook.is_notebook() and not env_var_bool("CHALK_ALLOW_REGISTRY_UPDATES"):
            raise ValueError(
...
def _parse_annotation_comments(class_source: str, cls_annotations: Dict[str, Any]) -> Mapping[str, str]:
...
def _process_class(
    ...
    for name, annotation in raw_cls_annotations.items():
        if name in ["features", "namespace", "items", "is_near"]:
            raise ValueError(
                f"Feature name '{name}' in '{namespace}' is a reserved name. Please rename this feature."
            )
    ...
    for name, member in inspect.getmembers(cls):
        ...
        if name not in cls_annotations and isinstance(member, Feature):
            ...
            if member._is_feature_time:
                # We must read the private variable to avoid parsing the annotation, which might contain forward references
    ...
    def __features__(cl: Features) -> List[Feature]:
        fs = list(cls_fields)
    ...
    if class_source is not None:
        try:
            ...
            comments = _parse_annotation_comments(class_source, cls.__annotations__)
        except:
            comments = {}
    ...
    set_new_attribute(
        ...
        name="__init__",
        value=_init_fn(
            feature_names_and_annotations=[(x.attribute_name, cls.__annotations__[x.attribute_name]) for x in cls_fields] + additional_inits,
