...
class DataFrameMeta(type):
        ...
        for a in item:
            if isinstance(a, Underscore):
                if pydantic_model:
                    ...
                    is_pydantic = False
                a = parse_underscore_in_context(exp=x, context=context, is_pydantic=is_pydantic)
    ...
    def _parse_underscore(self, exp: Underscore, pydantic_model: BaseModel = None) -> Any:
        if pydantic_model:
            context = self
            is_pydantic = True
        else:
            ...
            is_pydantic = False
        a = parse_underscore_in_context(exp=exp, context=context, is_pydantic=is_pydantic)
