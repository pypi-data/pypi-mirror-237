 ...
 * Follow the :ref:`version policy <pytango-version-policy>` and modify correspondingly `requires-python` and minimum runtime `dependencies` for NumPy in `pyproject.toml`.
...
Make a branch from ``develop`` to prepare the release
  ...
  * Bump the versions (``tango/release.py``, ``pyproject.toml`` and ``CMakeLists.txt``).
    E.g. ``version_info = (9, 4, 4)``, ``version = "9.4.4"``, and ``VERSION 9.4.4``
