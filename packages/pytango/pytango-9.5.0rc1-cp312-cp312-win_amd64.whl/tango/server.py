...
from tango._tango import AttrDataFormat, AttrWriteType, PipeWriteType
from tango._tango import DevFailed, GreenMode, SerialModel, ApiUtil
...
try:
    ...
    _disabled_via_env_var = ApiUtil.get_env_var("PYTANGO_DISABLE_DEBUG_TRACE_PATCHING")
...
_force_tracing = _debug_run_active or _coverage_run_active


def _forcefully_traced_method(fn):
    @functools.wraps(fn)
    def trace_wrapper(*args, **kwargs):
        original_tracer = "EMPTY"
...
def __get_wrapped_read_method(attribute, read_method):
    ...
    if _force_tracing:
        read_method = _forcefully_traced_method(read_method)
...
def __get_wrapped_write_method(attribute, write_method):
    ...
    if _force_tracing:
        write_method = _forcefully_traced_method(write_method)
...
def __get_wrapped_isallowed_method(attribute, isallowed_method):
    ...
    if _force_tracing:
        isallowed_method = _forcefully_traced_method(isallowed_method)
    ...
    if not attribute.has_dtype_kword:
        if not type_hint:
            if attribute.attr_write in (AttrWriteType.READ, AttrWriteType.READ_WRITE):
                ...
                type_hint = typing.get_type_hints(read_method).get("return", None)
            ...
            if not type_hint and attribute.attr_write in (
                ...
                type_hints = typing.get_type_hints(write_method)
                if type_hints:
                    type_hint = list(type_hints.values())[0]
...
def __patch_is_command_allowed_method(tango_device_klass, is_allowed_method, cmd_name):
    ...
    if _force_tracing:
        is_allowed_method = _forcefully_traced_method(is_allowed_method)
...
def __warn_if_any_standard_device_method_should_be_coroutine(klass):
    worker = get_worker()
    # get base device class:
    if isinstance(worker, AsyncioExecutor):
        for parent in klass.__bases__:
            if type(parent) == DeviceMeta:
                for method in (
                    "init_device",
                    "delete_device",
                    "dev_state",
                    ...
                    if user_method != base_method and not asyncio.iscoroutinefunction(
                        user_method
                    ):
                        warnings.warn(
                            f"Note: {method} function in Asyncio device should be coroutine!",
                            category=PyTangoUserWarning,
                        )
...
def __patch_standard_device_methods(klass):
    ...
    if not already_wrapped:
        if _force_tracing:
            init_device_orig = _forcefully_traced_method(init_device_orig)

        @functools.wraps(init_device_orig)
        def init_device(self):
            __warn_if_any_standard_device_method_should_be_coroutine(klass)
    ...
    if not already_wrapped:
        if _force_tracing:
            delete_device_orig = _forcefully_traced_method(delete_device_orig)
    ...
    if not already_wrapped:
        ...
        if _force_tracing:
            dev_state_orig = _forcefully_traced_method(dev_state_orig)
            dev_state_async = _forcefully_traced_method(dev_state_async)
    ...
    if not already_wrapped:
        if _force_tracing:
            dev_status_orig = _forcefully_traced_method(dev_status_orig)
    ...
    if not already_wrapped:
        if _force_tracing:
            read_attr_hardware_orig = _forcefully_traced_method(read_attr_hardware_orig)
    ...
    if not already_wrapped:
        if _force_tracing:
            always_executed_hook_orig = _forcefully_traced_method(
...
class _DeviceClass(DeviceClass):
    def __init__(self, name):
        ...
        if _force_tracing:
            ...
            setattr(self, "dyn_attr", _forcefully_traced_method(orig_dyn_attr))
...
def __create_tango_deviceclass_klass(tango_device_klass, attrs=None):
    ...
    klass_annotations = typing.get_type_hints(tango_device_klass, globalns={})
...
def command(
    ...
    if dtype_in is None and len(annotations):
        dtype_in, dformat_in, _, _ = parse_type_hint(
            list(annotations.values())[0], caller="command"
        )
    ...
    if _force_tracing:
        cmd = _forcefully_traced_method(cmd)
...
def __server_run(
    ...
    pre_init_callback = __to_cb(pre_init_callback, "pre_init_callback")
    post_init_callback = __to_cb(post_init_callback, "post_init_callback")
