...
# unintrusive when this plugin is not enabled.
rich_enabled = False
...
def pytest_cmdline_main(config):
    ...
    This aalso sets the rich_enabled global for othe hook functions to use.
    """
    # pylint: disable=global-statement
    global rich_enabled                                         # noqa: PLW0603

    if config.getoption('rich', default=False):
        rich_enabled = True                                     # noqa: PLW0603
...
def pytest_configure(config):
    ...
    if os.environ.get('PYTEST_XDIST_WORKER') is None:            # noqa: SIM102
        if sys.stdout.isatty() and config.getoption('rich', default=False):
