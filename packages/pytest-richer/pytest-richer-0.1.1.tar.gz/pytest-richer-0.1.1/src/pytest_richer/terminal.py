...
def not_a_worker() -> bool:
    """Check that this is not an pytest-xdist worker process."""
...
def main_process_only(func):
    """Decorate to allow execution only in the main pytest_xdist process."""
...
class Session(Helper):
    ...
    def finish(self, exitstatus: int | pytest.ExitCode) -> None:
        ...
        if not compat.run_phase_suppressed(self.config):
            ...
            # Replicate the standard TerminalReporter summary hook invocations.
...
class RichTerminalReporter(ReporterBase):
    ...
    def write_sep(
        ...
        """
        style_names = self.formatter.convert_markup(markup)
