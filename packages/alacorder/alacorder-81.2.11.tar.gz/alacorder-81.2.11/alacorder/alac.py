...
from typing import Annotated, Any, ClassVar, Optional, TypedDict, cast
...
try:
    __version__ = metadata("alacorder")["version"]
...
class AlacorderApp(App):
            ...
            case "start-pairing-tools":
                ...
                if autopair:
                    ...
                    if psr_path is None:
                        ...
                        pass_validation = False
                    elif not psr_path.is_file() or psr_path.suffix not in (
                        ".xlsx",
                        ".csv",
                        ".json",
                        ".parquet",
    ...
    def start_alacourt_driver(self, config: dict[str, Any]) -> None:
    ...
    def start_adoc_driver(self, config: dict[str, Any]) -> None:
        """Start ADOC Driver."""
    ...
    def make_tables(self, config: dict[str, Any]) -> None:
        """Make tables."""
    ...
    def start_archive_tools(self, config: dict[str, Any]) -> None:
        """Start archive tools."""
    ...
    def start_pairing_tools(self, config: dict[str, Any]) -> None:
        """Start pairing tools."""
        console = Console()
        if config["autopair"]:
            cases = Cases(config["input-path"])
