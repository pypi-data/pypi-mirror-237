...
class Utils:
    ...
    def gen_applyb(self, caller: wasmtime.Store, supertype_id: TypeId, data: Apply) -> Result[TypeId, Error]:
        ...
        ret = self.component.lift_callee63(caller, _clamp(supertype_id, 0, 4294967295), result16, len17)
    ...
    def add_graphql_endpoint(self, caller: wasmtime.Store, graphql: str) -> Result[int, Error]:
        ...
        ret = self.component.lift_callee64(caller, ptr, len0)
        assert(isinstance(ret, int))
        load = _load(ctypes.c_uint8, self.component._core_memory0, caller, ret, 0)
    ...
    def add_auth(self, caller: wasmtime.Store, data: Auth) -> Result[int, Error]:
        ...
        ret = self.component.lift_callee65(caller, ptr, len2, variant, result, len11)
        assert(isinstance(ret, int))
        load = _load(ctypes.c_uint8, self.component._core_memory0, caller, ret, 0)
        expected: Result[int, Error]
        if load == 0:
    ...
    def add_raw_auth(self, caller: wasmtime.Store, data: str) -> Result[int, Error]:
        ...
        ret = self.component.lift_callee66(caller, ptr, len0)
        assert(isinstance(ret, int))
        load = _load(ctypes.c_uint8, self.component._core_memory0, caller, ret, 0)
        expected: Result[int, Error]
        if load == 0:
    ...
    def oauth2(self, caller: wasmtime.Store, service_name: str, scopes: str) -> Result[str, Error]:
        ...
        ret = self.component.lift_callee67(caller, ptr, len0, ptr1, len2)
        assert(isinstance(ret, int))
        load = _load(ctypes.c_uint8, self.component._core_memory0, caller, ret, 0)
