...
class CommitValidator(Commit):
    ...
    def _validate_commit_merge(self) -> bool:
        if len(self.approvers) < 1:
            ...
            if self._check_approver_mr_same_author_child_commit():
                return False
            else:
                self.reporter.display_valid_merge_report(approvers=self.approvers)
                return True

    def _check_approver_mr_same_author_child_commit(self) -> bool:
        """
        Check if the MR approver is the same as the author of the child commit.
        """
        author_child_commit_same_mr_approver = []
        mr_approvers_mail = [approvers.split(" ")[-1] for approvers in self.approvers]
        for author in self.author_child_commit:
            if author in mr_approvers_mail:
                author_child_commit_same_mr_approver.append(author)
        if author_child_commit_same_mr_approver:
