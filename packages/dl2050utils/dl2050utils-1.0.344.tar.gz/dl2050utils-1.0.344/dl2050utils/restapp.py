...
class App():
    ...
    async def download_private(self, request):
        """
            ...
            User files are stored under subfolder f'/data/{service}/buckets/{email}/'.
        """
        u = await self.auth.check_auth(request)
        uid = u['uid']
        email = await self.auth.get_email_from_uid(uid)
        fname = request.path_params['fname']
        p = Path(f'/data/{self.service}/buckets/{email}/{fname}')
        if not p.is_file():
    ...
    async def download_workfile(self, request):
        """
            ...
            Workfiles are stored in the folder f'/data/{self.service}/workfiles/'.
    ...
    async def download_req(self, request):
        """
            Returns a download signed URL for the bucket/file specified.
        ...
        timeout = 7*24*3600
        url = fs_download_signed_url(self.fs_secret, args['base_url'], args['bucket'], args['key'], timeout=timeout) if FILE_SERVER_PROVIDER=='FS' else\
    ...
    async def upload_req(self, request):
        """
            Returns an upload signed URL for the bucket/file specified.
            The URL will be executed either by the File Server or the Cloud File Server (TO BE DEFINE HOW)
            A 24h default timeout is set.
        """
        if not self.fs_secret:
        ...
        self.perm(self.d, u, request.url.path, args)
        timeout = 7*24*3600
        url = fs_upload_signed_url(self.fs_secret, args['base_url'], args['bucket'], args['key'], args['size'], timeout=timeout) if FILE_SERVER_PROVIDER=='FS' else\
              gcs_download_signed_url(args['bucket'], args['key'], timeout=timeout)
        return rest_ok({'url': url})
