...
class App():
    ...
    async def download_req(self, request):
        ...
        args = ['base_url','bucket','key'] if FILE_SERVER_PROVIDER=='FS' else\
               ['bucket','key']
        ...
        timeout = 7*24*3600
        url = fs_download_signed_url(self.fs_secret, args['base_url'], args['bucket'], key, timeout=timeout) if FILE_SERVER_PROVIDER=='FS' else\
              gcs_download_signed_url(args['bucket'], args['key'], timeout=timeout)
    ...
    async def upload_req(self, request):
        """
            ...
            The URL will be executed either by the File Server or the Cloud File Server (TO BE DEFINE HOW)
            A 24h default timeout is set.
            TODO: Include methode to select the use of File Server or GCS.
        ...
        data = await request.json()
        FILE_SERVER_PROVIDER = 'FS'
        args = ['base_url', 'bucket','key','size'] if FILE_SERVER_PROVIDER=='FS' else\
               ['bucket','key']
        args = enforce_required_args(self.LOG, data, args, label='REST', label2='upload_req')
        self.perm(self.d, u, request.url.path, args)
        key = f"{email}/{args['key']}"
        timeout = 7*24*3600
        url = fs_upload_signed_url(self.fs_secret, args['base_url'], args['bucket'], key, args['size'], timeout=timeout) if FILE_SERVER_PROVIDER=='FS' else\
              gcs_download_signed_url(args['bucket'], args['key'], timeout=timeout)
        return rest_ok({'url': url})
