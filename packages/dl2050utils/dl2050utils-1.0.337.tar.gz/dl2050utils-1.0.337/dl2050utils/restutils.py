...
# ####################################################################################################
# Testing REST service requests from from client:
...
def fs_download_signed_url(secret, bucket, fname, timeout=7*24*3600):
    payload = {
        'bucket': bucket,
        'fname': fname,
...
def fs_upload_signed_url(secret, bucket, fname, size, timeout=7*24*3600):
    payload = {
        'bucket': bucket,
        'fname': fname,
        'size': size,
...
def gcs_download_signed_url(bucket_name, blob_name, duration=15):
    """
        Generates a v4 signed URL for downloading a blob.
        Signed url expires in <duration>, expressed in minutes.
    ...
    try:
        ...
        url = blob.generate_signed_url(version="v4", expiration=datetime.timedelta(minutes=duration), method="GET")
        return url
    except Exception as exc:
        print(f'EXCEPTION: {str(exc)}')
        return None

def gcs_upload_signed_url(bucket_name, blob_name, duration=15):
    """
    ...
    try:
        ...
        bucket = sc.bucket(bucket_name)
        blob = bucket.blob(blob_name)
        url = blob.generate_signed_url(version="v4", expiration=datetime.timedelta(minutes=15), method="PUT",
