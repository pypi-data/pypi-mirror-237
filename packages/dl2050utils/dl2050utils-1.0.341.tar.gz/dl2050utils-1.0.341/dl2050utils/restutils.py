...
def fs_download_signed_url(secret, bucket, key, timeout=7*24*3600):
    payload = {
        'bucket': bucket,
        'key': key,
...
def fs_upload_signed_url(secret, bucket, key, size, timeout=7*24*3600):
    payload = {
        'bucket': bucket,
        'key': key,
        'size': size,
...
def gcs_upload_signed_url(bucket_name, blob_name, timeout=15*60):
    """
    ...
    try:
        ...
        bucket = sc.bucket(bucket_name)
        blob = bucket.blob(blob_name)
        url = blob.generate_signed_url(version="v4", expiration=datetime.timedelta(seconds=timeout), method="PUT", content_type="application/octet-stream")
    except Exception as exc:
        print(f'EXCEPTION: {str(exc)}')
        return None
    return url
