...
class App():
    ...
    async def download_req(self, request):
        ...
        url = fs_download_signed_url(self.fs_secret, args['bucket'], args['fname'], timeout=7*24*3600)
        # url = gcs_download_signed_url(args['bucket'], args['fname'], timeout=timeout)
    ...
    async def upload_req(self, request):
        ...
        self.perm(self.d, u, request.url.path, args)
        timeout = 7*24*3600
        url = fs_upload_signed_url(self.fs_secret, args['bucket'], args['fname'], args['size'], timeout=timeout)
        # url = gcs_download_signed_url(args['bucket'], args['fname'], timeout=timeout)
        return rest_ok({'url': url})
