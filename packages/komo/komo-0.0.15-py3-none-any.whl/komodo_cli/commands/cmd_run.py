...
def _run_helper(
    ...
    try:
        validate_backend_config(
            backend_name, project_backend_config
        )
    except Exception as e:
        printing.error(
            f"Error while validating backend config: {str(e)}", bold=True
        )
        exit(1)
    ...
    if workspace:
        try:
            validate_overlay_images_repository(project_config)
        except Exception as e:
            printing.error(
                f"Error while validating overlay images repository: {str(e)}",
                bold=True,
            )
            exit(1)
        ...
        printing.info(
            f"Pushing an overlay image to repository {o.overlay_images_repository}",
            bold=True,
        )
