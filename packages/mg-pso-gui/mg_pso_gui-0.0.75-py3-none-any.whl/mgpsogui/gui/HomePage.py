...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.preset_selector = customtkinter.CTkOptionMenu(self.environment_editor, values=["Local"])
        ...
        self.create_environment_button = customtkinter.CTkButton(self.environment_editor, text="Deploy Environment", command=self.deploy_cluster)
    ...
    def deploy_cluster(self):
        ...
        selected_environment = self.preset_selector.get()
        if selected_environment == "Local":
            process = subprocess.Popen(["kubectl", "create", "namespace", "csip"], stdout=subprocess.PIPE)
            output, error = process.communicate()
            if error:
                print(f"An error occurred: {error}")
                self.custer_details.insert("0.0", error.decode('unicode-escape') + "\n\n")
            ...
            process = subprocess.Popen(["kubectl", "create", "-f", "../start.yaml"], stdout=subprocess.PIPE)
            output, error = process.communicate()
            if error:
                print(f"An error occurred: {error}")
                self.custer_details.insert("0.0", error.decode('unicode-escape') + "\n\n")
            ...
            process = subprocess.Popen(["minikube", "service", "pf8087-csu-csip-oms", "-n", "csip"], stdout=subprocess.PIPE)
            output, error = process.communicate()
            if error:
                print(f"An error occurred: {error}")
                self.custer_details.insert("0.0", error.decode('unicode-escape') + "\n\n")
