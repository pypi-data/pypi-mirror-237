...
.. note::
   In this doc, ``by default`` means that you used `kalabash installer
   <https://github.com/amonak/kinstaller>`_ to install kalabash
...
To do that, go to kinstaller directory then:
...
.. sourcecode:: bash
   ...
   > rm -rf frontend
   > cp -R <virtuenv_path>/lib/pythonX.X/site-packages/kalabash/frontend_dist/ frontend/
...
.. note::
   ...
   After updating the installer and kalabash with it,
   you only need to edit ``urls.py`` and ``settings.py``

* Django has been updated to 4.2LTS. Please upgrade all your extensions alongside kalabash
...
installation (this apply if you used ``kinstaller``):

1. Edit settings.py (``/srv/kalabash/instance/instance/settings.py`` by default) and add:
...
2. Create a new supervisord config (``/etc/supervisor/conf.d/kalabash-worker.conf`` by default) :

.. sourcecode:: ini

   [program:kalabash-worker]
   autostart=true
   autorestart=true
   command={%python env path%} {% manage.py instance path%} rqworker dkim
   directory={%kalabash home dir%}
   user=%{dkim manager user}
   numprocs=1
   stopsignal=TERM

``python env path`` : Python executable located in your virtual environment created for kalabash.
You will find it here  ``/srv/kalabash/venv/bin/python`` by default.
``manage.py instance path``: Path to manage.py of your kalabash instance.
You will find it here : ``/srv/kalabash/instance/manage.py``by default.
``Kalabash home dir``: Home dir of the user running kbashoba.
You will find it here ``/srv/kalabash/`` by default.
...
.. note::
   ...
   kalabash-webmail may not work with send-only user for now.
...
The ``kalabash-dmarc``, ``kalabash-pdfcredentials`` and ``kalabash-imapmigration``
...
Add ``'kalabash.dmarc'``, ``'kalabash.pdfcredentials'`` and ``'kalabash.imap_migration'``
to ``KALABASH_APPS``:
...
And remove any reference to ``'kalabash_dmarc'``, ``'kalabash_pdfcredentials'`` or ``'kalabash_imap_migration'``
in this same variable.

You need to add ``'kalabash.imap_migration.auth_backends.IMAPBackend',`` at the end of ``AUTHENTICATION_BACKENDS``:
...
After upgrading kalabash, run the following commands from your virtual environment:

.. sourcecode:: bash

   > python manage.py rename_app kalabash_dmarc dmarc
   > python manage.py rename_app kalabash_imap_migration imap_migration
...
* Add ``'kalabash.core.context_processors.new_admin_url',`` at the end of `context_processors` list:
...
Add the following cron job in order to generate DKIM keys::
  ...
  *       *       *       *       *       root    $PYTHON $INSTANCE/manage.py kbash manage_dkim_keys
...
following line to your crontab::
  ...
  */30 * * * * <optional_virtualenv_path/>python <kalabash_instance_dir>/manage.py kbash check_mx
