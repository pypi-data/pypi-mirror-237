...
from .constants import (
    ...
    ModelSelectorAPIConstants, ModelFamily
)
...
class ModelSelector:
    ...
    def convert_mlflow_model_to_pytorch_model(
        ...
        try:
            ...
            # copy the model
            logger.info("downloading mlflow model")
            shutil.copytree(model_dir, download_dir, dirs_exist_ok=True)

            # copy the config files
            logger.info("downloading mlflow config files")
    ...
    def _get_model_info_from_registry(self) -> requests.Response:
        ...
        if self.service_endpoint is None:
            ...
            return
        asset_id = ModelSelectorAPIConstants.MODEL_ASSET_ID.format(ModelSelectorDefaults.MODEL_REGISTRY,
                                                                   self.registry_model_name)
        registry_uri = ModelSelectorAPIConstants.REGISTRY_URI.format(self.service_endpoint, asset_id)
    ...
    def _get_blobstore_sas_credential(self, model_info: dict) -> requests.Response:
        ...
        asset_id = model_info[ModelSelectorAPIConstants.ASSET_ID]
        ...
        blob_reference_sas_request_dto = {
            ModelSelectorAPIConstants.BLOB_REF_ASSET_ID: asset_id,
