...
async def refresh(hub) -> int:
    ...
    Run "describe" for the given path, then run `idem state --test` on its output.
...
async def doc(hub) -> int:
    ...
    if hub.OPT.pop_tree.ref:
        ...
        ref = hub.OPT.pop_tree.ref.replace("[", ".").replace("]", "")
        tree = await hub.tree.init.traverse(ref=ref)
        ...
        if not result:
            result = {hub.OPT.pop_tree.ref: ret[ref]}
    else:
