...
parser.add_argument("-d", "--debug", dest="debug", default=1, help="debug level 0:fatal 1:errors (default), 2: success, 3: debug ")   
    ...
    if ( uid):
        ...
        if s == 200 :
            ...
            for item in items:
                ...
                match cmd:
                    case "list":
                        print (f"Auth {aid} Application {item['applicationName']}")
                    case "get":
...
syntax = { 
    ...
    "user" : { 
        ...
        "list-access":           {"params":["email"],"fct":uf_list_user_auths, "help":"list user access token"},
        "reset-access":          {"params":["email"],"fct":uf_del_user_auths, "help":"reset user access token"},
        "reset-access-in-csv":   {"params":["file"],"fct":reset_users_in_csv, "help":"reset user access token from CSV"},
