...
class HistoricalREST:
    ...
    @staticmethod
    def check_token_cache(tkn_cache_dir):
        if not os.path.exists(tkn_cache_dir) or not os.path.exists(f'{tkn_cache_dir}/tkn_cache_{dt.now().strftime("%d%m%y")}.pkl'):
            return False , None
        with open( f"{tkn_cache_dir}/tkn_cache_{dt.now().strftime('%d%m%y')}.pkl" , 'rb') as pkl:
            token_json = pickle.load(pkl)
        exp = token_json['.expires'].split(',')[-1]
        exp = datetime.strptime(exp, " %d %b %Y %H:%M:%S %Z")
        exp = exp + timedelta(hours=5, minutes=30)
        if  dt.now() > exp:
            return False , None 
        else:
            return True , token_json

    def get_new_token(self , tkn_cache_dir):
        ...
        shutil.rmtree(tkn_cache_dir, ignore_errors=True, onerror=None)
        os.makedirs(tkn_cache_dir , exist_ok=True )
        with open( f"{tkn_cache_dir}/tkn_cache_{dt.now().strftime('%d%m%y')}.pkl" , 'wb') as pkl:
            pickle.dump(token_json , pkl)
        return token_json

    def hist_login(self):
        if os.name == 'nt':
            tkn_cache_dir = '/'.join(__file__.split('\\')[:-1]) + '/cache/tkn_cache/'
        else: 
            tkn_cache_dir = '/'.join(__file__.split('/')[:-1]) + '/cache/tkn_cache/'
        valid , token_json = self.check_token_cache(tkn_cache_dir)
        if valid:
            self.logger.debug('Using cached token TrueData Historical Data Service')
        else:
            token_json = self.get_new_token(tkn_cache_dir)
        try:
