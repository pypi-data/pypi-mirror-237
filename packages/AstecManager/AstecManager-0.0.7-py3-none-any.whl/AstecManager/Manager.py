...
xml_metadata = "analysis_data.xml"
...
def decrease_brightness(img, value=30):
    ...
    img[img == img.max()] = 0
    v_min, v_max = np.percentile(img > 0, (0.2, 99.8))
...
def compute_source_astec_dir(astec_command, params_dict):
    ...
    if "mars" in astec_command.lower():
        if "EXP_FUSE" in params_dict:
            dir_to_create = "FUSE/FUSE_" + params_dict["EXP_FUSE"].replace("'", "")
        else:
    ...
    if "post" in astec_command.lower():
        if "EXP_SEG" in params_dict:
            dir_to_create = "SEG/SEG_" + params_dict["EXP_SEG"]
        else:
...
def compute_astec_dir(astec_command, params_dict):
    ...
    if "mars" in astec_command.lower():
        if "EXP_SEG" in params_dict:
            dir_to_create.append("SEG/SEG_" + params_dict["EXP_SEG"])
        else:
    ...
    if "post" in astec_command.lower():
        if "EXP_POST" in params_dict:
            dir_to_create.append("POST/POST_" + params_dict["EXP_POST"])
        else:
            dir_to_create.append("POST/POST_RELEASE")
    if "properties" in astec_command.lower():
        if "EXP_INTRAREG" in params_dict:
            dir_to_create.append("INTRAREG/INTRAREG_" + params_dict["EXP_INTRAREG"])
        else:
...
def compute_astec_tag(astec_command, params_dict):
    tag = ""
    if "fuse" in astec_command.lower():
        tag = "fuse"
...
def compute_astec_imagestep(astec_command, params_dict):
    tag = ""
    if "fuse" in astec_command.lower():
        tag = "fuse"
    elif "mars" in astec_command.lower():
        tag = "mars"
    elif "seg" in astec_command.lower() or astec_command.lower() == "astec_astec":
        tag = "seg"
    elif "post" in astec_command.lower():
        tag = "post"
    elif "intra" in astec_command.lower():
...
class start_astec_cleaner(Thread):
    ...
    def compute_recon_dir(self, params_dict):
        ...
        if "EXP_RECONSTRUCTION" in params_dict:
            reconstruction_dir.append(
                os.path.join(self.name_embryo, "REC-MEMBRANE/REC_" + params_dict["EXP_RECONSTRUCTION"]).replace('"',
                                                                                                                '').replace(
                    "'", ""))
            reconstruction_dir.append(
                os.path.join(self.name_embryo, "REC-SEED/REC_" + params_dict["EXP_RECONSTRUCTION"]).replace('"',
                                                                                                            '').replace(
                    "'", ""))
            reconstruction_dir.append(
                os.path.join(self.name_embryo, "REC-MORPHOSNAKE/REC_" + params_dict["EXP_RECONSTRUCTION"]).replace('"',
                                                                                                                   '').replace(
                    "'", ""))
        else:
            if "EXP_SEG" in params_dict:
                reconstruction_dir.append(
                    os.path.join(self.name_embryo, "REC-MEMBRANE/REC_" + params_dict["EXP_SEG"]).replace('"',
                                                                                                         '').replace(
                        "'", ""))
                reconstruction_dir.append(
                    os.path.join(self.name_embryo, "REC-SEED/REC_" + params_dict["EXP_SEG"]).replace('"', '').replace(
                        "'", ""))
                reconstruction_dir.append(
                    os.path.join(self.name_embryo, "REC-MORPHOSNAKE/REC_" + params_dict["EXP_SEG"]).replace('"',
                                                                                                            '').replace(
                        "'", ""))
    ...
    def run(self):
        ...
        source_logs = []
        for dir in dirs:
            self.cleaner_folder.append(str(self.name_embryo) + "/" + dir.replace('"', '').replace("'", ""))
            input_folder = str(self.name_embryo) + "/" + dir.replace('"', '').replace("'", "")
        for clean in self.cleaner_folder:
        ...
        pyom = None
        tag_step = compute_astec_tag(self.astec_command, self.params_dict)
        if self.omero_result:
            # Connect to OMERO
            pyom = omerolib.connect()
            ...
            self.omero_dataset_name = astec_dir.replace("POST/", "").replace("SEG/", "").replace("FUSE/", "").replace(
                '"', '').replace("'", "")
            # Project = embryo name
        ...
        if self.omero_result:
            onlyfiles = [f for f in os.listdir(self.cleaner_folder) if
                         os.path.isfile(os.path.join(self.cleaner_folder, f))]
            for file in onlyfiles:
                if not is_file_image_or_compressed(file):
                    if pyom is not None:
                        print("Upload attachment file")
                        pyom.add_file_to_dataset(self.dataset_id, os.path.join(self.cleaner_folder, file))
...
def compute_input_folder(astec_command, name_embryo, params_dict):
    dir_to_create = ""
    if "seg" in astec_command.lower() or astec_command.lower() == "astec_astec":
        if "EXP_FUSE" in params_dict:
            dir_to_create = "FUSE/FUSE_" + params_dict["EXP_FUSE"].replace("'", "")
        ...
        return None
    return os.path.join(name_embryo, dir_to_create)
...
class start_astec_command(Thread):
    ...
    def run(self):
        ...
        dirs = compute_astec_dir(self.astec_command, self.params_dict)
        for dir in dirs:
            dir_to_create.append(str(self.name_embryo) + "/" + dir)
        self.running_dir = dir_to_create
        if self.use_omero_input:
            folder_target = compute_input_folder(self.astec_command, self.name_embryo, self.params_dict)
        ...
        parameter_content = ""
        parameter_content += 'PATH_EMBRYO = "' + str(self.embryo_folder) + '"\n'
        parameter_content += 'EN = "' + str(self.name_embryo) + '"\n'
        ...
        print("Writing parameters file for " + self.astec_command + " on embryo " + self.embryo_folder)
        f = open(str(self.name_embryo) + "/" + parameters_name, "w+")
        f.write(parameter_content)
        f.close()
        commandrun = ""
        if self.keep_temp:
            commandrun = "cd " + self.name_embryo + " && " + " conda run -n " + self.astecenv + " " + final_command_arg0 + " -k -p " + parameters_name
        else:
            commandrun = "cd " + self.name_embryo + " && " + " conda run -n " + self.astecenv + " " + final_command_arg0 + " -p " + parameters_name
        os.system(commandrun)
        print("Command finished for " + self.astec_command + " on embryo " + self.embryo_folder)
        source_logs = []
        target_logs = []
        source_logs.append(
            os.path.join(compute_input_folder(self.astec_command, self.name_embryo, self.params_dict), "LOGS"))
        for inputfold in self.running_dir:
            target_logs = os.path.join(inputfold, "LOGS")
        self.copy_logs_files(source_logs, target_logs)
        os.system("rm " + str(self.name_embryo) + "/" + parameters_name)
...
class Manager:
    ...
    def compute_graphs_test_segmentation(self, embryo_name, begin, end):
        ...
        folder_out = os.path.join("./", embryo_name, os.path.join("analysis/", "test_segmentation"))
        lineages = [
            os.path.join("./", embryo_name) + "/POST/POST_test_maximum_gace/" + embryo_name + "_post_lineage.xml",
            os.path.join("./", embryo_name) + "/POST/POST_test_maximum_glace/" + embryo_name + "_post_lineage.xml",
            os.path.join("./", embryo_name) + "/POST/POST_test_addition_gace/" + embryo_name + "_post_lineage.xml",
            os.path.join("./", embryo_name) + "/POST/POST_test_addition_glace/" + embryo_name + "_post_lineage.xml"]
        ...
        apply_analysis(lineages, names, folder_out, embryo_name, mincells_test, maxcells_test, begin, end,
                       path=embryo_name + "_output_comparison.png", ref_lineage=ref_lineage, data_path=folder_out)
    ...
    def compute_graphs_post(self, embryo_name, begin, end,post_list):
        ...
        mincells_test = 64
        maxcells_test = 500
        folder_out = os.path.join("./", embryo_name, os.path.join("analysis/", "post_segmentation"))
        lineages = []
        names = []
        for exp_post in post_list:
            lineages.append( os.path.join("./", embryo_name) + "/POST/POST_" + str(exp_post) + "/" + embryo_name + "_post_lineage.xml")
    ...
    def plot_signal_to_noise(self, embryo_name, parameters):
        plotsignaltonoise(embryo_name, parameters)

    def plot_signal_to_noise_tofolder(self, folder,embryo_name):
        plotsignaltonoise_tofolder(folder,embryo_name)
    ...
    def generate_init_naming_parameters(self, cell_count, xml_folder, xml_file, embryo_name):
        ...
        txt += 'atlasFiles = []' + "\n"
        txt += "atlasFiles += ['" + str(
            os.path.join(atlas_path.replace("\n", "").replace(".", ""), "atlas/pm1.xml")) + "']" + "\n"
        txt += "atlasFiles += ['" + str(
            os.path.join(atlas_path.replace("\n", "").replace(".", ""), "atlas/pm3.xml")) + "']" + "\n"
        txt += "atlasFiles += ['" + str(
            os.path.join(atlas_path.replace("\n", "").replace(".", ""), "atlas/pm4.xml")) + "']" + "\n"
        txt += "atlasFiles += ['" + str(
            os.path.join(atlas_path.replace("\n", "").replace(".", ""), "atlas/pm5.xml")) + "']" + "\n"
        txt += "atlasFiles += ['" + str(
            os.path.join(atlas_path.replace("\n", "").replace(".", ""), "atlas/pm7.xml")) + "']" + "\n"
        txt += "atlasFiles += ['" + str(
            os.path.join(atlas_path.replace("\n", "").replace(".", ""), "atlas/pm8.xml")) + "']" + "\n"
        txt += "atlasFiles += ['" + str(
            os.path.join(atlas_path.replace("\n", "").replace(".", ""), "atlas/pm9.xml")) + "']" + "\n"
        txt += "check_volume=False" + "\n"
        f = open(os.path.join(os.path.join("./", embryo_name), parameters_name), "w+")
        f.write(txt)
        f.close()
        return parameters_name

    def generate_surface(self, exp_fuse, exp_post, begin, end, xmloutput, embryo_name, exp_intrareg):
        fuse_path = os.path.join(os.path.join("./", embryo_name),
                                 "INTRAREG/INTRAREG_" + str(exp_intrareg) + "/FUSE/FUSE_" + str(exp_fuse))
        fuse_template = os.path.join(fuse_path, embryo_name + "_fuse_t%03d.nii").replace("'", "").replace('"', '')
        post_path = os.path.join(os.path.join("./", embryo_name),
                                 "INTRAREG/INTRAREG_" + str(exp_intrareg) + "/POST/POST_" + str(exp_post))
        post_template = os.path.join(post_path, embryo_name + "_post_t%03d.nii").replace("'", "").replace('"', '')
        os.system(
            "conda run -n astec /Users/bgallean/Projects/GitHub/vt/build/bin/mc-cellProperties -fusion-format " + str(
                fuse_template) + " -segmentation-format " + str(post_template) + " -first " + str(
                begin) + " -last " + str(end) + " -o " + str(
                xmloutput) + " -feature contact-surface -feature barycenter")

        # cmd = "mc-cellProperties -fusion-format ./FUSE/FUSE_RELEASE/210714-Louis_fuse_t%03d.mha  -segmentation-format ./SEG/SEG_RELEASE/210714-Louis_seg_t%03d.mha -first 0 -last 199 -o output.xml -feature surface"

    def compute_atlas_path(self):
        path = ""
        dir = site.getsitepackages()[0]
        for directory in os.listdir(dir):
            if "astecmanager" in directory.lower():
                final_dir = os.path.join(dir, directory)
                if "egg-link" in final_dir:
                    f = open(final_dir, "r")
                    path = f.read()
                    f.close()
                else:
    ...
    def generate_prop_naming_parameters(self, xml_folder, xml_file, embryo_name):
        ...
        f.write(txt)
        f.close()
        return parameters_name,atlases_files
    ...
    def generate_init_naming(self, xml_folder, xml_file, begin_time_name, embryo_name, exp_fuse, exp_post, begin, end,
        ...
        os.system("cd ./" + str(embryo_name) + " && conda run -n astec-dev astec_atlas_init_naming -v -v -v -p " + str(
            parameter_file))
    ...
    def propagate_naming(self, xml_folder, xml_file, embryo_name):
        ...
        os.system("cd ./" + str(embryo_name) + " && conda run -n astec-dev astec_atlas_naming -v -v -v -p " + str(
            parameter_file))
        print("     - cleaning")
        os.system("rm " + str(parameter_file))
    ...
    def downscale_folder(self, parameters):
        ...
        omero_config_file = parameters["omero_config_file"]
        voxel_size = parameters["resolution"]
        user = compute_user(parameters)
        embryo_folder = os.path.join("." ,embryo_name)
    ...
    def upload_on_omero(self, config_file, omero_project_name, omero_dataset_name, input_folder, include_logs=False,embryo_name=None,user=None):
        ...
        metadata_xml = None
        if embryo_name is not None:
            embryo_folder = os.path.join("./",embryo_name)
            medata_xml = os.path.join(embryo_folder,"")
    ...
    def download_from_omero(self, parameters):
        config_file = parameters["omero_config_file"]
        omero_project_name = parameters["embryo_name"]
    ...
    def compute_contours(self, parameters):
        ...
        if omero_config_file is not None:
            contour_folder = os.path.join(os.path.join("./", embryo_name),
                                          "CONTOUR/CONTOUR_RELEASE_" + str(voxel_size).split(".")[1])
    ...
    def name_embryo(self, parameters):
        ...
        user = compute_user(parameters)
        lineage_path = os.path.join(os.path.join(embryo_name, "INTRAREG/INTRAREG_" + str(parameters["EXP_INTRAREG"])),
                                    "POST/POST_" + parameters["EXP_POST"] + "/")
    ...
    def downscale_mars(self, parameters):
        voxel_size = parameters["resolution"]
        embryo_folder = os.path.join(".",parameters["embryon"])
        user = compute_user(parameters)
    ...
    def test_fusion(self,parameters):
        ...
        self.start_fusion(parameters,run_now=False)
    ...
    def start_fusion(self,parameters,run_now=True):
        omero_config_file = parameters["omero_config_file"]
        embryo_name = parameters["embryo_name"]
        ...
        self.add_to_run("FUSE", ".", embryo_name, begin, real_parameters, end_time=end, compress_result=False,omero_result=(omero_config_file is not None),omero_config_file=omero_config_file)
        if run_now:
            self.start_running(thread_number=1)
        searcher = compute_user(parameters)
        embryo_dir = "./" + str(embryo_name)
        save_general_metadata_to_xml(embryo_dir, "fusion_user", "FUSE_"+real_parameters["EXP_FUSE"].replace('"','').replace("'",""), searcher,identifier_text="fusion_instance")
        save_general_metadata_to_xml(embryo_dir, "fusion_date", "FUSE_"+real_parameters["EXP_FUSE"].replace('"','').replace("'",""), datetime.now().strftime("%m/%d/%Y %H:%M:%S"),identifier_text="fusion_instance")
        if omero_config_file is not None:
            save_general_metadata_to_xml(embryo_dir, "fusion_upload_user",
                                         "FUSE_" + real_parameters["EXP_FUSE"].replace('"', '').replace("'", ""),
                                         searcher, identifier_text="fusion_instance")
            save_general_metadata_to_xml(embryo_dir, "fusion_upload_date", "FUSE_"+real_parameters["EXP_FUSE"].replace('"','').replace("'",""), datetime.now().strftime("%m/%d/%Y %H:%M:%S"),identifier_text="fusion_instance")
    ...
    def compute_fusion_movie(self,parameters):
        ...
        try:
            self.add_to_run("INTRAREG", ".", embryo_name, begin, real_parameters, end_time=end, compress_result=True)

            self.start_running(thread_number=1)

            self.compute_video_from_movie(embryo_name, real_parameters["EXP_INTRAREG"], real_parameters["EXP_FUSE"])
        except:
            print("Error during video computation: ", sys.exc_info()[0])
    def final_fusion(self, parameters):
        embryo_name = parameters["embryo_name"]
        real_parameters = self.start_fusion(parameters)
        self.compute_fusion_movie(real_parameters)

        try:
            self.plot_signal_to_noise(embryo_name, real_parameters)
        except:
            print("Error during signal plot: ", sys.exc_info()[0])
    ...
    def start_mars(self,parameters):
        ...
        try:
            ...
            print("Error  mars plot for add : ", sys.exc_info()[0])
    ...
    def start_segmentation(self,parameters,run_now=True):
        ...
        if omero_config_file is not None:
            save_general_metadata_to_xml(embryo_folder, "seg_upload_user",
                                         "SEG_" + real_parameters["EXP_SEG"].replace('"', '').replace("'", ""),
                                         user, identifier_text="segmentation_instance")
            save_general_metadata_to_xml(embryo_folder, "seg_upload_date", "SEG_"+real_parameters["EXP_SEG"].replace('"','').replace("'",""), datetime.now().strftime("%m/%d/%Y %H:%M:%S"),identifier_text="segmentation_instance")
    ...
    def start_post_correction(self,parameters,run_now=True):
        ...
        if omero_config_file is not None:
            save_general_metadata_to_xml(embryo_folder, "post_correction_upload_user",
                                         "POST_" + real_parameters["EXP_POST"].replace('"', '').replace("'", ""),
                                         user, identifier_text="post_correction_instance")
            save_general_metadata_to_xml(embryo_folder, "post_correction_upload_date", "POST_"+real_parameters["EXP_POST"].replace('"','').replace("'",""), datetime.now().strftime("%m/%d/%Y %H:%M:%S"),identifier_text="post_correction_instance")

        try :
            self.compute_graphs_post(embryo_name, begin, end)
        except :
            print("Error during computation of the Post correction graphs")
    ...
    def apply_intrareg(self,parameters):
        ...
        try:
            ...
            print("Error  during seg intrareg : ", sys.exc_info()[0])

        try:
            intra_folder = os.path.join(os.path.join("./", embryo_name),
                                        "INTRAREG/INTRAREG_" + str(parameters["EXP_INTRAREG"]) + "/POST/POST_" + str(
                                            parameters["EXP_POST"]))
            self.upload_on_omero(omero_config_file, embryo_name, "INT_POST_" + str(parameters["EXP_POST"]),
                                    intra_folder, include_logs=True)
        except:
            print("Error  during intrareg upload : ", sys.exc_info()[0])
        try:
            intra_fuse_folder = os.path.join(os.path.join("./", embryo_name), "INTRAREG/INTRAREG_" + str(
            ...
            print("Error  during intrareg fuse upload : ", sys.exc_info()[0])
        return real_parameters
    ...
    def compute_properties(self,parameters):
        ...
        try:
            ...
            print("Error  during seg properties : ", sys.exc_info()[0])
