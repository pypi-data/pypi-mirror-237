...
class S3API:
    ...
    def upload_items_for_select(self, file_name: str, item_list: [dict]):
        ...
        for idx, item in enumerate(item_list):
            ...
            if '_idx' in item:
                raise ValueError('item 에 _idx 필드는 예약되어있습니다.')
            item['_idx'] = idx
    ...
    def select(self, file_name, sql, idx_offset=0, limit=300):
        """
        ...
        """
        query = sql
        if 'S3Object s' not in query:
            raise error_util.SHOULD_USE_S3OBJECT_S_IN_QUERY
        if ';' in query:
            raise error_util.SHOULD_USE_S3OBJECT_S_IN_QUERY
        if idx_offset is None:
            raise error_util.IDX_OFFSET_MUST_BE_NUMBER
        if limit > 1000:
            raise error_util.LIMIT_MUST_LESS_EQUAL_1000
        input_serialization = {'JSON': {'Type': 'LINES'}}
        output_serialization = {'JSON': {'RecordDelimiter': '\n'}}
        query = make_query(query, idx_offset, limit)
        print(query)
        ...
        try:
            for event in response['Payload']:
                if 'Records' in event:
                    ...
                    items.extend(json.loads(record) for record in records.strip().split('\n') if record)
        except json.decoder.JSONDecodeError as ex:
            # 에러가 날시에는 limit 를 조정하여 다시 시도함.
            return self.select(file_name, sql, idx_offset, limit // 2)
        except Exception as ex:
            raise ex

        next_offset = items[-1]['_idx'] + 1 if items and len(items) == limit else None
        for item in items:
            if '_idx' in item:
                item.pop('_idx')
