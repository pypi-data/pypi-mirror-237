...
def get_clinvar_variants(page: fitz.Page) -> List[Dict[str, List[str]]]:
...
def get_variants(page: fitz.Page) -> List[Dict[str, List[str]]]:
    search1 = page.search_for('DNA Sequence Variants', hit_max=1)
    search2 = page.search_for('1 Based on Clinvar version', hit_max=1)
    search3 = page.search_for('Copy Number Variations', hit_max=1)
    ...
    if h9:
        ...
        headers = [GENE, VARIANT_AMINO_ACID_CHANGE, VARIANT_HGVS, VARIANT_COSMIC, VARIANT_POSITION,
                   VARIANT_ALLELE_FREQUENCY, VARIANT_ACCESSION, VARIANT_EFFECT]
    # set header_rows to 0 because no borderline above headers
    tab = pdf_util.parse_table_by_header(page, [0, search1[0].y1, 9999, bottom.y0], headers=header_pos, header_rows=0)
    return gen_variables(tab, headers)
...
def parse_variants(
        ...
        results: Dict[str, str | Dict[str, str] | List[Dict[str, str]]],
        doc: fitz.Document,
        func: Callable[[fitz.Page], List[Dict[str, List[str]]]],
    ...
    if all_variants:
        ...
        if VARIANT_COSMIC in keys:
            template = template + ' ' + template_cosmic
        if VARIANT_COVERAGE in keys:
            template = template + ' ' + template_coverage
        if CLIN_VAR in keys:
            template = template + ' ' + template_clinvar
    r = []
    for v in all_variants:
        r.append(substitute_variables(template, v))
    results['variant'] = r if r else None

    if maf:
        results['maf'] = maf.to_maf(pid, all_variants)

    if vcf:
        vcf.to_vcf('ONCOMINE', pid, all_variants, output_dir)


def get_cnv(page: fitz.Page, search1, search2) -> List[Dict[str, List[str]]]:
    ...
    cnv = gen_variables(tab, [GENE, VARIANT_POSITION, COPY_NUMBER])
    return list(filter(lambda i: check_cnv(i[VARIANT_POSITION]), cnv))


def parse_cnv(results: Dict[str, str | Dict[str, str] | List[Dict[str, str]]], doc: fitz.Document) -> None:
    # TODO:
    template_cnv = "In the chromosome {VARIANT_POSITION}, the genes {GENE} had a {COPY_NUMBER}-fold change in copy number."
    ...
    var = get_cnv(page, search1, search2)
    r = []
    for v in var:
        r.append(substitute_variables(template_cnv, v))
    results['cnv'] = r if r else None
...
def parse_brca(test_name: str, patient_id: str, maf: MAF, vcf: VCF, pdf_file_path: str, output_dir: str) -> None:
    (doc, results) = init(test_name, pdf_file_path, patient_id)
    parse_basic_info(patient_id, results, doc)
    parse_biomarkers(results, doc, template_onco_biomarker_sens, template_onco_biomarker_res, template_onco_biomarker_other)
    parse_variants(patient_id, results, doc, get_clinvar_variants,
                   template_onco_variant, template_onco_cosmic, template_onco_coverage, template_onco_clinvar,
                   output_dir, maf, vcf)
    parse_cnv(results, doc)

    write_json(results, output_dir, patient_id)


def parse_focus(test_name: str, patient_id: str, maf: MAF, vcf: VCF, pdf_file_path: str, output_dir: str) -> None:
    (doc, results) = init(test_name, pdf_file_path, patient_id)
    parse_basic_info(patient_id, results, doc)
    parse_biomarkers(results, doc, template_onco_biomarker_sens, template_onco_biomarker_res, template_onco_biomarker_other)
    parse_variants(patient_id, results, doc, get_variants,
                   template_onco_variant, template_onco_cosmic, template_onco_coverage, template_onco_clinvar,
                   output_dir, maf, vcf)
    parse_cnv(results, doc)
...
def parse_myeloid(test_name: str, patient_id: str, maf: MAF, vcf: VCF, pdf_file_path: str, output_dir: str) -> None:
    (doc, results) = init(test_name, pdf_file_path, patient_id)
    parse_basic_info(patient_id, results, doc)
    parse_biomarkers(results, doc, template_onco_biomarker_sens, template_onco_biomarker_res, template_onco_biomarker_other)
    parse_variants(patient_id, results, doc, get_variants,
...
def parse_tumor_mutation_load(test_name: str, patient_id: str, maf: MAF, vcf: VCF, pdf_file_path: str, output_dir: str) -> None:
    (doc, results) = init(test_name, pdf_file_path, patient_id)
    parse_basic_info(patient_id, results, doc)
    parse_biomarkers(results, doc, template_onco_biomarker_sens, template_onco_biomarker_res, template_onco_biomarker_other)
    parse_variants(patient_id, results, doc, get_variants,
...
def parse_oncomine(test_name: str, patient_id: str, maf: MAF, vcf: VCF, pdf_file_path: str, output_dir: str) -> None:
    pid = pdf_file_path[pdf_file_path.rfind('/') + 1: pdf_file_path.rfind('.')] \
        .replace('(', '') \
        .replace(')', '') if not patient_id else patient_id

    if 'BRCA' in test_name:
        parse_brca(test_name, pid, maf, vcf, pdf_file_path, output_dir)
    elif test_name == 'Oncomine Focus Assay':
        parse_focus(test_name, pid, maf, vcf, pdf_file_path, output_dir)
    elif test_name == 'Oncomine Myeloid Assay':
        parse_myeloid(test_name, pid, maf, vcf, pdf_file_path, output_dir)
    elif 'Tumor Mutation Load' in test_name:
        parse_tumor_mutation_load(test_name, pid, maf, vcf, pdf_file_path, output_dir)
    else:
