...
def parse_basic_info(pid: str, results: Dict[str, str | Dict[str, str] | List[Dict[str, str]]], doc: fitz.Document) -> None:
    template_report = "The report {REPORT_TEST_ASSAY} for the patient  {REPORT_PATIENT_ID}) is issued on {REPORT_DATE}. " \
                      "The diagnosis is {REPORT_DIAGNOSIS}."
...
def get_variant(page: fitz.Page, search1: fitz.IRect, search2: fitz.IRect) -> Dict[str, List[str]]:
...
def parse_variant(
    ...
    if page:
        ...
        all_variants = []
        for page, upper, lower in tables:
            var = get_variant(page, upper, lower)
            all_variants.append(var)
            r.append(substitute_variables(template_variant, var))

        if len(all_variants) > 0:
            print(f'Archer: {pid}')
            results['variant'] = r if r else None
            if maf:
                results['maf'] = maf.archer_to_maf(pid, all_variants)
            if vcf:
                vcf.archer_to_vcf('Archer', pid, all_variants, output_dir)
...
def parse_archer(
    ...
    parse_basic_info(pid, results, doc)
    parse_variant(pid, results, doc, output_dir, maf, vcf)
