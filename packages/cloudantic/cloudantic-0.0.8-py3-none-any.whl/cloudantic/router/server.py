...
from ..schema.json import jsonable_encoder
from ..schema.odm import DynaModel
...
class APIServer(Application):
    """
    Cloudantic APIServer class for handling API operations.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        schemas = DynaModel.__subclasses__()
        self.openapi = {
            "openapi": "3.1.0",
    ...
    def sse(
        ...
        import asyncio
        from Cloudantic import FaunaClient, APIServer, EventSourceResponse, to_json,q
    ...
    async def render_template(self, template: str, **kwargs: Any) -> Response:
        """Render a template using Jinja2"""
        template_ = self.env.get_template(template)
        return Response(
            text=await template_.render_async(**kwargs), content_type="text/html"
        )

    async def stream_template(self, template: str, **kwargs: Any) -> Response:
        """Render a template using Jinja2"""
        template_ = self.env.get_template(template)
        return Response(
            body=template_.generate_async(**kwargs), content_type="text/html"
        )


def component(
    func: Callable[P, Awaitable[Union[Dict[str, Any], BaseModel]]], *, env: Environment
) -> Callable[P, Awaitable[Response]]:
    @wraps(func)
    async def wrapper(*args: P.args, **kwargs: P.kwargs) -> Response:
        if func.__doc__ is None:
            raise ValueError("Component function must have a docstring")
        context = await func(*args, **kwargs)
        template = env.from_string(func.__doc__)
...
def stream_html(
    func: Callable[P, AsyncIterable[str]]
) -> Callable[P, Awaitable[Response]]:
    @wraps(func)
    async def wrapper(*args: P.args, **kwargs: P.kwargs) -> Response:
        async def stream():
        ...
        return Response(body=stream(), content_type="text/html")
