...
def 窗口宽度(修正值: int or _Callable = None) -> int:
    ...
    """
    if callable(修正值):
        return shutil.get_terminal_size().columns + 修正值()
    elif type(修正值) in [int, float]:
        return shutil.get_terminal_size().columns + int(修正值)
    else:
...
def 窗口高度(修正值: int or _Callable = None) -> int:
    ...
    """
    if callable(修正值):
        return shutil.get_terminal_size().lines + 修正值()
    elif type(修正值) in [int, float]:
        return shutil.get_terminal_size().lines + int(修正值)
    else:
    ...
    def 解析入参(self, 画板: 打印模板 = None):
        ...
        for 参数 in self._参数字典.values():
            if 'jsonCfg' != 参数.名称:
                ...
                if 简写:
                    if 简写 in 简写字典.keys():
                        # 如果这个简写符号重复了,则标记这个符号不可简写
                        简写字典[简写] = False
                    else:
                        # 如果这个简写符号尚不存在,则添加这个简写符号,并标记其可简写
                        简写字典[简写] = True
            ...
            if 参数.有效:
                if 参数.无值型:
                    if len(参数.名称) == 1:
                        ...
                        简写符: str = 参数.名称[0]
                        if 简写符 in 简写字典.keys() and 简写字典[简写符]:
                            # 如果这个简写符号存在于字典中,并且支持简写
        ...
        解析器.add_argument('匿名参数', nargs='*', help='请将所有匿名参数放在关键字参数前,或者后')  # 用于接收其它未明确的参数
