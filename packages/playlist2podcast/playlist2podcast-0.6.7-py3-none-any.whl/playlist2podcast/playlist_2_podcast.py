...
def main(
    ...
    for current_play_list in config.play_lists:
        ...
        videos_to_download = determine_videos_to_download(
            current_play_list, videos_list
        )
...
def add_episodes(
    ...
    for video in play_list_info:
        ...
        if number_episodes_added < config.number_of_episodes:
            local_audio_file = (
                f"{config.publish_dir}/{config.media_dir}/{video_id}.opus"
            )
            host_audio_file = (
                f"{config.podcast_host}/{config.media_dir}/{video_id}.opus"
            )
