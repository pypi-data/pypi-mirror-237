...
class Validate:
    ...
    def terragit_notify(self, message):
        ...
        url = (
            "https://gitlab.allence.cloud/api/v4/projects/"
            + str(project_id)
            + "/merge_requests/?state=merged&per_page=30&page=1"
        )
        ...
        payload = {
            "content": "The user "
            + str(GITLAB_USER_NAME)
            + " launch a "
            + message
            + "https://gitlab.allence.cloud"
            + "/"
            + str(CI_OPEN_MERGE_REQUESTS)
            + "\n"
            + data
            + "\n"
            + text
        ...
        if discord_api != None:
            ...
            res = requests.post(discord_api, data=json_message, headers=headers)
        else:
            print(
                self.bcolor.WARNING
