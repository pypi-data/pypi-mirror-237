...
class Member(User, RestModel, MetaDataModel):
    ...
    def block(self, reason, request=None):
        ...
        self.reportIncident(
            "account", f"account blocked: {reason}", level=2,
            request=request,
            details=reason)
    ...
    def GetWithNotification(perm, email_list=False):
        if type(perm) is list:
            queries = [Q(properties__category="notify", properties__key=p, properties__int_value=1) for p in perm]
            query = queries.pop()
            for item in queries:
                query |= item
            qset = Member.objects.filter(is_active=True).filter(query).distinct()
    ...
    @staticmethod
    def notifyWith(setting, subject, message=None, template=None, context=None, email_only=False, sms_msg=None, force=False):
        NotificationRecord = RestModel.getModel("account", "NotificationRecord")
        NotificationRecord.notify(Member.GetWithNotification(setting), subject, message, template, context, email_only, sms_msg, force)
