...
class TestGetFolder(AWSS3FolderTestCase, GetOneMixin):
    ...
    async def test_copy(self):
        ...
        async with self.client.get(href2, headers={hdrs.ACCEPT: nvpjson.MIME_TYPE}) as resp:
            expected = [{'bucket_id': 'arp-scale-2-cloud-bucket-with-tags11', 'created': '2022-05-17T00:00:00+00:00',
                         ...
                         'source': 'AWS Simple Cloud Storage (S3)', 'storage_class': 'STANDARD',
...
class TestDeleteProject(AWSS3ProjectTestCase, DeleteMixin):
    pass
...
class TestGetProject(AWSS3ProjectTestCase, GetOneMixin):
    ...
    async def test_copy(self):
        ...
        async with self.client.get(href2, headers={hdrs.ACCEPT: nvpjson.MIME_TYPE}) as resp:
            expected = [{'bucket_id': 'arp-scale-2-cloud-bucket-with-tags11', 'created': '2022-05-17T00:00:00+00:00',
                         ...
                         'modified': '2022-05-17T00:00:00+00:00', 'name': 'VGVzdEZvbGRlci9UZXN0UHJvamVjdDIv',
                         'owner': 'system|none',
                         's3_uri': 's3://arp-scale-2-cloud-bucket-with-tags11/TestFolder/TestProject2/', 'shares': [],
                         'source': 'AWS Simple Cloud Storage (S3)', 'type': 'heaobject.project.AWSS3Project'}]
            self._assert_equal_ordered(expected, await resp.json())
    ...
    async def test_not_project(self):
        ...
        async with self.client.get(href) as resp:
            if resp.status != 404:
                self.fail('The project API returned a folder')
