...
import logging
import asyncio
from .util import get_async_status, response_folder_as_zip
...
@routes.get('/volumes/{volume_id}/buckets/{bucket_id}/awss3folders/{id}/duplicatorasyncstatus')
...
@routes.get('/volumes/{volume_id}/buckets/{bucket_id}/awss3folders/{id}/updaterasyncstatus')
...
async def post_folder_duplicator_async(request: web.Request) -> web.Response:
    ...
    if not await _is_folder(request, volume_id, bucket_id, id_):
        return response.status_not_found()
    return await awsservicelib.copy_object_async(request, f'{request.url}status', activity_cb=publish_desktop_object)
...
async def _move(s3_client: S3Client, source_bucket_id: str, source_key: str, target_bucket_id: str, target_key: str):
    ...
    await awsservicelib._queued_processing(gen, item_processor)
