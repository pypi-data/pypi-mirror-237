...
from .util import response_folder_as_zip, get_async_status
        ...
        async with DesktopObjectActionLifecycle(request=request,
            ...
            async with aws.S3ClientContext(request=request, volume_id=volume_id) as s3_client:
                try:
                    ...
                    await _move(s3_client=s3_client, source_bucket_id=bucket_id, source_key=key, target_bucket_id=target_bucket_id, target_key=target_key)
...
async def post_project_duplicator_async(request: web.Request) -> web.Response:
    ...
    return await awsservicelib.copy_object_async(request, f'{request.url}status', activity_cb=publish_desktop_object, done_cb=update_metadata)
            ...
            try:
                ...
                async def objects():
                    async for obj in awsservicelib.list_objects(s3_client, bucket_id, prefix=object_key, loop=loop):
                        ...
                        if not awsservicelib.is_folder(obj['Key']) and obj.get("StorageClass") == S3StorageClass.STANDARD.value:
                            yield obj
                ...
                await awsservicelib._queued_processing(objects, get_links)
            except BotoClientError as e:
                activity.status = Status.FAILED
                return awsservicelib.handle_client_error(e)
            if not urls:
...
async def put_project(request: web.Request) -> web.Response:
    ...
    async def coro():
        ...
        if async_requested:
            sub = request.headers.get(SUB, NONE_USER)
            status_location = f'{request.url}/updaterasyncstatus'
            task_name = f'{sub}^{status_location}'
            await request.app[HEA_BACKGROUND_TASKS].add(coro(), name=task_name)
            return response.status_see_other(status_location)
        else:
...
async def post_new_project(request: web.Request) -> web.Response:
    ...
    if id_ == awsservicelib.ROOT_FOLDER.id:
        ...
        try:
            ...
            if not awsservicelib.is_folder(item_folder_id) or not await _is_project(request, volume_id, bucket_id, item_folder_id):
                item_folder_id = None
        except KeyDecodeException:
            item_folder_id = None
...
@routes.get('/volumes/{volume_id}/buckets/{bucket_id}/awss3projects/{id}/duplicatorasyncstatus')
...
@routes.get('/volumes/{volume_id}/buckets/{bucket_id}/awss3projects/{id}/updaterasyncstatus')
async def get_project_async_status(request: web.Request) -> web.Response:
    return await get_async_status(request)
...
async def _get_items_in_container(request: web.Request, volume_id: str, bucket_id: str, folder_id: str) -> web.Response:
    ...
    async with aws.S3ClientContext(request=request, volume_id=volume_id) as s3:
        ...
        async with DesktopObjectActionLifecycle(request,
            ...
            async with mongo.MongoContext(request=request, volume_id=volume_id) as mongo_client:
                try:
                    async for obj in awsservicelib.list_objects(s3, bucket_id=bucket_id, prefix=folder_key, loop=loop, delimiter='/'):
                        key = obj['Key'] if 'Key' in obj else obj['Prefix']
