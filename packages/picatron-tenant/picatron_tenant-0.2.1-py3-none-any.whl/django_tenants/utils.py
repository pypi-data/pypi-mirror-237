...
class tenant_context(ContextDecorator):
    ...
    def __enter__(self):
        ...
        self.previous = {
            'main_tenant': connection.main_tenant,
            'sub_tenant': connection.sub_tenant,
            'descendant_tenants': connection.descendant_tenants,
        }
        self.connection.set_main_tenant(self.tenant.get_root())
        self.connection.set_sub_tenant(self.tenant)
        self.connection.set_descendant_tenants(self.tenant.get_descendants(include_self=True))

    def __exit__(self, *exc):
        self.connection.set_main_tenant(self.previous.get('main_tenant'))
        self.connection.set_sub_tenant(self.previous.get('sub_tenant'))
        self.connection.set_descendant_tenants(self.previous.get('descendant_tenants'))
