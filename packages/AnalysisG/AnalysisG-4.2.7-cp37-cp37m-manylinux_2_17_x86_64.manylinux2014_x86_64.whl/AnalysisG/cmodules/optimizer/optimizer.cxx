...
namespace Optimizer
{
    ...
    void CyEpoch::add_kfold(int fold, std::map<std::string, data_t>* var)
    {
        ...
        for (; itr != var -> end(); ++itr){
            ...
            this -> merge_this(&(other -> mass_pred), &(self -> mass_pred)); 
            this -> merge_this(&(other -> mass_truth), &(self -> mass_truth)); 
        }
    } 
    ...
    std::map<std::string, metric_t> CyEpoch::metrics()
    {
        ...
        for (itr = this -> container.begin(); itr != this -> container.end(); ++itr){
            ...
            for (itd = data_map -> begin(); itd != data_map -> end(); ++itd){
                ...
                this -> merge_this(&atom_d -> mass_truth, &(output[name].mass_truth[var_n])); 
                atom_d -> mass_truth.clear(); 

                this -> merge_this(&atom_d -> mass_pred , &(output[name].mass_pred[var_n]));
                atom_d -> mass_pred.clear(); 
