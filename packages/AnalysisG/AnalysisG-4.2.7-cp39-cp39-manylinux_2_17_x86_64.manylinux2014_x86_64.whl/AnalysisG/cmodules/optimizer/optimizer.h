...
namespace Optimizer
{
    class CyEpoch
    {
        public:
            ...
            std::map<std::string, metric_t> compiled = {}; 
        ...
        private:
                    ...
                    std::map<int, std::vector<std::vector<float>>>* out)
            {
                ...
                for (itr = in -> begin(); itr != in -> end(); ++itr){
                    this -> merge_this(&(*in)[itr -> first], &(*out)[itr -> first]); 
                }
            }; 
                    ...
                    std::map<int, std::vector<float>>* out)
            {
                std::map<int, std::vector<std::vector<float>>>::iterator itr; 
                for (itr = in -> begin(); itr != in -> end(); ++itr){
                    std::vector<float>* vec = &((*out)[itr -> first]); 
                    for (unsigned int x(0); x < itr -> second.size(); ++x){
                        std::vector<float>* vec2 = &(itr -> second.at(x)); 
                    ...
                    std::string name)
            {
                ...
                float std_ = 0; 
                for (unsigned int x = 0; x < len; ++x){
                    std_ += std::pow((vec -> at(x))[0] - yhat, 2)/len; 
                }
                (*out)[name + "_average"] = yhat; 
                (*out)[name + "_stdev"] = std::pow(std_, 0.5);
                (*out)[name + "_up"] = up; 
                (*out)[name + "_down"] = down; 
            }; 
