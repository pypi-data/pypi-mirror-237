...
{
    std::string variable = ""; 
    std::map<int, std::vector<float>> pred = {}; 
    std::map<int, std::vector<float>> truth = {}; 
};
...
struct atomic_t
{
    ...
    std::map<std::string, mass_t>  masses = {};
...
struct epoch_t
{
    ...
    void append_masses(std::string mode, std::string kfold, metric_t* met){
        std::string fold = get_fold(kfold); 
        int mode_i = get_mode(mode); 
        atomic_t* atm = &(data[mode_i][fold]); 

        std::map<int, std::vector<float>>::iterator xtr; 
        std::map<std::string, std::map<int, std::vector<float>>>::iterator itr; 
        for (itr = met -> mass_truth.begin(); itr != met -> mass_truth.end(); ++itr){
            atm -> masses[itr -> first].variable = itr -> first; 
            for (xtr = itr -> second.begin(); xtr != itr -> second.end(); ++xtr){
                std::vector<float>* vec = &(atm -> masses[itr -> first].truth[xtr -> first]); 
                vec -> insert(vec -> end(), xtr -> second.begin(), xtr -> second.end()); 
            }
        }

        for (itr = met -> mass_pred.begin(); itr != met -> mass_pred.end(); ++itr){
            atm -> masses[itr -> first].variable = itr -> first; 
            for (xtr = itr -> second.begin(); xtr != itr -> second.end(); ++xtr){
                std::vector<float>* vec = &(atm -> masses[itr -> first].pred[xtr -> first]); 
                vec -> insert(vec -> end(), xtr -> second.begin(), xtr -> second.end()); 
            }
        }
    }
...
class CyMetric
{
    public:
        ...
        void BuildMasses(std::vector<abstract_plot>*);  
