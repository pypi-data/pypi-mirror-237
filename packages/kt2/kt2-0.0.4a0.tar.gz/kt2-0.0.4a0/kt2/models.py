...
class PersonMixin:
    ...
    def primary_email(self) -> str:
        return next(
            (contact.value for contact in self.telecom if contact.system == 'email'), ''
        )

    @property
    def first_name(self) -> str:
        return " ".join(next((humanname.given for humanname in self.name), ''))

    @property
    def middle_name(self) -> str:
        return " ".join(next((humanname.prefix for humanname in self.name), ''))

    @property
    def last_name(self) -> str:
        return next((humanname.family for humanname in self.name), '')
...
class KT2Patient(PersonMixin, Patient):
    """
...
class KT2Practitioner(PersonMixin, Practitioner):
    """
