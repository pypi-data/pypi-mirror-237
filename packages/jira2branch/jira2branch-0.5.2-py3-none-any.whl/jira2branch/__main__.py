    ...
    if secrets_file.exists():
        ...
        with secrets_file.open('w') as f:
            f.writelines(["[JIRA CREDENTIALS]\n\n",
                          ...
                          "# token = \n"])
        click.echo(
            f'Created empty secrets file under {secrets_file}, please configure it first')
        return None
...
def get_gitlab_credentials() -> Optional[GitLabCredentials]:
    secrets = GitLabCredentials()
    secrets_file_path = Path.home().joinpath('.j2b')
    secrets_file_name = 'gitlab_secrets.ini'
    secrets_file = secrets_file_path.joinpath(secrets_file_name)
    ...
    if secrets_file.exists():
        parser = ConfigParser()
        parser.read(secrets_file)
        gitlab_credentials_section = parser["GITLAB CREDENTIALS"]
        secrets.url = gitlab_credentials_section.get("url")
        secrets.token = gitlab_credentials_section.get("token")
    else:
        os.makedirs(secrets_file_path, exist_ok=True)
        secrets_file.touch()
        with secrets_file.open('w') as f:
...
def create_branch(branch_name, source_branch, push=False, target: Path = Path.cwd()):
    try:
        ...
        if not check_if_branch_exists(repo, branch_name):
            ...
            click.secho(
                'A remote branch by that name already exists... aborting', fg='green')
            raise Exception(
...
def create_merge_request(target: Path,
                         source_branch: str,
                         target_branch: str,
                         issue_id: str,
                         issue_url: str,
    ...
    if hasattr(project, 'merge_requests_template'):
        ...
        if mr_template:
            ...
            try:
                ...
                click.secho(
                    f"Failed to edit MR description, using defaults instead", fg='red')
    ...
    if merge_requests_for_issue and not dry_run:
        spinner.stop()
        click.secho(
            f'One or more pending MR already exist for this issue... aborting', fg='red')
        exit(1)
    ...
    if set_upstream_branch:
        ...
        if not pending_commits and not pending_merges:
            ...
            try:
                ...
                click.secho(
                    f'Something failed while trying to push...', fg='red')
                click.echo(err)
                exit(1)
...
def cli(issue_id_or_url,
    ...
    if name_only:
        ...
        if check_if_branch_exists(Repo(target), branch_name):
            click.secho(
                f'\nWARNING: a remote branch by that name already exists', fg='red')
    elif dry_run:
