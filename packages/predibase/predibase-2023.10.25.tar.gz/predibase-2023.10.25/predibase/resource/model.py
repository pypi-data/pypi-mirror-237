    ...
    def upload_model(self, model_path: str, dataset: Optional[Dataset] = None) -> "Model":
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for idx, model_weight_file in enumerate(model_weights_files):
                ...
                log_info(f"presigned_url {presigned_url}")
...
class Model:
    ...
    def deploy(self, name: str) -> "interface.LLMDeploymentJob":
        ...
        self.session.post_json(
            ...
            json={
                "name": name,
                "modelUri": f"pb://models/{self.repo.name}/{self.version}",
            },
        )

        return interface.LLMDeploymentJob(name, self.session)
    ...
    def _wait_until_ready(self, launch_tensorboard=False):
        ...
        :return: None
        """
        wait_until_model_ready(self._id, self.session, None, launch_tensorboard)

    def launch_tensorboard(self):
        self.session.get_model_logs(model_id=self._id)
        launch_tb(self._id)
...
def wait_until_model_ready(
    ...
    return session.get_json_until_with_logging(
        f"/models/version/{model_id}?withRuns={launch_tb_parameter}",
...
class ModelFuture:
    ...
    def get(
        ...
        resp = self._wait_for_ready(
            start_time=pd.to_datetime(resp["modelVersion"]["created"]),
    ...
    def _wait_for_ready(self, start_time=None, launch_tensorboard=False) -> Dict[str, Any]:
        return wait_until_model_ready(self.version_id, self.session, start_time, launch_tensorboard)
