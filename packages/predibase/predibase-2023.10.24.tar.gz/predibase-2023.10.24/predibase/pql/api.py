...
class Session:
    ...
    def get_model_logs(
        ...
        sync_event: Optional[Event] = None,
        parent_dir: [str] = "",
    ) -> None:
        ...
        Args:
            ...
            sync_event: The event to use to indicate when to stop syncing logs between the client and blob storage.
                    Use for actively training models.
        ...
        while len(logfiles) == 0:
            # Allow this loop to be stopped when running the function in a thread
            if sync_event is not None and not sync_event.is_set():
                break

            logfiles = self.get_model_logs_urls(model_id)
            ...
            # For models that are in a terminal state, we should try to grab the logs once so we don't infinite loop
            # This is particularly problematic for failed models where the assets don't exist
            if sync_event is None:
                break

            # If there are no logfiles, wait 30 seconds and try again
            if len(logfiles) == 0:
                time.sleep(30)
    ...
    def get_json_until_with_logging(
        ...
        while True:
            ...
            if curr_status == "training":
                ...
                if tensorboard_thread is None and run_id is not None:
                    ...
                    tensorboard_thread = Thread(
                        ...
                        args=[model_id, run_id, tensorboard_event],
                    )
            ...
            if training_pbar is not None and not training_pbar.disable:
                training_pbar.bar_format = "  " + curr_status.capitalize() + f"... {self._get_elapsed(start_time)}"
