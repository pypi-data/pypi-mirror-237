    ...
    for sym, trans_sym in transactions.groupby("symbol"):
        ...
        for dt, t in trans_sym.iterrows():
            ...
            split_info = bundle.adjustment_reader.load_adjustments(
                                dates=period,
                                assets=pd.Int64Index([trans_sym.symbol[0].sid]),
            ...
            if ((len(price_stack) == 0) or (copysign(1, price_stack[-1]) == copysign(1, t.amount))) and len(split_info) != 0:
                ...
                # 20230927用以紀錄每次split_ratio所得之股票(int(tmp_stack * split_ratio)加總每期所購買之股票(indiv_prices)
                tmp_stack = int(tmp_stack * split_ratio) + len(indiv_prices)
            ...
            # 20230927若一開始沒倉位(price_stack=0)或是最新的成本價與建倉數正負號相同(意味加倉)，則將最新一期的成本價加入price_stack中
                ...
                if (copysign(1, price_stack[-1]) != copysign(1, t.amount)) and len(split_info) != 0:
                    ...
                    tmp_stack = int(tmp_stack * split_ratio)

                pnl = 0
                invested = 0
                cur_open_dts = []

                if tmp_stack == len(price_stack): #20230927若累加過的股票數(含股票股利) 相等於 經由購買而得之總股票數則不需調整額外股票所得
                ...
                elif tmp_stack != len(price_stack): # 20230927若累加過的股票數(含股票股利) 不等於 經由購買而得之總股票數則需要調整額外股票所得

                    indiv_prices = [indiv_prices[-1]] * len(price_stack) # 20230927重新修正本次所能實際賣出之股票數，也就是調降為實際經由買入而得之總股數
                    ...
                    total_diff = tmp_stack - len(indiv_prices)

                    pnl += -(total_diff * price)  
