...
def plot_txn_time_hist(
    transactions, bin_minutes=5, tz="Asia/Taipei", ax=None, **kwargs   # 20230801 (by MRC)  "America/New_York"->Asia/Taipei 否則full tear sheet圖形出不來
):
    """
...
def dist_mae_gmfe_bmfe(mae:pd.DataFrame, ax1:None, ax2:None, ax3:None):    
    ...
    sns.histplot(mae, x=mae['MAE'].abs(), hue='profit', palette={'Profit': 'r', 'Loss': 'g'}, multiple='dodge', ax=ax1, label=[1,2], alpha=0.8)
    ...
    #===============================================================================================================================================================
    sns.histplot(mae, x=mae['GMFE'].abs(), hue='profit', palette={'Profit': 'r', 'Loss': 'g'}, multiple='dodge', ax=ax2, label=[1,2], alpha=0.8)
    ...
    #===============================================================================================================================================================
    sns.histplot(mae, x=mae['BMFE'].abs(), hue='profit', palette={'Profit': 'r', 'Loss': 'g'}, multiple='dodge', ax=ax3, label=[1,2], alpha=0.8)
    ...
    ax1.legend(handles=[handles[3], handles[0], handles[2], handles[1]], labels=['Profit',  f"P.Q3：{mae_q3_p:.2f}%", 'Loss', f"L.Q3：{mae_q3_l:.2f}%"])

    handles, labels = ax1.get_legend_handles_labels()
    ax2.legend(handles=[handles[3], handles[0], handles[2], handles[1]], labels=['Profit', f"P.Q3：{gmfe_q3_p:.2f}%", 'Loss', f"L.Q3：{gmfe_q3_l:.2f}%"])

    handles, labels = ax1.get_legend_handles_labels()
    ax3.legend(handles=[handles[3], handles[0], handles[2], handles[1]], labels=['Profit', f"P.Q3：{bmfe_q3_p:.2f}%", 'Loss', f"L.Q3：{bmfe_q3_l:.2f}%"])
