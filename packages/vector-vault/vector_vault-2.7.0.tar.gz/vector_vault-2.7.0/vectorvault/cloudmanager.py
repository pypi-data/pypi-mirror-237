...
class CloudManager:
    ...
    def rename_item(self, vault, old_item, new_item):
        old_path = self.cloud_name(vault, old_item, self.user, self.api, item=True)
        new_path = self.cloud_name(vault, new_item, self.user, self.api, item=True)
        old_meta_path = self.cloud_name(vault, old_item, self.user, self.api, meta=True)
        new_meta_path = self.cloud_name(vault, new_item, self.user, self.api, meta=True)
        ...
        if old_meta_blob.exists(self.storage_client):
            self.cloud.copy_blob(old_meta_blob, self.cloud, new_meta_blob.name)
    ...
    def delete_and_rename_all_items_after(self, item_id):
        """
        Deletes an item specified by item_id and renames all subsequent items 
        by decrementing their item_ids by one.
        """
        
        # Delete the specified item
        self.delete_item(self.cloud_name(self.vault, item_id, self.user, self.api, item=True))
        ...
        while True:
            ...
            if current_blob.exists(self.storage_client):
                ...
                self.rename_item(self.vault, current_item_id, next_item_id)
                current_item_id += 1
            else:
