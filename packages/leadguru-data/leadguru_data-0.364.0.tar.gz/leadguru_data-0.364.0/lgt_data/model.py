...
class DedicatedBotModel(BaseBotModel):
    def __init__(self):
        ...
        self.active_servers: List[ActiveServer] = []
        self.state = 0
        self.source: Source | None = None
    ...
    def to_dic(self):
        ...
        result['active_servers'] = [ActiveServer.to_dic(server) for server in self.active_servers]
        return result
    ...
    def from_dic(cls, dic: dict):
        ...
        model.source = Source.from_dic(dic.get("source"))
        model.active_servers = [ActiveServer.from_dic(server) for server in dic.get("active_servers", [])]
        return model
...
class ActiveServer:
    ...
    def from_dic(cls, dic: dict):
        ...
        model.active_channels = [ActiveChannel.from_dic(channel) for channel in dic.get("active_channels", [])]
        return model

    def to_dic(self):
        result = copy.deepcopy(self.__dict__)
        result['active_channels'] = [ActiveChannel.to_dic(channel) for channel in self.active_channels]
        return result
