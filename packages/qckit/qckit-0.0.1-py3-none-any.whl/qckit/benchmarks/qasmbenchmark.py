...
class QASMBenchmark:
    ...
    def __init__(self, path, category, num_qubits_list = None, remove_final_measurements = False, do_transpile = False, **transpile_args):
        ...
        if do_transpile and "backend" not in transpile_args:
            raise ValueError("`backend` must be provided for transpile")
    ...
    def items(self):
        return list(zip(self.circ_name_list, self.circ_list))
    ...
    def get(self, circ_name):
        ...
        Args:
            circ_name (int, str or list): The name of the circuit or a list of circuit names.
        ...
        if isinstance(circ_name, str):
            ...
            return self.get(self._circ_name_list[circ_name])
        else:
            raise TypeError
    ...
    def statevector(self, circ_name):
        from qiskit import Aer, QiskitError
        simulator = Aer.get_backend('aer_simulator_statevector')

        circ = self[circ_name]
        if isinstance(circ, list):
            return [self.statevector(c) for c in circ]
        else:
            circ.remove_final_measurements()
            circ.save_statevector()
            try:
                final_statevector = simulator.run(circ).result().get_statevector()
            except QiskitError:
...
if __name__ == "__main__":
    ...
    path = settings.path_qasmbench
    category = "small"
    num_qubits_list = list(range(3, 5))
    # num_qubits_list = list(range(8))
    remove_final_measurements = True
    
    bm = QASMBenchmark(path, category, num_qubits_list=num_qubits_list, remove_final_measurements=remove_final_measurements)
    # bm = QASMBenchmark(path, category, remove_final_measurements=remove_final_measurements)
    # print(bm)
    # print(len(bm))
    # print(bm.circ_list)
    # print(next(bm).draw("text"))
    # print(next(bm).draw("text"))
    # print(bm.get("adder_n4"))
    print(bm.get(["teleportation_n3", "adder_n4"]))
