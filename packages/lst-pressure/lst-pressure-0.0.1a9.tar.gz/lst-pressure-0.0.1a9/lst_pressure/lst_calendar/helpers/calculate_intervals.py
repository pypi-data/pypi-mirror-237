...
def calculate_AVOID_SUNRISE_SUNSET(start, end):
    if end < start:
        δ = 24 - start
        end = start + δ + end
    return (start, end)
...
def calculate_AVOID_SUNSET_SUNRISE(start, end):
    if end < start:
        δ = 24 - start
        end = start + δ + end
    return (start, end)
...
def calculate_NIGHT_ONLY(start, end):
    if end < start:
        δ = 24 - start
        end = start + δ + end
    return (start, end)
...
def calculate_OBSERVATION_WINDOW(start, end):
    if end < start:
        δ = 24 - start
        end = start + δ + end
    return (start, end)
...
def calculate_intervals(
    ...
    return (
        ...
        LSTInterval(
            *calculate_AVOID_SUNRISE_SUNSET(today_sunrise_lst, today_sunset_lst),
            today,
        ...
        LSTInterval(
            *calculate_AVOID_SUNSET_SUNRISE(today_sunset_lst, tomorrow_sunrise_lst),
            today,
        ...
        # NIGHT_ONLY
        LSTInterval(
            *calculate_NIGHT_ONLY(today_dusk_lst, tomorrow_dawn_lst),
            today,
        ...
        # ALL_DAY
        LSTInterval(0, 24, today, LSTIntervalType.ALL_DAY, today_sun, tomorrow_sun),
    )
