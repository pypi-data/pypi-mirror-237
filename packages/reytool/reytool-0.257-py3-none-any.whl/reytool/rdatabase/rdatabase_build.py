...
class RBuild(object):
    ...
    def build(
        ...
        for params in databases:
            ...
            # Exist.
            exist = self.rengine.build.exist((database, None, None))
            if exist: continue
        ...
        for params in tables:
            path = params["path"]
            if path.__class__ == str:
                database, table, column = self.rengine.extract_path(path)

            # Exist.
            exist = self.rengine.build.exist((database, table, column))
            if exist: continue
