...
public:
	...
	int Sreceivers_raw(i_t i, std::array<i_t, 8>& arr, std::array<f_t, 8>& arrdx, i_t& trec, f_t& tdx) const
	{
		...
		for (int j = 0; j < nn; ++j){
			arr[j] += i;
			f_t ttSS = (this->data->_surface[i] - this->data->_surface[arr[j]])/arrdx[j];
			if(ttSS>tSS){
				tSS = ttSS;
				trec = arr[j];
				tdx = arrdx[j];
