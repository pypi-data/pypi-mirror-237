...
_all__ = ["FoldWalshQLEP", "FoldWalshStateGenerator",
          "FoldWalshLeaderElectionAlgorithm",
          "FoldWalshMaliciousAttacker", "FoldWalsh"]


class FoldWalshQLEP(qlep.core.QuantumLeaderElectionProtocolwithPoS):
    r"""
    FoldWalshQLEP class for Fold-Walsh codes superposition
    of Walsh codes. It is a quantum leader election protocol.
    """
    ...
    def __init__(
        ...
        super().__init__(
            election_type=qlep.core.ElectionType.FOLDWALSH,
            no_nodes=no_nodes,
        ...
        if ((2 ** self.no_data_bits * self.fold) !=
                (self.quantum_no_nodes * self.fold)):
            logging.error("""[FoldWalshQLEP] no_nodes/committee size for
                          fold walsh must be 2^n * fold""")
    ...
    def _get_stake_quantum_circuits(
        ...
        return [FoldWalshStateGenerator.get_quantum_circuits(
    ...
    def get_leader_election_algorithm(
        ...
        return FoldWalshLeaderElectionAlgorithm(
            no_data_bits=self.no_data_bits,
            fold=self.fold,
            limit=self.limit
        )
    ...
    def get_malicious_attacker(self) -> qlep.core.MaliciousAttacker:
        return FoldWalshMaliciousAttacker(
            no_data_bits=self.no_data_bits,
            fold=self.fold,
            limit=self.limit
        )
...
class FoldWalshStateGenerator:
    r"""
    FoldWalshStateGenerator class for Fold-Walsh codes superposition.
    """
    @staticmethod
    def get_quantum_circuits(
        ...
        Args:
            ...
            fold :
                The number of folds in the Fold-Walsh codes superposition.
            limit :
                The maximum number of qubits connection for a node to a given

        Returns:
            quantum_circuit
                The quantum circuit for the Fold-Walsh state for the given
                number of data bits.
        """
        # the number of nodes
        no_nodes = 2 ** no_data_bits
        # the number of qubits = the number of no_nodes multiply by
        # the number of folds
        no_qubits = no_nodes * fold
        # the index matrix
        index_matrix = FoldWalsh.get_index_matrix(
        ...
        # generate data qubits superposition
        ...
        # generate firs segment of hamming code
...
class FoldWalshLeaderElectionAlgorithm(qlep.core.LeaderElectionAlgorithm):
    r"""
    FoldWalshLeaderElectionAlgorithm class for Fold-Walsh codes.
    """
    ...
    The number of data bits
    """
    fold_walsh_coder: FoldWalsh = None
    r"""
    The FoldWalsh object. The coder is used to encode and decode the
    ...
    def __init__(
        ...
        self.fold: int = fold
        self.limit: int = limit
        self.fold_walsh_coder: FoldWalsh = FoldWalsh(
            no_data_bits=self.no_data_bits,
            fold=self.fold,
            limit=self.limit
        )
    ...
    def elect(self, data: np.ndarray) -> int:
        ...
        # make multiple folds one long bitstirng
        bits_array.shape = self.fold * len(data[0, 0])
        leader_index = self.fold_walsh_coder.decode(bits_array)
        return leader_index


class FoldWalshMaliciousAttacker(qlep.core.MaliciousAttacker):
    r"""
    FoldWalshMaliciousAttacker class for Fold-Walsh codes.
    """
    ...
    def attack(
        ...
        if ((leader_index != -1) and
            ...
            differences: np.ndarray = self.fold_walsh_coder.differences(
                bits_array
            )
            ...
            for index in range(0, len(register_ids)):
                ...
                if register_ids[index] in malicious_ids:
                    if ((distance <=
                            self.fold_walsh_coder.correcting_distance) and
                            (distance < min_distance)):
                        change_leader = True
                        change_leader_index = index
                        min_distance = distance
                # if the election can be at least invalidate
                if (distance == (int(np.floor(
                        self.fold_walsh_coder.correcting_distance)) + 1)):
            ...
            if change_leader or invalidate_leader:
                ...
                for dataIndex in range(0, self.no_data_bits):
                    if (binary_representation[dataIndex] == '1'):
                        ...
                        temp[dataIndex] = 0
                # the encoded value
                to_send = self.fold_walsh_coder.encode(temp)
...
class FoldWalsh:
    r"""
    FoldWalsh class for Fold-Walsh codewords. It can be
    ...
    def __init__(
        ...
        self.no_nodes: int = 2 ** self.no_data_bits
        # the number of folds
        self.fold: int = fold
        # the number of final bits
        self.no_bits: int = self.no_nodes * self.fold
        # the distance
        self.distance: int = self.fold * int(np.floor(self.no_nodes / 2))
        # correcting distance
        ...
        self.index_matrix = FoldWalsh.get_index_matrix(
            no_data_bits=no_data_bits,
            fold=self.fold,
            limit=limit
        )
        ...
        for kindex in range(1, self.fold):
            ...
            for index in range(0, self.no_nodes):
                ...
                for data_index in range(0, self.no_data_bits):
                    ...
                    # from the first fold
                    # the index matrix use value from 1 to n inclusive,
                    # by subtracting 1 we go in the 0 to n-1 domain
                    self.encode_matrix[
                        data_index,
        ...
        self.decode_matrix: np.ndarray = np.zeros(
            shape=(self.no_nodes, self.no_bits),
            dtype=int
        )
        temp: np.ndarray = np.zeros(shape=self.no_data_bits)
        for index in range(0, self.no_nodes):
    ...
    def get_index_matrix(
        ...
        """
        no_nodes = 2 ** no_data_bits
        matrix = np.zeros(shape=(fold, no_nodes), dtype=int)
        maxValue = no_nodes
        if (fold == 2 and limit == 1):
            for index in range(0, no_nodes - 2):
                matrix[0, index] = index + 1
                matrix[1, index] = maxValue - 1 - (index + 1)
            matrix[0, no_nodes - 2] = no_nodes - 1
            matrix[1, no_nodes - 2] = maxValue
            matrix[0, no_nodes - 1] = maxValue
            matrix[1, no_nodes - 1] = no_nodes - 1
        return matrix
    ...
    def decode(self, bits_array: np.ndarray) -> int:
        ...
        """
        distances = np.zeros(shape=self.no_nodes, dtype=int)
        differences = self.differences(bits_array=bits_array)
        for index in range(0, self.no_nodes):
