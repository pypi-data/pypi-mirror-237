...
class TalismanAPIAdapter:
    def __init__(
        ...
        self.concept_candidate_fact_fields = ("id", "name")
    ...
    def _configure_output_document_fields(
        ...
        *,
        ...
        if with_candidate_facts:
            candidate_fact = doc_object.list_fact().__as__(ConceptCandidateFact)
            candidate_fact.__fields__(*self.concept_candidate_fact_fields)
            candidate_fact.concept_type().__fields__(*self.concept_type_fields)
            candidate_fact.list_concept().__fields__("id")
        if with_children:
    ...
    # @prettify
    def get_document(self, document_id: str, with_candidate_facts: bool = False) -> Document:
    ...
    def update_document_node(self, id: str, node_id: str, source: str, target: str, translation: str) -> Document:
        ...
        udn: Document = op.update_document_node(
            form=DocumentNodeUpdateInput(
                ...
                translation=TranslationInput(
                    language=LanguageInput(id=target),
                    text=translation
                )
            )
        )
    ...
    def update_platform(
        ...
        up: Platform = op.update_platform(
            form=PlatformUpdateInput(
                platform_id=platform_id, name=name, new_id=new_platform_id, platform_type=platform_type,
                url=url, country=country, language=language, markers=markers if markers else [],
