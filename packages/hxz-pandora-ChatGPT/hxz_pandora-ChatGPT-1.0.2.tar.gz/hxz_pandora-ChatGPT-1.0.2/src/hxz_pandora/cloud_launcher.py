...
def main():
    ...
    Console.debug_b(
        '''
            hxz_pandora-Cloud - A web interface to ChatGPT
            Github: https://github.com/shoot82003/hxz_pandora
    ...
    try:
        from hxz_pandora_cloud.server import ChatBot as CloudServer

        return CloudServer(args.proxy, args.verbose, login_local=args.local).run(args.server, args.threads)
    except (ImportError, ModuleNotFoundError):
        Console.error_bh('### You need `pip install hxz_pandora-ChatGPT[cloud]` to support cloud mode.')
