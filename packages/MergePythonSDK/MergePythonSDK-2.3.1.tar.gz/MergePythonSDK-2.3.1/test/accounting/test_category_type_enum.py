"""
    Merge Accounting API

    The unified API for building rich integrations with multiple Accounting & Finance platforms.  # noqa: E501

    The version of the OpenAPI document: 1.0
    Contact: hello@merge.dev
    Generated by: https://openapi-generator.tech
"""


import sys
import unittest
from unittest.mock import MagicMock

import MergePythonSDK.accounting
from MergePythonSDK.accounting.model.category_type_enum import CategoryTypeEnum
from MergePythonSDK.shared.api_client import ApiClient


class TestCategoryTypeEnum(unittest.TestCase):
    """CategoryTypeEnum unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def testCategoryTypeEnum(self):
        """Test CategoryTypeEnum"""
        # FIXME: construct object with mandatory attributes with example values
        # model = CategoryTypeEnum()  # noqa: E501

        # no tests for enums
        pass

if __name__ == '__main__':
    unittest.main()
