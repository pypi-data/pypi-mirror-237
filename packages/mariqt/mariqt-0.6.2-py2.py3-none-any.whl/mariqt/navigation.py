...
def smoothPositionsSliding2dMedianLatLonGaussianDepth(positions:miqtg.Positions,median_half_width:int=60,gauss_half_width:int=60):
	...
	for utc in time_points:
		...
		x,y = [],[]
		start_index = j
		timePointsInWindow_latlon, start_index = getPositionsInTimeRange(time_points,utc + (-1 * median_half_width * 1000 -1), utc + (median_half_width+1) * 1000, start_index)
		if not lat is None and not lon is None:
		...
		# Compute the variance of the new points towards all the contributing points
		if not new_dep is None:
			var_dep = 0.0
			k = 0
			for i in range(-gauss_half_width, gauss_half_width):
				for time_point in timePointsInWindowSections_dep[k]:
					if not positions[time_point].dep is None:
						dist = (positions[time_point].dep - new_dep) * (positions[time_point].dep - new_dep) * gauss[i]
						var_dep += dist 
				k += 1
			var_dep /= g_sum
			sd_dep = 0.0
			if var_dep != 0:
				sd_dep = math.sqrt(var_dep)
			past_sd_dep = {'utc':utc,'sd':sd_dep}
		elif not past_sd_dep['sd'] is None:
			sd_dep = past_sd_dep['sd'] + (utc - past_sd_dep['utc']) / 1000 * 0.05 # uncert increases with 0.05 m/s
		else: 
			sd_dep = None
		...
		if not sd_dep is None and not sd_latLon is None:
			sd = (sd_dep + sd_latLon) / 2
		else:
			sd = None

		# Store new positions
		smoothed_pos_tmp[utc] = miqtg.Position(utc,new_lat,new_lon,new_dep,hgt,sd)
		uncertainty[utc] = sd
