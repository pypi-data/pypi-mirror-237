...
def _generic_filter(filtering_dict, filter_list, current_path="", allow_mode=False):
    ...
    for key in filtering_dict.keys():
        path = "{current_path}.{key}".format(current_path=current_path, key=key) if current_path != "" else key

        if allow_mode and not check_path(path, filter_list):
            delete_list.append(key)
        elif not allow_mode and check_path(path, filter_list):
            delete_list.append(key)

        elif issubclass(type(filtering_dict[key]), dict) and filtering_dict[key] != {}:
            _generic_filter(filtering_dict=filtering_dict[key], current_path=path, filter_list=filter_list, allow_mode=allow_mode)
            ...
            if issubclass(type(next(iter(filtering_dict[key] or []), None)), dict):
                for item in filtering_dict[key]:
                    _generic_filter(filtering_dict=item, current_path=path, filter_list=filter_list, allow_mode=allow_mode)
...
def none_values_filter(filtering_dict, filter_list, current_path="", recursive=True, is_reversed=False):
    ...
    for key in filtering_dict.keys():
        ...
        if not is_reversed and check_path(path, filter_list) and filtering_dict[key] is None:
            delete_list.append(key)
        elif is_reversed and not check_path(path, filter_list) and filtering_dict[key] is None:
            delete_list.append(key)

        elif issubclass(type(filtering_dict[key]), dict) and filtering_dict[key] != {} and recursive:
            none_values_filter(filtering_dict=filtering_dict[key], current_path=path, filter_list=filter_list, recursive=True, is_reversed=is_reversed)
        elif issubclass(type(filtering_dict[key]), list) and recursive:
            if issubclass(type(next(iter(filtering_dict[key] or []), None)), dict):
                for item in filtering_dict[key]:
                    none_values_filter(filtering_dict=item, current_path=path, filter_list=filter_list, recursive=True, is_reversed=is_reversed)
