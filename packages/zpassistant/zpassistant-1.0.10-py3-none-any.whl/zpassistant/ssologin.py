...
class SsoLoginUtil:
    def __init__(self,sso_url = None):
        ...
        if os.path.exists(self.base_cookie_jar.filename):
            ...
            for cookie in self.base_cookie_jar:
    ...
    def ssologinByBrowser(self,url, checkFunction):
        ...
        session_cookie_jar = self.session.cookies
        ...
        for cookie in browser_cookies:
            ...
            if cookie["name"] in ["INNER_AUTHENTICATION"]:
                self.base_cookie_jar.set_cookie(http.cookiejar.Cookie(**cookie_dict))
    ...
    def ssoLoginByUserNamePassword(self,url, userName = None, password=None,type = None):
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 200:
                for cookie in session.cookies:
                    if "INNER_AUTHENTICATION" == cookie.name:
                        self.base_cookie_jar.set_cookie(cookie)
                        self.base_cookie_jar.save()
                return session
