...
class BaseSignalHandler(BaseHandler):
    ...
    def getSignals(self):
        ...
        logger.debug("Signal Data: {}".format(self.getData()))
        logger.debug("Policy Id: {}".format(self.getPolicyIdFromEnv()))
        logger.debug("Matched Signals: {}".format(signals))
        if not signals:
            logger.error("No signals found for data: {}.".format(self.getData()))
            return []
