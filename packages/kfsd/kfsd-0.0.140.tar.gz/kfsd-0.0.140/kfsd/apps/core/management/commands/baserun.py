...
class Command(BaseCommand):
    ...
    def __init__(self, config):
        self.config = config
        self.working_dir = DictUtils.get(self.config, "working_dir")
        self.service_id = DictUtils.get(self.config, "service_id")
    ...
    def handle(self, *args, **options):
        ...
        logger.info(
            "Running fixtures for env: {}, type: {}".format(self.env, self.type)
        )
        self.run()
