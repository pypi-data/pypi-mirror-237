...
"""
import os
from . import get_event_key_js,make_component
from abstract_utilities import eatAll
class AbstractBrowser:
    def __init__(self, window_mgr=None):
        self.window_mgr = window_mgr
        if self.window_mgr and self.window_mgr.window:
            self.window = self.window_mgr.window
        else:
            self.window = sg.Window('File/Folder Scanner', layout=self.get_scan_browser_layout())
        self.event = None
        self.values = None

        self.scan_mode = "all"
        self.history = [os.getcwd()]
        self.key_list = ["-FILES_BROWSER-","-DIR-","-DIRECTORY_BROWSER-","-FILES_LIST-","-SCAN_MODE_ALL-","-SELECT_HIGHLIGHTED-","-SCAN-", "-SELECT_HIGHLIGHTED-", "-MODE-", "-BROWSE_BACKWARDS-", "-BROWSE_FORWARDS-"]
    def handle_event(self,values,event,window):
        ...
        if self.event_key_js['found']:
            return self.while_static(self.window, self.event_key_js, self.values)
        else:
            return self.event, self.values
    def scan_it(self,directory):
        self.last_scan=os.path.isfile(self.values[self.event_key_js['-DIR-']])
        self.scan_results = self.scan_directory(directory, self.scan_mode)
        self.browse_update(key=self.event_key_js['-FILES_LIST-'],args={"values":self.scan_results})
    @staticmethod
    def get_scan_browser_layout(section=None,extra_buttons=[]):
        ...
        [make_component("Text",'Directory to scan:'), make_component("InputText",os.getcwd(),key=text_to_key(text='dir',section=section)),
         ...
         make_component("FileBrowse","Files", enable_events=True, key=text_to_key(text='file browser',section=section))],
        [make_component("Listbox",[],size=(50, 10),key=text_to_key(text='files list',section=section),enable_events=True),make_component("Checkbox",'all',default=True,key=text_to_key(text='scan mode all',section=section))],
        ...
        make_component("Button","->",key=text_to_key(text='browse forwards',section=section),enable_events=True)]+extra_buttons
        ]
    def while_static(self,window,event_key_js,values):
        self.values,self.window=values,window
        ...
        if self.event_key_js['found'] == "-SELECT_HIGHLIGHTED-":
            if len(self.values[self.event_key_js['-FILES_LIST-']])>0:
                self.browse_update(key=self.event_key_js['-DIR-'],args={"value":os.path.join(self.return_directory(), self.values[self.event_key_js['-FILES_LIST-']][0])})
        ...
        if self.event_key_js['found'] == "-BROWSE_BACKWARDS-":
            ...
            self.browse_update(key=self.event_key_js['-FILES_LIST-'],args={"values":self.scan_directory(directory, self.scan_mode)})
            self.scan_it(directory)
        if self.event_key_js['found'] in ["-BROWSE_FORWARDS-",'-FILES_LIST-']:
            # Navigate down into the selected directory or move to the next history path
            if self.values[self.event_key_js['-FILES_LIST-']]:  # If there's a selected folder in the listbox
                directory = os.path.join(self.return_directory(), self.values[self.event_key_js['-FILES_LIST-']][0])
