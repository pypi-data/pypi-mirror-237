...
from flowtask.exceptions import FlowTaskError, ConfigError
    ...
    def clone_repository(self):
        ...
        try:
            if self.use_ssh is False:
                self.git_url = self.git_url.replace('https://', f'https://{self.git_user}:{self.git_password}@')
                Repo.clone_from(
                    self.git_url,
                    self.path
                )
                ...
                if self.use_ssh is True:
                    ...
                    if len(repo.remote().refs) > 0:
                        ...
                        for remote_ref in repo.remote().refs:
                            print('REMOTE ', remote_ref.remote_head)
                            if remote_ref.remote_head == 'master' or remote_ref.remote_head == 'main':
                        ...
                        repo.git.branch('--set-upstream-to', f'origin/{upstream_branch}')
