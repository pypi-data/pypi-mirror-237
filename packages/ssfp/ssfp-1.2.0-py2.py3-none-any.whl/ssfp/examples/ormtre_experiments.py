...
from ssfp import fimtre as ormtre
...
def simsetup(N: int, TRs: list, alpha: float, npcs: list, sigma: float, avoid_sing: bool, short_df: bool) -> list:
    ...
    for TR, npcs0 in zip(TRs, npcs):
        ...
        if sigma == 0:
            ...
            SNR = np.sqrt(1/sig.size*np.sum(np.abs(sig.flatten())**2))/np.sqrt(1/n.size*np.sum(np.abs((n.flatten())**2)))
            #SNR = 1/sig.size*np.sum(np.abs(sig.flatten())**2)/(1/n.size*np.sum(np.abs(n.flatten())**2))
            SNR = 10*np.log10(SNR)
        #sig_power = np.sqrt(1/sig.size*np.sum(np.abs(sig.flatten())**2))
        #n_power = np.sqrt(1/n.size*np.sum(np.abs(n.flatten())**2))
        #n_power_new = sig_power/sigma
        #n *= n_power_new/n_power
        #SNR = sigma
...
def dfexp(N: int, TRs: list, alpha: float, sigma: float, use4=False, t1t2_cmp=True, disp=True, short_df=False):
    ...
    if t1t2_cmp and disp:
        ...
        phi = ormtre(s0.I, s1.I, s0.TR, s1.TR, rad=True)
        ...
        theta = ormtre(s0.I, s1.I, s0.TR, s1.TR, rad=False)
    ...
    if not disp:
        # do offres estimate using two gs_recon solutions
        theta = ormtre(s0.I, s1.I, s0.TR, s1.TR)
...
if __name__ == '__main__':
    ...
    plt.subplot(nx, ny, 3)
    plt.semilogy(df_err[0, :], 'k:', label=f'PLANET (FA={alpha}$^o$)', linewidth=1.5)
    plt.semilogy(df_err[2, :], 'r-', label=f'Multi-TR (FA={alpha}$^o$)', linewidth=1.5)
    plt.semilogy(df_err_lo_alpha[0, :], 'k-.', label=f'PLANET (FA={alpha_lo}$^o$)', linewidth=1.5)
    plt.semilogy(df_err_lo_alpha[2, :], 'r--', label=f'Multi-TR (FA={alpha_lo}$^o$)', linewidth=1.5)
