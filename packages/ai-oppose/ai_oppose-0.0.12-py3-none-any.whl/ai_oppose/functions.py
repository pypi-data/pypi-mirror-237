            ...
            if not use_google:
                try:
                    ...
                    if new_ref["Abstract Note"] == "":
                        ...
                        new_ref = get_abstract_from_google(dict(df.loc[i,:]), bias = ["pubmed", "springer"], serp_key=serp_key)
...
def get_abstract_from_google(ref: dict, bias: list, query_only = False, serp_key = None)-> any:
    ...
    if query_only:
        ref_dicts = []
        urls = search_with_backoff(f"""{ref["query"]}""", num_results = 10, serp_key = serp_key)
        print(f"urls: {urls}")
        if urls == None:
            return None
        for url in urls:
