...
def get_abstract_from_google(ref: dict, bias: list, query_only = False, serp_key = None)-> any:
    ...
    if query_only:
        ref_dicts = []
        urls = search_with_backoff(f"""{ref["query"]}""", num_results = 2, serp_key = serp_key)
        print(f"urls: {urls}")
        if urls == None:
            return None
        for url in urls:
        ...
        if url:
            print("validated url")
            print(url)
            print(clean_string(ref["Title"]))
            print(clean_string(ref["Author"], min_word_length=2))
            t = extract_abstract(url, title = clean_string(ref["Title"]), authors = clean_string(ref["Author"], min_word_length=2))
            print(f"t:: {t}")
            ref["Abstract Note"] = t
