...
class ReplayManager:
    ...
    def register_message(
        ...
        with self.__lock__:
            ...
            if isinstance(message, UploadInMemoryMessage):
                message = convert_upload_in_memory_to_file_message(
                    message, self.tmp_dir
                )

            LOGGER.debug(
                "Register message, id: %d, message: %r",
                message.message_id,
    ...
    def update_message(self, message_id: int, status: MessageStatus) -> None:
        with self.__lock__:
            ...
            with self.conn:
                ...
                LOGGER.debug(
                    "Update message status, id: %d, status: %r", message_id, status
                )

            # delete saved message callbacks
            if (
                status == MessageStatus.delivered
                and message_id in self.message_callbacks
            ):
    ...
    def get_message(self, message_id: int) -> BaseMessage:
        db_message = self.get_db_message(message_id)
        return db_message_to_message(db_message)

    def get_db_message(self, message_id: int) -> DBMessage:
        with self.conn:
            c = self.conn.execute(
