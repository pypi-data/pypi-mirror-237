...
from comet_ml.system.network.network_logging import (
    NetworkLoggingThread,
    is_network_info_available,
)
...
from .system.cpu.cpu_logging import CPULoggingThread, is_cpu_info_available
from .system.disk.disk_utilization_logging import (
    DiskUtilizationLoggingThread,
    is_disk_info_available,
)
...
class BaseExperiment(CommonExperiment):
    ...
    def __init__(
        ...
        self.network_monitor_thread = None  # type: Optional[NetworkLoggingThread]
        self.disk_utilization_thread = (
            None
        )  # type: Optional[DiskUtilizationLoggingThread]
    ...
    def _on_end(self, wait=True):
        ...
        successful_clean = self._close_system_metrics_logging_threads(wait)
    ...
    def _close_system_metrics_logging_threads(self, wait: bool) -> bool:
        ...
        for thread in logger_threads:
            ...
            if wait is True:
                ...
                if thread.is_alive():
                    LOGGER.debug("%s didn't clean successfully after 2s", thread.name)
                    successful_clean = False
                else:
                    LOGGER.debug("%s cleaned successfully", thread.name)
        ...
        if self.log_env_details:
            ...
            try:
                if self.log_env_network:
                    if is_network_info_available():
                        self._start_network_monitor_thread()
                    else:
                        LOGGER.debug(
                            "Network details unavailable, don't start the network logging thread"
                        )
            except Exception:
                LOGGER.error(
                    "Failed to start the network rate logging thread", exc_info=True
                )

            try:
                if self.log_env_disk:
                    if is_disk_info_available():
                        self._start_disk_utilization_monitor_thread()
                    else:
                        LOGGER.debug(
                            "Disk utilization details unavailable, don't start the disk utilization logging thread"
                        )
            except Exception:
                LOGGER.error(
                    "Failed to start the disk utilization logging thread", exc_info=True
                )
    ...
    def log_remote_model(
        ...
        """
        Logs the model data from the URI provided under the name of the model.
    ...
    def _start_gpu_thread(self):
        ...
        self.gpu_thread = GPULoggingThread(
            self.config["comet.system_gpu_interval"], self._log_gpu_details
        )
    ...
    def _start_network_monitor_thread(self):
        ...
        self.network_monitor_thread = NetworkLoggingThread(
            initial_interval=self.config["comet.system_network_interval"],
            callback=self._log_network_rate,
        )
        self.network_monitor_thread.start()

    def _start_disk_utilization_monitor_thread(self):
        if not self.alive:
            return

        self.disk_utilization_thread = DiskUtilizationLoggingThread(
