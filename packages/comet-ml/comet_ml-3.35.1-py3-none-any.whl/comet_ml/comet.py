...
class Streamer(BaseStreamer):
    ...
    def __init__(
        ...
        self.on_message_sent_callback = None  # type: Optional[OnMessageSentCallback]
        ...
        def _callback(response):
            if (
                ...
                self._on_messages_sent(
                    [message_id], success=False, connection_error=True
                )
    ...
    def _send_ws_message(self, message: Union[WebSocketMessage, BaseMessage]) -> None:
        ...
        try:
            ...
            self._on_messages_sent(
                messages_ids=[message.message_id], success=False, connection_error=True
            )
            # report experiment error
    ...
    def _process_rest_api_send(
        ...
        **kwargs
        ...
        try:
            ...
            self._on_messages_sent(messages_ids, success=False, connection_error=True)
    ...
    def _on_messages_sent(
        ...
        if self.on_message_sent_callback is not None:
            for message_id in messages_ids:
                self.on_message_sent_callback(message_id, success, connection_error)
    ...
    def _process_register_model_message(self, message: RegisterModelMessage) -> None:
        try:
            ...
            self.on_message_sent_callback(message.message_id, True, False)
            ...
            self.on_message_sent_callback(message.message_id, False, True)
...
class FallbackStreamer(BaseStreamer):
    ...
    def __init__(
        ...
        self.online_streamer.on_message_sent_callback = self._track_message
    ...
    def _track_message(self, message_id: int, delivered: bool, connection_error: bool):
        if connection_error:
            self.server_connection_monitor.has_server_connection = False
    ...
    def put_message_in_q(self, message) -> None:
        with self.__lock__:
            ...
            if message is not None:
                # update message_id to be the same for both streamers
                self._counter += 1
                message.message_id = self._counter
                ...
                if self.has_connection_to_server():
                    ...
                    self.replay_manager.register_message(
    ...
    def close(self) -> None:
        with self.__lock__:
            ...
            # mark as closed to block any new message while online/offline streamers are closing
            self.closed = True
    ...
    def wait_for_finish(
        ...
        # Close replay manager after online streamer finish
