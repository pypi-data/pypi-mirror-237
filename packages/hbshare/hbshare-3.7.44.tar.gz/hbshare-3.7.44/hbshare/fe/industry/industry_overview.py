...
class IndustryOverview:
    def __init__(self, industry_name, start_date, end_date, report_date, data_path, sub_remove=True):
        ...
        self.industry_universe = pd.read_excel('{0}{1}/{2}_信息.xlsx'.format(self.data_path, self.industry_name, self.industry_name), sheet_name='股票池-v1')
    ...
    def load(self):
        ...
        for date in report_df['REPORT_DATE'].unique().tolist():
            ...
            stock_finance_date = stock_finance_date[['END_DATE', 'PUBLISH_DATE', 'TICKER_SYMBOL', 'SEC_SHORT_NAME', 'OPER_REVENUE_YOY', 'NET_PROFIT_YOY', 'ROE_TTM', 'EPS_TTM', 'NAPS', 'OCF_TTM', 'DIVIDEND_TTM', 'ROA_TTM', 'ROIC_TTM', 'GrossIncomeRatio'.upper(), 'NetProfitRatio'.upper(), 'NPParentCompanyYOY'.upper(), 'NPParentCompanyCutYOY'.upper(), 'OperProfitGrowRate'.upper(), 'TotalAssetGrowRate'.upper()]] if len(stock_finance_date) > 0 else pd.DataFrame(columns=['END_DATE', 'PUBLISH_DATE', 'TICKER_SYMBOL', 'SEC_SHORT_NAME', 'OPER_REVENUE_YOY', 'NET_PROFIT_YOY', 'ROE_TTM', 'EPS_TTM', 'NAPS', 'OCF_TTM', 'DIVIDEND_TTM', 'ROA_TTM', 'ROIC_TTM', 'GrossIncomeRatio'.upper(), 'NetProfitRatio'.upper(), 'NPParentCompanyYOY'.upper(), 'NPParentCompanyCutYOY'.upper(), 'OperProfitGrowRate'.upper(), 'TotalAssetGrowRate'.upper()])
            star_stock_finance_date = HBDB().read_star_stock_finance_jy2(date)
            star_stock_finance_date = star_stock_finance_date[['END_DATE', 'PUBLISH_DATE', 'TICKER_SYMBOL', 'SEC_SHORT_NAME', 'OPER_REVENUE_YOY', 'NET_PROFIT_YOY', 'ROE_TTM', 'EPS_TTM', 'NAPS', 'OCF_TTM', 'DIVIDEND_TTM', 'ROA_TTM', 'ROIC_TTM', 'GrossIncomeRatio'.upper(), 'NetProfitRatio'.upper(), 'NPParentCompanyYOY'.upper(), 'NPParentCompanyCutYOY'.upper(), 'OperProfitGrowRate'.upper(), 'TotalAssetGrowRate'.upper()]] if len(star_stock_finance_date) > 0 else pd.DataFrame(columns=['END_DATE', 'PUBLISH_DATE', 'TICKER_SYMBOL', 'SEC_SHORT_NAME', 'OPER_REVENUE_YOY', 'NET_PROFIT_YOY', 'ROE_TTM', 'EPS_TTM', 'NAPS', 'OCF_TTM', 'DIVIDEND_TTM', 'ROA_TTM', 'ROIC_TTM', 'GrossIncomeRatio'.upper(), 'NetProfitRatio'.upper(), 'NPParentCompanyYOY'.upper(), 'NPParentCompanyCutYOY'.upper(), 'OperProfitGrowRate'.upper(), 'TotalAssetGrowRate'.upper()])
        ...
        self.stock_consensus.to_hdf('{0}{1}/stock_consensus_{2}_{3}.hdf'.format(self.data_path, self.industry_name, type, self.industry_name), key='table', mode='w')
        self.stock_consensus = pd.read_hdf('{0}{1}/stock_consensus_{2}_{3}.hdf'.format(self.data_path, self.industry_name, type, self.industry_name), key='table')
...
class IndustryFinance:
    ...
    def get_all(self):
        if self.industry_name == '动力电池':
            from hbshare.fe.industry.industry_finance.donglidianchi import IndustryFinanceData
            IndustryFinanceData(self.industry_name, self.start_date, self.end_date, self.report_date, self.data_path).get_all()
        if self.industry_name == '光伏':
            from hbshare.fe.industry.industry_finance.guangfu import IndustryFinanceData
            IndustryFinanceData(self.industry_name, self.start_date, self.end_date, self.report_date, self.data_path).get_all()
        return
...
if __name__ == '__main__':
    ...
    sub_remove = False
    IndustryOverview(industry_name, start_date, end_date, report_date, data_path, sub_remove).get_all()
    StockOverview(industry_name, start_date, end_date, report_date, data_path).get_all()
    IndustryFinance(industry_name, start_date, end_date, report_date, data_path).get_all()

    # import openpyxl
    # import os
    # data_path = 'D:/Git/hbshare/hbshare/fe/xwq/data/industry_overview/'
    # wb = openpyxl.load_workbook('{0}申万个股行业分类.xlsx'.format(data_path))
    # sheets = wb.sheetnames
    # for industry_name in sheets:
    #     print(industry_name)
    #     if not os.path.exists(data_path + industry_name):
    #         os.makedirs(data_path + industry_name)
