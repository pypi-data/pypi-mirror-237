...
class IndustryFinanceData:
    def __init__(self, industry_name, start_date, end_date, report_date, data_path):
        ...
        self.industry_universe_HK['TICKER_SYMBOL'] = self.industry_universe_HK['TICKER_SYMBOL'].apply(lambda x: x.zfill(5))
    ...
    def load(self):
        ...
        self.stock_daily_k = self.stock_daily_k[self.stock_daily_k['TICKER_SYMBOL'].isin(self.industry_universe_A['TICKER_SYMBOL'].unique().tolist())]
        ...
        self.stock_bs = self.stock_bs[self.stock_bs['TICKER_SYMBOL'].isin(self.industry_universe_A['TICKER_SYMBOL'].unique().tolist())]
        self.stock_is = self.stock_is[self.stock_is['TICKER_SYMBOL'].isin(self.industry_universe_A['TICKER_SYMBOL'].unique().tolist())]
        self.stock_cf = self.stock_cf[self.stock_cf['TICKER_SYMBOL'].isin(self.industry_universe_A['TICKER_SYMBOL'].unique().tolist())]
    ...
    def get_all(self):
        ...
        industry_finance.to_excel(writer, sheet_name=self.industry_name)
        data_list = []
        ...
        data.to_excel(writer, sheet_name='相关个股')
        writer.save()
        return
