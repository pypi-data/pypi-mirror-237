    ...
    def get_location_ids_by_profile_id(profile_id: int, limit: int = 1, datetime_range: tuple = None) -> List[Location]:
        ...
        if datetime_range is None:
            ...
            if is_valid_date_range(datetime_range):
                where_clause = f"profile_id = {profile_id} AND updated_timestamp BETWEEN '{datetime_range[0]} 00:00:00' AND '{datetime_range[1]} 23:59:59'"
            else:
                raise ValueError(
                    "Invalid time_range format. It should be 'YYYY-MM-DD'.")

        location_ids = LocationProfiles.select_multi_by_where(LocationProfiles(
        ), view_table_name="location_profile_view", select_clause_value="location_id", where=where_clause, limit=limit, order_by="updated_timestamp desc")
