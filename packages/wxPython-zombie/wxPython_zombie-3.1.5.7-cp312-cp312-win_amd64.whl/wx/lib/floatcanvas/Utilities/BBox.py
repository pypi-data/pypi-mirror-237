...
def fromPoints(Points):
    ...
    """
    Points = N.asarray(Points, N.float_).reshape(-1,2)
...
def fromBBArray(BBarray):
 ...
 #   BBarray = N.asarray(BBarray, N.float_).reshape(-1,2)
 #   arr = N.vstack( (BBarray.min(0), BBarray.max(0)) )
    BBarray = N.asarray(BBarray, N.float_).reshape(-1,2,2)
...
def NullBBox():
    ...
    arr = N.array(((N.nan, N.nan),(N.nan, N.nan)), N.float_)
    return N.ndarray.__new__(BBox, shape=arr.shape, dtype=arr.dtype, buffer=arr)
...
def InfBBox():
    ...
    arr = N.array(((-N.inf, -N.inf),(N.inf, N.inf)), N.float_)
    return N.ndarray.__new__(BBox, shape=arr.shape, dtype=arr.dtype, buffer=arr)
