...
class Circle(XYObjectMixin, LineAndFillMixin, DrawObject):
    ...
    def __init__(self, XY, Diameter,
        ...
        self.XY = N.array(XY, N.float_)
        self.WH = N.array((Diameter/2, Diameter/2), N.float_) # just to keep it compatible with others
        self.CalcBoundingBox()
...
class Arc(XYObjectMixin, LineAndFillMixin, DrawObject):
    ...
    def __init__(self,
        ...
        self.XY = N.asarray( XY, N.float_).reshape((2,))
        self.WH = N.asarray( WH, N.float_).reshape((2,))

        self.StartXY = N.asarray(StartXY, N.float_).reshape((2,))
        self.CenterXY = N.asarray(CenterXY, N.float_).reshape((2,))
        self.EndXY = N.asarray(EndXY, N.float_).reshape((2,))
...
class PieChart(XYObjectMixin, LineOnlyMixin, DrawObject):
    ...
    def __init__(self,
        ...
        self.XY = N.asarray(XY, N.float_).reshape( (2,) )
        self.Diameter = Diameter
        self.Values = N.asarray(Values, dtype=N.float_).reshape((-1,1))
    ...
    def SetValues(self, Values):
        ...
        """
        Values = N.asarray(Values, dtype=N.float_).reshape((-1,1))
        self.Values = Values
        self.CalculatePoints()
    ...
    def _Draw(self, dc , WorldToPixel, ScaleWorldToPixel, HTdc=None):
        ...
        WH = N.array((Diameter,Diameter), dtype = N.float_)
