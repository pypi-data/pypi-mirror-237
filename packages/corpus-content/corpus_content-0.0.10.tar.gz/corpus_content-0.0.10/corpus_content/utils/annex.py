...
class Footnote(NamedTuple):
    ...
    @classmethod
    def slice_by(cls, text: str) -> Iterator[str]:
        """Using a given text, get possible passages based
        on the footnote sentence pattern."""
        while True:
            if match := footnote_sentence_pattern.search(text):
                if partial := text[: match.end()].strip():
                    yield partial
                text = text[match.end() :]
            else:
