...
class Rabbit():
    ...
    def __on_connection_closed(self, connection, reply_code, reply_text):
        ...
        if self.reconnect:
            ...
            logging.debug(
                f'Será tentado reconectar em {str(self.reconnect_delay)} segundos, a conexão foi fechada pelo erro: {str(reply_code)} - {str(reply_text)} ')
            self.SelectConn.ioloop.call_later(
                self.reconnect_delay, self.reconnect)
