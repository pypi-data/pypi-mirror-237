...
from zipline.errors import (
    ...
    ZeroCapitalError,
    IllegalValueException,           # 20230804 (by MRC)
    RegisterTradingPolicyPostInit,   # 20230804 (by MRC)
)
...
from zipline.finance.controls import (
    ...
    MinLeverage,
    RestrictedListOrder,
    ExcludeIlliquidAsset    # 20230804 (by MRC)
)
...
from zipline.sources.TEJ_Api_Data import PandasRequestsTEJ_API, LIQUIDITY_RISK_COLUMNS  # 20230210 (by MRC)
...
""".strip().format(
    ...
    def _create_clock(self):
        ...
        # Changed this to local timezone.
        '''
    ...
    def _create_daily_stats(self, perfs):
        ...
        #daily_dts = make_utc_aware(daily_dts)  #20230213 (by MRC) for timezone in perf
        daily_dts = daily_dts.tz_convert(tz=tz) #20230213 (by MRC) for timezone in perf

        daily_stats = pd.DataFrame(daily_perfs, index=daily_dts)

        #--------------------------------------------------------------------
        #20230213 (by MRC) for timezone in perf #start#        
        ...
        #20230213 (by MRC) for timezone in perf #end#  
        #--------------------------------------------------------------------
    ...
    #-----------------------------------------------------------------------------
    #20230804 (by MRC) TEJ_Api_data_source #start#  
    @api_method
    def fetch_tej_api(
        self,
        columns,
    ...
    # 20230831 (by MRC) 了解在order_target_percent下會需要購買多少amount的股票
    @api_method
    @disallowed_in_before_trading_start(OrderInBeforeTradingStart())
    def calculate_order_target_percent_amount(self, asset, target):
        """Calculate order target percent amount
        Parameters
        ----------
        asset : Asset
            The asset that this order is for.
