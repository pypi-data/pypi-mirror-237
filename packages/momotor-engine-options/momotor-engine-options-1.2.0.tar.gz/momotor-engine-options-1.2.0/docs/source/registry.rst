...
To configure a checklet to use a certain tool version, two options are needed: the :py:ref:`tools@scheduler option`
to indicate to the scheduler which tools are required by the checklet, and options in the
:py:ref:`tools domain <tools domain>` to define the actual tool versions to use.

Registry functions and classes
------------------------------
