    ...
    try:
        ...
        try:
            ...
            if args.part in ['all', 'collab']:
                try:
                    import mldev_collab
                    exec_tool_command(f'init_collab.sh .', extra='mldev_collab', environ=env)
                except ModuleNotFoundError as ex:
                    if args.part == 'collab':
                        err_msg = f"Please install collab extra using ./install_mldev.sh collab"
...
def do_main():
    ...
    collab_merge_driver_parser = collab_subparsers.add_parser(
        "merge-driver",
        help="Internal command used by git. Custom merge driver realisation."
    )
    ...
    collab_precommit_parser = collab_subparsers.add_parser(
        "precommit",
        help="Internal command used by git. Pre commit hook realisation."
    )
