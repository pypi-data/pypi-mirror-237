...
from devvio_util.errors import DevvError
...
class PsqlMixin:
    ...
    def create_record(self, relation: str, record_props: Dict[str, Any]) -> bool or None:
        stmt = "insert into {} (".format(relation)
        counter = 0
        if not record_props:
            raise DevvError('Missing record_props argument in PsqlMixin.create_record()')
        ...
        new_record_props['created'] = self.get_devvtime()
        new_record_props['modified'] = self.get_devvtime()
        self._logger.debug("Create record query: {}; values: {}".format(stmt, new_record_props))
        try:
            self.dbquery(stmt, new_record_props)
            return True
        except psycopg2.InternalError as i:
            self._logger.exception("PSQL error in CreateRecord, attempt rollback: {}".format(i))
            self.dbquery("rollback")
            return None
        except Exception as e:
            return self.handle_psql_exception(e)

    def update_record(self, relation: str, update_props: Dict[str, Any], constraints: Dict[str, Any]) -> int or None:
        stmt = "update {} set ".format(relation)
        counter = 0
        if not update_props:
            raise DevvError('Missing record_props argument in PsqlMixin.update_record()')
        ...
        counter = 0
        if not constraints:
            raise DevvError('Missing constraint argument in PsqlMixin.update_record()')
        ...
        try:
            ...
            self._logger.debug("PSQL error in UpdateRecord, attempt rollback: {}".format(str(i)))
            self.dbquery("rollback")
            return None
        except Exception as e:
            return self.handle_psql_exception(e)
    ...
    def delete_record(self, relation: str, constraints: Dict[str, Any]):
        ...
        counter = 0
        if not constraints:
            raise DevvError('Missing constraint argument in PsqlMixin.delete_record()')
        for k in constraints:
            if counter > 0:
                stmt += " and "
            counter += 1
        ...
        try:
            ...
            self._logger.debug("PSQL error in DeleteRecord, attempt rollback: {}".format(str(i)))
            self.dbquery("rollback")
            return None
        except Exception as e:
            return self.handle_psql_exception(e)

    def set_record(self, relation: str, record_props: Dict[str, Any], constraints: Dict[str, Any]) -> int or None:
        try:
            stmt = "select count(*) from {} where ".format(relation)
            num_records = 0
            counter = 0
            if not constraints:
                raise DevvError('Missing constraint argument in PsqlMixin.set_record()')
            for k in constraints:
                if counter > 0:
                    stmt += " and "
                stmt += "{} = %({})s".format(k, k)
            ...
            try:
                ...
                self._logger.debug("PSQL error in select portion of SetRecord, attempt create: {}".format(str(i)))
            if num_records < 1:
            ...
            self._logger.debug("PSQL error in SetRecord, attempt rollback: {}".format(str(i)))
            self.dbquery("rollback")
            return None
        except Exception as e:
            return self.handle_psql_exception(e)

    def select_single_record(self, relation: str, select_props: List[str],  # noqa: C901
                             constraints: Dict[str, Any]) -> dict or None:
        stmt = "select "
        counter = 0
        if not select_props:
            raise DevvError('Missing select_props argument in PsqlMixin.select_single_record()')
        ...
        try:
            ...
            self._logger.debug("PSQL error in SelectSingleRecord: {}".format(str(i)))
            return None
        except Exception as e:
            return self.handle_psql_exception(e)

    def select_all_records(self, relation: str, select_props: List[str],  # noqa: C901
                           constraints: Dict[str, Any]) -> List[Dict[str, Any]] or None:
        stmt = "select "
        counter = 0
        if not select_props:
            raise DevvError('Missing select_props argument in PsqlMixin.select_all_record()')
        for k in select_props:
            if counter > 0:
                stmt += ", "
            counter += 1
        ...
        try:
            ...
            self._logger.debug("PSQL error in SelectAllRecords: {}".format(str(i)))
            return None
        except Exception as e:
            return self.handle_psql_exception(e)

    def paged_query(self, select_props: List[str], constraint_filters: str, args: Dict[str, Any],  # noqa: C901
                    page: int, per_page: int) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
        ...
        counter = 0
        if not select_props:
            raise DevvError('Missing select_props argument in cache.paged_query()')
        if not constraint_filters:
            raise DevvError('Missing constraint_filters argument in cache.paged_query()')
        if per_page < 1:
            raise DevvError('Invalid per_page argument in cache.paged_query()')
        if page < 1:
            raise DevvError('Invalid page argument in cache.paged_query()')
        for k in select_props:
            if counter > 0:
                stmt += ", "
            counter += 1
        ...
        try:
            ...
            self._logger.debug("PSQL error in paged_query: {}".format(str(i)))
    ...
    def get_uuid(self) -> str:
        try:
            ...
            raise DevvError("Unknown error encountered while creating UUID")
