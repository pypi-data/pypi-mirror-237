...
[Gitee](https://gitee.com/bluepang2021/testapis_project)

HTTP automation testing framework based on pytest.

> 基于pytest 的 HTTP自动化测试框架
...
```shell
> pip install -i https://pypi.tuna.tsinghua.edu.cn/simple testapis
```
...
[demo](/demo) 提供了丰富实例，帮你快速了解testapis的用法。
...
class TestParameter(testapis.TestCase):
    """
    参数化demo
    """

    @testapis.data(LIST_DATA)
    def test_list(self, param):
        logger.info(param)

    @testapis.file_data(file='../static/data.json')
    def test_json(self, param):
        logger.info(param)

    @testapis.file_data(file='../static/data.yml', key='names')
...
testapis.main()  # 当前文件，pycharm中需要把默认的单元测试框架改成unittests
testapis.main(case_path="./")  # 当前目录
testapis.main(case_path="./test_dir/")  # 指定目录
testapis.main(case_path="./test_dir/test_api.py")  # 指定特定文件
testapis.main(case_path="./test_dir/test_api.py::TestCaseClass:test_case1") # 指定特定用例
```
...
testapis 提供了随机获取测试数据的方法。

```python
import testapis
from testapis import testdata


class TestYou(testapis.TestCase):
...
```python
import testapis
from testapis import depend


class TestDepend(testapis.TestCase):
...
```python
import testapis
from testapis.utils.mail import Mail
...
```python
import testapis
from testapis.utils.dingtalk import DingTalk
