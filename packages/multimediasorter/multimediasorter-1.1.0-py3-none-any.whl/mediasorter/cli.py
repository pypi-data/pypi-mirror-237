...
from mediasorter.config import (
    config,
...
# def _load_config(console, config_files, verbose=False) -> MediaSorterConfig:
...
def cli_root(
    ...
    if log_file := logfile or (config.file.loging and config.file.loging.logfile):
        ...
        try:
            ...
            file_handler.setLevel(loglevel or config.file.loging.loglevel or 'INFO')
    ...
    if metainfo_tag is not None:
        config.file.parameters.movie.allow_metadata_tagging = metainfo_tag
    if max_threads:
        config.file.maximum_concurrent_requests = max_threads
    ...
    if scans:
        config.file.scan_sources = scans
    elif not config.file.scan_sources:
        console.print(Text("No scans requested.", style="bold red"))
        log.error("No scans requested.")
        sys.exit(1)

    from mediasorter.sorter import MediaSorter
    sorter = MediaSorter()
