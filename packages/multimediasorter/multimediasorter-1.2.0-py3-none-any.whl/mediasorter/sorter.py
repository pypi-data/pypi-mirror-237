...
class MediaSorter:
    ...
    async def scan_all(self) -> List[Operation]:
        ...
        scan_ops = [self.scan(**scan.__dict__) for scan in config.file.scan_sources]
            ...
            filter(
                lambda x: x is not None, filter(
                    lambda provider: isinstance(provider, type_), map(
                        lambda api: self._get_api(api), config.file.api)
                )
            )
        )
    ...
    async def suggest_tv_show(self, src_path: str):
        parsed_tv_show = parse_season_and_episode(
            src_path,
            config.file.parameters.split_characters,
            config.file.parameters.tv.min_split_length,
        ...
        if parsed_tv_show:
            ...
            if config.file.parameters.tv.suffix_the:
                result.series_title = fix_leading_the(result.series_title)

            # Build the final path+filename
            season_dir = config.file.parameters.tv.dir_format.format(**result.__dict__)
            filename = config.file.parameters.tv.file_format.format(**result.__dict__)
    ...
    async def suggest_movie(self, src_path: str) -> Tuple[Optional[str], str]:
        ...
        try:
            ...
            if parse_season_and_episode(
                    src_path,
                    config.file.parameters.split_characters,
                    config.file.parameters.movie.min_split_length,
        ...
        movie, year, metainfo = parse_movie_name(
            ...
            config.file.parameters.split_characters,
            config.file.parameters.movie.min_split_length,
            config.file.metainfo_map
        )
        log.debug(f"Parsed {os.path.basename(src_path)}, {movie=} {year=}")
        result = await self._query(MovieMetadataApi, movie, year)

        for title in config.file.parameters.movie.name_overrides:
            if title == result.title:
                result.title = config.file.parameters.movie.name_overrides[title]
                break

        filename = config.file.parameters.movie.file_format.format(**result.__dict__)

        # Sort movie files in a directory.
        if config.file.parameters.movie.subdir:
            subdir = config.file.parameters.movie.dir_format.format(**result.__dict__)
        else:
            subdir = None

        if config.file.parameters.movie.allow_metadata_tagging and metainfo:
    ...
    async def suggest(
        ...
        if not extension:
            ...
            return None
        elif extension and extension not in config.file.parameters.valid_extensions:
            log.warning(
                f"{os.path.basename(src_path)}: extension '{extension}' not allowed, "
                f"not in {config.file.parameters.valid_extensions}."
            )
            return None
