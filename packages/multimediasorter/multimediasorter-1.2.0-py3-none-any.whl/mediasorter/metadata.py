...
from .config import MetadataProviderApi, config, read_search_overrides
...
semaphore = asyncio.Semaphore(config.file.maximum_concurrent_requests)
...
class TvMaze(TvShowMetadataApi):
    ...
    async def query(self, title: str, season_id: int, episode_id: int) -> TvShowMetadata:
        if override_title := config.file.parameters.tv.search_overrides.get(title.lower()):
...
class TMDB(MovieMetadataApi):
    ...
    def __init__(self, provider_config: MetadataProviderApi = None) -> None:
    ...
    async def query(self, title: str, search_year: int) -> Optional[MovieMetadata]:
        if override_title := config.file.parameters.movie.search_overrides.get(title.lower()):
            title = override_title
