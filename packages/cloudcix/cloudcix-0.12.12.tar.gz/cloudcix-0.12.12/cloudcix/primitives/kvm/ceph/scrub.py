...
def scrub(host_ip: str, device_name: str, pool_name: str) -> Tuple[int, str]:
    logger = logging.getLogger(__name__)
    logger.debug('Compiling data for ceph.scrub')

    success_id = hashlib.sha256(
        (device_name + pool_name).encode(),
    ).hexdigest()[:10]
    template_data = {
    ...
    template = JINJA_ENV.get_template('kvm/ceph/scrub.sh.j2')
