...
def attach(host_ip: str, vm_name: str, ceph_name: str, pool_name: str) -> Tuple[int, str]:
    logger = logging.getLogger(__name__)
    logger.debug('Compiling data for ceph.attach')

    success_id = hashlib.sha256(
        (vm_name + ceph_name + pool_name).encode(),
    ).hexdigest()[:10]
    template_data = {
        'vm_name': vm_name,
        'ceph_name': ceph_name,
    ...
    template = JINJA_ENV.get_template('kvm/ceph/attach.sh.j2')
