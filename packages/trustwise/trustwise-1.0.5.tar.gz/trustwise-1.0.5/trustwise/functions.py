...
def validate_api_key(api_key):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logging.error("API Key is invalid!, Please visit -> http://api.trustwise.ai:8080/github-login")
            return None
...
class Observability:
    ...
    def evaluate(self, query, response):
        ...
        if res.status_code == 200:
            ...
            logging.error(f"Evaluate request failed with status code {res.status_code}, Please try again later.")
            return None
