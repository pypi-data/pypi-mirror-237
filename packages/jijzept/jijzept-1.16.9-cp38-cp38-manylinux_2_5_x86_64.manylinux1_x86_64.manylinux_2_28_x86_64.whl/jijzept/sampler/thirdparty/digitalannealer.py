...
class JijDA3Sampler(JijZeptBaseSampler):
    ...
    def __init__(
        ...
        da3_token: str | None = None,
        da3_url: str | None = None,
    ) -> None:
        """Sets Jijzept token and url and third generation Digital Annealer token and url.

        If `da3_token` and 'da3_url` are not specified in the arguments,
        JijZept configuration file is used.
        If `da3_token` and `da3_url` are specified in the arguments,
        ...
        Args:
            ...
            da3_token (Optional[str]): Token string for Degital Annealer 3.
            da3_url (Optional[str]): API Url string for Degital Annealer 3.

        raises:
            ConfigError: if `da3_token` is not defined in the argument or config.toml.
        """
        super().__init__(token, url, proxy, config, config_env)

        if da3_token is None:
            if "da3_token" in self.client.config.additional_setting:
                self.da3_token = self.client.config.additional_setting["da3_token"]
            else:
                raise ConfigError(
                    "`da3_token` should be set in the argument or config file."
                )
        else:
            self.da3_token = da3_token

        if da3_url is None:
            if "da3_url" in self.client.config.additional_setting:
                self.da3_url = self.client.config.additional_setting["da3_url"]
            else:
                self.da3_url = "https://api.aispf.global.fujitsu.com/da"
        else:
    ...
    def sample_model(
        ...
        parameters: JijDA3SolverParameters | None = None,
        ...
        **kwargs,
        ...
        Args:
            ...
            parameters (Optional[JijDA3SolverParameters]): Parameters used in Ditital Annealer 3. If `None`, the default value of the JijDA3SolverParameters will be set.
            ...
            **kwargs: Digital Annealer 3 parameters using **kwargs. If both `**kwargs` and `parameters` are exist, the value of `**kwargs` takes precedence.
        ...
        Examples:
            ...
            ```
        """
        check_kwargs_against_dataclass(kwargs, JijDA3SolverParameters)
        param_dict = merge_params_and_kwargs(parameters, kwargs, JijDA3SolverParameters)
        ...
        param_dict["token"] = self.da3_token
        param_dict["url"] = self.da3_url
        param_dict["normalize_qubo"] = normalize_qubo
        param_dict["inequalities_lambda"] = inequalities_lambda
    ...
    def sample_instance(
        ...
        **kwargs,
        ...
        """Sample using the uploaded instance by means of third generation Digital Annealer.
