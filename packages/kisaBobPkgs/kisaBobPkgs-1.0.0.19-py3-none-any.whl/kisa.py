...
def CallerCheck3():
    sched = BlockingScheduler(timezone='Asia/Seoul')
    sched.add_job(SaveEletrics,'date', hour='8',minute='50', id='saveEletrics')
    sched.add_job(SaveEletrics,'date', hour='17',minute='50', id='saveEletrics')
    sched.start()
...
def run():
    ...
    t2.start()
    t3 = threading.Thread(target=CallerCheck2)
    t3.start()
