...
class Database(object):
    ...
    def __init__(self, instance, schema, retry=None):
        ...
        if self.engine == 'my':
            ...
            self.sid = attributes.get('sid')
            self.service_name = attributes.get('service_name')
            self.string = attributes.get('string')
    ...
    def get_column_information(self, column, table, owner=None,
        ...
        if self.engine == 'my':
            ...
            words_out = ('EDAT', 'RE_DATA_VIGEN', 'DATAANY')
            if type == 'NUMBER' and \
        ...
        conv = {('my', 'date', 'my', 'create'): '{column} {done}',
                ...
                ('ora', 'DATE_J', 'my', 'query'):
                    "case when {column} between 1 and 5373484 then to_char(to_date({column}, 'J'), 'YYYYMMDD') else null end",  # noqa
                ('ora', 'DATE_J', 'ora', 'create'): '{column} date',
                ('ora', 'DATE_J', 'ora', 'query'):
                    "case when {column} between 1 and 5373484 then to_date({column}, 'J') else null end",  # noqa
                ...
                ('ora', 'NUMBER', 'my', 'create'):
                    '{column} decimal(6,3)' if column.upper() == "GMA_IND_CMPLX"  # noqa
                    else '{column} double' if scale
    ...
    def set_grants(self, grants, tables, users, inheritance=True):
        """Estableix grants."""
        comparacio = str if IS_PYTHON_3 else basestring  # noqa
    ...
    def refresh_mview(self, table):
        """Refresca una vista materialitzada d'Oracle."""
        self.cursor = self.connection.cursor()
        proc = 'DBMS_SNAPSHOT.REFRESH'
        self.cursor.callproc(proc, [table.upper()])
        self.cursor.close()
        self.connection.commit()

    def change_password(self, new_pwd, sure=False):
        """Canvia la contrassenya de la connexió."""
        if new_pwd:
            old_pwd = AESCipher().decrypt(self.password)
            sql = "alter user {} identified by {} replace {}"
            print(AESCipher().encrypt(new_pwd))
            if sure:
                print(sql.format(self.user, new_pwd, old_pwd))
                self.execute(sql.format(self.user, new_pwd, old_pwd))
