...
class lyymysql_class():
    ...
    def get_tdx_server_list(self):
        df = pd.read_sql_table('stock_tdx_server', self.conn)
        #print(df)
        return df.ip.to_list()

    def 获取股票代码表(self, debug=False):
        if debug: print(sys._getframe().f_code.co_name)
        df = pd.read_sql_table('stock_all_codes', self.conn)
        code_list = df['code'].tolist()
        if debug: print("code_list:", len(code_list))
        return code_list, df

    def get_date_list_mysql(self):
        df = pd.read_sql_query('SELECT day FROM stock_trade_calendars', self.conn)
    ...
    def insert_mysql(self, wmdf, start_date, stk_code_num):
        ...
        if 结果长度 > 0:
            print(结果长度)
            inserted_rows = self.mysql_insert(newdf, self.engine, stk_code_num, output=True)
    ...
    def mysql_insert(self, df, table_name, output=True):
        ...
        if not self.engine.dialect.has_table(self.engine, table_name):
            ...
            for row in df.iterrows():
                ...
                try:
                    current_row = pd.read_sql(query, self.engine)
                except:
                ...
                if current_row.empty:
                    # 表无对应行,插入新行
                    print(df.iloc[index])
                    df.iloc[index].to_sql(table_name, self.engine, index=False, if_exists='replace')
