...
##                strQ += '<p><b>DATA:</b><br>'+'<br>'.join(f'{v}' for v in dataA)+'</p>'
        ...
        if isinstance(answers[indxJ], dict) and 'choices' in answers[indxJ] and answers[indxJ]['choices']: # changed 2023-10-28
            choices=answers[indxJ]['choices']
            try:
                if len(set(choices)) != len(choices):
                    return f'<font color="#dd0000"><bold>Error</bold></font>::질문 #{indxJ+1}의 중복된 보기문항 in choices....', None, None, None
            except: # added 2023-10-26
