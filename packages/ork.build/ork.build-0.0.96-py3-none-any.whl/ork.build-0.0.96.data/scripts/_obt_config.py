...
def configFromCommandLine(parser_args=None):
  ...
  if _config.inplace:
    ...
    os.environ["OBT_SCRIPTS_DIR"] = str(_genpath(_config.root_dir/"obt"/"scripts"))
    ...
    os.environ["PYTHONPATH"]=str(_config.scripts_dir)
    #os.environ["PATH"]=str(root_dir/"bin_priv")+":"+os.environ["PATH"]
    sys.path = [str(_config.scripts_dir)]+sys.path
  ...
  obt.env.append("PYTHONPATH",_config.scripts_dir)
