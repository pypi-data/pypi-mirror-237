  ...
  def __init__(self): # default initialization from env vars
    ...
    if (self.root_dir!=None):
      if "OBT_INPLACE" in os.environ:
        self._inplace = True
        self._bin_priv_dir = self.root_dir/"bin_priv"
        self._bin_pub_dir = self.root_dir/"bin_pub"
      else:
        self._bin_priv_dir = self.root_dir/"obt"/"bin_priv"
        self._bin_pub_dir = self.root_dir/"bin_pub"
...
def configFromCommandLine(parser_args=None):
  ...
  if not env_is_set("OBT_ROOT"):
    ...
    os.environ["OBT_ROOT"] = str(_genpath(_config._bin_pub_dir/".."))
    os.environ["OBT_SCRIPTS_DIR"] = str(_config.root_dir/"obt"/"scripts")

    sys.path.append(str(_config._scripts_dir))
  ...
  if _config.inplace:
    _config._bin_priv_dir = _genpath(_config.root_dir/"bin_priv")
    _config._scripts_dir = _genpath(_config.root_dir/"scripts")
  else:
    _config._bin_priv_dir = _genpath(_config.root_dir/"obt"/"bin_priv")
    _config._scripts_dir = _genpath(_config.root_dir/"obt"/"scripts")
  ...
  if _config.inplace:
    _config._module_paths = [_config.root_dir/"modules"]
  else:
    _config._module_paths = [_config.root_dir/"obt"/"modules"]
  ...
  if _config.valid: # Valid StageDir ?
    ...
    #obt.env.prepend("PKG_CONFIG_PREFIX",_config.stage_dir)
    ...
    obt.env.append("LD_LIBRARY_PATH",_config.stage_dir/"python-3.9.13"/"lib")
