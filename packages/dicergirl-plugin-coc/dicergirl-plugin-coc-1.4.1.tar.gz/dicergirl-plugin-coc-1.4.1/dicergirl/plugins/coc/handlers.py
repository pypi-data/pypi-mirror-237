...
from .cards import coc_cards, coc_cache_cards, coc_rolls
from .backend import sc, ti, li

from dicergirl.utils.formatters import format_msg, format_str
from dicergirl.utils.operator import get_status
...
async def coc_handler(matcher: Matcher, event: MessageEvent):
    """ COC 车卡指令 """
...
async def sccommandhandler(matcher: Matcher, event: MessageEvent):
    """ COC 疯狂检定指令 """
    if not get_status(event) and not event.to_me:
        return
