...
def ra(event: Event, args: list, bp=None):
    ...
    if not card_data:
        if len(args) == 1:
            return str(judger(event, Dicer(), 0))

        return str(judger(event, Dicer(), int(args[1])))
    ...
    if not exp:
        ...
        return judger(event, Dicer(), exp).detail
    elif exp and len(args) > 1:
        if not args[1].isdigit():
            return "技能值应当为整型数, 使用`.help ra`查看技能检定指令使用帮助."

        reply = [f"你已经设置了技能 {args[0]} 为 {exp}, 但你指定了检定值, 使用指定检定值作为替代."]
        reply.append(str(judger(event, Dicer(), int(args[1]), name=inv.name)))
        return reply

    time = 1
    r = judger(event, Dicer(), exp)

    for _ in range(time-1):
        r += judger(event, Dicer(), exp, name=inv.name)
